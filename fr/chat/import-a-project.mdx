...
description: "Importez du code existant ou des éléments de conception dans Replit, découvrez la différence entre l’importation et l’ajout d’un fichier, puis poursuivez dans le projet créé."
...
Si vous travaillez déjà sur une app ou un projet, vous pouvez l’importer dans Replit et poursuivre votre travail dans un projet. L’importation est différente de l’ajout d’un fichier comme pièce justificative ou contexte dans une Conversation.

<div id="choose-a-supported-source">
  ## Choisissez une source prise en charge
</div>

La source détermine ce que vous fournissez et la façon dont le travail existant est importé dans Replit. Le guide d’importation actuel décrit les options suivantes :

| Travail existant                         | Sources prises en charge                                                                              |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Dépôts et projets créés par des builders | **GitHub**, **Bitbucket**, **Vercel**, **Bolt**, **Lovable**, **Base44** et **Previous Agent export** |
| Designs                                  | **Figma** et **Claude**                                                                               |
| Fichiers et données locaux               | **ZIP** et **Spreadsheet** (`.xlsx`, `.csv` ou une URL publique Google Sheets)                        |

Certaines importations de projets créés par des builders utilisent un dépôt GitHub exporté depuis le service d’origine. Consultez le guide spécifique à la source avant d’importer. Les paramètres, Secrets, valeurs de base de données ou services propres au fournisseur peuvent nécessiter une configuration supplémentaire par la suite.

Dans le champ de prompt, sélectionnez le menu **plus**, puis **Import an existing project** pour ouvrir le processus d’importation.

<div id="import-or-attach">
  ## Importer ou joindre ?
</div>

* **Importez un projet** si vous souhaitez poursuivre le développement à partir de code existant, d’un design ou d’un travail de projet structuré.
* **Téléversez un fichier** si un document, une feuille de calcul, une image ou une capture d’écran fournit du contexte pour une question ou une tâche dans la Conversation en cours.

Pour la seconde option, consultez [Téléverser et utiliser des fichiers](/fr/chat/upload-and-use-files).
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/fr/build/import-from-providers">
    ...
    <p>Choisissez une source compatible et suivez la procédure d’importation complète.</p>
  ...
  <a className="chat-example has-visual" href="/fr/features/projects-and-artifacts/projects">
    ...
    <p>Découvrez l’espace structuré dans lequel vous poursuivez le travail importé.</p>
    <div className="chat-card-link">Découvrir projets →</div>
  </a>
</section>

Une fois le projet dans Replit, [utilisez un système de design](/fr/chat/use-a-design-system) si le travail doit respecter une identité visuelle établie.