---
title: "Canvas"
description: "了解如何借助 Canvas 与 Agent 一起探索和迭代设计，并在 frame 中查看你的构件。"
---
...
Canvas 是一个可以与 Agent 一起探索和迭代设计的空间。在这里，你的构件 (如应用、视频和幻灯片) 会以 frame 的形式呈现。你可以与 Agent 协作创建原型稿、对比不同的设计方向，并将修改应用回你的应用——这一切都在同一个可视化面板中完成。

<Frame>
  <img src="/assets/mirror/canvas-desktop-85e8bfec38a2.png" alt="Canvas 中应用预览与 Agent 聊天及任务更新并排显示" width="3452" height="2006" data-path="images/replitai/canvas-desktop.png" />
</Frame>
...
Canvas 是一块无限延展的画板，你可以在这里以可视化的方式查看和处理所有内容。

<Frame>
  <img src="/assets/mirror/canvas-mockups-057e738a78da.png" alt="Canvas 中并排排列的多个设计原型图——深色主题、移动端视图与详情页" width="3452" height="1620" data-path="images/replitai/canvas-mockups.png" />
</Frame>

* **你的应用**：正在运行的应用会以实时交互预览的形式直接显示在 Canvas 上。你可以像在浏览器中一样点击并使用它们。
* **设计原型图**：Agent 为你创建的可视化原型。它们看起来就像真实页面——你可以与之交互、调整大小，也可以让 Agent 进行修改——但它们与线上应用彼此独立，因此你可以放心大胆地尝试。
* **形状与图形**：矩形、圆形、星形、箭头、心形等等。用它们来勾勒想法、标注设计，或梳理各个页面之间的跳转关系。
* **文本与便签**：在 Canvas 的任意位置添加标签或备注，帮助你整理思路。
* **图片**：将截图或参考图拖放到 Canvas 上，Agent 可以复现所看到的内容，或从中获取灵感。
* **视频**：将视频文件放到 Canvas 上，与其他内容并列展示。

<div id="colocando-designs-no-canvas">
  ### 将设计放到 Canvas 上
</div>

开始创建设计有几种方式，选择你觉得最顺手的一种即可。

<Steps>
  <Step title="在 Chat 中向 Agent 提问">
    在 Chat 输入框中输入你想看到的内容，Agent 就会把它作为一个可交互的 frame 放到 Canvas 上。

    <AgentInput defaultPrompt="Show me a mockup of my landing page on the canvas" showTypewriter={true} />
  </Step>

  <Step title="选择输出类型">
    你也可以选择想要的输出类型——网页、移动端、幻灯片、设计等等。发送 prompt 之前，用轮播控件进行选择。

    <AgentCarousel />
  </Step>

  <Step title="将已有页面引入 Canvas">
    已经有上线的应用了？可以让 Agent 把某个页面放到 Canvas 上做重新设计——例如，&quot;把我的仪表盘放到 canvas 上。&quot; Agent 会创建一份副本，你的线上应用不会受到任何影响。
  </Step>
</Steps>
...
在 Canvas 上创建好设计后，你可以通过多种方式对其进行迭代。

<div id="pea-ao-agent-por-atualizaes">
  ### 让 Agent 进行更新
</div>

在 Canvas 中选中一个 frame，然后在其下方出现的输入框中输入你想要的修改。按下回车后，Agent 会接收该请求、执行更新，并直接在 Canvas 中刷新该原型图。
...
使用底部工具栏中的绘图工具，直接在 Canvas 上勾画。用箭头指向想要修改的地方，圈出有问题的区域，或用文字备注写下你的反馈。然后让 Agent 根据你的标注进行修改。
...
查看设计在不同屏幕尺寸下的呈现效果。让 Agent 展示手机版、平板版或桌面版：

* **手机** — 390 × 844
* **平板** — 768 × 1024
* **桌面** — 1280 × 720

例如，输入“给我看看这个页面的手机版”，Agent 就会按该尺寸渲染。
...
对某个设计满意后，你可以将更改应用回主版本。Agent 会显示本次更改的摘要，你可以选择应用或放弃。
...
当你不确定该往哪个方向走时，可以让 Agent 生成多个版本，方便并排比较。
...
* **同一页面的多个版本**：&quot;给我看这个落地页的五个版本。&quot;Agent 会逐一创建，并将它们并排排列。
* **多页面流程**：&quot;设计一个完整的新手引导流程：欢迎、个人资料设置、偏好设置和确认。&quot;Agent 会构建每个界面，并按顺序排列。
* **混搭组合**：&quot;给我看落地页和定价页的极简风格版和大胆风格版。&quot;Agent 会创建每种组合，让你看到全貌。

<div id="mapeando-fluxos-de-usurio">
  ## 梳理用户流程
</div>

在 Canvas 上铺开完整的用户旅程——注册、新手引导、仪表板、设置——并加以整理，看清各环节如何衔接。

* 将每个界面单独放置为一个 frame
* 让 Agent 生成完整流程：&quot;设计我应用的完整新手引导流程&quot;
* 用箭头和形状展示用户在各界面之间的流转路径
* 让 Agent 把粗略草图转化为精致的设计

<div id="transformando-um-design-em-um-aplicativo-real">
  ## 将设计变成真正的应用
</div>

当你对某个设计满意，想把它变为现实时，有两条路径可选：

<div id="torne-o-um-aplicativo-completo">
  ### 将其变成完整应用
</div>

如果你的设计需要真实的功能——比如保存数据、用户账户或连接其他服务——可以让 Agent 把它变成一个完整的应用。你可以这样说:&quot;把它变成一个真正的应用&quot;或&quot;给它加一个后端&quot;。Agent 会构建好一切,让它成为可真正运行并发布的应用。
...
你也可以将设计 frame 转换为特定类型的构件：

<Steps>
  <Step title="选择一个 frame">
    点击你想要转换的 frame。
  </Step>

  <Step title="让 Agent 进行转换">
    告诉 Agent 你的需求——例如“把它转换成一个 Web 应用”或“把它变成幻灯片”。
  </Step>

  <Step title="选择一种类型">
    可选择 Web 应用、移动应用、幻灯片、数据可视化等多种类型。
  </Step>
</Steps>
...
<Accordion title="设计 frame 和我正在运行的应用是一回事吗？">
  不是。设计 frame 是视觉原型——看起来和真实产品一样，但背后并没有可运行的后端或数据库。而 Canvas 中的应用预览，才是你真正在运行的应用。
</Accordion>

<Accordion title="我可以在不影响线上应用的情况下重新设计某个页面吗？">
  可以。让 Agent 把某个页面放到 Canvas 上——例如，“把我的仪表板放到画布上。”Agent 会生成一份副本供你随意试验。在你选择应用更改之前，线上应用不会受到任何影响。
</Accordion>

<Accordion title="如何把设计变成真正的应用？">
  让 Agent 将其变成完整的应用 (“把这个变成真的”) ，或者转换为特定类型 (“把这个转换成 Web 应用”) 。Agent 会搭建所需的一切，使其成为可运行、可发布的应用。
</Accordion>

<Accordion title="我可以转换成不同类型的 Web 应用吗？">
  可以。你可以把 Canvas 中的 frame 转换成 Web 应用、移动应用、幻灯片、数据可视化等等。
</Accordion>

<Accordion title="我可以从 Figma 或其他工具导入设计吗？">
  可以。将 Figma 设计引入 Replit 有以下几种方式：

  * **从 Figma 导入**：访问 [replit.com/import](https://replit.com/import)，连接你的 Figma 账户，粘贴某个 frame 的 URL，即可将设计转换为可运行的 React 应用。详情请参阅[快速入门：从 Figma 导入](/zh/pt/getting-started/quickstarts/import-from-figma)。
  * **在 Agent 聊天中使用 Figma MCP**：直接在 Agent 聊天中粘贴 Figma 链接，让它生成代码、提取设计数据或根据设计构建组件。请参阅 [Figma MCP 集成](/zh/replitai/mcp/figma)指南。
  * **以截图作为参考**：把截图拖放到 Canvas 上，让 Agent 据此还原设计。
</Accordion>

<Accordion title="Canvas 上还能放些什么？">
  除了设计原型图和应用预览之外，你还可以添加形状、箭头、文本标签、便签、图片和视频——非常适合勾勒创意、为设计添加注释或梳理思路。
</Accordion>
...
Canvas 在所有套餐中均可使用，包括创建 frame、内联编辑、生成变体以及转换为构件。若要将设计转化为完整的应用，则需要付费套餐。