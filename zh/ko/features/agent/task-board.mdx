---
title: "任务看板"
description: "使用任务看板审核、启动、监控和应用 Agent 任务，了解其在各个阶段的进展。"
---
...
任务看板提供了一个统一的空间，让你可以查看、启动、监控并应用 Agent 的工作。当 Agent 把较大的请求拆分成多个部分、你想让任务在后台运行，或需要确认项目中已有哪些改动时，都可以使用它。

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="任务看板，显示按草稿、进行中、已就绪、已完成分列整理的任务" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

任务按状态分列整理。看板会显示 Agent 已规划、正在执行、可供检查以及已经应用的内容。

每个任务会随着推进从左向右移动：

* **草稿**：Agent 已规划但尚未开始的任务
* **进行中**：Agent 正在构建的任务，包括因活动任务上限而排队等待的任务
* **已就绪**：Agent 已完成、等待检查的任务
* **已完成**：已完成、已归档或已取消的任务
...
**草稿**是 Agent 已规划但尚未开始构建的任务。每个草稿都包含标题、说明和分步计划。

如果你想在任务启动前先审查 Agent 提出的方案，可以使用草稿。你可以查看计划、要求 Agent 进行调整，也可以在准备就绪后启动任务。

当你想在确定后台任务之前先规划较大的功能时，草稿会非常有用。例如，Agent 可以把一个新手引导项目拆分成多个独立的草稿，分别负责身份验证、个人资料设置、仪表板界面和部署收尾工作。

<div id="active">
  ## 进行中
</div>

批准任务后，任务会移至 **进行中** 列，Agent 随即开始构建。每项任务都会在项目的独立副本中运行，因此在应用更改前，不会影响主版本。

有些任务依赖于其他任务。例如，构建仪表板可能需要先完成数据库架构相关任务。存在依赖关系的任务会保持 **排队中** 状态，直到前置条件完成。当达到套餐允许的活跃后台任务上限时，任务也会进入队列。

<KanbanColumns
  columns={[
{ title: '草稿', count: 2, cards: [
{ title: '为家人和朋友添加送礼流程', description: '让家长能够为其他孩子创作故事并作为礼物发送，开辟新的获客渠道。', status: 'draft-static', timestamp: '2m' },
{ title: '带深色主题的睡前阅读模式', description: '通过采用暖色调深色主题和环境音的专属睡前模式，让应用成为完整的睡前仪式的一部分。', status: 'draft-static', timestamp: '2m' },
] },
{ title: '进行中', count: 3, cards: [
{ title: '故事续篇：续集', description: '添加“继续冒险”按钮后，即可生成从上一个故事结束处开始的续集。', status: 'active', timestamp: '12m' },
{ title: '重新生成故事：重写页面', description: '添加重新生成单个页面的功能，让家长拥有更多掌控权并减少不满。', status: 'active', timestamp: '8m' },
{ title: '为家人和朋友添加送礼流程', description: '让家长能够为其他孩子创作故事并作为礼物发送。', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
任务构建完成后会进入**已就绪**状态。这意味着任务已完成，但尚未对主版本做出任何更改。

应用更改的方式有两种：

* **在会话中**：打开该任务的会话，选择**将更改应用到主版本**，将任务合入项目；或选择**删除**将其丢弃
* **在看板中**：在处于已就绪状态的任务卡片上打开三点菜单，然后选择**应用更改**

<TaskReviewDrawer />

应用之前，请先检查已就绪的任务。查看任务日志、测试输出和预览，了解 Agent 做了哪些更改。

<div id="done">
  ## 已完成
</div>

应用完成后，任务卡片会移动到 **已完成** 列。

已完成 列中还包含已归档的规划会话和已取消的任务。这样一来，已完成的任务不会再混入 草稿 或 进行中，同时你仍能回顾整个过程的记录。

<KanbanColumns
  columns={[
{ title: '已就绪', count: 3, cards: [
{ title: '带深色主题的睡前阅读模式', description: '带有暖色调深色主题和环境音效的专属睡前模式。', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: '带 OG 预览的可分享故事链接', description: '添加可公开分享的链接是最大的增长杠杆。', status: 'ready-drop', timestamp: '31m', showMenu: true },
{ title: '增强的角色自定义', description: '添加外观选项后，插画才会真正有个性化的感觉。', status: 'ready-drop', timestamp: '28m', showMenu: true },
] },
{ title: '已完成', count: 4, cards: [
{ title: '带单词高亮的朗读模式', description: '通过文本转语音，孩子们可以独立享受故事。', status: 'done', timestamp: '4h' },
{ title: '带 OG 预览的可分享故事链接', description: '最大的增长杠杆——可公开分享的链接。', status: 'done', timestamp: '1h' },
{ title: '增强的角色自定义', description: '为真正个性化的插画提供的外观选项。', status: 'done', timestamp: '4h' },
{ title: '移动端响应式与生产环境收尾', description: '改进的错误状态、移动端布局和加载动画。', status: 'done', timestamp: '4h' },
] }
]}
/>

在应用多个任务的更改时，Agent 会自动处理冲突。如果发现哪里不对劲，可以让 Agent 修复。

<div id="section">
  ## 查找和筛选任务
</div>

当项目中有大量进行中的任务时，可以使用任务看板。你可以按任务标题或任务编号搜索、筛选草稿，并通过编辑模式批量管理多个任务。

例如，当有人在会话、评审或 Slack 对话中提到某个任务时，可以按任务编号搜索。如果你只想专注于已规划的任务，而不想在活跃或已完成的任务中翻找，可以筛选草稿。

关于建议的后续任务，请参阅 [Follow-up tasks](/zh/ko/features/agent/follow-up-tasks)。

<div id="section-2">
  ## 重命名任务
</div>

随着任务范围变化，持续时间较长的任务列表可能会变得难以一目了然。要重命名任务，请打开卡片的三点菜单 (位于任务列表侧边栏或任务看板中) ，然后选择**重命名**。输入更清晰的标题，然后选择**保存**。

重命名任务只会改变任务向您和团队成员显示的名称。Agent 已完成的工作、任务历史记录和待处理的更改都会保持不变。

<div id="section-3">
  ## 每个任务的设置
</div>

每个任务的标题旁都有各自的设置下拉菜单。打开该菜单，可控制 Agent 如何完成任务，以及结果如何应用到项目中：

* **自动应用更改** (草稿和进行中) ：任务就绪后立即将其应用到主版本，无需等待审核。
* **自动批准计划** (草稿) ：当 Agent 为任务提出计划时，自动批准并开始构建。
* **应用更改** (已就绪) ：将已完成的任务应用到主版本。
* **重命名**：更改卡片上显示的标题。
* **审核更改** (已就绪) ：直接跳转到已就绪任务的差异对比。
* **取消**：停止任务或删除草稿。

自动应用和自动批准都是一次性的快捷操作，并非可持续开启的开关。即使打开多个任务，菜单也会保持固定宽度，避免标签位置跳动。

<Frame>
  <img src="/assets/mirror/task-settings-dropdown-7ea7868303b4.png" alt="从任务卡片打开的任务设置下拉菜单，在固定宽度的弹出窗口中显示应用更改、重命名、审核更改和取消操作" width="858" height="536" data-path="images/replitai/task-settings-dropdown.png" />
</Frame>