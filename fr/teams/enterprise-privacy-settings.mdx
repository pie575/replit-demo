...
description: "Configurez les paramètres de confidentialité, de sécurité et de contrôle de code source à l'échelle de l'organisation pour protéger le code et les déploiements de votre équipe."
...
<CardGroup cols={2}>
  ...
  <Card title="URLs de développement privées" icon="shield-halved">
...
Lorsque ce paramètre est activé, toutes les nouvelles applications doivent être publiées en privé. Les applications privées ne sont accessibles qu'aux membres authentifiés de votre organisation.
...
Contrôlez si les membres de l'équipe peuvent créer des déploiements protégés par mot de passe. Lorsque ce paramètre est activé, les builders peuvent ajouter une couche de sécurité supplémentaire en exigeant un mot de passe pour accéder à leurs applications publiées.
...
Lorsque ce paramètre est activé, toutes les URLs de développement d'applications nécessitent une authentification. Si votre organisation a activé SAML, les URLs de développement nécessitent une authentification SSO.
...
Lorsque ce paramètre est activé, les membres de l'équipe ne peuvent pas exporter le code source d'une application sous forme de fichier zip. Cela empêche le téléchargement et le partage du code en dehors de la plateforme Replit.
...
Configurez si toutes les applications doivent réussir une analyse de sécurité avant la publication. Les analyses de sécurité détectent les vulnérabilités, les secrets exposés et autres problèmes de sécurité dans votre code.
...
Lorsque ce paramètre est activé, toutes les applications doivent avoir leurs modifications locales poussées vers un dépôt Git distant avant la publication. Cela garantit que toutes les modifications de code sont suivies dans le système de contrôle de version de votre organisation.
...
Contrôlez si les membres de l'équipe peuvent utiliser des dépôts Git publics. Lorsque la configuration exige des dépôts distants privés, les membres de l'équipe ne peuvent pousser que vers des dépôts privés sur des plateformes comme GitHub.
...
<AccordionGroup>
  <Accordion title="Commencer par privé par défaut" icon="shield">
    Activez **Déploiements privés** pour vous assurer que toutes les nouvelles applications publiées sont privées. Cela contribue à éviter l'exposition accidentelle d'applications sensibles.
  </Accordion>
...
* [Sécurité de l'information](/fr/teams/information-security/overview) - En savoir plus sur les pratiques de sécurité et les standards de conformité de Replit
* [Authentification SAML](/fr/teams/identity-and-access-management/saml) - Configurer l'authentification unique pour votre organisation
* [Groupes et permissions](/fr/teams/identity-and-access-management/groups-and-permissions) - Gérer l'accès des membres de l'équipe avec des rôles et des groupes
