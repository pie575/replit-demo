...
description: "Создайте приложение для телефона с помощью Agent, протестируйте его в симуляторе и на своём телефоне и поделитесь им, чтобы получить обратную связь."
sidebarTitle: "Создание мобильного приложения"
icon: "mobile-screen"
---
...
Мобильное приложение должно быть удобным на телефоне: быстро запускаться, легко управляться касаниями и комфортно использоваться одной рукой.

Вы создадите **Pace Mobile** — трекер пробежек для телефона, вдохновлённый примером трекера пробежек из статьи [Что такое Replit?](/ru/welcome). Цель — протестировать продуманный мобильный сценарий, прежде чем проходить полный путь публикации в App Store.

<Frame>
  <img src="/assets/mirror/mobile-app-4203ff45ecd5.png" alt="Пример современного мобильного приложения с экранами отслеживания фитнес-активности, разработанными для использования на телефоне" width="1440" height="850" data-path="images/build-examples/mobile-app.png" />
</Frame>
...
* Создадите проект мобильного приложения с помощью Agent.
* Подготовите первую версию с одним сценарием, удобным для использования на телефоне.
* Запустите приложение в симуляторе или эмуляторе.
* Запустите приложение на телефоне с помощью Expo Go.
* Создадите версию, которой можно поделиться и отправить кому-нибудь для получения обратной связи.
...
* Создать мобильное приложение с помощью промпта.
* Учесть ограничения мобильных устройств при разработке первой версии.
* Тестировать приложение в симуляторе или эмуляторе.
* Тестировать приложение на реальном устройстве с помощью Expo Go.
* Определить, когда стоит продолжить доработку приложения, прежде чем переходить к App Store или TestFlight.

<div id="who-this-is-for">
  ## Для кого это руководство
</div>

Используйте это руководство, если вашей идеей будут пользоваться на телефоне.

Оно пригодится для трекеров, сервисов бронирования, полевых инструментов, приложений для мероприятий, обучения, формирования привычек и всего, чем люди будут пользоваться не за настольным компьютером.

<Note>
  Публикация в TestFlight и App Store выходит за рамки этого руководства. Когда вы
  будете готовы к этому этапу, см. [Создание и запуск мобильного приложения](/ru/build/mobile-app).
</Note>
...
Подробнее о симуляторах, эмуляторах и предварительном просмотре на устройствах см. в разделе [Нативные мобильные приложения](/ru/features/artifact-types/building-mobile-apps).

<div id="define-the-mobile-flow">
  ## Определите сценарий для мобильного приложения
</div>

Перед началом разработки решите, что пользователь должен делать в приложении на телефоне.

Для Pace Mobile:

* **Аудитория**: бегуны, которые хотят быстро записывать пробежки.
* **Основной сценарий**: выбрать тип пробежки → ввести расстояние и время → сохранить пробежку → посмотреть сводку за сегодня.
* **Ограничение мобильного интерфейса**: основное действие должно быть легко доступно для большого пальца.
* **Первая версия**: без аккаунтов, без социальной ленты и без расширенной аналитики.

Так первый мобильный вариант останется достаточно компактным для тестирования.

<div id="build-the-first-version">
  ## Создайте первую версию
</div>

Откройте Replit и создайте новое приложение. Опишите, что хотите создать, затем выберите **Mobile App** в качестве типа приложения.

<Frame>
  <img src="/assets/mirror/prompt-5cb144699b45.png" alt="Поле ввода запроса на главном экране Replit, где в качестве типа приложения выбрано Mobile App" width="1440" height="900" data-path="images/native-mobile-apps/prompt.png" />
</Frame>

Используйте такой промпт:

<AiPrompt>
  Создай Pace Mobile — трекер пробежек для телефона.

  <br />

  Пользователь должен иметь возможность выбрать тип пробежки, указать дистанцию и время, сохранить
  пробежку и посмотреть сводку за сегодня.

  <br />

  Разработай приложение для управления одной рукой: с крупными областями нажатия, простой навигацией и
  продуманным дизайном в стиле фитнес-приложения.

  <br />

  Сделай первую версию достаточно компактной, чтобы протестировать её уже сегодня в симуляторе и на реальном
  телефоне.
</AiPrompt>

Если хотите, чтобы Agent сначала всё продумал, используйте [режим Plan](/ru/features/agent/plan-mode). Попросите Agent проверить экраны, навигацию, области нажатия и ограничения, связанные с использованием на телефоне, прежде чем он начнёт разработку.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Поле ввода запроса Replit с выбранным режимом Plan, прежде чем Agent определит объём работ для мобильного приложения" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Когда Agent завершит первую сборку, откройте [Preview](/ru/features/editor/preview). В меню выбора устройства выберите **iOS Simulator** или **Android Emulator**.

Протестируйте приложение так, как если бы вы пользовались им на телефоне.

<Frame>
  <img src="/assets/mirror/workspace-dda0f81e6b83.png" alt="Project Editor Replit с предварительным просмотром мобильного приложения и возможностью открыть его на мобильном устройстве" width="1440" height="900" data-path="images/native-mobile-apps/workspace.png" />
</Frame>
...
* Сохранить пробежку.
* Увидеть обновлённую сводку за сегодня.
* Перемещаться по приложению, не заходя в тупик.
...
Тестирование в симуляторе полезно, но только на телефоне можно понять, насколько удобно пользоваться приложением.

Откройте панель предпросмотра на мобильном устройстве и отсканируйте QR-код телефоном. Приложение откроется в Expo Go.

<Frame>
  <img src="/assets/mirror/mobile-screen-e5685fc26793.png" alt="Мобильное приложение, запущенное в предпросмотре Replit Project Editor, с панелью Preview на устройстве и QR-кодом для Expo Go" width="1440" height="900" data-path="images/native-mobile-apps/mobile-screen.png" />
</Frame>

Проверьте на телефоне:

* Можно ли выполнить основной сценарий одной рукой?
* Достаточно ли велики кнопки и поля ввода, чтобы по ним было удобно нажимать?
* Легко ли дотянуться до основного действия?
* Не закрывает ли клавиатура важные поля?
* Остаётся ли приложение понятным на маленьком экране?

Если что-то кажется неудобным, попросите Agent исправить конкретную часть, а не переделывать всё приложение.

<AiPrompt>
  Улучши Pace Mobile для использования одной рукой.

  <br />

  Сделай кнопку Save Run более доступной, при необходимости увеличь области нажатия
  и сохрани простоту формы пробежки.
...
<div id="share-for-feedback">
  ## Поделитесь приложением для сбора отзывов
</div>

Когда всё будет работать на телефоне, опубликуйте приложение, чтобы создать версию Expo Go, которой можно поделиться и собрать первые отзывы.

Поделитесь ею, попросив выполнить одну конкретную задачу:

> Откройте Pace Mobile на телефоне, сохраните одну пробежку и расскажите обо всём, что было неудобно нажимать, читать или понимать.

Конкретные отзывы проще использовать, чем общие впечатления. Если отзыв указывает на одну явную проблему, попросите Agent внести минимальное исправление и снова протестируйте приложение на телефоне.

<div id="publish-to-the-app-store">
  ## Публикация в App Store
</div>

После тестирования Pace Mobile в симуляторе и на телефоне через Expo Go вы можете отправить это же App в TestFlight и App Store. Для этого требуется членство в **Apple Developer Program** с включённой двухфакторной аутентификацией. Подробнее о регистрации, индивидуальных и организационных аккаунтах, проверке в TestFlight и App Store, а также внешнем тестировании см. в разделе [Создание и запуск мобильного приложения](/ru/build/mobile-app).

<Steps>
  <Step title="Откройте вкладку Publishing и начните публикацию в App Store">
    Когда предпросмотр Expo Go станет доступен, откройте вкладку **Publishing**. В разделе **Production** вы увидите карточку **Ready to publish Pace to the App Store?**. Нажмите **Start publishing to the App Store**.

    <Frame>
      <img src="/assets/mirror/app-store-02-start-publishing-190eb7841bcf.png" alt="Вкладка Publishing в Project Editor со статусом Production и карточкой Ready to publish Pace to the App Store? с кнопкой Start publishing to the App Store" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-02-start-publishing.png" />
    </Frame>
  </Step>

  <Step title="Выберите проект Expo">
    Replit откроет **Launch** — встроенный в Replit процесс Expo. Введите или подтвердите название и slug проекта Expo, затем нажмите **Continue**. Если у вас уже есть проект Expo, нажмите **Use existing**.

    <Frame>
      <img src="/assets/mirror/app-store-03-choose-expo-project-4707fafffafe.png" alt="Диалоговое окно Launch с активным шагом Choose Expo project: поля Project name и Project slug предварительно заполнены значением Pace, кнопка Continue и вариант Use existing" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-03-choose-expo-project.png" />
    </Frame>
  </Step>

  <Step title="Войдите с помощью Apple">
    Подключите учётную запись Apple Developer, которой будет принадлежать карточка приложения в App Store. Учётная запись должна быть зарегистрирована в Apple Developer Program. Apple требует двухфакторную аутентификацию, поэтому будьте готовы подтвердить код на доверенном устройстве.

    <Frame>
      <img src="/assets/mirror/app-store-04-sign-in-apple-459dfc9b84ba.png" alt="Диалоговое окно Launch на шаге Sign in with Apple: поля для адреса электронной почты или номера телефона Apple Developer и пароля, а также примечание, что пароль не покидает устройство и учётная запись должна быть зарегистрирована в Apple Developer Program" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-04-sign-in-apple.png" />
    </Frame>
  </Step>

  <Step title="Выберите или создайте приложение Apple">
    Введите **App name** и **Bundle ID** (например, `app.replit.pace`), которые будут отображаться в App Store. Идентификатор пакета нельзя изменить после первой сборки, поэтому выбирайте его внимательно.

    <Frame>
      <img src="/assets/mirror/app-store-05-choose-apple-app-3971aea62f14.png" alt="Диалоговое окно Launch на шаге Choose Apple app: для App name указано Pace, Bundle ID — app.replit.pace, есть примечание, что идентификатор пакета нельзя изменить позднее, и кнопка Creating app…" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-05-choose-apple-app.png" />
    </Frame>
  </Step>

  <Step title="Запустите публикацию в App Store">
    Replit настроит сертификаты Apple от вашего имени. Когда станет активен шаг **Ready for launch**, нажмите **Launch to App Store**. Replit соберёт нативное приложение в облаке и отправит сборку в App Store Connect для бета-тестирования через TestFlight.

    <Frame>
      <img src="/assets/mirror/app-store-06-ready-for-launch-ab02659ee0bb.png" alt="Диалоговое окно Launch на шаге Ready for launch, где шаги Choose Apple app и Configure Apple certificates отмечены как завершённые, а также показаны кнопка Launch to App Store и Optional launch parameters" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-06-ready-for-launch.png" />
    </Frame>

    Когда Launch завершится, появится подтверждение, что приложение готовится к публикации в App Store. Затем диалоговое окно закроется, и вы вернётесь в Project Editor.

    <Frame>
      <img src="/assets/mirror/app-store-targeting-f8797071c9e0.png" alt="Диалоговое окно Launch, в котором шаг Targeting App Store отмечен как завершённый и показано сообщение Redirecting back to Replit — состояние подтверждения после выбора Launch to App Store" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-targeting.png" />
    </Frame>
  </Step>
</Steps>

<Note>
  Первая сборка версии обычно должна пройти бета-проверку TestFlight,
  прежде чем тестировщики смогут её установить. После одобрения последующие сборки, как правило,
  быстрее становятся доступны тестировщикам. Чтобы открыть доступ внешним тестировщикам или отправить приложение на
  публикацию в App Store, завершите оформление карточки в **App Store Connect** — полный контрольный список App Store Connect см. в разделе [Создание и запуск
  мобильного приложения](/ru/build/mobile-app).
</Note>
...
* Приложение открывается на вашем телефоне через Expo Go.
* Вы можете пройти основной мобильный сценарий.
* Текст, кнопки и навигация удобны на телефоне.
* Вы опубликовали мобильную версию для тестирования или поделились ею.
* Хотя бы один человек может попробовать её и оставить отзыв.
* (Необязательно) Вы отправили приложение в TestFlight через App Store.
...
<CardGroup cols={2}>
  <Card title="Добавьте вход" icon="user-lock" href="/ru/build/add-login">
    Дайте каждому пользователю возможность отслеживать свой прогресс.
  </Card>

  <Card title="Добавьте базу данных" icon="database" href="/ru/build/add-database">
    Сохраняйте историю пробежек после первого сеанса тестирования.
  </Card>
  ...
  <Card title="Создайте и запустите мобильное приложение" icon="mobile-screen" href="/ru/build/mobile-app">
...
* **Agent создаёт веб-приложение:** начните заново и перед сборкой выберите **Mobile app** в качестве типа приложения.
* **Предпросмотр в Expo Go не обновился:** встряхните телефон, откройте меню разработчика и выберите **Refresh**.
* **Области нажатия слишком малы:** попросите Agent увеличить их и разместить основные действия в зоне досягаемости большого пальца.
* **Навигация выглядит перегруженной:** попросите Agent сократить приложение до минимального числа экранов, необходимых для основного сценария.
* **Симулятор работает, а телефон — нет:** протестируйте поведение, зависящее от конкретного оборудования, на реальном устройстве и ознакомьтесь с [Native Mobile Apps](/ru/features/artifact-types/building-mobile-apps).
* **Не удаётся выполнить Publishing:** см. [Устранение неполадок мобильного приложения](/ru/features/troubleshooting/mobile-app).

<div id="try-it-now">
  ## Попробуйте прямо сейчас
</div>

<AgentInput highlightStart defaultPrompt="Создай Pace Mobile — трекер пробежек для телефона. Пользователь должен иметь возможность выбрать тип пробежки, указать дистанцию и время, сохранить пробежку и посмотреть сводку за сегодня. Сделай интерфейс удобным для использования одной рукой: с крупными областями нажатия, простой навигацией и стильным дизайном фитнес-приложения. Первая версия должна быть достаточно компактной, чтобы протестировать её уже сегодня в симуляторе и на реальном телефоне." showTypewriter={false} />

<div id="related">
  ## См. также
</div>

* [Нативные мобильные приложения](/ru/features/artifact-types/building-mobile-apps)
* [Создание и запуск мобильного приложения](/ru/build/mobile-app)
* [Vibe coding 101](/ru/learn/foundations/vibe-coding-101)
* [Устранение неполадок мобильного приложения](/ru/features/troubleshooting/mobile-app)