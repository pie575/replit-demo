---
title: "使用 Agent 构建"
description: "掌握与 Agent 协作的五个实用习惯：描述具体、规划工作、补充上下文、复查与测试，以及善用 检查点。"
...
Agent 是 Replit 的 AI 构建助手，也是帮你把想法变成软件的搭档。Agent 可以规划改动、编写代码、解释行为、调试问题并改进你的 app。

[Vibe coding 101](/zh/pt/learn/foundations/vibe-coding-101) 讲的是思维方式，本页则讲如何在实践中与 Agent 协作。[Effective prompting](/zh/pt/learn/effective-prompting) 说明如何写出更清晰的指令。

可以把 Agent 想象成场上的四分卫：它会研判局势、选定路线并推进工作。你则是教练：制定策略、下达战术、复盘结果并决定下一步怎么走。

Agent 能做的事很多，但由你主导整个过程时效果最好：描述要具体、先规划工作、补充上下文、审查并测试，并在需要回退时使用 检查点。
...
Agent 可以处理复杂的任务，但需要清晰的指引。

一份有效的请求应说明你想要什么、哪些方面最重要，以及哪些内容需要保持不变。你不必描述每个实现细节，但应为 Agent 提供足够的指导，让它能够做出恰当的决策。
...
<AiPrompt>
  改进注册页面，让访客在创建账户前了解产品的功能。<br />
  保留当前的配色方案和表单字段，并让主按钮更加醒目。
</AiPrompt>

第二个提示词为 Agent 提供了目标、重点和约束条件。

当你的请求包含多个部分时，请说明最重要的内容。

<AiPrompt>
  添加一个用于管理餐饮请求的简单管理员界面。<br />
  最重要的是能够查看新请求、将其标记为已联系，以及按状态筛选。<br />
  第一版请保持简单。
</AiPrompt>

Agent 可以将这项工作拆分为多个步骤，但你的指引能让它明确成功的标准。

如需查看更多具体指令示例，请参阅 [Effective prompting](/zh/pt/learn/effective-prompting)。

<div id="planeje-o-trabalho">
  ## 规划工作
</div>

当任务存在不确定性、涉及多个步骤、需要权衡取舍，或者你希望在文件被改动前先行确认时，做一份规划会很有帮助。

你可以在对话中让 Agent 制定一份计划：
...
如果你希望 Agent 先思考，并在修改文件前等待你的批准，请使用 [Plan mode](/zh/pt/features/agent/plan-mode)。发送前，在提示词输入框中启用 **Plan**。

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Replit 的初始提示词输入框，其中输入了一个运行应用的提示词，并勾选了 Plan 复选框" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Agent 思考完成后，你会看到一条 **task plan is ready for review** 横幅，上面带有 **Review now** 按钮。
...
上下文可帮助 Agent 了解哪些内容最重要。

你可以通过文本、屏幕截图、草图、文件、数据、错误信息或 Canvas 注释添加上下文。请选择最能清晰说明问题的形式。

当改动涉及行为、目标或限制条件时，请使用文本：

<AiPrompt>
  上下文：此 App 面向为孩子订购生日蛋糕的家长。<br />
  保持表单简洁，让自取信息一目了然，并避免任何显得过于企业化的内容。
</AiPrompt>

当布局难以用文字描述时，请使用屏幕截图或草图：

<AiPrompt>
  将此屏幕截图作为请求表单的布局参考。<br />
  保持整体结构一致，但保留现有字段和提交方式。
</AiPrompt>

当改动涉及视觉效果，并且你希望指出 App 的特定部分时，请使用 [Canvas](/zh/pt/design/canvas) 注释。

<Frame>
  <img src="/assets/mirror/agent-canvas-annotations-c88ab5856966.png" alt="Canvas 上的健身 App 画面，其中矩形、箭头和文本标签指向进度卡片，表单区域还有手绘草图和标签" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-canvas-annotations.png" />
</Frame>

<AiPrompt>
  使用我的 Canvas 注释更新首屏区域。<br />
  仅将这些备注应用于标题、图片和主按钮。<br />
  不要更改餐饮请求表单。
</AiPrompt>

良好的上下文还应包括不应更改的内容。这有助于 Agent 改进某个区域，而不会意外重写其他区域。

<div id="revise-e-teste">
  ## 审阅并测试
</div>

Agent 可以在构建前提出计划，并在完成后总结所做的更改。请仔细查看这两部分内容。

在任务计划卡片中选择 **View** 以打开计划。展开后的计划会显示 Agent 打算构建的内容、成功标准、范围外的内容，以及 Agent 将采取的步骤。

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="展开的任务计划，显示“内容和原因”、“完成”标准、“范围外”项目及编号构建步骤" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>
...
<AiPrompt>
  这个计划范围太大。<br />
  请将其缩减为我今天能测试的最小版本。<br />
  将其他所有内容移至未来改进列表。
</AiPrompt>

Agent 完成构建后，请亲自测试应用。打开 Preview，像目标用户一样使用它。
...
<AiPrompt>
  已经很接近了，但表单现在太长了。<br />
  保留新的视觉风格，移除可选备注字段，并让取件日期更易于浏览。
</AiPrompt>

当 App 比之前更糟、重要功能出现故障，或 Agent 的更改超出预期时，使用检查点。

在 Agent 面板中打开 **History** 查看检查点。每个检查点都会显示更改内容，并提供 **Rollback here** 和 **Changes** 控件。

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="Agent 的 History 面板显示一个标记为“Transitioned from Plan to Build mode”的检查点，其中包含 Rollback here 和 Changes 按钮" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

选择 **Rollback here** 后，Agent 会在执行回滚前确认将要恢复的内容。

<Frame>
  <img src="/assets/mirror/agent-checkpoints-rollback-dialog-28de278b44da.png" alt="Rollback to checkpoint 确认对话框列出目标检查点及将受影响的内容：文件、Agent 记忆、任务和数据库复选框" width="900" height="880" data-path="images/fitstart/work-with-agent/agent-checkpoints-rollback-dialog.png" />
</Frame>

如果您不确定应继续修复还是回滚，可以让 Agent 帮您比较。

<AiPrompt>
  上一次更改让 App 变得更糟。<br />
  请将当前版本与上一个正常运行的版本进行比较。<br />
  告诉我继续修复还是回滚更安全，并说明原因。
</AiPrompt>

走弯路很正常。检查点让您可以放心探索，同时保留返回更安全版本的能力。

如需了解更多恢复相关的指导，请参阅 [Checkpoints and rollbacks](/zh/pt/features/version-control/checkpoints-and-rollbacks)。

<div id="como-uma-boa-colaborao-com-agent">
  ## 与 Agent 良好协作是什么样的
</div>

如果符合以下这些情况，说明你的方向是对的：

* 你的请求说明了目标和约束条件
* Agent 的计划足够清晰，便于你审阅
* 当文字说不清楚时，你会补充截图、文件或 Canvas 中的备注
* 变更完成后，你会测试关键流程
* 出现问题时，你会给出有针对性的反馈
* 你会使用 检查点，而不是没完没了地去理清一个糟糕的变更
...
<CardGroup cols={2}>
  <Card icon="comment-dots" href="/zh/pt/learn/effective-prompting">
    学习如何编写更清晰的指令，让 Agent 能够放心执行。
  </Card>

  <Card title="构建你的第一个应用" icon="rocket" href="/zh/pt/build/your-first-app">
    从第一条 提示词 到有针对性的优化，练习与 Agent 协作。
  </Card>

  <Card title="检查点 与回滚" icon="rotate-left" href="/zh/pt/features/version-control/checkpoints-and-rollbacks">
    了解如何在需要时回到更安全的状态。
  </Card>
</CardGroup>