...
description: "Découvrez les états des tâches, le travail en file d'attente, l'application des modifications, l'archivage et l'annulation des tâches de l'Agent."
---
...
Chaque tâche de l’Agent suit un cycle de vie : planifiée, en cours d’exécution, prête à être révisée et terminée. Ce cycle de vie permet de maintenir le travail isolé jusqu’à ce que vous décidiez de l’intégrer à votre version principale.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Tableau des tâches montrant des tâches organisées dans les colonnes Brouillons, Actif, Prêt et Terminé" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>
...
Un **Brouillon** est une tâche que l’Agent a planifiée, mais qu’il n’a pas encore commencé à réaliser. Les brouillons peuvent être consultés sans risque, car ils ne contiennent que le travail proposé : le titre, la description et le plan de la tâche.

Utilisez les brouillons pour vérifier si l’Agent a compris l’objectif. Vous pouvez lancer la tâche, demander à l’Agent de revoir le plan ou archiver le brouillon pour plus tard.

<div id="ativo">
  ## Ativo
</div>

Une tâche au statut **actif** s’exécute en arrière-plan. Agent y travaille dans une copie isolée de votre projet afin que votre version principale reste inchangée pendant l’exécution de la tâche.

Vous pouvez continuer à utiliser le fil de discussion principal pendant l’exécution des tâches actives. Si une tâche nécessite un retour, ouvrez son fil de discussion et poursuivez la conversation depuis celui-ci.

<div id="na-fila">
  ## En file d&#39;attente
</div>

Une tâche **en file d&#39;attente** est approuvée, mais attend de démarrer. Une tâche peut être mise en file d&#39;attente lorsqu&#39;elle dépend d&#39;une autre tâche ou lorsque vous avez atteint la limite de tâches actives en arrière-plan de votre forfait.

Les tâches en file d&#39;attente démarrent automatiquement lorsque leurs prérequis sont remplis et qu&#39;un emplacement de tâche active se libère.

<div id="pronto">
  ## Pronto
</div>

Une tâche **Prête** est terminée, mais ses modifications n’ont pas encore été appliquées à votre version principale. Examinez le journal de travail de la tâche, les résultats des tests et l’aperçu avant d’appliquer les modifications.

Si le travail est correct, appliquez-le à la version principale. Sinon, abandonnez-le et démarrez une nouvelle tâche.

<div id="aplicando">
  ## Application
</div>

Une tâche **Application** intègre ses modifications à votre version principale. Agent résout automatiquement les conflits lors de l’application des modifications provenant de plusieurs tâches.

Attendez la fin de l’application avant d’évaluer le résultat final. Si quelque chose semble incorrect par la suite, demandez à Agent de le corriger dans le fil de discussion principal.

<div id="concludo">
  ## Terminé
</div>

Une tâche **Terminée** est finalisée et ne nécessite plus aucune action. Terminé peut signifier que la tâche a été intégrée à votre version principale, archivée avant de commencer ou annulée après le passage à l’étape de construction.

Utilisez la colonne **Terminé** comme historique des tâches terminées, archivées et annulées.

<div id="exemplos-de-estados">
  ## Exemples d’états
</div>

Chaque tâche passe par une série d’étapes, d’une idée à un élément de votre app. Chaque étape possède sa propre icône pour vous permettre de voir d’un coup d’œil ce qui se passe.

<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Ajouter un parcours cadeau pour la famille et les amis" description="Permettez aux parents de créer une histoire pour l'enfant d'une autre personne et de l'offrir en cadeau." status="draft-static" timestamp="2m" width="100%" />
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Suites d’histoires" description="L’ajout d’un bouton Continuer l’aventure génère une suite qui reprend là où la précédente s’est arrêtée." status="active" timestamp="12m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Actif</strong> : l’Agent est en train de construire cette tâche. Elle s’exécute dans sa propre copie de votre projet afin que votre version principale reste intacte.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Ajouter une fonctionnalité de cadeau pour la famille et les amis" description="Permettre aux parents de créer une histoire pour l'enfant de quelqu'un d'autre et de l'offrir en cadeau." status="active-queued" timestamp="1m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>En file d&#39;attente</strong> : Cette tâche est approuvée, mais attend qu&#39;un siège se libère avant de pouvoir démarrer.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Pronto</strong> : Cette tâche est terminée. Vous pouvez examiner les modifications et les appliquer à votre version principale quand vous le souhaitez.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Liens partageables avec aperçus OG" description="L’ajout de liens partageables publics est le principal levier de croissance." status="applying" timestamp="31m" width="100%" />
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Lecture à voix haute avec surlignage des mots" description="La synthèse vocale permet aux enfants d'écouter des histoires en toute autonomie tout en suivant les mots surlignés." status="done" timestamp="4h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Conclué</strong> : Cette tâche est finalisée et ses modifications font partie de votre version principale.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Visite guidée de prise en main pour les nouveaux lecteurs" description="Une courte visite guidée qui présente aux parents le parcours de lecture principal lors de la première ouverture." status="draft-static" timestamp="1h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Archivé</strong> : une session de planification que vous avez mise de côté avant de commencer à développer. Les sessions de planification archivées peuvent être restaurées ultérieurement dans la colonne <strong>Concluído</strong>.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Générateur expérimental de packs d'autocollants" description="Générez des autocollants illustrés personnalisés à partir des personnages et des scènes d'une histoire." status="done" timestamp="2h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Annulée</strong> : une tâche que vous avez interrompue après son passage à l’étape de construction. Les tâches annulées ne peuvent pas être restaurées. Démarrez une nouvelle tâche si vous changez d’avis.</span>
  </div>
</div>
...
Vous pouvez interrompre une tâche à tout moment de son cycle de vie, mais l’option disponible dépend de l’étape où elle se trouve dans le tableau. **Archiver** et **Annuler** déplacent tous deux une tâche vers la colonne **Terminé**, mais ils fonctionnent différemment :

| Action       | Quand elle est disponible                                                                                                               | Ce qui se passe                                                                                                           | Réversible ?                                                                                                                 |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **Archiver** | Tant que la tâche est encore une session de planification — un **Brouillon** dont la construction n’a pas commencé                      | La session de planification est mise de côté sans être exécutée. Son plan est conservé.                                   | Oui. Ouvrez la carte archivée dans la colonne **Terminé** et restaurez-la dans **Brouillons**.                               |
| **Annuler**  | Une fois que la tâche est passée de la planification à la construction — toute tâche **Active**, **en file d&#39;attente** ou **Prête** | Agent arrête la tâche et abandonne tout travail en cours. Aucune modification n’est appliquée à votre version principale. | Non. Les tâches annulées ne peuvent pas être restaurées. Démarrez une nouvelle tâche si vous souhaitez reprendre le travail. |

Utilisez **Archiver** pour les sessions de planification que vous souhaitez reprendre ultérieurement. Utilisez **Annuler** comme bouton d’arrêt définitif pour les tâches en file d’attente, déjà en cours d’exécution ou dont la construction est terminée.

Vous trouverez les sessions de planification archivées et les tâches annulées dans la colonne **Terminé**, à l’extrémité droite du tableau de tâches, avec les tâches terminées.