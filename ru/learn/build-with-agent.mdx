---
title: "Создание приложений с Agent"
description: "Освойте пять практичных привычек работы с Agent: формулируйте задачи конкретно, планируйте работу, добавляйте контекст, проверяйте и тестируйте, используйте checkpoints."
sidebarTitle: "Создание с Agent"
icon: "robot"
---
...
Agent — это ИИ-разработчик от Replit и ваш помощник в превращении идей в готовые продукты. Agent умеет планировать изменения, писать код, объяснять поведение приложения, находить и устранять ошибки и улучшать ваше приложение.

Страница [Vibe coding 101](/ru/learn/foundations/vibe-coding-101) объясняет сам подход, а эта — как работать с Agent на практике. В разделе [Эффективные промпты](/ru/learn/effective-prompting) рассказано, как формулировать более понятные инструкции.

Представьте, что Agent — это квотербек на поле: он оценивает ситуацию, выбирает путь и двигает работу вперёд. Вы — тренер: определяете стратегию, назначаете розыгрыш, разбираете результат и решаете, каким будет следующий шаг.

Agent способен на многое, но лучше всего он работает, когда процессом руководите вы: формулируйте конкретно, планируйте работу, добавляйте контекст, проверяйте и тестируйте, а также используйте checkpoints, когда нужно вернуться назад.

<div id="five-tips-for-building-with-agent">
  ## Пять советов по созданию приложений с Agent
</div>

Придерживайтесь этих правил в работе с Agent:

1. Формулируйте конкретно.
2. Планируйте работу.
3. Добавляйте контекст.
4. Проверяйте и тестируйте.
5. Используйте checkpoints.

<div id="be-specific">
  ## Формулируйте конкретно
</div>

Agent справляется со сложными задачами, но ему нужны чёткие указания.

Хороший запрос объясняет, чего вы хотите, что для вас важно и что должно остаться без изменений. Описывать каждую деталь реализации не нужно, но указаний должно хватать, чтобы Agent мог принимать верные решения.

Вместо:

<AiPrompt>Сделай моё приложение лучше.</AiPrompt>

Попробуйте:

<AiPrompt>
  Улучши страницу регистрации, чтобы посетители понимали, что делает продукт, ещё
  до создания учётной записи.

  <br />

  Сохрани текущую цветовую палитру, оставь поля формы прежними и сделай
  основную кнопку заметнее.
</AiPrompt>

Второй prompt задаёт Agent цель, область работы и ограничения.

Если запрос состоит из нескольких частей, укажите, что важнее всего.

<AiPrompt>
  Добавь простой раздел администратора для управления заявками на кейтеринг.

  <br />

  Самое важное — видеть новые заявки, отмечать их как обработанные и
  фильтровать по статусу.

  <br />

  Первую версию сделай простой.
</AiPrompt>

Agent может разбить эту работу на шаги, но именно ваши указания определяют, каким должен быть результат.

Больше примеров конкретных инструкций смотрите в разделе [Эффективные prompt](/ru/learn/effective-prompting).

<div id="plan-the-work">
  ## Планирование работы
</div>

Планирование полезно, когда в задаче есть неопределённость, несколько этапов, компромиссы или что-то, что вы хотите утвердить до того, как изменятся файлы.

Вы можете попросить Agent составить план прямо в Conversation:

<AiPrompt>
  Прежде чем вносить изменения, составь план по добавлению онлайн-заказов.

  <br />

  Включи пользовательский сценарий, необходимые данные, страницы или компоненты,
  которые изменятся, риски и то, как мне это протестировать.
</AiPrompt>

Используйте [режим Plan](/ru/features/agent/plan-mode), когда хотите, чтобы Agent сначала всё обдумал и дождался вашего подтверждения, прежде чем менять файлы. Перед отправкой включите переключатель **Plan** в поле ввода промпта.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Поле ввода промпта на главной странице Replit с введённым промптом о работающем приложении и включённым флажком Plan" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>

<AgentInput defaultPrompt="Прежде чем вносить изменения, составь план по добавлению онлайн-заказов. Включи пользовательский сценарий, необходимые данные, страницы или компоненты, которые изменятся, риски и то, как мне это протестировать." highlightPlan={true} showTypewriter={false} />

Когда Agent закончит обдумывание, вы увидите баннер **task plan is ready for review** с кнопкой **Review now**.

<Frame>
  <img src="/assets/mirror/agent-plan-ready-banner-5fbcf58a1668.png" alt="Баннер о том, что план задачи готов к проверке, с кнопкой Review now внизу панели Agent" width="560" height="550" data-path="images/fitstart/work-with-agent/agent-plan-ready-banner.png" />
</Frame>
...
Контекст помогает Agent понять, что действительно важно.

Контекст можно передать текстом, скриншотами, набросками, файлами, данными, сообщениями об ошибках или аннотациями на Canvas. Выбирайте тот формат, который лучше всего объясняет задачу.

Используйте текст, когда изменение касается поведения, целей или ограничений:

<AiPrompt>
  Контекст: это приложение для родителей, которые заказывают торты на день рождения.

  <br />

  Форма должна быть короткой, детали самовывоза — очевидными; избегай всего, что
  выглядит слишком по-корпоративному.
</AiPrompt>

Используйте скриншоты или наброски, когда раскладку сложно описать словами:

<AiPrompt>
  Используй этот скриншот как ориентир по раскладке для формы заявки.

  <br />

  Повтори общую структуру, но сохрани существующие поля и поведение при
  отправке.
</AiPrompt>

Используйте аннотации на [Canvas](/ru/design/canvas), когда изменение визуальное и нужно указать на конкретную часть приложения.

<Frame>
  <img src="/assets/mirror/agent-canvas-annotations-c88ab5856966.png" alt="Frame фитнес-приложения на Canvas с прямоугольником, стрелкой и текстовой подписью, указывающими на карточку прогресса, а также с рисунком пером и подписью в области формы" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-canvas-annotations.png" />
</Frame>

<AiPrompt>
  Используй мои аннотации на Canvas, чтобы обновить главный раздел.

  <br />

  Примени заметки только к заголовку, изображению и основной кнопке.

  <br />

  Не меняй форму заявки на кейтеринг.
</AiPrompt>

Хороший контекст включает и то, что менять не нужно. Так Agent доработает одну область, случайно не переписав другую.

<div id="review-and-test">
  ## Проверка и тестирование
</div>

Agent может предложить план перед началом работы, а по завершении — подвести итог изменений. Прочитайте и то, и другое.

Откройте план, нажав **View** на карточке плана задачи. В развёрнутом виде план показывает, что Agent намерен создать, критерии успеха, что не входит в задачу и какие шаги Agent выполнит.

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="Развёрнутый план задачи с разделами «Что и почему», критериями завершённости, пунктами вне задачи и пронумерованными шагами сборки" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>

Перед тем как Agent начнёт работу, изучите план:

* Решает ли он нужную задачу?
* Не упущено ли что-то?
* Нет ли лишнего?
* Можно ли сначала сделать более компактную версию?
* Объясняет ли план, как проверить результат?

Если план слишком широкий, попросите Agent сузить его.
...
Когда Agent закончит, протестируйте приложение самостоятельно. Откройте Preview и поработайте с приложением так, как это делал бы тот, для кого оно создано.

Проверьте:

* Может ли человек выполнить основное действие?
* Произошло ли запрошенное изменение?
* Работает ли приложение на мобильных устройствах?
* Не сломалось ли что-то важное?
* Не изменил ли Agent что-то постороннее?
* Достаточно ли хорош результат, чтобы развивать его дальше?

Если что-то не так, опишите, что вы увидели.

Вместо:

<AiPrompt>Fix the form.</AiPrompt>

Попробуйте:

<AiPrompt>
  When I submit the catering request form, the success message appears, but the
  form still shows the old text.
...
Конкретная обратная связь помогает Agent исправить именно то, что нужно.

<div id="use-checkpoints">
  ## Используйте checkpoint&#39;ы
</div>

Checkpoint&#39;ы помогают восстановиться, если изменение пошло не в том направлении.

Если результат уже близок к нужному, дайте точечную обратную связь:

<AiPrompt>
  Уже близко, но форма стала слишком длинной.

  <br />

  Сохрани новый визуальный стиль, убери необязательное поле для заметок и сделай
  дату самовывоза более заметной.
</AiPrompt>

Используйте checkpoint, если приложение стало хуже, чем было, сломалось важное поведение или Agent изменил больше, чем ожидалось.

Откройте **History** на панели Agent, чтобы увидеть checkpoint&#39;ы. Для каждого checkpoint&#39;а показано, что изменилось, а также кнопки **Rollback here** и **Changes**.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="Панель Agent History с checkpoint'ом «Transitioned from Plan to Build mode» и кнопками Rollback here и Changes" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

Когда вы выбираете **Rollback here**, Agent показывает, что именно будет восстановлено, и просит подтвердить откат.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-rollback-dialog-28de278b44da.png" alt="Диалог подтверждения Rollback to checkpoint со списком целевого checkpoint'а и того, что будет затронуто: файлы, память Agent, задачи и флажок Database" width="900" height="880" data-path="images/fitstart/work-with-agent/agent-checkpoints-rollback-dialog.png" />
</Frame>

Если вы не уверены, исправлять дальше или откатиться, попросите Agent помочь со сравнением.

<AiPrompt>
  Из-за последнего изменения приложение стало хуже.

  <br />

  Сравни текущую версию с предыдущей рабочей версией.

  <br />

  Скажи, что безопаснее: исправлять дальше или откатиться, и объясни почему.
</AiPrompt>

Неудачные попытки — это нормально. Checkpoint&#39;ы позволяют экспериментировать, не теряя возможности вернуться к более надёжной версии.

Больше рекомендаций по восстановлению см. в разделе [Checkpoint&#39;ы и откаты](/ru/features/version-control/checkpoints-and-rollbacks).

<div id="what-good-agent-collaboration-feels-like">
  ## Каким должно быть хорошее взаимодействие с Agent
</div>

Вы на верном пути, если:

* В запросах вы объясняете цель и ограничения
* План Agent достаточно понятен, чтобы его можно было оценить
* Вы добавляете скриншоты, файлы или заметки в Canvas, когда слов недостаточно
* Вы проверяете важные сценарии после изменений
* Вы даёте конкретную обратную связь, когда что-то пошло не так
* Вы используете checkpoints, а не пытаетесь бесконечно распутывать неудачное изменение
...
<CardGroup cols={2}>
  <Card icon="comment-dots" href="/ru/learn/effective-prompting">
    Узнайте, как формулировать более понятные инструкции, по которым Agent сможет уверенно действовать.
  </Card>

  <Card title="Создайте своё первое приложение" icon="rocket" href="/ru/build/your-first-app">
    Попрактикуйтесь в работе с Agent: от первого промпта до точечной
    доработки.
  </Card>

  <Card title="Checkpoints и откаты" icon="rotate-left" href="/ru/features/version-control/checkpoints-and-rollbacks">
    Узнайте, как при необходимости вернуться к более надёжному состоянию.
  </Card>
</CardGroup>