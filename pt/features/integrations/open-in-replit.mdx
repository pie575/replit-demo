---
title: "Abrir no Replit"
description: "Aprenda a criar um botão Open in Replit que permite aos visitantes começar a criar a partir do seu app ou site."
---
...
<Frame caption="Clicar neste selo abre o Replit automaticamente e preenche o prompt e as configurações.">
...
| ---------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `stack`    | string | O modo de construção a ser usado. Deve ser `Design` ou `Build`. `Design` abre o [Canvas](/pt/design/canvas) para prototipagem visual. `Build` abre o [Agent](/pt/features/agent/overview) para desenvolvimento completo de aplicativos. | Obrigatório          |
| `prompt`   | string | Texto descrevendo o aplicativo a ser construído, **compactado usando [LZ-string](https://pieroxy.net/blog/pages/lz-string/index.html)**. O prompt descompactado pode ter até 50.000 caracteres.                                         | Obrigatório          |
| `referrer` | string | O nome ou a URL da página que contém este link. Usado para atribuição.                                                                                                                                                                  | Opcional             |

<Note>
  O parâmetro `prompt` deve ser compactado usando compressão LZ-string antes de
  ser adicionado à URL. Isso ajuda a manter as URLs dentro dos limites de tamanho do navegador
  e a garantir que sejam interpretadas corretamente.
</Note>

### Construtor de link interativo

Use a ferramenta abaixo para gerar seu link **Open in Replit**. Digite seu prompt, selecione o modo stack e o link será gerado automaticamente com a compressão LZ-string aplicada.

<CreateUrlLinkBuilder />

### Compactando prompts

Para criar um link **Open in Replit** válido, você deve compactar seu prompt usando compressão LZ-string. Veja como:
...
<Tip>
  Use `compressToEncodedURIComponent` para garantir que a string compactada seja
  segura para uso em URLs.
</Tip>
...
**Build Mode com prompt detalhado**
...
Use `stack=Design` para abrir seu prompt no [Canvas](/pt/design/canvas), que é otimizado para:
...
Use `stack=Build` para abrir seu prompt no [Agent](/pt/features/agent/overview), que é otimizado para:
...
* Mantenha os prompts concisos e focados nos recursos principais para obter melhores resultados de geração.
* Use linguagem clara e descritiva que explique o que você quer construir.
* Escolha o modo `stack` apropriado para seu caso de uso (Design para UI/UX, Build para apps completos).
* Teste suas URLs compactadas para garantir que sejam interpretadas corretamente antes de compartilhá-las.
* Considere o nível técnico do seu público ao elaborar prompts.

### Autenticação do usuário

Independentemente do seu estado de autenticação no Replit, você precisará fornecer uma entrada antes de criar o app. Geralmente, isso significa enviar o prompt.

### Tratamento de erros

Se a URL estiver malformada ou os parâmetros forem inválidos, o Replit não preencherá o prompt. As possíveis causas incluem:
...
<Warning>
  Os links **Open in Replit** podem ser usados para criar apps com comportamento não intencional
  ou vulnerabilidades de segurança. Clique apenas em links de fontes confiáveis.
</Warning>

Ao compartilhar links **Open in Replit**:

* Revise os prompts cuidadosamente antes de compartilhá-los publicamente
* Considere as implicações de segurança dos apps que estão sendo gerados
* Evite incluir informações sensíveis nos prompts
* Seja transparente sobre o que o link vai criar

## Recursos relacionados

* [Documentação do Canvas](/pt/design/canvas)
* [Documentação do Agent](/pt/features/agent/overview)
* [Integrações de IA do Replit](/pt/features/integrations/overview)
