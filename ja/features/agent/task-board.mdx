---
title: "タスクボード"
description: "タスクボードを使用して、各ステージを進むAgentのタスクを確認、開始、監視、適用します。"
---
...
タスクボードは、Agentのタスクを確認、開始、監視、適用するための一元的な場所です。Agentが大きなリクエストを複数のパーツに分割するとき、バックグラウンドで作業を実行したいとき、またはプロジェクトで何が変更済みかを把握したいときに使用します。

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="ドラフト、アクティブ、準備完了、完了の各列にタスクが整理されたタスクボード" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

タスクはステータスに基づいて列に整理されます。ボードには、Agentが計画したもの、実行中のもの、レビュー待ちのもの、すでに適用されたものが表示されます。

各タスクは進行に応じて左から右へ移動します：

* **ドラフト**：Agentが計画したが、まだ開始していないタスク
* **アクティブ**：Agentが現在構築中のタスク（アクティブタスク制限の後ろにキューされているタスクを含む）
* **準備完了**：Agentが完了し、レビューの準備ができているタスク
* **完了**：終了、アーカイブ、またはキャンセルされたタスク

## ドラフト

**ドラフト**は、Agentがまだ構築を開始していない計画済みタスクです。各ドラフトにはタイトル、説明、および段階的な計画が含まれます。

ドラフトを使用して、作業が開始される前にAgentの提案アプローチを確認します。プランを確認し、Agentに調整を依頼するか、準備ができたらタスクを開始できます。

ドラフトは、バックグラウンド作業を確定する前に大きな機能を計画したいときに役立ちます。例えば、Agentはオンボーディングプロジェクトを、認証、プロフィール設定、ダッシュボードUI、デプロイの仕上げの個別ドラフトに分割することがあります。

## アクティブ

タスクを承認すると、**アクティブ**列に移動し、Agentが構築を開始します。各タスクはプロジェクトの独立したコピーで実行されるため、作業を適用するまでメインバージョンは変更されません。

タスクによっては他のタスクに依存することがあります。例えば、ダッシュボードを構築するタスクは、データベーススキーマのタスクが先に完了する必要がある場合があります。依存するタスクは、前提条件が完了するまで**キュー済み**として待機します。プランのアクティブなバックグラウンドタスクの制限に達した場合もタスクはキューに入ります。

<KanbanColumns
  columns={[
{ title: 'ドラフト', count: 2, cards: [
{ title: '家族や友人へのギフトフローを追加', description: '親が他の子供のためにストーリーを作成して贈り物として送れるようにします。新しい獲得チャンネルが開きます。', status: 'draft-static', timestamp: '2m' },
{ title: 'ダークテーマを使った就寝前読書モード', description: '暖色系ダークテーマとアンビエントサウンドを備えた専用の就寝モードで、アプリを完全な就寝前の儀式にします。', status: 'draft-static', timestamp: '2m' },
] },
{ title: 'アクティブ', count: 3, cards: [
{ title: 'ストーリーの続き：続編', description: '「冒険を続ける」ボタンを追加すると、前回の終わりから続く続編が生成されます。', status: 'active', timestamp: '12m' },
{ title: 'ストーリーの再生成：ページのやり直し', description: '個々のページを再生成する機能を追加することで、親がコントロールでき、フラストレーションが軽減されます。', status: 'active', timestamp: '8m' },
{ title: '家族や友人へのギフトフローを追加', description: '親が他の子供のためにストーリーを作成して贈り物として送れるようにします。', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
タスクの構築が完了すると、**準備完了**に移動します。これは作業が完成したことを意味しますが、メインバージョンにはまだ何も変更されていません。

変更を適用する方法は2つあります：

* **スレッドから**：タスクのスレッドを開き、**メインバージョンに変更を適用**を選択して作業をプロジェクトに取り込むか、**却下**を選択して破棄します
* **ボードから**：準備完了のタスクカードの3点メニューを選択し、**変更を適用**を選択します

<TaskReviewDrawer />

変更を適用する前に準備完了のタスクを確認します。作業ログ、テスト出力、プレビューを確認して、Agentが何を変更したかを把握します。

## 完了

適用されると、タスクカードは**完了**列に移動します。

完了列には、アーカイブされた計画セッションやキャンセルされたタスクも含まれます。これにより、完了した作業をドラフトやアクティブに混ぜることなく、何が起きたかの履歴を確認できます。

<KanbanColumns
  columns={[
{ title: '準備完了', count: 3, cards: [
{ title: 'ダークテーマを使った就寝前読書モード', description: '暖色系ダークテーマとアンビエントサウンドを備えた専用の就寝モード。', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: 'OGプレビュー付きの共有可能なストーリーリンク', description: '公開の共有可能なリンクの追加は、最大の成長レバーです。', status: 'ready-drop', timestamp: '31m', showMenu: true },
{ title: 'キャラクターカスタマイズの強化', description: '外見オプションの追加により、イラストが本当にパーソナライズされた感じになります。', status: 'ready-drop', timestamp: '28m', showMenu: true },
] },
{ title: '完了', count: 4, cards: [
{ title: '単語ハイライト付きの読み上げモード', description: 'テキスト読み上げにより、子供たちが独立してストーリーを楽しめます。', status: 'done', timestamp: '4h' },
{ title: 'OGプレビュー付きの共有可能なストーリーリンク', description: '最大の成長レバーとしての公開共有リンク。', status: 'done', timestamp: '1h' },
{ title: 'キャラクターカスタマイズの強化', description: '真にパーソナライズされたイラストのための外見オプション。', status: 'done', timestamp: '4h' },
{ title: 'モバイルレスポンシブと本番環境の仕上げ', description: 'より良いエラー状態、モバイルレイアウト、読み込みアニメーション。', status: 'done', timestamp: '4h' },
] }
]}
/>

複数のタスクから変更を適用する際、Agentは自動的に競合を処理します。何かおかしいと思ったら、Agentに修正を依頼してください。

## タスクの検索とフィルタリング

プロジェクトに多くのタスクが進行中の場合は、タスクボードを使用します。タスクのタイトルまたはタスク番号で検索し、ドラフトをフィルタリングし、編集モードを使用してタスクのグループをまとめて管理できます。

例えば、スレッド、レビュー、またはSlackの会話でタスクが参照された場合はタスク番号で検索します。アクティブや完了したタスクをスキャンせずに計画済みの作業に集中したい場合はドラフトをフィルタリングします。

推奨されるフォローアップについては、[フォローアップタスク](/ja/features/agent/follow-up-tasks)を参照してください。

## タスクの名前を変更する

長期実行タスクリストは、スコープが変わると見づらくなることがあります。タスクの名前を変更するには、カード（タスクリストサイドバーまたはタスクボード上）の3点メニューを開き、**名前を変更**を選択します。より明確なタイトルを入力し、**保存**を選択します。

タスクの名前変更は、あなたとチームメイトへの表示方法のみ変更します。Agentがすでに行った作業、タスクの履歴、および保留中の変更には影響しません。

## 各タスクの設定

各タスクには、タイトルの横に独自の設定ドロップダウンがあります。開くと、Agentがタスクを完了する方法と、結果がプロジェクトにどのように反映されるかを制御できます：

* **変更を自動適用**（ドラフトとアクティブ）：タスクが準備完了になったら、レビューを待たずにすぐにメインバージョンに適用します。
* **プランを自動承認**（ドラフト）：Agentがタスクのプランを提案したとき、自動的に承認して構築を開始します。
* **変更を適用**（準備完了）：完成した作業をメインバージョンに適用します。
* **名前を変更**：カードに表示されるタイトルを変更します。
* **変更を確認**（準備完了）：準備完了のタスクの差分に直接ジャンプします。
* **キャンセル**：タスクを停止するかドラフトを削除します。

自動適用と自動承認は、オンのままにするトグルではなく、一度選択するクイックアクションです。メニューは固定幅を保つため、異なるタスクを開いてもラベルがずれません。

<Frame>
  <img src="/assets/mirror/task-settings-dropdown-7ea7868303b4.png" alt="タスクカードの設定ドロップダウンが開いており、変更を適用、名前を変更、変更を確認、キャンセルのアクションが固定幅のポップオーバーに表示されている" width="858" height="536" data-path="images/replitai/task-settings-dropdown.png" />
</Frame>
