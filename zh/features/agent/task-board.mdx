---
title: "任务看板"
description: "使用任务看板查看、启动和监控 Agent 任务，并在各个阶段推进时应用这些任务。"
sidebarTitle: "任务看板"
icon: "table-columns"
---
...
任务看板让你在同一处审阅、启动、监控和应用 Agent 任务。当 Agent 将一个较大的请求拆分为多个部分时、当你希望让工作在后台运行时，或者当你需要了解 project 中已经发生了哪些改动时，都可以使用它。

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="任务看板中的任务按 草稿、进行中、已就绪 和 Done 各列组织显示" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

任务按状态分列组织。看板会显示 Agent 已规划的内容、正在运行的内容、可供审阅的内容，以及已经应用的内容。

每个任务会随着进展从左向右移动：

* **草稿**：Agent 已规划但尚未开始的任务
* **进行中**：Agent 正在构建的任务，包括因活动任务数量上限而排队等待的任务
* **已就绪**：Agent 已完成、等待你审阅的任务
* **Done**：已完成、已归档或已取消的任务
...
**草稿**是 Agent 尚未开始构建的计划任务。每个草稿都包含标题、描述和分步计划。

在动手之前，你可以通过草稿审阅 Agent 提出的实现思路：查看计划、要求 Agent 进行调整，或在准备就绪后启动该任务。

如果想在投入后台工作前先规划一项较大的功能，草稿会很有帮助。例如，Agent 可能会将一个用户引导 project 拆分为多个独立草稿，分别对应身份验证、个人资料设置、仪表板界面和部署优化。

<div id="active">
  ## 进行中
</div>

接受任务后，它会移入 **进行中** 列，Agent 随即开始构建。每个任务都在 project 的独立副本中运行，因此在你应用这些改动之前，主版本不会发生任何变化。

有些任务依赖于其他任务。例如，构建仪表板的任务可能需要先完成数据库架构任务。存在依赖关系的任务会保持 **Queued** 状态，直到其前置任务完成。如果达到套餐的后台活动任务数量上限，任务同样会进入队列。

<KanbanColumns
  columns={[
{ title: '草稿', count: 2, cards: [
{ title: '为家人和朋友添加赠送流程', description: '让家长为别人的孩子创建故事并作为礼物送出，开辟新的获客渠道。', status: 'draft-static', timestamp: '2m' },
{ title: '深色主题的睡前阅读模式', description: '专门的睡前模式，配以暖色调深色主题和环境音效，让应用成为完整的睡前仪式。', status: 'draft-static', timestamp: '2m' },
] },
{ title: '进行中', count: 3, cards: [
{ title: '故事续写：续集', description: '添加“继续冒险”按钮，生成承接上一段结尾的续集。', status: 'active', timestamp: '12m' },
{ title: '故事重新生成：重做页面', description: '支持重新生成单个页面，让家长掌握控制权，减少挫败感。', status: 'active', timestamp: '8m' },
{ title: '为家人和朋友添加赠送流程', description: '让家长为别人的孩子创建故事并作为礼物送出。', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
任务构建完成后，会进入 **已就绪** 状态。这表示工作已完成，但主版本中尚未发生任何改动。

你可以通过两种方式应用更改：

* **在线程中**：打开该任务的线程，选择 **Apply changes to main version** 将成果合并到 project 中，或选择 **Dismiss** 放弃该成果
* **在看板中**：在任意 已就绪 任务卡片上打开三点菜单，然后选择 **Apply changes**

<TaskReviewDrawer />

应用之前，请先审阅处于 已就绪 状态的任务。查看工作日志、测试输出和 Preview，了解 Agent 具体改动了哪些内容。
...
Done 列还包含已归档的规划会话和已取消的任务，让你可以回顾过往的处理记录，同时避免已完成的工作重新混入 Drafts 或 Active。

<KanbanColumns
  columns={[
{ title: '已就绪', count: 3, cards: [
{ title: '带深色主题的睡前阅读模式', description: '专门的睡前模式，采用暖色调深色主题并配有环境音效。', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: '带 OG 预览的可分享故事链接', description: '添加公开分享链接是最有力的增长手段。', status: 'ready-drop', timestamp: '31m', showMenu: true },
{ title: '增强的角色自定义', description: '增加外观选项，让插图真正个性化。', status: 'ready-drop', timestamp: '28m', showMenu: true },
] },
{ title: 'Done', count: 4, cards: [
{ title: '带单词高亮的朗读模式', description: '文本转语音让孩子可以自己享受故事。', status: 'done', timestamp: '4h' },
{ title: '带 OG 预览的可分享故事链接', description: '公开分享链接是最有力的增长手段。', status: 'done', timestamp: '1h' },
{ title: '增强的角色自定义', description: '通过外观选项打造真正个性化的插图。', status: 'done', timestamp: '4h' },
{ title: '移动端适配与上线前打磨', description: '更完善的错误状态、移动端布局和加载动画。', status: 'done', timestamp: '4h' },
] }
]}
/>

当你应用来自多个任务的更改时，Agent 会自动处理冲突。如果发现哪里不对，可以让 Agent 修复。

<div id="find-and-filter-tasks">
  ## 查找和筛选任务
</div>

当 project 中同时进行的任务较多时，可以使用任务看板。你可以按任务标题或任务编号搜索、筛选草稿，也可以使用编辑模式批量管理多个任务。

例如，当有人在讨论串、评审或 Slack 对话中提到某个任务时，可以按任务编号搜索；当你想专注于计划中的工作，而不必逐一查看进行中或已完成的任务时，可以筛选草稿。

关于建议的后续事项，请参阅[后续任务](/zh/features/agent/follow-up-tasks)。

<div id="rename-a-task">
  ## 重命名任务
</div>

随着工作范围的变化，冗长的任务列表会越来越难以快速浏览。要重命名任务，请打开任务卡片上的三点菜单 (位于任务列表侧边栏或任务看板中) ，然后选择 **Rename**。输入更清晰的标题，再选择 **Save**。

重命名任务只会改变它在你和团队成员眼中的显示方式。Agent 已完成的工作、任务的历史记录以及任何待处理的更改都不会受到影响。

<div id="settings-on-each-task">
  ## 每个任务的设置
</div>

每个任务的标题旁都有各自的设置下拉菜单。打开它即可控制 Agent 如何完成该任务，以及结果如何落入你的 project：

* **Auto-apply changes** (草稿和进行中) ：任务就绪后立即应用到主版本，无需等待审核。
* **Auto-approve plan** (草稿) ：当 Agent 为该任务提出方案时，自动接受并开始构建。
* **Apply changes** (已就绪) ：将完成的工作应用到主版本。
* **Rename**：更改卡片上显示的标题。
* **Review changes** (已就绪) ：直接跳转到已就绪任务的差异对比。
* **Cancel**：停止该任务或删除草稿。

自动应用和自动批准是只需选择一次的快捷操作，而不是可以一直开着的开关。菜单宽度保持一致，因此打开不同任务时标签不会移位。

<Frame>
  <img src="/assets/mirror/task-settings-dropdown-7ea7868303b4.png" alt="任务卡片上打开的任务设置下拉菜单，在固定宽度的弹出框中显示 Apply changes、Rename、Review changes 和 Cancel 操作" width="858" height="536" data-path="images/replitai/task-settings-dropdown.png" />
</Frame>