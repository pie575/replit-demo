...
现在你已了解集成，不妨将其用于咖啡店的开业筹备。

你已经选定方向：打造一家**社区咖啡馆**，而不是面向车站通勤者的酒吧。现在，让我们向
Replit 提供将这一方向转化为可执行计划所需的材料。

集成可将 Replit 连接到你已在使用的工具和文件。这样，Replit 就能获取相关信息，
并执行完成任务所需的操作。你可以连接超过 450 项服务，包括
**[Slack](/zh/ja/features/integrations/overview#connectors)**、
**[Google Drive](/zh/ja/features/integrations/overview#connectors)**、
**[Notion](/zh/ja/features/integrations/overview#connectors)**、
**[Salesforce](/zh/ja/features/integrations/overview#connectors)**等。

对于这家咖啡店，使用 Google Drive 可让 Replit 访问预算、选址相关
备注、菜单创意、照片和视觉参考资料。Notion 则提供了一个集中记录
开业计划、已作决定和待解决问题的地方。

详细了解如何[连接你的工具](/zh/ja/chat/integrate-your-tools)。

<Steps>
  <Step title="请 Replit 整理素材">
    请先发送以下请求。Replit 会确认完成计划所需的内容，
    并在需要访问权限时请求连接相应服务。

    <AiPrompt>
      请使用我咖啡馆的文件，为一家社区咖啡馆制定首月开业计划。
      更新相关假设，然后将概念、关键决策、
      待解决问题和开业检查清单记录到 Notion。
    </AiPrompt>
  </Step>

  <Step title="Replit 请求时连接 Google Drive">
    授权连接 Google Drive，以便 Replit 查看需要使用的素材。
    Replit 会直接在对话中打开连接请求。

    <CoffeeShopScreenshotFrame src="/assets/mirror/connect-google-drive-f7951617b078.png" alt="Replit 请求连接 Google Drive 以查看咖啡馆文件" />
  </Step>

  <Step title="Replit 需要记录计划时连接 Notion">
    当 Replit 请求创建开业简报时，授权连接 Notion。
    这样，您就可以将概念、决策、待解决问题和检查清单集中
    记录在一个地方。
  </Step>
</Steps>

Google Drive 为 Replit 提供开展工作所依据的资料。Notion 则将不断完善的计划集中在一处，
方便您回答新问题并作出决策。

详细了解[添加上下文](/zh/ja/chat/add-context)、[上传文件](/zh/ja/chat/upload-and-use-files)、
[集成工具](/zh/ja/chat/integrate-your-tools)。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/ja/home/watch-the-competition">
    ...
    <div className="chat-card-title">持续关注竞争对手</div>
    <p>在准备发布的同时，设置例行任务以持续监测变化。</p>
