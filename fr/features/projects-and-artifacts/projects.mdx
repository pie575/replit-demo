...
description: "Découvrez ce que sont les projets sur Replit — le conteneur pour tout ce que vous créez, des applications web et mobiles aux présentations et vidéos d'animation."
---
...
Un projet est le conteneur pour tout ce que vous construisez sur Replit. Il contient votre code, vos données et tous les artefacts que vous créez — applications web, applications mobiles, présentations, tableaux de bord de données et vidéos. Lorsque vous indiquez à Agent ce que vous voulez, il crée un projet et construit les artefacts à l'intérieur.

Pensez à un projet comme à un espace de travail sur votre bureau. Tout se trouve au même endroit et peut partager les mêmes informations, mais chaque élément a un rôle différent.

Par exemple, si vous demandez à Agent de créer une application de suivi de fitness, il crée un projet avec un artefact d'application mobile, ainsi que le backend, la base de données et le stockage nécessaires à son fonctionnement.
...
* **Ressources** : chaque artefact bénéficie d'un accès automatique à un backend, une base de données et un stockage de fichiers — sans aucune configuration

<Note>
  La publication met en ligne l'ensemble de votre projet en une seule fois.
  Tous les artefacts sont publiés ensemble, afin qu'ils restent toujours
  synchronisés.
</Note>
...
| -------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| **[Application web](/fr/features/artifact-types/web-apps)**                | Un site ou une application web que les visiteurs consultent dans leur navigateur | Une boutique en ligne, un outil de réservation, un site portfolio                     |
| **[Application mobile](/fr/features/artifact-types/building-mobile-apps)** | Une application native que les utilisateurs installent sur leur téléphone        | Un suivi de fitness, une application de commande pour un restaurant                   |
| **[Visualisation de données](/fr/features/artifact-types/data-apps)**      | Un tableau de bord interactif avec graphiques, filtres et données en direct      | Un rapport de ventes, un suivi de métriques, une vue analytique                       |
| **[Présentation](/fr/features/artifact-types/slide-decks)**                | Une présentation que vous pouvez présenter ou exporter                           | Un pitch deck, une mise à jour d'équipe, une présentation de produit                  |
| **[Animation](/fr/features/artifact-types/animated-videos)**               | Une vidéo d'animation que vous pouvez exporter en MP4                            | Une vidéo de lancement de produit, un explicatif, du contenu pour les réseaux sociaux |
...
| **[Design](/fr/design/canvas)**                                            | Des directions visuelles et des prototypes explorés sur le canevas de design     | Des mood boards, des maquettes, de l'exploration de design                            |

Un même projet peut contenir plusieurs artefacts. Par exemple, vous pourriez créer une application web pour vos clients, une application mobile pour votre équipe et une présentation pour des investisseurs — le tout dans le même projet. Lorsque des artefacts vivent dans le même projet, ils partagent automatiquement les mêmes données et la même logique backend. Pour en savoir plus, consultez [Artefacts](/fr/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project).

## Gérer vos projets

Votre page Projets répertorie chaque projet que vous avez créé ou auquel vous avez été invité. C'est le moyen le plus rapide de reprendre un travail récent et de retrouver le bon projet dans un espace de travail chargé.

### Voir en un coup d'œil ce que contient chaque projet

Chaque carte de projet affiche de petites icônes pour les artefacts qu'elle contient, pour chaque application web, application mobile, présentation, visualisation de données ou animation du projet. Survolez une icône pour prévisualiser la capture d'écran de cet artefact directement sur la carte, afin de distinguer les projets sans avoir à les ouvrir.

<Frame>
  <img src="/assets/mirror/projects-page-artifacts-a9eb175ffbaf.png" alt="Carte de projet sur la page Projets avec des icônes d'artefacts et un menu déroulant listant chaque artefact du projet" width="721" height="1022" data-path="images/replitai/projects-page-artifacts.png" />
</Frame>

### Filtrer par artefact

Utilisez le menu déroulant **Type de build** en haut de la page Projets pour filtrer votre liste par artefact. Choisissez-en un pour ne voir que les projets qui contiennent ce type d'artefact :
...
* **3D Game**, **Agent & Automation**, et plus

Sélectionnez **Any build type** pour supprimer le filtre.

<Frame>
  <img src="/assets/mirror/projects-page-build-type-filter-9e23df1b365a.png" alt="Menu déroulant Type de build sur la page Projets filtrant par artefact, avec les options Design, Web, Data, Mobile, 3D Game et Slides" width="472" height="622" data-path="images/replitai/projects-page-build-type-filter.png" />
</Frame>

### Trier par dernière ouverture

La page Projets est triée par **dernière ouverture par vous** par défaut — les projets dans lesquels vous avez passé du temps le plus récemment apparaissent en haut. C'est particulièrement utile dans les espaces de travail d'équipe, où les modifications récentes des autres membres pourraient sinon masquer votre travail en cours.
...
Épinglez un projet pour le garder en haut de votre liste. Les épingles sont **privées** — épingler un projet dans un espace de travail d'équipe ne change pas ce que voient les autres membres.

Pour épingler un projet, ouvrez le menu débordant (trois points) sur la carte du projet et sélectionnez **Pin**. Sélectionnez **Unpin** dans le même menu pour retirer l'épingle.

## Prochaines étapes

<CardGroup cols={3}>
  <Card title="Artefacts" href="/fr/features/projects-and-artifacts/artifacts" icon="layer-group">
    Découvrez comment créer, basculer entre et gérer des artefacts
  </Card>
  ...
  <Card title="Présentations" href="/fr/features/artifact-types/slide-decks" icon="presentation-screen">
    Créez des présentations par la conversation
  </Card>
</CardGroup>
