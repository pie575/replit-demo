---
title: "Replit Agent"
description: "Replit Agent 只需自然语言，就能把你的想法变成应用、设计、幻灯片等成果，无需编写代码。"
...
<YouTubeEmbed videoId="bAUVQfz_SIY" title="隆重推出 Replit Agent 4" />

<div id="what-is-replit-agent">
  ## 什么是 Replit Agent？
</div>

Agent 是你的创意伙伴。它会理解你的想法，帮你打磨完善，再把它们变为现实。与只会回答问题的聊天机器人不同，Agent 会实际动手：搭建 project、创建应用、检查自己的成果，并随时修复出现的问题。

用日常语言描述你想要什么即可，无需编写代码，也不需要技术背景。从规划到 deployment，其余的都交给 Agent。
...
<Steps>
  <Step title="描述你的需求">
    在 [Project Editor](/zh/learn/projects-and-artifacts/project-editor) 中直接开始对话即可。描述你想构建的应用、提出问题、研究某个主题，或者从 BigQuery、Slack、Notion 等[已连接的服务](/zh/features/agent/general-agent)中提取数据。没有任何限制，无论你提出什么需求，Replit Agent 都能应对。

    <AgentInput />
  </Step>

  <Step title="选择要构建的内容">
    你也可以选择一种 project 类型：web app、移动应用、幻灯片、design、数据可视化等等。如果你在第 1 步中已经描述了需求，Agent 会自动确定合适的配置。

    <AgentCarousel />
  </Step>

  <Step title="Agent 开始构建">
    Agent 会编写代码、搭建基础设施并测试结果。进入 project 后，你可以在不同的 [Agent 模式](#agent-modes)之间切换，以控制它的构建方式。
  </Step>

  <Step>
    与 Agent 对话来完善你的 project，准备就绪后即可发布。所有 artifact 会一并发布。
  </Step>
</Steps>

<Note>你不会被局限于某一种类型。可以先从 web app 开始，之后再添加移动应用、幻灯片或视频——全部都在同一个 project 中。</Note>

<div id="plan-mode">
  ### Plan 模式
</div>

启用 Plan 模式，可以在 Agent 改动任何代码或数据之前，先进行头脑风暴、提问并梳理你的 project 思路。在 Plan 模式下，Agent 会：

* **拆解复杂 project**，形成有序的任务列表
* **探索不同方案**并权衡利弊
* 在编写任何代码前先**审查和完善**

在聊天输入框中点击“Plan”，或直接告诉 Agent，即可切换到 Plan 模式。例如：“制定一个计划，为我的团队搭建一个项目跟踪工具”

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent 会生成一份有序的任务列表，供你审查和完善。确认计划无误后，批准它，Agent 就会开始构建。

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="任务计划已创建对话框，显示四项拟定任务以及“接受任务”和“修改计划”选项" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

详见 [Plan 模式指南](/zh/features/agent/plan-mode)。

<div id="agent-modes">
  ### Agent 模式
</div>

选择 Agent 构建 project 的方式：

<Frame caption="包含 Free、Power 和 Max 模式的 Agent 模式弹出面板。">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Agent 模式弹出面板，显示 Free、Power 和 Max 模式，其中选中了 Power，并带有主模型列表和 Effort 滑块" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**：在套餐额度内免费完成日常 Agent 工作。Core 和 Pro 的额度每五小时重置一次，并设有每周上限。Starter 层级沿用原有限制。前往 **Settings → Usage** 查看用量。
* **Auto**：在 Core 和 Pro 套餐下使用 Power 或 Max 进行付费工作时，自动选择模型。
* **Power**：付费的中端模式，以实惠的价格提供强大的模型。
* **Max**：付费的顶级模式，用最前沿的模型应对最具挑战性的工作。

如果某个请求所需的能力超出 Free Mode，Agent 可能会提供付费的 **Allow once** 选项。付费操作开始前，Replit 会先请求确认，操作完成后 Agent 会回到 Free Mode。

<Note>提示：你还可以启用 **[Plan 模式](/zh/features/agent/plan-mode)**，在开始构建前审阅并调整 Agent 的方案。</Note>

<div id="what-you-can-build-with-the-replit-agent">
  ## 你可以用 Replit Agent 构建什么
</div>

* **web app、移动应用、数据看板和 AI 驱动的工具**
* **视觉设计与原型** — 在 [Design Canvas](/zh/design/canvas) 上探索设计稿，再决定是否着手编码
* **同一个 project 中的多种产出** — web app、移动应用、幻灯片和视频共用同一套后端
* **文件与文档** — CSV、PDF、PowerPoint 文件、Markdown 文档
* **已连接服务的数据查询** — 直接在 Chat 中从 BigQuery、Linear、Slack、Notion 等服务拉取数据

<Card title="立即开始构建" icon="arrow-up-right" href="https://replit.com">
  描述你的想法，让 Agent 将它变为现实 — 无需任何配置。
</Card>
...
<Accordion title="我需要会写代码吗？">
  不需要。编写代码、搭建基础设施、配置数据库等技术工作都由 Agent 完成。你只需用日常语言描述想要什么，Agent 就会把它做出来。
</Accordion>

<Accordion title="Auto 与 Free、Power、Max 有什么区别？">
  **Free Mode** 是在套餐额度内免费完成的日常 Agent 工作。在 Power 或 Max 中，**Auto** 会为每个付费任务选择合适的模型。**Power** 是付费的中端模式，以实惠的价格提供强大的模型。**Max** 是付费的顶级模式，适合要求最高的工作。

  付费操作在开始前需要确认。
</Accordion>

<Accordion title="我可以在一个 project 中构建多个内容吗？">
  可以。你可以把 web app、移动应用、幻灯片、视频和数据可视化添加到[同一个 project](/zh/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) 中，它们共享同一套后端和数据。
</Accordion>

<Accordion title="如果 Agent 出错了怎么办？">
  Agent 会定期测试自己的工作成果，并在工作过程中创建 checkpoint，因此你可以回滚到之前的任意状态。你也可以在 Chat 中向 Agent 描述出了什么问题，它会帮你修复。
</Accordion>
...
Enterprise workspace 统一使用 Auto 模式，而非 Free Mode。Auto 会为每个任务选择相应的模式和允许使用的模型。账户管理员可通过 [Enterprise 模型控制](/zh/features/collaboration/enterprise-model-controls)管理模型访问权限。

有关详细定价，请参阅 [Agent 计费](/zh/billing/ai-billing#agent-billing)。
...
<CardGroup cols={2}>
  <Card title="Agent Skills" icon="puzzle-piece" href="/zh/features/agent/skills">
    让 Agent 掌握专业知识——使用预置 skills 或自行创建。
  </Card>

  <Card title="任务系统" icon="list-check" href="/zh/core-concepts/agent/task-system">
    看板式规划、后台任务，以及将改动合并回主版本。
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/zh/design/canvas">
    可视化原型与可直接上手的 Visual Editor。
  </Card>

  <Card title="Connectors" icon="plug" href="/zh/features/agent/general-agent">
    连接 BigQuery、Linear、Slack、Notion 等服务。
  </Card>

  <Card title="账单" icon="credit-card" href="/zh/billing/ai-billing#agent-billing">
    Agent 定价、套餐对比与支出管理。
  </Card>

  <Card title="Vibe coding 指南" icon="wand-magic-sparkles" href="/zh/learn/build-with-agent">
    高效编写 prompt、借助 AI 构建应用的技巧。
  </Card>
</CardGroup>