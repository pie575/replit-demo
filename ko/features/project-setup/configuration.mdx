---
title: "Replit App 구성"
description: ".replit 및 replit.nix 파일을 사용하여 Replit App을 구성하는 방법을 알아보세요."
---
...
`Dependencies` 도구를 통해 Nix 패키지를 시각적으로 관리할 수 있습니다. 자세한 내용은 [시스템 의존성](/ko/features/project-setup/dependency-management#system-dependencies) 가이드를 참조하세요.
...
**실행 명령:** Run 버튼을 선택했을 때 실행되는 명령을 지정합니다. 각 템플릿에는 즉시 코드를 실행할 수 있도록 기본 실행 명령이 있습니다. 더 맞춤화되고 복잡한 앱의 경우 [Workflows](/ko/features/workspace-tools/workflows)를 사용하세요.

**언어 서버 프로토콜(LSP)**: 자동 완성, 코드 탐색, 코드 하이라이팅, 실시간 린팅 및 오류 표시 등의 기능을 제공합니다.

**환경 변수:** 애플리케이션이 올바르게 실행되는 데 필요한 환경 변수를 설정하고 관리합니다.

**의존성 및 패키지:** `.replit` 파일을 통해 패키지 설치 및 구성을 직접 관리하여 Replit App 시작 시 필요한 모든 도구가 준비되도록 합니다. `Dependencies` 도구를 통해 의존성을 시각적으로 관리할 수 있습니다. 자세한 내용은 [시스템 모듈](/ko/features/project-setup/dependency-management#advanced-configuration) 가이드를 참조하세요.
...
| <a name="nix" />`[nix]`                         |                                        | 시스템 의존성을 관리하는 패키지 관리자인 Nix 사용을 위한 설정을 지정합니다. 자세한 내용은 [의존성 관리](/ko/features/project-setup/dependency-management) 문서를 참조하세요. |
...
`&`로 여러 명령을 동시에 실행할 수 있지만, 서비스 간 로그를 더 명확하게 구분하고 싶을 수 있습니다. [`process-compose`](https://github.com/F1bonacc1/process-compose)와 같은 [시스템 의존성](/ko/features/project-setup/dependency-management#system-dependencies)을 추가하면 여러 프로세스를 더 효과적으로 관리할 수 있습니다.
...
`.replit` 파일에서 `audio = true`를 설정하면 개발 환경을 재시작해야 할 수 있습니다. `Cmd+K`(Mac) 또는 `Ctrl+K`(Windows)로 명령 팔레트를 열고 **Restart compute**를 검색하여 선택하세요.
