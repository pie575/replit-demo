...
description: "Restaure seu banco de dados de desenvolvimento para um checkpoint ou reverta seu banco de dados de produção para um ponto no tempo."
---
...
| Banco de dados  | Método de recuperação                   | Disponibilidade                                                                   |
| --------------- | --------------------------------------- | --------------------------------------------------------------------------------- |
| Desenvolvimento | Rollback de checkpoint                  | Todos os planos                                                                   |
| Produção        | Restauração pontual e backups agendados | Core: até 7 dias de histórico. Pro e Enterprise: até 28 dias (7 dias por padrão). |

## Restaure seu banco de dados de desenvolvimento

Você pode revertar seu app e banco de dados de desenvolvimento para um estado anterior usando o [recurso de rollback](/pt/features/version-control/checkpoints-and-rollbacks/). Isso restaura seu banco de dados para qualquer checkpoint criado pelo Agent.
...
<Frame>
  <img src="/assets/mirror/database-rollbacks-f13dd20874ae.jpg" alt="Interface de rollbacks de banco de dados mostrando opções de rollback" width="1082" height="708" data-path="images/databases/database-rollbacks.jpg" />
</Frame>
...
<Warning>
  Restaurar seu banco de dados não restaura o código do seu app, e fazer rollback do seu app não restaura seu banco de dados. Para trazer os dois de volta ao mesmo momento: restaure o banco de dados e depois [faça rollback para o checkpoint correspondente](/pt/features/version-control/checkpoints-and-rollbacks/) e publique seu app novamente.
</Warning>
...
| Categoria de armazenamento                  | Preço por GiB por mês |
| ------------------------------------------- | --------------------: |
| Armazenamento lógico do banco de dados      |                \$0.35 |
| Armazenamento de restauração pontual (PITR) |                \$0.20 |
| Armazenamento de backup agendado            |                \$0.09 |

<Note>Os custos de armazenamento do banco de dados estão atualmente com desconto de 100%.</Note>

A cobrança de \$0.35 pelo armazenamento lógico se aplica independentemente de você usar PITR, backups agendados ou ambos. O armazenamento de backup agendado custa menos por GiB do que o armazenamento PITR. Se pontos de restauração diários atenderem às suas necessidades, backups agendados com uma janela de PITR mais curta podem reduzir os custos de armazenamento de recuperação. Seu custo total de backup depende do volume de backup armazenado e do período de retenção.

O PITR oferece mais pontos de restauração dentro da sua janela. Os backups agendados oferecem um ponto de restauração por dia, então selecione o método de recuperação que atenda às suas necessidades.

<Note>
  A programação usa seu deslocamento UTC atual. Uma mudança de horário de verão
  pode deslocar o horário do backup em uma hora até que você atualize a
  programação. Fusos horários com deslocamentos fracionários usam a hora mais
  próxima.
</Note>
...
* [Desenvolvimento e produção](/pt/features/data-and-storage/development-and-production): Como seus dois bancos de dados funcionam juntos.
* [Checkpoints e rollbacks](/pt/features/version-control/checkpoints-and-rollbacks/): Reverta todo o seu app.
