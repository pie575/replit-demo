---
title: "Replit Agent"
description: "Replit Agent 可将想法转化为应用、设计、幻灯片等，无需编程知识。"
---
...
Agent 是您的创意伙伴。它能将您的想法打磨并变为现实。与只会回答问题的聊天机器人不同，Agent 会付诸行动：设置项目、创建应用、检查工作成果，并在过程中解决问题。

只需用日常语言描述您想要的内容，无需编写代码或具备技术知识。Agent 会处理从规划到部署的所有工作。
...
<Steps>
  <Step title="描述你的需求">
    在[项目编辑器](/zh/ja/learn/projects-and-artifacts/project-editor)中开始对话。你可以描述想构建的应用、提出问题、研究主题，或从 BigQuery、Slack、Notion 等[已连接的服务](/zh/ja/features/agent/general-agent)中提取数据。没有限制，Replit Agent 都能帮你完成。

    <AgentInput />
  </Step>

  <Step title="选择要构建的内容">
    你也可以选择项目类型：Web 应用、移动应用、幻灯片、设计、数据可视化等。如果你已在第 1 步描述了需求，Agent 会自动确定合适的设置。

    <AgentCarousel />
  </Step>

  <Step title="让 Agent 构建">
    Agent 会编写代码、配置基础设施并测试结果。进入项目后，你可以切换 [Agent 模式](#agent-modes)，控制构建方式。
  </Step>

  <Step>
    与 Agent 对话以改进项目，准备就绪后即可发布。所有产物都会一并部署。
  </Step>
</Steps>

<Note>你不必局限于一种类型。可以从 Web 应用开始，之后再添加移动应用、幻灯片和视频，全部都在同一个项目中。</Note>

<div id="plan">
  ### Plan 模式
</div>

启用 Plan 模式，让 Agent 在修改代码或数据前进行头脑风暴、提出问题并梳理项目。在 Plan 模式下，Agent 会：

* **拆解复杂项目**，创建有序的 task 列表
* **探索不同方案**，评估各种权衡
* **在编写代码前进行确认和完善**

点击聊天输入框中的“Plan”，或直接让 Agent 切换到 Plan 模式。例如：“请为构建团队项目跟踪器制定计划”

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent 会创建有序的 task 列表，供你确认和完善。对计划满意后，批准即可让 Agent 开始构建。

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="显示四个建议 task，以及“批准 task”和“修改计划”选项的任务规划对话框" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

有关详细信息，请参阅 [Plan 模式指南](/zh/ja/features/agent/plan-mode)。
...
<Frame caption="显示 Free、Power 和 Max 模式的 Agent 模式弹出菜单。">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="显示 Free、Power 和 Max 模式的 Agent 模式弹出菜单，其中选中了 Power，并显示主模型列表和 Effort 滑块" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**：为 Core 和 Pro 订阅者免费提供日常 Agent 使用。额度每 5 小时重置一次，并设有每周上限。Starter 套餐保持现有上限。你可以在**设置 → 使用量**中查看使用情况。
* **Power**：付费中档模式，以实惠的价格提供强大的模型。
* **Max**：付费顶级模式，为最具挑战性的任务提供顶尖的前沿模型。在**高级设置**中，你还可以开启 **Turbo**，以获得更快的执行速度。

对于超出 Free Mode 能力范围的请求，Agent 有时会提供付费的**仅此一次允许**选项。Replit 会在付费操作开始前要求你确认，之后 Agent 会返回 Free Mode。

<Note>提示：你还可以启用 **[Plan 模式](/zh/ja/features/agent/plan-mode)**，在开始构建前查看并改进 Agent 的计划。</Note>

<div id="replit-agent-3">
  ## 可使用 Replit Agent 构建的内容
</div>

* **Web 应用、移动应用、数据控制台和 AI 驱动的工具**
* **视觉设计和原型** — 在提交代码前，先在[设计 Canvas](/zh/ja/design/canvas)中探索设计稿
* **一个项目中的多种输出** — 共享同一后端的 Web 应用、移动应用、幻灯片和视频
...
<Accordion title="我需要会写代码吗？">
  不需要。Agent 会处理所有技术工作：编写代码、配置基础设施和设置数据库。您只需用日常语言描述想要的内容，Agent 就会帮您构建。
</Accordion>

<Accordion title="Free、Power 和 Max 有什么区别？">
  **Free Mode** 为 Core 和 Pro 订阅用户提供日常 Agent 工作，可在额度范围内免费使用。**Power** 是价格实惠的付费中档模式，可使用强大的模型。**Max** 是面向要求最严苛任务的付费顶级模式。**Turbo** 是高级设置中的可选开关，仅限 Max 使用。

  付费操作需要在开始前确认。
</Accordion>

<Accordion title="我可以在一个项目中构建多个内容吗？">
  可以。您可以将 Web 应用、移动应用、幻灯片、视频和数据可视化添加到[同一个项目](/zh/ja/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project)中，它们会共享相同的后端和数据。
</Accordion>

<Accordion title="如果 Agent 出错了怎么办？">
  Agent 会定期测试自己的工作。它还会在工作过程中创建检查点，因此您可以回滚到任何之前的状态。您可以与 Agent 聊天并说明问题所在，它会帮您修复。
</Accordion>
...
Turbo 默认关闭。选择 Max 后，可在需要时针对各个项目启用。Free Mode 在发布时暂不适用于 Enterprise。Enterprise 工作区随后将很快获得专属的 Agent 体验。详情请咨询管理员。

有关详细定价，请参阅 [Agent 计费](/zh/ja/billing/ai-billing#agent-billing)。
...
<CardGroup cols={2}>
  <Card title="Agent 技能" icon="puzzle-piece" href="/zh/ja/features/agent/skills">
    向 Agent 传授专业知识——使用预构建技能，或创建自己的技能。
  </Card>

  <Card title="task 系统" icon="list-check" href="/zh/ja/core-concepts/agent/task-system">
    看板规划、后台 task，以及将更改应用到主版本。
  </Card>

  <Card title="设计 Canvas" icon="paintbrush" href="/zh/ja/design/canvas">
    视觉模型和实用的可视化编辑器。
  </Card>

  <Card title="连接器" icon="plug" href="/zh/ja/features/agent/general-agent">
    连接 BigQuery、Linear、Slack、Notion 等服务。
  </Card>

  <Card title="计费" icon="credit-card" href="/zh/ja/billing/ai-billing#agent-billing">
    Agent 定价、套餐比较和支出管理。
  </Card>

  <Card title="氛围编程指南" icon="wand-magic-sparkles" href="/zh/ja/learn/build-with-agent">
    使用 AI 的提示词和构建技巧。
  </Card>
</CardGroup>