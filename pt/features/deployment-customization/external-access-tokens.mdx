...
[uma URL de Dev Privada](/pt/features/publishing/private-deployments)
ou por um [Private Deployment](/pt/features/publishing/private-deployments).
...
<Note>
  Os tokens de acesso externo estão disponíveis quando você seleciona acesso
  privado para um deployment. Não é necessária nenhuma adesão do workspace. Em
  um workspace Enterprise, apenas administradores do workspace podem
  gerenciar tokens.
</Note>
...
Um token `Production` sobrevive a uma republicação, incluindo a publicação de
uma nova versão. O Replit o revoga quando você:

* Remove a publicação do deployment.
* Exclui o deployment.
* Altera o acesso do app de privado para público.

Um token também deixa de funcionar quando expira ou quando você o revoga.
...
<Accordion title="Meu token de produção parou de funcionar de repente">
  Os tokens de produção permanecem válidos em republicações normais. Verifique
  se o token expirou ou se você o revogou. Verifique também se você removeu a
  publicação, excluiu o deployment ou alterou o app de privado para público.
  Se o app não tiver mais um deployment privado, publique-o novamente como
  privado. Em seguida, crie um novo token. Não é possível restaurar um token
...
* [Private Deployments](/pt/features/publishing/private-deployments):
