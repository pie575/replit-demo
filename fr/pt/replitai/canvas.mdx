---
title: "Canvas"
description: "Découvrez comment utiliser Canvas pour explorer et itérer sur des designs avec Agent, et afficher vos artefacts dans des frames."
---
...
Canvas est un espace où vous pouvez explorer différents designs et les affiner avec Agent. Vous y verrez vos artefacts — applications, vidéos et diapositives, par exemple — affichés dans un frame. Vous pouvez travailler avec Agent pour créer des maquettes, comparer différentes pistes de design et appliquer les modifications à votre application, le tout depuis un même panneau visuel.

<Frame>
  <img src="/assets/mirror/canvas-desktop-85e8bfec38a2.png" alt="Canvas affichant un aperçu d'application à côté du Chat d'Agent avec les mises à jour des tâches" width="3452" height="2006" data-path="images/replitai/canvas-desktop.png" />
</Frame>

<div id="o-que-h-no-canvas">
  ## Ce que contient le Canvas
</div>

Le Canvas est un espace infini où vous pouvez visualiser et manipuler tous vos éléments.

<Frame>
  <img src="/assets/mirror/canvas-mockups-057e738a78da.png" alt="Canvas affichant plusieurs maquettes de design disposées côte à côte — thème sombre, aperçus mobiles et pages de détails" width="3452" height="1620" data-path="images/replitai/canvas-mockups.png" />
</Frame>

* **Vos applications** : Vos applications en cours d’exécution apparaissent directement dans le Canvas sous forme d’aperçus interactifs en temps réel. Vous pouvez cliquer dessus et les utiliser comme dans un navigateur.
* **Maquettes de design** : Des prototypes visuels que l’Agent crée pour vous. Ils ressemblent à de vraies pages : vous pouvez interagir avec eux, les redimensionner et demander à l’Agent de les ajuster. Ils restent toutefois distincts de votre application en ligne, afin que vous puissiez expérimenter librement.
* **Formes et dessins** : Rectangles, cercles, étoiles, flèches, cœurs et bien plus encore. Utilisez-les pour esquisser des idées, annoter des designs ou représenter les liens entre les écrans.
* **Texte et notes adhésives** : Ajoutez des libellés ou des notes n’importe où dans le Canvas pour organiser vos idées.
* **Images** : Déposez des captures d’écran ou des images de référence dans le Canvas. L’Agent peut les recréer ou s’en inspirer.
* **Vidéos** : Placez des fichiers vidéo dans le Canvas à côté de vos autres contenus.

<div id="colocando-designs-no-canvas">
  ### Ajouter des designs au Canvas
</div>

Il existe plusieurs façons de commencer à créer des designs. Choisissez celle qui vous semble la plus naturelle.

<Steps>
  <Step title="Demandez à l’Agent dans le Chat">
    Saisissez ce que vous souhaitez voir dans le champ de Chat, et l’Agent le placera sous la forme d’un frame interactif sur le Canvas.

    <AgentInput defaultPrompt="Montre-moi une maquette de ma page de destination sur le Canvas" showTypewriter={true} />
  </Step>

  <Step title="Choisissez un type de sortie">
    Vous pouvez également choisir le type de sortie souhaité — web, mobile, diapositives, design et bien plus encore. Utilisez le carrousel pour faire votre choix avant d’envoyer votre prompt.

    <AgentCarousel />
  </Step>

  <Step title="Ajoutez une page existante au Canvas">
    Vous avez déjà une application en ligne ? Demandez à l’Agent de placer une page sur le Canvas pour en revoir le design — par exemple, « Ajoute mon dashboard au Canvas. » L’Agent en crée une copie afin que votre application en ligne reste intacte.
  </Step>
</Steps>
...
Une fois votre design créé dans Canvas, vous pouvez le faire évoluer de différentes façons.

<div id="pea-ao-agent-por-atualizaes">
  ### Demandez des modifications à Agent
</div>

Sélectionnez un frame dans Canvas et saisissez vos modifications dans le champ qui s’affiche en dessous. Lorsque vous appuyez sur Entrée, Agent traite votre demande, apporte les modifications et met à jour la maquette directement dans Canvas.
...
Utilisez les outils de dessin de la barre d&#39;outils inférieure pour dessiner directement sur le Canvas. Tracez des flèches vers ce que vous souhaitez modifier, entourez les zones problématiques ou ajoutez des notes avec vos retours. Demandez ensuite à Agent d&#39;apporter les modifications en fonction de vos annotations.
...
<div id="visualize-em-diferentes-dispositivos">
  ### Visualisez vos designs sur différents appareils
</div>

Découvrez le rendu de vos designs selon différentes tailles d’écran. Demandez à Agent d’afficher une version mobile, tablette ou desktop :

* **Mobile** — 390 × 844
* **Tablette** — 768 × 1024
* **Desktop** — 1280 × 720

Par exemple, demandez « Montre-moi la version mobile de cette page », et Agent l’affichera à cette taille.
...
<div id="aplique-alteraes-ao-seu-aplicativo">
  ### Appliquez les modifications à votre application
</div>

Une fois satisfait d’un design, appliquez les modifications à votre version principale. Agent affiche un résumé des modifications apportées et vous permet de les appliquer ou de les ignorer.
...
<div id="comparando-direes-de-design">
  ## Comparer différentes orientations de design
</div>

Lorsque vous hésitez sur l’orientation à adopter, demandez à Agent de générer plusieurs versions afin de pouvoir les comparer côte à côte.
...
* **Plusieurs versions d’une page** : « Montrez-moi cinq versions de cette page de destination. » Agent crée chacune d’elles et les dispose côte à côte.
* **Flux de plusieurs pages** : « Concevez un flux d’onboarding complet : bienvenue, configuration du profil, préférences et confirmation. » Agent crée chaque écran et les organise dans l’ordre.
* **Combinez les styles** : « Montrez-moi la page de destination et la page des tarifs dans un style minimaliste et dans un style audacieux. » Agent crée chaque combinaison afin que vous puissiez avoir une vue d’ensemble.

<div id="mapeando-fluxos-de-usurio">
  ## Cartographier les parcours utilisateur
</div>

Organisez des parcours utilisateur complets sur le Canvas — inscription, onboarding, dashboard, paramètres — pour visualiser les liens entre chaque étape.

* Placez chaque écran dans un frame distinct
* Demandez à l’Agent de générer le parcours complet : « Dessine le parcours d’onboarding complet de mon application »
* Utilisez des flèches et des formes pour montrer comment les utilisateurs passent d’un écran à l’autre
* Demandez à l’Agent de transformer des croquis sommaires en designs aboutis

<div id="transformando-um-design-em-um-aplicativo-real">
  ## Transformer un design en véritable application
</div>

Lorsque vous êtes satisfait d’un design et souhaitez le concrétiser, deux options s’offrent à vous :

<div id="torne-o-um-aplicativo-completo">
  ### Transformez-le en application complète
</div>

Si votre design nécessite de véritables fonctionnalités — comme l’enregistrement de données, des comptes utilisateurs ou la connexion à d’autres services — demandez à Agent de le transformer en application complète. Dites par exemple « Transforme ceci en véritable application » ou « Ajoute un backend à ceci. » Agent crée tout le nécessaire pour en faire une véritable application publiable.
...
<div id="converta-para-um-tipo-especfico">
  ### Convertir vers un type spécifique
</div>

Vous pouvez également convertir un frame de Design en un type d&#39;artefact précis :

<Steps>
  <Step title="Sélectionnez un frame">
    Cliquez sur le frame que vous souhaitez convertir.
  </Step>

  <Step title="Demandez à Agent de le convertir">
    Indiquez à Agent ce que vous souhaitez — par exemple, « Convertis ceci en application web » ou « Transforme ceci en diapositives ».
  </Step>

  <Step title="Choisissez un type">
    Choisissez parmi application web, application mobile, diapositives, visualisation de données et bien plus encore.
  </Step>
</Steps>
...
<Accordion title="Les frames de design sont-elles identiques à mon application en cours d'exécution ?">
  Non. Les frames de design sont des prototypes visuels : elles ressemblent au résultat final, mais ne disposent pas d&#39;un backend ni d&#39;une base de données fonctionnels. Les aperçus de votre application dans le Canvas, eux, correspondent à votre véritable application en cours d&#39;exécution.
</Accordion>

<Accordion title="Puis-je repenser une page sans affecter mon application en ligne ?">
  Oui. Demandez à Agent d&#39;ajouter une page au Canvas, par exemple : « Ajoute mon tableau de bord au Canvas. » Agent crée une copie sur laquelle vous pouvez expérimenter. Votre application en ligne n&#39;est pas affectée tant que vous ne choisissez pas d&#39;appliquer les modifications.
</Accordion>

<Accordion title="Comment transformer un design en véritable application ?">
  Demandez à Agent d&#39;en faire une application complète (« Fais-en une véritable application ») ou de le convertir en un type spécifique (« Convertis-le en application web »). Agent met en place tous les éléments nécessaires pour en faire une application fonctionnelle et publiable.
</Accordion>

<Accordion title="Puis-je convertir vers différents types d'applications web ?">
  Oui. Vous pouvez transformer des frames du Canvas en applications web, applications mobiles, diapositives, visualisations de données et bien plus encore.
</Accordion>

<Accordion title="Puis-je importer des designs depuis Figma ou d'autres outils ?">
  Oui. Il existe plusieurs façons d&#39;importer des designs Figma dans Replit :

  * **Importer depuis Figma** : Accédez à [replit.com/import](https://replit.com/import), connectez votre compte Figma et collez l&#39;URL d&#39;une frame pour convertir un design en une application React fonctionnelle. Consultez le [Guide de démarrage rapide : Importer depuis Figma](/fr/pt/getting-started/quickstarts/import-from-figma) pour plus de détails.
  * **MCP Figma dans le Chat d&#39;Agent** : Collez un lien Figma directement dans le Chat d&#39;Agent et demandez-lui de générer du code, d&#39;extraire des données de design ou de créer un composant à partir du design. Consultez le guide [Intégration MCP Figma](/fr/replitai/mcp/figma).
...
<Accordion title="Que puis-je ajouter d'autre au Canvas ?">
  En plus des maquettes de design et des aperçus d&#39;applications, vous pouvez ajouter des formes, des flèches, des libellés de texte, des notes adhésives, des images et des vidéos, utiles pour esquisser des idées, annoter des designs ou organiser votre réflexion.
</Accordion>
...
Canvas est disponible dans toutes les formules, y compris pour créer des frames, les modifier en ligne, générer des variantes et les convertir en artefacts. La transformation de designs en applications complètes nécessite un plan payant.