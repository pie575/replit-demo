---
title: "Replit Agent"
description: "Replit Agent transforme vos idées en applications, designs, présentations et bien plus, à partir de langage naturel. Aucune compétence en programmation requise."
---
...
Agent est votre partenaire créatif. Agent prend vos idées, vous aide à les affiner, puis les concrétise. Contrairement à un chatbot qui se contente de répondre aux questions, Agent passe à l'action : il configure votre projet, crée des applications, vérifie son travail et résout les problèmes en cours de route.

Décrivez ce que vous voulez en langage courant. Aucune compétence en programmation ou technique n'est requise. Agent gère le reste, de la planification au déploiement.
...
<Steps>
  <Step title="Décrivez ce que vous voulez">
    Dans l'[Éditeur de projet](/fr/learn/projects-and-artifacts/project-editor), commencez simplement à discuter. Décrivez une application que vous souhaitez créer, posez une question, effectuez des recherches sur un sujet, ou extrayez des données de [services connectés](/fr/features/agent/general-agent) comme BigQuery, Slack ou Notion. Il n'y a aucune contrainte. Replit Agent gère tout ce que vous lui soumettez.

    <AgentInput />
  </Step>

  <Step title="Choisissez ce que vous voulez créer">
    Optionnellement, sélectionnez un type de projet : application web, application mobile, présentation, design, visualisation de données, et plus encore. Si vous avez déjà décrit ce que vous voulez à l'étape 1, Agent détermine automatiquement la configuration appropriée.

    <AgentCarousel />
  </Step>

  <Step title="Agent construit votre projet">
    Agent écrit le code, configure l'infrastructure et teste le résultat. Une fois dans votre projet, vous pouvez basculer entre les [modes Agent](#agent-modes) pour contrôler la façon dont il construit.
  </Step>

  <Step>
    Discutez avec Agent pour affiner votre projet, puis publiez quand vous êtes prêt. Tous les artefacts se publient ensemble.
  </Step>
</Steps>

<Note>Vous n'êtes pas limité à un seul type. Commencez par une application web et ajoutez une application mobile, des diapositives ou une vidéo ultérieurement — tout dans le même projet.</Note>

### Mode Plan

Activez le mode Plan pour réfléchir, poser des questions et organiser votre projet avant qu'Agent ne modifie du code ou des données. En mode Plan, Agent va :

* **Décomposer les projets complexes** en listes de tâches ordonnées
* **Explorer différentes approches** et évaluer les compromis
* **Réviser et affiner** avant qu'aucun code ne soit écrit

Cliquez sur « Plan » dans la zone de saisie du chat — ou demandez simplement à Agent — pour passer en mode Plan. Exemple : « Crée un plan pour construire un gestionnaire de projets pour mon équipe »

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent crée une liste de tâches ordonnée que vous pouvez réviser et affiner. Lorsque vous êtes satisfait du plan, approuvez-le et Agent commence la construction.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="Boîte de dialogue des plans de tâches créés montrant quatre tâches proposées avec les options Accepter les tâches et Réviser le plan" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

Apprenez-en plus dans le [guide du mode Plan](/fr/features/agent/plan-mode).

### Modes Agent

Choisissez la façon dont Agent construit votre projet :

<Frame caption="Le menu des modes Agent avec les modes Free, Power et Max.">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Menu des modes Agent affichant les modes Free, Power et Max avec Power sélectionné, une liste de modèles principaux et un curseur d'effort" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode** : Travail quotidien gratuit avec Agent pour les abonnés Core et Pro. Son allocation se renouvelle toutes les cinq heures et comporte une limite hebdomadaire. Le palier Starter conserve ses limites existantes. Consultez **Paramètres → Utilisation** pour voir votre utilisation.
* **Power** : Un mode payant de milieu de gamme avec des modèles puissants à un prix abordable.
* **Max** : Un mode payant haut de gamme avec les meilleurs modèles de pointe pour le travail le plus exigeant. Dans les **Paramètres avancés**, vous pouvez également activer **Turbo** pour des exécutions plus rapides.

Si une requête nécessite plus de capacité que le Free Mode, Agent peut proposer une option payante **Autoriser une fois**. Replit demande une confirmation avant le début d'une action payante, et Agent revient ensuite au Free Mode.

<Note>Conseil : Vous pouvez également activer le **[mode Plan](/fr/features/agent/plan-mode)** pour réviser et itérer sur le plan d'Agent avant le début de la construction.</Note>

## Ce que vous pouvez construire avec Replit Agent

* **Applications web, applications mobiles, tableaux de bord de données et outils alimentés par l'IA**
* **Designs visuels et prototypes** — explorez des maquettes sur le [Canevas de design](/fr/design/canvas) avant de vous engager dans le code
* **Plusieurs sorties dans un seul projet** — applications web, applications mobiles, diapositives et vidéos partageant le même backend
* **Fichiers et documents** — CSV, PDF, fichiers PowerPoint, documents Markdown
* **Requêtes de services connectés** — extrayez des données de BigQuery, Linear, Slack, Notion et plus directement depuis le chat

<Card title="Commencez à construire maintenant" icon="arrow-up-right" href="https://replit.com">
...
<Accordion title="Dois-je savoir programmer ?">
  Non. Agent gère tout le travail technique — écriture du code, configuration de l'infrastructure, configuration des bases de données. Décrivez ce que vous voulez en langage courant et Agent le construit.
</Accordion>

<Accordion title="Quelle est la différence entre Free, Power et Max ?">
  **Free Mode** est un travail quotidien gratuit avec Agent pour les abonnés Core et Pro, dans la limite d'une allocation. **Power** est un mode payant de milieu de gamme avec des modèles puissants à un prix abordable. **Max** est le mode payant haut de gamme pour le travail le plus exigeant. **Turbo** est un bouton optionnel réservé à Max dans les Paramètres avancés.

  Les actions payantes nécessitent une confirmation avant de démarrer.
</Accordion>

<Accordion title="Puis-je construire plus d'une chose dans un projet ?">
  Oui. Vous pouvez ajouter des applications web, des applications mobiles, des diapositives, des vidéos et des visualisations de données au [même projet](/fr/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) — le tout partageant le même backend et les mêmes données.
</Accordion>

<Accordion title="Que se passe-t-il si Agent fait une erreur ?">
  Agent teste régulièrement son propre travail. Agent crée également des points de contrôle au fur et à mesure, vous pouvez donc revenir à n'importe quel état précédent. Vous pouvez également discuter avec Agent pour décrire ce qui s'est passé et il corrigera le problème.
</Accordion>
...
| ------------------------------- | ---- | --- |
| Chat et construction avec Agent | ✅    | ✅   |
| Free Mode                       | ✅    | ✅   |
| Mode Power                      | ✅    | ✅   |
| Mode Max                        | ✅    | ✅   |
| Canevas de design               | ✅    | ✅   |
...
Turbo est désactivé par défaut — activez-le par projet lorsque vous en avez besoin, avec Max sélectionné. Le Free Mode n'est pas disponible pour Enterprise au lancement. Les espaces de travail Enterprise bénéficieront d'une expérience Agent dédiée peu après ; leur administrateur en aura les détails.

Pour la tarification détaillée, consultez la [facturation Agent](/fr/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  ...
  <Card title="Système de tâches" icon="list-check" href="/fr/core-concepts/agent/task-system">
    Planification Kanban, tâches en arrière-plan et modifications appliquées à la version principale.
  </Card>

  <Card title="Canevas de design" icon="paintbrush" href="/fr/design/canvas">
    Maquettes visuelles et éditeur visuel interactif.
  </Card>

  <Card title="Connecteurs" icon="plug" href="/fr/features/agent/general-agent">
    Connectez BigQuery, Linear, Slack, Notion et plus encore.
  </Card>

  <Card title="Facturation" icon="credit-card" href="/fr/billing/ai-billing#agent-billing">
    Tarification Agent, comparaison des plans et gestion des dépenses.
  </Card>

  <Card title="Guide de vibe coding" icon="wand-magic-sparkles" href="/fr/learn/build-with-agent">
    Conseils pour un prompting efficace et la construction avec l'IA.
  </Card>
</CardGroup>
