---
title: "Canvas"
description: "Узнайте, как с помощью Canvas исследовать и дорабатывать дизайны вместе с Agent, а также просматривать свои артефакты во фреймах."
---
...
<YouTubeEmbed videoId="Qv9cREjl9vQ" title="Обзор Replit Canvas" />

<div id="o-que-o-canvas">
  ## Что такое Canvas?
</div>

Canvas — это пространство, где можно вместе с Agent прорабатывать и дорабатывать дизайны. Здесь ваши артефакты — например, приложения, видео и слайды — отображаются во фрейме. Работайте с Agent: создавайте макеты, сравнивайте варианты дизайна и применяйте изменения к своему приложению — всё в одной визуальной панели.

<Frame>
  <img src="/assets/mirror/canvas-desktop-85e8bfec38a2.png" alt="Canvas с предпросмотром приложения рядом с чатом Agent и обновлениями задач" width="3452" height="2006" data-path="images/replitai/canvas-desktop.png" />
</Frame>
...
Canvas — это бесконечная доска, на которой всё можно видеть и всем можно управлять визуально.

<Frame>
  <img src="/assets/mirror/canvas-mockups-057e738a78da.png" alt="Canvas с несколькими макетами дизайна, расположенными рядом — тёмная тема, мобильные виды и страницы с подробностями" width="3452" height="1620" data-path="images/replitai/canvas-mockups.png" />
</Frame>

* **Ваши приложения**: запущенные приложения отображаются прямо на Canvas в виде живых интерактивных предпросмотров. По ним можно кликать и пользоваться ими так же, как в браузере.
* **Макеты дизайна**: визуальные прототипы, которые создаёт для вас Agent. Они выглядят как настоящие страницы — с ними можно взаимодействовать, менять их размер и просить Agent их скорректировать, — но они отделены от вашего работающего приложения, так что экспериментировать можно свободно.
* **Фигуры и рисунки**: прямоугольники, круги, звёзды, стрелки, сердца и многое другое. Используйте их, чтобы набрасывать идеи, отмечать элементы дизайна или выстраивать связи между экранами.
* **Текст и стикеры**: добавляйте подписи или заметки в любом месте Canvas, чтобы упорядочить свои мысли.
* **Изображения**: перетаскивайте скриншоты или референсные изображения на Canvas. Agent может воссоздать увиденное или вдохновиться им.
* **Видео**: размещайте видеофайлы на Canvas рядом с остальным содержимым.

<div id="colocando-designs-no-canvas">
  ### Добавление дизайнов на Canvas
</div>

Создать дизайн можно несколькими способами. Выберите наиболее удобный для вас.

<Steps>
  <Step title="Обратитесь к Agent в чате">
    Введите в поле чата то, что хотите увидеть, и Agent добавит это на Canvas в виде интерактивного фрейма.

    <AgentInput defaultPrompt="Покажи макет моей целевой страницы на Canvas" showTypewriter={true} />
  </Step>

  <Step title="Выберите тип результата">
    Также можно выбрать нужный тип результата: веб, мобильное приложение, слайды, дизайн и многое другое. Выберите его в карусели перед отправкой prompt.

    <AgentCarousel />
  </Step>

  <Step title="Перенесите существующую страницу на Canvas">
    У вас уже есть работающее приложение? Попросите Agent добавить страницу на Canvas для редизайна — например: «Добавь мою панель управления на Canvas». Agent создаст копию, поэтому работающее приложение останется без изменений.
  </Step>
</Steps>
...
Если у вас есть дизайн в Canvas, его можно дорабатывать разными способами.

<div id="pea-ao-agent-por-atualizaes">
  ### Попросите Agent внести изменения
</div>

Выберите фрейм на Canvas и опишите нужные изменения в поле, которое появится под ним. После нажатия Enter Agent примет запрос, внесёт правки и обновит макет прямо на Canvas.
...
<div id="desenhe-e-anote">
  ### Рисуйте и добавляйте пометки
</div>

Используйте инструменты рисования на нижней панели, чтобы делать наброски прямо на Canvas. Нарисуйте стрелки, указывающие на то, что нужно изменить, обведите проблемные области или добавьте текстовые заметки с отзывами. Затем попросите Agent внести изменения на основе ваших пометок.
...
<div id="visualize-em-diferentes-dispositivos">
  ### Просматривайте на разных устройствах
</div>

Посмотрите, как ваши дизайны выглядят на экранах разного размера. Попросите Agent показать версию для мобильного устройства, планшета или настольного компьютера:

* **Мобильное устройство** — 390 × 844
* **Планшет** — 768 × 1024
* **Настольный компьютер** — 1280 × 720

Например, скажите: «Покажи мобильную версию этой страницы», и Agent отобразит её в этом размере.
...
<div id="aplique-alteraes-ao-seu-aplicativo">
  ### Примените изменения к приложению
</div>

Когда вас устроит дизайн, примените изменения к основной версии приложения. Agent покажет сводку выполненных действий и предложит применить или отменить изменения.
...
* **Несколько вариантов одной страницы**: «Покажи пять вариантов этой лендинг-страницы». Agent создаст и расположит их рядом.
* **Многостраничные сценарии**: «Спроектируй полный сценарий онбординга: приветствие, настройка профиля, предпочтения и подтверждение». Agent создаст каждый экран и расположит их по порядку.
* **Смешивайте и сочетайте**: «Покажи лендинг-страницу и страницу с ценами в минималистичном и смелом стилях». Agent создаст все комбинации, чтобы вы могли увидеть общую картину.

<div id="mapeando-fluxos-de-usurio">
  ## Создание карты пользовательских сценариев
</div>

Разместите в Canvas полный пользовательский путь — регистрацию, онбординг, панель управления, настройки — и упорядочьте экраны, чтобы увидеть, как всё связано.

* Разместите каждый экран в отдельном фрейме
* Попросите Agent создать полный сценарий: «Создай полный сценарий онбординга для моего приложения»
* Используйте стрелки и фигуры, чтобы показать, как пользователи переходят между экранами
* Попросите Agent превратить грубые наброски в проработанные дизайны

<div id="transformando-um-design-em-um-aplicativo-real">
  ## Превращение дизайна в реальное приложение
</div>

Когда дизайн вас устраивает и вы хотите воплотить его в жизнь, есть два пути:

<div id="torne-o-um-aplicativo-completo">
  ### Превратите его в полноценное приложение
</div>

Если вашему дизайну нужна настоящая функциональность — например, сохранение данных, учётные записи пользователей или подключение к другим сервисам — попросите Agent превратить его в полноценное приложение. Напишите что-то вроде «Сделай из этого настоящее приложение» или «Добавь сюда бэкенд». Agent создаст всё необходимое, чтобы это работало как настоящее приложение, готовое к публикации.

<Note>
  Для превращения дизайна в полноценное приложение требуется платный тариф.
</Note>
...
<Steps>
  ...
  <Step title="Попросите Agent преобразовать его">
    Сообщите Agent, что нужно сделать, например: «Преобразуй это в веб-приложение» или «Преобразуй это в слайды».
  </Step>
...
<Accordion title="Design-фреймы — это то же самое, что моё работающее приложение?">
  Нет. Design-фреймы — это визуальные прототипы: они выглядят как настоящее приложение, но за ними нет работающего бэкенда или базы данных. А вот предпросмотры вашего приложения на Canvas — это уже реально работающее приложение.
</Accordion>

<Accordion title="Могу ли я переработать дизайн страницы, не навредив работающему приложению?">
  Да. Попросите Agent перенести страницу на Canvas — например: «Помести мою панель управления на канвас». Agent сделает копию, с которой можно экспериментировать. Ваше работающее приложение останется без изменений, пока вы сами не решите применить правки.
</Accordion>

<Accordion title="Как превратить дизайн в настоящее приложение?">
  Попросите Agent сделать из него полноценное приложение («Сделай это настоящим») или преобразовать в конкретный тип («Преобразуй это в веб-приложение»). Agent создаст всё необходимое, чтобы получилось рабочее приложение, готовое к публикации.
</Accordion>

<Accordion title="Можно ли преобразовать дизайн в разные типы веб-приложений?">
  Да. Фреймы с Canvas можно превращать в веб-приложения, мобильные приложения, слайды, визуализации данных и многое другое.
</Accordion>

<Accordion title="Могу ли я импортировать дизайны из Figma или других инструментов?">
  Да. Перенести дизайны из Figma в Replit можно несколькими способами:

  * **Импорт из Figma**: перейдите на [replit.com/import](https://replit.com/import), подключите свою учётную запись Figma и вставьте URL фрейма, чтобы преобразовать дизайн в работающее приложение на React. Подробности см. в [Кратком руководстве: импорт из Figma](/ru/pt/getting-started/quickstarts/import-from-figma).
  * **Figma MCP в чате Agent**: вставьте ссылку на Figma прямо в чат Agent и попросите сгенерировать код, извлечь данные дизайна или собрать компонент по дизайну. См. руководство [Интеграция Figma MCP](/ru/replitai/mcp/figma).
  * **Ориентир по скриншоту**: перетащите скриншот на Canvas и попросите Agent воссоздать дизайн по нему.
</Accordion>

<Accordion title="Что ещё можно разместить на Canvas?">
  Помимо макетов дизайна и предпросмотров приложений, вы можете добавлять фигуры, стрелки, текстовые подписи, стикеры, изображения и видео — это удобно для набросков идей, аннотирования дизайнов или структурирования мыслей.
</Accordion>
...
Canvas доступен на всех тарифах: можно создавать фреймы, редактировать их прямо на Canvas, генерировать варианты и преобразовывать в артефакты. Для преобразования дизайнов в полноценные приложения требуется платный тариф.