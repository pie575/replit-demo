...
description: "Comprenez les états des tâches, la mise en file d'attente, l'application des modifications, l'archivage et l'annulation des tâches Agent."
---
...
Chaque tâche de l&#39;Agent suit un cycle de vie : planifiée, en cours, prête pour révision et terminée. Ce cycle de vie isole le travail jusqu&#39;à ce que vous décidiez de ce qui doit être intégré à votre version principale.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Tableau des tâches affichant les tâches organisées dans les colonnes Brouillons, Actif, Prêt et Terminé" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>
...
Un **Brouillon** est une tâche que l&#39;Agent a planifiée, mais dont il n&#39;a pas encore commencé la réalisation. Les Brouillons peuvent être consultés en toute sécurité, car ils ne contiennent que le travail proposé : le titre de la tâche, sa description et le plan.

Utilisez les Brouillons pour vérifier si l&#39;Agent a bien compris l&#39;objectif. Vous pouvez lancer la tâche, demander à l&#39;Agent de revoir le plan ou archiver le brouillon pour plus tard.

<div id="ativo">
  ## Active
</div>

Une tâche **Active** s&#39;exécute en arrière-plan. Agent y travaille dans une copie isolée de votre projet : votre version principale reste donc inchangée pendant l&#39;exécution de la tâche.

Vous pouvez continuer à utiliser le fil principal pendant que les tâches Actives s&#39;exécutent. Si une tâche nécessite un retour de votre part, ouvrez son fil et poursuivez la conversation à cet endroit.

<div id="na-fila">
  ## En file d&#39;attente
</div>

Une tâche **En file d&#39;attente** est approuvée, mais son démarrage est en attente. Une tâche peut être mise en file d&#39;attente lorsqu&#39;elle dépend d&#39;une autre tâche ou lorsque vous avez atteint la limite de tâches en arrière-plan actives prévue par votre plan.
...
Une tâche **Prête** est terminée, mais ses modifications n&#39;ont pas encore été appliquées à votre version principale. Consultez le journal de travail de la tâche, les résultats des tests et le Preview avant d&#39;appliquer les modifications.

Si le résultat vous convient, appliquez-le à la version principale. Sinon, supprimez-le et lancez une nouvelle tâche.

<div id="aplicando">
  ## Application en cours
</div>

Une tâche à l&#39;état **Application en cours** est en train de reporter ses modifications sur votre version principale. Agent gère automatiquement les conflits lorsqu&#39;il applique les modifications de plusieurs tâches.

Attendez la fin de l&#39;application avant d&#39;évaluer le résultat final. Si quelque chose vous semble incorrect par la suite, demandez à Agent de le corriger dans le fil principal.

<div id="concludo">
  ## Terminé
</div>

Une tâche **Terminée** est finalisée et ne nécessite plus aucune action. « Terminé » peut signifier que la tâche a été appliquée à votre version principale, archivée avant d&#39;avoir été lancée ou annulée après être passée en phase de construction.

Utilisez la colonne **Terminé** comme historique des travaux achevés, archivés et annulés.

<div id="exemplos-de-estados">
  ## Exemples d&#39;états
</div>

Chaque tâche passe par une série d&#39;étapes, de l&#39;idée initiale jusqu&#39;à son intégration dans votre app. Chaque étape possède sa propre icône, ce qui vous permet de comprendre rapidement ce qui se passe.
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Suites d'histoires : les épisodes suivants" description="L'ajout d'un bouton Continuer l'aventure génère une suite qui reprend là où la précédente s'est arrêtée." status="active" timestamp="12 min" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Actif</strong> : Agent est en train de créer cette tâche. Elle s&#39;exécute dans sa propre copie de votre projet, afin que votre version principale reste intacte.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>En file d&#39;attente</strong> : cette tâche est approuvée, mais attend qu&#39;un créneau se libère avant que la création puisse démarrer.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Prêt</strong> : cette tâche est terminée. Vous pouvez examiner les modifications et les appliquer à votre version principale dès que vous le souhaitez.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Liens partageables avec aperçus OG" description="L'ajout de liens publics partageables est le principal levier de croissance." status="applying" timestamp="31m" width="100%" />
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Terminé</strong> : cette tâche est achevée et ses modifications font désormais partie de votre version principale.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Visite guidée d'introduction pour les nouveaux lecteurs" description="Une courte visite guidée qui présente aux parents le parcours de lecture principal dès le premier lancement." status="draft-static" timestamp="1h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Archivé</strong> : une session de planification que vous avez mise de côté avant que sa construction ne commence. Les sessions de planification archivées peuvent être restaurées ultérieurement depuis la colonne <strong>Terminé</strong>.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Annulée</strong> : une tâche que vous avez interrompue alors qu&#39;elle était déjà passée en phase de construction. Les tâches annulées ne peuvent pas être restaurées. Lancez une nouvelle tâche si vous changez d&#39;avis.</span>
  </div>
</div>
...
Vous pouvez arrêter une tâche à tout moment de son cycle de vie, mais l&#39;option proposée dépend de l&#39;endroit où se trouve la tâche sur le tableau. **Archiver** et **Annuler** déplacent tous deux une tâche vers la colonne **Terminé**, mais leur comportement diffère :

| Action       | Quand l&#39;option est disponible                                                                                         | Ce qui se passe                                                                                                                     | Réversible ?                                                                                                               |
| ------------ | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **Archiver** | Tant que la tâche est encore une session de planification : un **Brouillon** dont la construction n&#39;a pas démarré     | La session de planification est mise de côté sans être exécutée. Votre plan est conservé.                                           | Oui. Ouvrez la carte archivée dans la colonne **Terminé** et restaurez-la dans les **Brouillons**.                         |
| **Annuler**  | Une fois que la tâche est passée de la planification à la construction : toute tâche **Active**, **en file d&#39;attente** ou **Prête** | L&#39;Agent arrête la tâche et abandonne tout travail en cours. Aucune modification n&#39;est appliquée à votre version principale. | Non. Les tâches annulées ne peuvent pas être restaurées. Lancez une nouvelle tâche si vous souhaitez reprendre le travail. |

Utilisez **Archiver** pour les sessions de planification que vous souhaitez reprendre plus tard. Utilisez **Annuler** comme bouton d&#39;arrêt définitif pour les tâches en file d&#39;attente, déjà en cours d&#39;exécution ou dont la construction est terminée.

Vous retrouverez les sessions de planification archivées comme les tâches annulées dans la colonne **Terminé**, à l&#39;extrémité droite du tableau des tâches, aux côtés des tâches achevées.