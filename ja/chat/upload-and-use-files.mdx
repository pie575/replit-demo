---
title: "ファイルをアップロードして使う"
description: "関連するドキュメント、スプレッドシート、画像、スクリーンショットをConversationに追加し、その情報をどのようにリクエストに役立てるべきかをReplitに伝えましょう。"
sidebarTitle: "ファイルをアップロードして使う"
icon: "file-arrow-up"
...
ドキュメント、スプレッドシート、画像、スクリーンショットをアップロードすると、説明するよりも簡単に、証拠や要件、視覚的な方向性をReplitに伝えられます。

## ファイルをアップロードする方法

プロンプトボックスから**プラス**ボタンを選択し、**ファイルをアップロード**を選びます。ファイルをプロンプトボックスにドラッグ&ドロップすることもできます。そのファイルに依存するリクエストを送信する前に、ファイルを追加しておきましょう。

<Frame>
  <img src="/assets/mirror/prompt-box-plus-menu-8c1e2d807e0b.png" alt="最初のオプションとして「ファイルをアップロード」があるプロンプトボックスのプラスメニュー" width="1558" height="556" data-path="images/chat/prompt-box-plus-menu.png" />
</Frame>
...
* スプレッドシートを分析する。
* PDFについて質問する。
* スクリーンショットからデザインを作成または改良するようReplitに依頼する。
* 構築したいアプリについての方向性や情報をReplitに伝える。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ja/chat/upload-and-use-files">
    ...
    <div className="chat-card-title">ファイルを使って作業する</div>
    <p>ファイルを理解し分析する完全な例を見てみましょう。</p>
