...
如果您已在开发应用或项目，可以将其导入 Replit，并在 Project 中继续进行。导入不同于将文件作为证据或上下文附加到对话中。

<div id="escolha-uma-fonte-suportada">
  ## 选择受支持的来源
</div>

来源决定了您提供的内容，以及如何将现有工作导入 Replit。当前导入指南涵盖以下方式：

| 现有工作      | 支持的来源                                                                                   |
| --------- | --------------------------------------------------------------------------------------- |
| 构建者的仓库和项目 | **GitHub**、**Bitbucket**、**Vercel**、**Bolt**、**Lovable**、**Base44** 以及**从之前的 Agent 导出** |
| 设计        | **Figma** 和 **Claude**                                                                  |
| 本地文件和数据   | **ZIP** 和**电子表格** (`.xlsx`、`.csv` 或公开的 Google Sheets URL)                               |

部分构建者导入会使用从原始服务导出的 GitHub 仓库。请在导入前查阅相应来源的专门指南。设置、Secrets、数据库值或模型提供方特定的服务可能需要在之后进行额外配置。

在提示词输入框中，选择**更多 (+)** 菜单，然后选择**导入现有项目**，以打开导入流程。
...
* 如果文档、电子表格、图片或截图能为当前 Chat 中的问题或任务提供上下文，请**上传文件**。

如需了解第二种方式，请参阅[上传和使用文件](/zh/pt/chat/upload-and-use-files)。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/pt/build/import-from-providers">
    ...
    <div className="chat-card-title">从模型提供方导入</div>
    <p>选择受支持的来源，然后按照完整的导入流程操作。</p>
  ...
  <a className="chat-example has-visual" href="/zh/pt/features/projects-and-artifacts/projects">
    ...
    <p>了解可继续开展导入工作所在的结构化空间。</p>
    <div className="chat-card-link">详细了解 Projects →</div>
  </a>
</section>

将 Project 导入 Replit 后，如果工作需要遵循既定的视觉语言，请[使用设计系统](/zh/pt/chat/use-a-design-system)。