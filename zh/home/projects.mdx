---
title: "项目"
description: "了解 项目 如何将代码、数据和 artifact 汇聚在一起，承载重要的构建与设计工作。"
sidebarTitle: "项目"
...
项目是您在 Replit 上构建的所有内容的容器。它包含您的代码、数据以及创建的所有 artifact——Web 应用、移动应用、幻灯片、数据仪表板和视频。当您告诉 Agent 您想要什么时，它会创建一个项目，并在其中构建 artifact。

您可以把项目想象成桌面上的工作区。所有内容都集中在同一个地方，可以共享相同的信息，但每个部分各司其职。

例如，如果您让 Agent 构建一个健身追踪应用，它会创建一个项目，其中包含移动应用 artifact，以及运行所需的后端、数据库和存储。

<ArtifactGrid />

核心概念：

* **Project**：容纳所有内容的容器——您的代码、数据和所有 artifact
* **Artifact**：项目中可发布的产出物 (Web 应用、幻灯片、视频等)
* **资源**：每个 artifact 都能自动使用后端、数据库和文件存储，无需任何设置

<Note>
  Publishing 会一次性发布您的整个项目。所有 artifact 都会
  同时上线，因此始终保持同步。
</Note>
...
| 类型                                                        | 说明                            | 示例                 |
| --------------------------------------------------------- | ----------------------------- | ------------------ |
| **[Web app](/zh/features/artifact-types/web-apps)**          | 用户在浏览器中访问的网站或 Web 应用          | 在线商店、预订工具、作品集网站    |
| **[移动应用](/zh/features/artifact-types/building-mobile-apps)** | 用户安装到手机上的原生应用                 | 健身追踪器、餐厅点餐应用       |
| **[数据可视化](/zh/features/artifact-types/data-apps)**           | 包含图表、筛选器和实时数据的交互式仪表板          | 销售报表、指标追踪器、分析视图    |
| **[幻灯片](/zh/features/artifact-types/slide-decks)**           | 可用于演示或导出的演示文稿                 | 融资路演材料、团队进展汇报、产品概览 |
...
一个 项目 可以包含多个 artifact。例如，你可以在同一个 项目 中构建面向客户的 Web 应用、面向团队的移动应用，以及面向投资人的幻灯片。当多个 artifact 处于同一个 项目 中时，它们会自动共享相同的数据和后端逻辑。要了解更多信息，请参阅 [Artifacts](/zh/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project)。

<div id="managing-your-projects">
  ## 管理你的项目
</div>

项目 页面会列出你创建的以及受邀加入的所有项目。当 工作区 中项目众多时，这是回到近期工作、快速找到目标项目的最快途径。

<div id="see-at-a-glance-whats-inside-each-project">
  ### 一眼看清每个项目里有什么
</div>

每张项目卡片都会以小图标展示项目中包含的 artifact，无论是 Web 应用、移动应用、幻灯片、数据可视化还是动画。将鼠标悬停在任意图标上，即可直接在卡片上预览该 artifact 的截图，不用打开项目就能区分不同项目。

<Frame>
  <img src="/assets/mirror/projects-page-artifacts-a9eb175ffbaf.png" alt="项目页面上的项目卡片，显示 artifact 图标以及列出项目内所有 artifact 的下拉菜单" width="721" height="1022" data-path="images/replitai/projects-page-artifacts.png" />
</Frame>
...
使用 项目 页面顶部的 **Build type** 下拉菜单，按 artifact 筛选列表。选择某一项后，将只显示包含该类 artifact 的 项目：
...
<Frame>
  <img src="/assets/mirror/projects-page-build-type-filter-9e23df1b365a.png" alt="项目 页面上按 artifact 筛选的 Build type 下拉菜单，包含 Design、Web、Data、Mobile、3D Game 和 Slides 选项" width="472" height="622" data-path="images/replitai/projects-page-build-type-filter.png" />
</Frame>
...
项目 页面默认按**你最近打开的时间**排序——你近期使用最多的 项目 会排在最上方。这在团队 工作区 中尤其有用，否则其他成员的最新修改会把你正在进行的工作挤到下面。

你可以选择其他列标题来切换排序方式 (例如**最近更新**或**名称**) 。

<div id="pin-the-projects-you-use-most">
  ### 置顶最常用的 项目
</div>

将 项目 置顶后，它会始终显示在列表顶部。置顶状态**仅对你自己可见**——在团队 工作区 中置顶 项目 不会影响其他成员看到的内容。

要置顶 项目，请打开 项目 卡片上的更多菜单 (三个点) ，然后选择 **Pin**。在同一菜单中选择 **Unpin** 即可取消置顶。
