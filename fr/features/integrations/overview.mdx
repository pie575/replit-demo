...
description: "Découvrez les intégrations prises en charge par l'Agent et comment elles s'activent automatiquement en fonction de vos invites."
---
...
<Frame>
  <img src="/assets/mirror/agent-logo-e171ec584553.png" alt="Logo de l'Agent avec des connexions d'intégration illustrées autour" width="1280" height="720" data-path="images/replitai/agent-logo.png" />
</Frame>
...
### Laisser l'Agent suggérer un connecteur

Parfois, plusieurs services peuvent accomplir la même tâche, comme stocker des fichiers ou envoyer des e-mails. Lorsque vous demandez une fonctionnalité sans nommer de fournisseur, l'Agent affiche une courte liste de connecteurs adaptés. Choisissez celui que vous voulez.

Sélectionnez un fournisseur, puis connectez-vous une seule fois pour lier votre compte. L'Agent utilise votre choix au fur et à mesure qu'il construit. Vous pouvez aussi demander un service spécifique par son nom pour ignorer la liste.

<Frame>
  <img src="/assets/mirror/agent-connector-suggestions-0cb07754b84c.png" alt="Sélecteur « Choisir une intégration à connecter » de l'Agent listant Google Drive, Dropbox, Microsoft OneDrive, Box et SharePoint Online" width="2000" height="786" data-path="images/replitai/agent-connector-suggestions.png" />
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="Communication" icon="comments">
    ...
    * **Slack** : Rechercher dans votre Slack, lire les conversations et fils de discussion, envoyer des messages et gérer les canevas
  ...
  <Accordion title="Paiements et monétisation" icon="credit-card">
    ...
    * **Stripe** : Traiter les paiements et gérer les abonnements. Consultez le [guide des paiements Stripe](/fr/features/monetization/stripe-payments).
    * **Square** : Accepter les paiements et gérer les commandes, clients et inventaires
    * **Plaid** : Accéder aux soldes de comptes financiers, transactions et données d'investissement là où l'institution connectée le permet. Consultez le [guide du connecteur Plaid](/fr/features/integrations/plaid-connector).
    * **Shopify** : Provisionner une boutique Shopify, créer un storefront personnalisé et gérer vos produits et inventaires depuis l'Agent. Consultez le [guide du connecteur Shopify](/fr/features/integrations/shopify).
  </Accordion>
...
### Utiliser Slack avec l'Agent

Connectez Slack pour donner à l'Agent le contexte des conversations auxquelles vous avez accès. Demandez à l'Agent de rechercher des messages, fichiers, canaux ou personnes, puis utilisez les résultats pour répondre à des questions ou poursuivre votre travail.

Avec votre permission, l'Agent peut :

* Lire les canaux privés, les messages directs et les messages directs de groupe auxquels vous appartenez.
* Lire les réponses complètes des fils de discussion lorsque vous avez besoin du contexte environnant.
* Envoyer des messages en votre nom.
* Créer, lire et mettre à jour les canevas Slack.

<AiPrompt>
  Résume la discussion dans mon message direct avec Alex à propos du plan de lancement, puis rédige un brouillon de suivi pour moi.
</AiPrompt>

<Info>
  L'Agent ne peut accéder qu'aux conversations et contenus Slack auxquels votre compte Slack connecté a accès.
</Info>
...
<Frame>
  <img src="/assets/mirror/connectors-overview-4295e541a5ab.png" alt="Interface des connecteurs d'application montrant les intégrations disponibles incluant GitHub, OneDrive, Outlook, Spotify, Linear, Dropbox, Monday.com, Google Docs et YouTube avec leur statut de connexion et les options de gestion" width="3456" height="1688" data-path="images/replitai/connectors-overview.png" />
</Frame>
...
<Frame>
  <img src="/assets/mirror/connectors-oauth-flow-3873e6fcdaf4.png" alt="Flux d'authentification OAuth montrant la boîte de dialogue de connexion Linear avec des informations de confidentialité et de sécurité, expliquant l'utilisation des données et le contrôle utilisateur" width="1345" height="1213" data-path="images/replitai/connectors-oauth-flow.png" />
</Frame>
...
<Frame>
  <img src="/assets/mirror/connector-oauth-configuration-100911ce0c48.png" alt="Boîte de dialogue de configuration OAuth du connecteur avec des champs pour le Client ID, le Client Secret et les périmètres du connecteur, et une URL de redirection définie sur https://replit.com/connectors/oauth/callback" width="1112" height="1218" data-path="images/replitai/connector-oauth-configuration.png" />
</Frame>
...
<Tip>
  Vous cherchez à gérer les connecteurs de l'espace de travail ou de l'organisation ? Consultez [Gérer vos connecteurs](/fr/replitai/managing-connectors).
</Tip>
...
<Note>
  Les clés API sont stockées de manière sécurisée dans les [Secrets](/fr/core-concepts/project-editor/app-setup/secrets) de votre application. Agent vous demandera d'ajouter la clé lorsque nécessaire.
</Note>
...
<AccordionGroup>
  <Accordion title="Google Workspace : « Cette application est bloquée »">
    ...
    [Résoudre « Cette application est bloquée » pour les connecteurs Google](/fr/features/troubleshooting/debugging-google-connectors)
  </Accordion>
