---
title: "构建移动应用"
description: "使用 Agent 构建一款以手机为先的应用，在模拟器和手机上测试，并分享给他人以收集反馈。"
sidebarTitle: "构建移动应用"
icon: "mobile-screen"
---
...
移动应用在手机上用起来应该很自然：打开快、点按方便、单手操作也顺手。

你将构建 **Pace Mobile**——一款以手机为先的跑步追踪应用，灵感来自 [Replit 是什么？](/zh/welcome) 中的跑步追踪示例。目标是在投入完整的 App Store 上架流程之前，先验证一个聚焦的移动端流程。

<Frame>
  <img src="/assets/mirror/mobile-app-4203ff45ecd5.png" alt="一个精致的移动应用示例，包含专为手机使用设计的健身追踪界面" width="1440" height="850" data-path="images/build-examples/mobile-app.png" />
</Frame>
...
* 在模拟器中测试应用。
* 使用 Expo Go 在真机上测试应用。
* 判断在着手 App Store 或 TestFlight 相关工作之前，何时应继续迭代。
...
本指南适用于追踪工具、预订流程、现场工具、活动 App、学习工具、习惯养成 App，以及任何需要脱离桌面设备使用的产品。

<Note>
  TestFlight 和 App Store 发布不在本指南涵盖范围内。准备好进行这一步时，请参阅 [构建并发布移动 App](/zh/build/mobile-app)。
</Note>
...
关于模拟器、仿真器和设备预览的具体说明，请参阅[原生移动应用](/zh/features/artifact-types/building-mobile-apps)。

<div id="define-the-mobile-flow">
  ## 定义移动端流程
</div>

在开始构建前，先确定用户应如何在手机上操作。

对于 Pace Mobile：

* **目标用户**：希望快速记录跑步的跑者。
* **主要流程**：选择跑步类型 → 输入距离和时间 → 保存跑步记录 → 查看今日摘要。
* **移动端限制**：主要操作应能用单手拇指轻松触及。
* **首个版本**：不提供账户、社交动态或高级分析功能。

这样可使首个移动端功能模块足够精简，便于测试。

<div id="build-the-first-version">
  ## 构建第一个版本
</div>

打开 Replit 并新建一个 App。描述你想构建的内容，然后选择 **Mobile app** 作为 App 类型。

<Frame>
  <img src="/assets/mirror/prompt-5cb144699b45.png" alt="Replit 主屏幕上的提示词输入框，已选择 Mobile app 作为 App 类型" width="1440" height="900" data-path="images/native-mobile-apps/prompt.png" />
</Frame>

使用以下提示词：

<AiPrompt>
  构建 Pace Mobile，一款以手机为先的跑步追踪应用。

  <br />

  跑者应能选择跑步类型、输入距离和时间、保存跑步记录，
  并查看今日汇总。

  <br />

  请针对单手操作进行设计，采用较大的触控目标、简洁的导航，以及
  精致的健身 App 风格。

  <br />

  保持第一个版本足够精简，以便今天就能在模拟器和真实
  手机上进行测试。
</AiPrompt>

如果你希望 Agent 在构建前先进行规划，请使用 [Plan 模式](/zh/features/agent/plan-mode)。让 Agent 在开始构建前检查屏幕、导航、触控目标以及手机端的特定限制。

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Agent 确定移动 App 的构建范围前，Replit 提示词编辑器中已选择 Plan 模式" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Agent 完成首次构建后，打开 [Preview](/zh/features/editor/preview)。在设备选择器中，选择 **iOS Simulator** 或 **Android Emulator**。

像在手机上使用应用一样，逐个点按体验一遍。

<Frame>
  <img src="/assets/mirror/workspace-dda0f81e6b83.png" alt="Replit Project Editor 显示移动应用预览以及在移动设备上预览的选项" width="1440" height="900" data-path="images/native-mobile-apps/workspace.png" />
</Frame>
...
模拟器测试很有用，但只有在手机上才能判断 App 的实际使用体验是否合适。

打开移动设备预览面板，用手机扫描二维码，即可在 Expo Go 中打开 App。

<Frame>
  <img src="/assets/mirror/mobile-screen-e5685fc26793.png" alt="在 Replit Project Editor 的 Preview 中运行的移动 App，显示“在设备上预览”面板和用于 Expo Go 的二维码" width="1440" height="900" data-path="images/native-mobile-apps/mobile-screen.png" />
</Frame>
...
如果某处感觉不够顺手，请让 Agent 修复该部分，而不是重新设计整个 App。

<AiPrompt>
  优化 Pace Mobile，使其更适合单手使用。

  <br />

  让 Save Run 按钮更容易触及，在需要时增大触控目标，
  并保持跑步表单简洁。
...
当手机端流程运行正常后，发布 App，创建可分享的 Expo Go 版本，以便尽早收集反馈。

附上一个明确的 task 与他人分享：

> 在手机上打开 Pace Mobile，保存一次跑步记录，并告诉我哪些地方让你觉得点按、阅读或理解起来不够顺畅。

与笼统的意见相比，聚焦的反馈更容易采纳。如果反馈指出一个明确的问题，请让 Agent 做出最小改动，然后再次在手机上测试 App。

<div id="publish-to-the-app-store">
  ## 发布到 App Store
</div>

在模拟器中以及通过 Expo Go 在手机上测试完 Pace Mobile 后，你可以将同一个应用发布到 TestFlight 和 App Store。此流程需要拥有已开启双重身份验证的 **Apple Developer Program** 会员资格。有关注册流程、个人账户与组织账户的区别、TestFlight 与 App Store 审核以及外部测试的背景信息，请参阅[构建并发布移动应用](/zh/build/mobile-app)。

<Steps>
  <Step title="打开 Publishing 选项卡，开始发布到 App Store">
    Expo Go 预览上线后，打开 **Publishing** 选项卡。在 **Production** 下，你会看到 **Ready to publish Pace to the App Store?** 卡片。选择 **Start publishing to the App Store**。

    <Frame>
      <img src="/assets/mirror/app-store-02-start-publishing-190eb7841bcf.png" alt="Project Editor 中的 Publishing 选项卡，显示 Production 状态，以及包含 Start publishing to the App Store 按钮的 Ready to publish Pace to the App Store? 卡片" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-02-start-publishing.png" />
    </Frame>
  </Step>

  <Step title="选择 Expo 项目">
    Replit 会打开 **Launch**，这是嵌入 Replit 的 Expo 流程。输入或确认 Expo 项目名称和 slug，然后选择 **Continue**。如果你已有 Expo 项目，请选择 **Use existing**。

    <Frame>
      <img src="/assets/mirror/app-store-03-choose-expo-project-4707fafffafe.png" alt="Launch 对话框，当前处于 Choose Expo project 步骤——Project name 和 Project slug 字段已预填为 Pace，包含 Continue 按钮和 Use existing 选项" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-03-choose-expo-project.png" />
    </Frame>
  </Step>

  <Step title="使用 Apple 登录">
    连接将拥有 App Store 上架信息的 Apple Developer 账户。该账户必须已加入 Apple Developer Program。Apple 要求使用双重身份验证，因此请准备好在受信任设备上确认验证码。

    <Frame>
      <img src="/assets/mirror/app-store-04-sign-in-apple-459dfc9b84ba.png" alt="Launch 对话框的 Sign in with Apple 步骤——包含 Apple Developer 电子邮件地址或电话号码和密码字段，并注明密码不会离开设备，且账户必须已加入 Apple Developer Program" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-04-sign-in-apple.png" />
    </Frame>
  </Step>

  <Step title="选择或创建 Apple app">
    输入将在 App Store 中显示的 **App name** 和 **Bundle ID** (例如 `app.replit.pace`) 。首次构建后无法更改 Bundle ID，因此请谨慎选择。

    <Frame>
      <img src="/assets/mirror/app-store-05-choose-apple-app-3971aea62f14.png" alt="Launch 对话框的 Choose Apple app 步骤——App name 设为 Pace，Bundle ID 为 app.replit.pace，提示之后无法更改 Bundle ID，并显示 Creating app… 按钮" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-05-choose-apple-app.png" />
    </Frame>
  </Step>

  <Step title="发布到 App Store">
    Replit 会代你配置 Apple 证书。当 **Ready for launch** 步骤就绪后，选择 **Launch to App Store**。Replit 会在云端构建原生 app，并将构建提交到 App Store Connect 进行 TestFlight Beta 审核。

    <Frame>
      <img src="/assets/mirror/app-store-06-ready-for-launch-ab02659ee0bb.png" alt="Launch 对话框的 Ready for launch 步骤，其中 Choose Apple app 和 Configure Apple certificates 标记为已完成，并提供 Launch to App Store 按钮和 Optional launch parameters" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-06-ready-for-launch.png" />
    </Frame>

    Launch 完成后，你会看到确认信息，表明该 app 将发布到 App Store；随后对话框会关闭并返回 Project Editor。

    <Frame>
      <img src="/assets/mirror/app-store-targeting-f8797071c9e0.png" alt="Launch 对话框中 Targeting App Store 标记为已完成，并显示 Redirecting back to Replit 消息——选择 Launch to App Store 后的确认状态" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-targeting.png" />
    </Frame>
  </Step>
</Steps>

<Note>
  某个版本的首个构建通常需要经过 TestFlight
  测试审核，测试人员才能安装。审核通过后，后续构建通常能更快推送给测试人员。若要共享给外部测试人员或提交至
  App Store 发布，请在 **App Store Connect** 中完善应用信息 —— 完整的 App Store Connect
  检查清单请参阅[构建并发布移动应用](/zh/build/mobile-app)。
</Note>
...
* App 可在 iOS Simulator 或 Android Emulator 中打开。
* App 可通过 Expo Go 在你的手机上打开。
* 你可以完成主要的移动端流程。
* 文本、按钮和导航在手机上使用起来很舒适。
* 你已发布或分享了可供测试的移动版本。
* 至少有一人可以试用并提供反馈。
* (可选) 你已通过 App Store 流程将 App 提交至 TestFlight。
...
<CardGroup cols={2}>
  <Card title="添加登录功能" icon="user-lock" href="/zh/build/add-login">
    让每位用户查看自己的进度。
  </Card>
  ...
  <Card title="构建并发布移动 App" icon="mobile-screen" href="/zh/build/mobile-app">
    将可用的移动 App 推向 TestFlight 和 App Store。
  </Card>
</CardGroup>
...
* **Agent 构建成了 web app：**重新开始，并在构建前选择 **Mobile app** 作为应用类型。
* **Expo Go 预览未更新：**摇晃手机，打开开发者菜单，然后选择 **Refresh**。
* **触控目标太小：**让 Agent 增大触控区域，并将主要操作置于拇指可触及的范围内。
* **导航过于拥挤：**让 Agent 将应用精简为主流程所需的最少页面。
* **模拟器可以运行，但手机无法运行：**在真实设备上测试硬件相关行为，并参阅 [Native Mobile Apps](/zh/features/artifact-types/building-mobile-apps)。
* **Publishing 失败：**请参阅[移动应用故障排除](/zh/features/troubleshooting/mobile-app)。

<div id="try-it-now">
  ## 立即试用
</div>

<AgentInput highlightStart defaultPrompt="构建 Pace Mobile：一款以手机为先的跑步追踪应用。跑者应能选择跑步类型、输入距离和时间、保存跑步记录，并查看今日摘要。请针对单手操作进行设计，采用较大的触控目标、简洁的导航和精致的健身 App 风格。请将首个版本保持精简，以便今天就在模拟器和真实手机上测试。" showTypewriter={false} />

<div id="related">
  ## 相关内容
</div>

* [原生移动应用](/zh/features/artifact-types/building-mobile-apps)
* [构建并发布移动应用](/zh/build/mobile-app)
* [氛围编程 101](/zh/learn/foundations/vibe-coding-101)
* [移动应用故障排除](/zh/features/troubleshooting/mobile-app)