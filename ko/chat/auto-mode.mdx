...
description: "Replit이 각 작업에 적합한 모델을 선택하도록 하고, 필요한 경우에만 수동으로 모델을 선택하세요."
...
<div className="chat-mkt" role="img" aria-label="지능형 모델 라우팅을 나타내는 Anthropic, OpenAI, Kimi 마크" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
지능형 모델 라우팅을 사용하면 품질과 비용의 균형을 맞추면서 Replit이 각 작업에 적합한 모델을 선택합니다. 제품에서 **Auto**로 표시되어 있다면 Replit이 지능형 모델 라우팅을 실행하고 있는 것입니다.

## 지능형 모델 라우팅 활성화하기

1. 프롬프트 입력창에서 **Agent Mode**를 선택합니다. 메뉴에 사용 가능한 모드(**Free**, **Power**, **Max**)가 표시됩니다.
2. **Power** 또는 **Max**를 선택한 다음 **Primary model** 섹션을 찾습니다. 이 레이블은 선택한 모드에 따라 달라지며, 예를 들어 **Primary Max model**처럼 표시될 수 있습니다.
3. 프라이머리 모델이 이미 **Auto**로 설정되어 있다면 지능형 모델 라우팅이 활성화된 상태입니다. 특정 모델이 선택되어 있다면 프라이머리 모델 메뉴를 열고 **Automatically choose**를 선택하세요.

Free 모드는 항상 지능형 모델 라우팅을 사용하며 모델을 수동으로 선택할 수 없습니다.

## 작업에 따라 모델이 결정되도록 하기

작업이 변할 때 Replit이 모델 선택을 처리하기를 원한다면 지능형 모델 라우팅이 유용합니다. 명확한 요청에는 저비용 모델을 사용할 수 있고, 복잡한 작업에는 더 성능이 뛰어난 모델이 필요할 수 있습니다.

Power 모드나 Max 모드에서는 Core 및 Pro 사용자가 대신 사용 가능한 모델을 수동으로 선택할 수 있습니다.

<Frame caption="Agent 설정의 지능형 모델 라우팅, 모델 선택, Effort.">
  <img src="/assets/mirror/auto-mode-model-and-effort-selector-fdcabef57097.jpg" alt="Automatically choose recommended, 사용 가능한 모델, Effort 컨트롤, 지능형 모델 라우팅을 사용하는 Max 모드가 표시된 Agent 설정" width="1839" height="855" data-path="images/chat/auto-mode-model-and-effort-selector.jpg" />
</Frame>

## 더 알아보기

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ko/features/agent/model-selector#지능형-모델-라우팅">
    ...
    <div className="chat-card-title">지능형 모델 라우팅 참조</div>

    <p>
      Replit이 모델을 선택하는 방식과 수동 선택이 가능한 시점을 알아보세요.
    </p>
  ...
  <a className="chat-example has-visual" href="/ko/chat/model-selector">
    ...
    <p>더 세밀한 제어가 필요할 때 기본 모델을 선택하고 Effort를 설정하세요.</p>
    <div className="chat-card-link">계속하기 →</div>
  </a>
</section>

지능형 모델 라우팅이 제공하는 것보다 더 세밀한 제어가 필요하다면 [모델 선택하기](/ko/chat/model-selector)로 이동하세요.
