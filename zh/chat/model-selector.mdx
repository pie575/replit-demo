...
<div className="chat-mkt" role="img" aria-label="代表模型选择器的 Anthropic、OpenAI 和 Qimi 标识" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
智能模型路由可让 Replit 为 task 自动选择模型。如需自行选择，请在提示框中选择 **Agent Mode**，再选择 **Power** 或 **Max**，然后在 **主模型** 部分选择模型。

Free Mode 始终使用智能模型路由。Core 和 Pro 的构建者可在 Power Mode 或 Max Mode 中选择可用模型。Replit 中的模型列表是您账户信息的权威来源，因为可用模型可能会有所不同。

<Frame caption="Agent 设置菜单中的模型和 Effort 控件。">
  <img src="/assets/mirror/model-selector-gpt-5-6-sol-effort-8e4d256bf497.jpg" alt="演示 Workspace 的模型选择器：在 Max Mode 中选中了 GPT-5.6 Sol，Effort 设置为 Medium" width="1821" height="864" data-path="images/chat/model-selector-gpt-5-6-sol-effort.jpg" />
</Frame>
...
Effort 控制所选模型在生成回复时投入的推理量。这是针对每个模型的设置，而非独立模式。支持的级别从 **Low** 到 **Max**，每个模型都会显示推荐设置。

仅当 task 需要更深入的推理时，才使用更高的 Effort。对于复杂任务，更高的设置可能需要更长时间，成本也更高。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/features/agent/model-selector">
  ...
  <a className="chat-example has-visual" href="/zh/features/agent/model-selector#effort">
    ...
    <div className="chat-card-link">了解 Effort →</div>
  </a>
</section>

模型和 Effort 设置会影响 Replit 的响应方式。[语音模式](/zh/chat/voice-mode)让你可以通过语音而非输入文字来提出请求。