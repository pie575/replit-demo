---
title: "Replit Agent"
description: "Replit Agent превращает идеи в App, дизайн, слайды и многое другое. Знание программирования не требуется."
---
...
Agent — ваш творческий партнёр. Он помогает развивать ваши идеи и воплощать их в жизнь. В отличие от чат-бота, который лишь отвечает на вопросы, Agent действует: настраивает проекты, создаёт приложения, проверяет свою работу и исправляет возникающие проблемы.

Просто опишите обычными словами, что вам нужно. Знание кода или технические навыки не требуются. Agent берёт на себя всё — от планирования до деплоя.

<div id="replit-agent-2">
  ## Как пользоваться Replit Agent
</div>
...
<Steps>
  <Step title="Опишите, что хотите создать">
    Начните чат в [редакторе проекта](/ru/ja/learn/projects-and-artifacts/project-editor). Опишите App, которое хотите создать, задайте вопросы, изучите тему или получите данные из [подключённых сервисов](/ru/ja/features/agent/general-agent), таких как BigQuery, Slack и Notion. Никаких ограничений нет. Replit Agent справится с чем угодно.

    <AgentInput />
  </Step>

  <Step title="Выберите, что создать">
    При желании выберите тип проекта: веб-App, мобильное App, слайды, дизайн, визуализация данных и другие. Если на шаге 1 вы уже описали желаемое, Agent автоматически определит подходящие настройки.

    <AgentCarousel />
  </Step>

  <Step title="Agent приступает к созданию">
    Agent пишет код, настраивает инфраструктуру и тестирует результат. В проекте вы можете переключать [режимы Agent](#agent-modes), чтобы управлять процессом создания.
  </Step>

  <Step>
    Общайтесь с Agent, чтобы улучшить проект, а когда всё будет готово — опубликуйте его. Все артефакты разворачиваются вместе.
  </Step>
</Steps>

<Note>Необязательно ограничиваться одним типом. Начните с веб-App, а затем добавьте мобильное App, слайды или видео — всё в рамках одного проекта.</Note>

<div id="plan">
  ### Режим Plan
</div>

Включите режим Plan, чтобы перед внесением изменений в код или данные Agent мог провести мозговой штурм, задать вопросы и изучить структуру проекта. В режиме Plan Agent:

* **разбивает сложные проекты** на упорядоченный список задач
* **рассматривает разные подходы** и оценивает компромиссы
* **даёт проверить и доработать план до написания кода**

Нажмите «Plan» в поле ввода чата или просто попросите Agent перейти в режим Plan. Например: «Составь план создания трекера проектов для команды»

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent создаст упорядоченный список задач, который можно проверить и доработать. Когда план вас устроит, подтвердите его, и Agent начнёт работу.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="Диалог создания плана задач с четырьмя предложенными задачами и вариантами «Подтвердить задачи» и «Изменить план»" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

Подробнее см. в [руководстве по режиму Plan](/ru/ja/features/agent/plan-mode).

<div id="agent">
  ### Режимы Agent
</div>

Выберите, как Agent будет создавать проект:

<Frame caption="Всплывающее окно режимов Agent с режимами Free, Power и Max.">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Всплывающее окно режимов Agent с режимами Free, Power и Max: выбран Power, отображаются список основных моделей и ползунок уровня усилий" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**: бесплатный режим для повседневных задач с Agent для подписчиков Core и Pro. Лимит сбрасывается каждые 5 часов, при этом действует недельное ограничение. Для тарифа Starter сохраняются текущие лимиты. Использование можно проверить в разделе **Настройки → Использование**.
* **Power**: платный режим среднего уровня с мощными моделями по доступной цене.
* **Max**: платный режим верхнего уровня с лучшими передовыми моделями для самых требовательных задач. В **Дополнительных настройках** можно также включить **Turbo** для более быстрого выполнения.

Для запросов, требующих возможностей сверх Free Mode, Agent может предложить платную опцию **Разрешить один раз**. Replit запросит подтверждение до начала платного действия, после чего Agent вернётся в Free Mode.

<Note>Совет: вы также можете включить **[режим Plan](/ru/ja/features/agent/plan-mode)**, чтобы просмотреть и улучшить план Agent до начала создания.</Note>

<div id="replit-agent-3">
  ## Что можно создать с помощью Replit Agent
</div>

* **Веб- и мобильные App, панели мониторинга данных и инструменты на базе ИИ**
* **Визуальные дизайны и прототипы** — изучайте макеты в [Design Canvas](/ru/ja/design/canvas), прежде чем переходить к коду
* **Несколько результатов в одном проекте** — веб- и мобильные App, слайды и видео с общим бэкендом
* **Файлы и документы** — CSV, PDF, файлы PowerPoint и документы Markdown
* **Запросы к подключённым сервисам** — получайте данные из BigQuery, Linear, Slack, Notion и других сервисов прямо в чате
...
<Accordion title="Нужно ли уметь писать код?">
  Нет. Agent берёт на себя всю техническую работу: написание кода, настройку инфраструктуры и баз данных. Просто опишите желаемое обычными словами, и Agent всё создаст.
</Accordion>

<Accordion title="Чем отличаются Free, Power и Max?">
  **Free Mode** позволяет подписчикам Core и Pro бесплатно выполнять повседневные задачи с Agent в пределах доступного лимита. **Power** — платный режим среднего уровня с доступом к мощным моделям по доступной цене. **Max** — платный режим высшего уровня для самых требовательных задач. **Turbo** — дополнительный переключатель в расширенных настройках, доступный только для Max.

  Перед началом платные действия требуют подтверждения.
</Accordion>

<Accordion title="Можно ли создавать несколько вещей в одном проекте?">
  Да. В [один и тот же проект](/ru/ja/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) можно добавить веб-App, мобильные App, слайды, видео и визуализации данных — все они используют общий бэкенд и данные.
</Accordion>

<Accordion title="Что произойдёт, если Agent допустит ошибку?">
  Agent регулярно тестирует свою работу. Он также создаёт чекпоинты во время работы, поэтому вы можете откатиться к любому предыдущему состоянию. Напишите Agent в чате, что пошло не так, и он исправит проблему.
</Accordion>
...
| Функция                           | Core | Pro |
| --------------------------------- | ---- | --- |
| Чат и создание приложений с Agent | ✅    | ✅   |
| Free Mode                         | ✅    | ✅   |
| Режим Power                       | ✅    | ✅   |
| Режим Max                         | ✅    | ✅   |
| Design Canvas                     | ✅    | ✅   |
...
Turbo по умолчанию отключён — при необходимости включайте его для отдельных проектов, выбрав Max. Free Mode недоступен в Enterprise на момент запуска. Вскоре для рабочих пространств Enterprise станет доступен специальный интерфейс Agent. Подробнее уточняйте у администратора.

Подробную информацию о ценах см. в разделе [Биллинг Agent](/ru/ja/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  <Card title="Навыки Agent" icon="puzzle-piece" href="/ru/ja/features/agent/skills">
    Научите Agent специальным знаниям — используйте готовые навыки или создайте свои.
  </Card>

  <Card title="Система задач" icon="list-check" href="/ru/ja/core-concepts/agent/task-system">
    Планирование на канбан-доске, фоновые задачи, применение изменений к основной версии.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/ru/ja/design/canvas">
    Визуальные макеты и удобный визуальный редактор.
  </Card>

  <Card title="Коннекторы" icon="plug" href="/ru/ja/features/agent/general-agent">
    Подключайте BigQuery, Linear, Slack, Notion и другие сервисы.
  </Card>

  <Card title="Биллинг" icon="credit-card" href="/ru/ja/billing/ai-billing#agent-billing">
    Стоимость Agent, сравнение планов, управление расходами.
  </Card>

  <Card title="Руководство по вайб-кодингу" icon="wand-magic-sparkles" href="/ru/ja/learn/build-with-agent">
    Советы по эффективным промптам и созданию приложений с помощью ИИ.
  </Card>
</CardGroup>