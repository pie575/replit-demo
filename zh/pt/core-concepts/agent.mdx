---
title: "Replit Agent"
description: "Replit Agent 能将你的想法变成应用、设计、幻灯片等成果，只需用自然语言描述，无需编写代码。"
---
...
Agent 是您的创意伙伴。它接收您的想法，帮助您将其打磨完善，并将其变为现实。与只会回答问题的聊天机器人不同，Agent 能够付诸行动：配置您的项目、创建应用、检查您的成果，并在过程中解决问题。

用日常语言描述您的需求即可，无需编写代码或具备技术知识。Agent 会负责其余工作，从规划到部署。
...
<Steps>
  <Step title="描述你的需求">
    在 [Project Editor](/zh/pt/core-concepts/project-editor) 中，只需开始对话。描述你想创建的应用、提出问题、研究某个主题，或从 BigQuery、Slack、Notion 等[已连接的服务](/zh/pt/core-concepts/agent/general-agent)中获取数据。没有任何限制。Replit Agent 会处理你需要的一切。

    <AgentInput />
  </Step>

  <Step title="选择要创建的内容">
    你也可以选择项目类型：Web 应用、移动应用、幻灯片、设计、数据可视化等。如果你已在第 1 步描述了需求，Agent 会自动确定合适的配置。

    <AgentCarousel />
  </Step>

  <Step title="Agent 开始构建">
    Agent 会编写代码、配置基础设施并测试结果。进入项目后，你可以切换 [Agent 模式](#agent-modes)，控制它的构建方式。
  </Step>

  <Step title="迭代、测试和部署">
    与 Agent 对话以完善你的项目，并在准备就绪后发布。所有产物都会一同部署。
  </Step>
</Steps>

<Note>你不必局限于单一类型。可以先创建 Web 应用，之后再添加移动应用、幻灯片或视频——一切都在同一个项目中完成。</Note>

<div id="modo-planejamento">
  ### 规划模式
</div>

启用规划模式，在 Agent 修改任何代码或数据之前先探索想法、提出问题并梳理项目。在规划模式下，Agent 将会：

* **拆分复杂项目**，生成有序的任务清单
* **探索不同方案**并权衡利弊
* **审查并完善**，再开始编写代码

点击聊天输入框中的“Plan”，或直接让 Agent 制定计划，即可切换到规划模式。例如：“为我的团队制定一个构建项目跟踪器的计划”

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent 会创建一份有序的任务清单，供你审查和完善。确认计划无误后，批准它，Agent 就会开始构建。

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="已创建任务计划的对话框，显示四项建议任务，并提供接受任务和审查计划选项" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

前往[规划模式指南](/zh/pt/core-concepts/agent/plan-mode)了解更多。
...
* **Lite**：快速完成轻量、经济的改动。Lite 模式适合视觉微调、缺陷修复以及范围明确的功能。
* **Economy**：使用 Agent 的成本优化模型处理日常任务。展开**高级设置**可管理 App 测试和代码优化等功能。
* **Power**：使用 Agent 能力最强的模型处理更棘手的问题、更大规模的改动和更长时间的构建。在**高级设置**中，你还可以开启 **Turbo**，以更高的成本获得最高 2.5 倍的构建速度 (仅限 Pro) 。

<Note>提示：你也可以开启**[规划模式](/zh/pt/core-concepts/agent/plan-mode)**，在构建开始前审阅并迭代 Agent 的计划。Max 模式已不再提供；如需能力最强的标准构建，请使用 Power。</Note>
...
* **视觉设计和原型** — 在开始编写代码前，先在 [Design Canvas](/zh/pt/replitai/canvas) 中探索设计稿
* **一个项目中的多种产出** — 共享同一 backend 的 Web 应用、移动应用、幻灯片和视频
* **文件和文档** — CSV、PDF、PowerPoint 文件和 Markdown 文档
* **查询已连接的服务** — 直接在 Chat 中从 BigQuery、Linear、Slack、Notion 等服务获取数据

<Card title="立即开始创建" icon="arrow-up-right" href="https://replit.com">
...
<Accordion title="我需要会编程吗？">
  不需要。Agent 会处理所有技术工作，包括编写代码、配置基础设施和设置数据库。您只需用简单的语言描述需求，Agent 就会完成构建。
</Accordion>

<Accordion title="Lite、Economy 和 Power 有什么区别？">
  **Lite** 适合快速、目标明确的修改 (10–60 秒) ，例如视觉调整、修复错误和范围明确的功能。**Economy** 是大多数 Build 的最佳默认选择，可兼顾成本与质量。**Power** 使用能力最强的模型处理更困难的任务；如需最快的执行速度，可在高级设置中启用仅适用于 Power 的可选 **Turbo** 开关。
</Accordion>

<Accordion title="我可以在一个项目中创建多个内容吗？">
  可以。您可以将 Web 应用、移动应用、幻灯片、视频和数据可视化添加到[同一个项目](/zh/pt/replitai/artifacts#multiple-artifacts-in-one-project)中——它们都共享同一后端和数据。
</Accordion>

<Accordion title="如果 Agent 出错了怎么办？">
  Agent 会定期测试自己的工作。它还会在工作过程中创建检查点，因此您可以还原到之前的任意状态。您也可以与 Agent 对话，描述出了什么问题，它会修复问题。
</Accordion>
...
有关详细定价，请参阅 [Agent 计费](/zh/pt/billing/ai-billing#agent-billing)。
...
<CardGroup cols={2}>
  <Card title="Agent skills" icon="puzzle-piece" href="/zh/pt/core-concepts/agent/skills">
    向 Agent 传授专业知识——使用预构建的 skills，或创建自己的 skills。
  </Card>

  <Card title="并行构建" icon="list-check" href="/zh/pt/core-concepts/agent/task-system">
    使用看板进行规划、处理后台任务，并将更改应用回主版本。
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/zh/pt/replitai/canvas">
    视觉模型和直观的可视化编辑器。
  </Card>

  <Card title="连接器" icon="plug" href="/zh/pt/core-concepts/agent/general-agent">
    连接 BigQuery、Linear、Slack、Notion 等众多服务。
  </Card>

  <Card title="账单" icon="credit-card" href="/zh/pt/billing/ai-billing#agent-billing">
    Agent 定价、套餐比较和支出管理。
  </Card>

  <Card title="vibe coding 指南" icon="wand-magic-sparkles" href="/zh/pt/tutorials/how-to-vibe-code">
    编写有效提示词和使用 AI 构建应用的技巧。
  </Card>
</CardGroup>