---
title: "Skills 사용하기"
description: "재사용 가능한 지침과 보조 자료로 Replit이 반복 작업을 처리하는 방식을 맞춤 설정하세요."
sidebarTitle: "Skills 사용하기"
icon: "puzzle-piece"
canonical: "https://docs.replit.com/chat/agent-skills"
---

<div className="chat-mkt" role="img" aria-label="Agent skills를 나타내는 기하학적 도형" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
Skill은 재사용 가능한 지침과 보조 파일을 모아 둔 폴더입니다. Skill을 사용하면 반복되는 작업이나 규칙, workflow를 Replit이 어떤 방식으로 처리할지 원하는 대로 설정할 수 있습니다.

prompt box에서 **더하기** 메뉴를 선택한 다음 **Use a skill**을 선택하세요. 작업에 맞는 Skill을 고른 뒤 원하는 결과를 Replit에 알려주세요.

<div id="choose-the-right-connection">
  ## 알맞은 연결 방식 선택
</div>

* 재사용 가능한 지침, 예시, 보조 자료에는 **Skill**을 사용하세요.
* 지원되는 서비스와 그 서비스의 승인된 정보나 작업에는 [integration](/ko/chat/integrate-your-tools)을 사용하세요.
* 호환되는 MCP 서버나 클라이언트를 통해 제공되는 도구에는 [MCP](/ko/chat/connect-through-mcp)를 사용하세요.
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ko/features/agent/skills">
    ...
    <p>skill의 구조와 탐색 방식, 활용 방법을 알아보세요.</p>
    <div className="chat-card-link">기능 가이드 열기 →</div>
  </a>

  <a className="chat-example has-visual" href="/ko/build/use-agent-skills">
    ...
    <div className="chat-card-title">project에서 skill 사용하기</div>
    <p>skill을 추가하고 사용하는 자세한 workflow를 따라해 보세요.</p>
    <div className="chat-card-link">가이드 열기 →</div>
  </a>
</section>

skill은 Replit 안에서 재사용할 수 있는 작업 방식을 제공합니다. 호환되는 클라이언트나 서버용 프로토콜 기반 확장이 필요하다면 [MCP로 연결하기](/ko/chat/connect-through-mcp)로 이어서 진행하세요.