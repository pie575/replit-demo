---
title: "使用 Agent 构建"
description: "与 Agent 高效协作的五个实用习惯：描述具体、制定计划、补充上下文、审查与测试、善用检查点。"
...
Agent 是 Replit 的 AI 构建者，也是将创意转化为软件的伙伴。Agent 可以规划变更、编写代码、解释行为、调试问题并改进应用。

如果说 [Vibe coding 101](/zh/ko/learn/foundations/vibe-coding-101) 讲的是思维方式，那么本页讲的就是如何在实践中与 Agent 协作。[高效提示](/zh/ko/learn/effective-prompting)则介绍如何写出更清晰的指令。

可以把 Agent 想象成球场上的四分卫：解读局势、选择路线、推动进攻向前。而你是教练：制定策略、决定战术、复盘结果并决定下一步行动。

Agent 能做的事情很多，但当你主导整个流程时，它的表现最佳：描述具体、先做计划、补充上下文、检查并测试，必要时使用检查点。
...
Agent 能够处理复杂任务，但需要明确的方向。

好的请求会说明你想要什么、什么最重要，以及哪些内容不能改动。你不必交代每一个实现细节，但要给出足够的方向，让 Agent 能做出合理的判断。
...
<AiPrompt>
  改进注册页面，让访客在创建账户之前就能理解产品的用途。<br />
  保留当前的配色方案，表单字段维持原样，并让主按钮更加醒目。
</AiPrompt>

第二个提示词为 Agent 提供了目标、关注重点和约束条件。

当一个请求包含多个部分时，请说明哪一部分最重要。

<AiPrompt>
  添加一个简单的管理员视图，用于管理餐饮服务请求。<br />
  最重要的是能够查看新请求、将其标记为已联系，并按状态筛选。<br />
  第一个版本请保持简单。
</AiPrompt>

Agent 可以把任务拆解成多个步骤，但你给出的方向决定了怎样才算成功。

更多具体指令示例，请参阅[高效提示](/zh/ko/learn/effective-prompting)。

<div id="section-2">
  ## 制定计划
</div>

当任务存在不确定性、涉及多个步骤、需要权衡取舍,或者你希望在文件被改动前先行确认时,计划都会很有帮助。

你可以在对话中让 Agent 制定计划:

<AiPrompt>
  在做任何改动之前,先为添加在线订购功能制定一个计划。<br />
  请包含用户流程、所需数据、会变动的页面或组件、风险点,以及应该如何测试。
</AiPrompt>

如果你希望 Agent 在修改文件之前先思考并等待你确认,请使用 [Plan 模式](/zh/ko/features/agent/plan-mode)。提交之前,在提示词编辑器中打开 **Plan** 开关。

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Replit 首页提示框中输入了运行中应用的提示词,且 Plan 复选框已勾选" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>

<AgentInput defaultPrompt="在做任何改动之前,先为添加在线订购功能制定一个计划。请包含用户流程、所需数据、会变动的页面或组件、风险点,以及应该如何测试。" highlightPlan={true} showTypewriter={false} />

Agent 思考完成后,会显示带有**立即查看**按钮的**工作计划已可供查看**横幅。

<Frame>
  <img src="/assets/mirror/agent-plan-ready-banner-5fbcf58a1668.png" alt="Agent 面板底部显示带有立即查看按钮的工作计划已可供查看横幅" width="560" height="550" data-path="images/fitstart/work-with-agent/agent-plan-ready-banner.png" />
</Frame>
...
上下文能帮助 Agent 理解什么才是重要的。

你可以通过文本、截图、草图、文件、数据、报错信息或 Canvas 批注来添加上下文。选择最能说明问题的形式即可。

当改动涉及行为、目标或约束条件时，使用文本：

<AiPrompt>
  上下文：这个应用面向订购生日蛋糕的家长。<br />
  请让表单保持简短，取货信息清晰明了，并避免任何过于商务化的感觉。
</AiPrompt>

当布局难以用语言描述时，使用截图或草图：

<AiPrompt>
  请把这张截图作为申请表单的布局参考。<br />
  整体结构照此调整，但保留现有的字段和提交行为。
</AiPrompt>

当改动偏视觉、且你想指向应用的某个特定部分时，使用 [Canvas](/zh/ko/design/canvas) 批注。

<Frame>
  <img src="/assets/mirror/agent-canvas-annotations-c88ab5856966.png" alt="Canvas 中的健身应用 frame，带有指向进度卡片的矩形、箭头和文本标签，表单区域有手绘草图和标签" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-canvas-annotations.png" />
</Frame>

<AiPrompt>
  请根据我的 Canvas 批注更新主视觉区域。<br />
  只把这些备注应用到标题、图片和主按钮上。<br />
  请不要改动餐饮预订表单。
</AiPrompt>

好的上下文还会说明哪些内容不该改动。这样 Agent 才能只优化某一个区域，而不会无意中重写其他部分。

<div id="section-4">
  ## 检查与测试
</div>

Agent 会在构建前提出计划，并在构建后总结所做的更改。两者都要读一读。

在工作计划卡片中选择**查看**即可打开计划。展开后的计划会显示 Agent 打算构建什么、成功标准、哪些内容不在范围内，以及 Agent 将遵循的步骤。

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="展开的工作计划，显示 What and Why、完成标准、范围外事项以及带编号的构建步骤" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>
...
<AiPrompt>
  提交餐饮服务申请表单后会出现成功提示，但表单里仍显示之前输入的文字。<br />
  提交后请清空表单，并让成功提示继续可见。
</AiPrompt>

具体的反馈能帮助 Agent 修复真正的问题。
...
<AiPrompt>
  已经很接近了，但表单变得太长。<br />
  保留新的视觉风格，移除可选备注字段，并让取货日期更易于阅读。
</AiPrompt>

如果应用变得比之前更差、重要功能失效，或 Agent 的更改超出预期，请使用检查点。

在 Agent 面板中打开 **History** 查看检查点。每个检查点都会显示更改内容，以及 **回滚到此处** 和 **更改** 控件。

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="Agent History 面板，显示一个标有“已从 Plan 模式切换到 Build 模式”的检查点，以及“回滚到此处”和“更改”按钮" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

选择 **回滚到此处** 后，Agent 会在执行回滚前确认要恢复的内容。

<Frame>
  <img src="/assets/mirror/agent-checkpoints-rollback-dialog-28de278b44da.png" alt="检查点回滚确认对话框，列出将受影响的内容，包括目标检查点，以及文件、Agent 记忆、任务和数据库的复选框" width="900" height="880" data-path="images/fitstart/work-with-agent/agent-checkpoints-rollback-dialog.png" />
</Frame>

如果不确定该继续修复还是回滚，请让 Agent 帮你比较。

<AiPrompt>
  最后的更改让应用变得更差。<br />
  请将当前版本与之前可用的版本进行比较。<br />
  告诉我继续修复还是回滚更安全，以及原因。
</AiPrompt>

偏离方向是正常的。使用检查点，你可以放心探索，同时始终保留回到更安全版本的能力。

有关更多恢复指南，请参阅[检查点和回滚](/zh/ko/features/version-control/checkpoints-and-rollbacks)。

<div id="agent-2">
  ## 高效协作 Agent 的感觉
</div>

出现以下情况，说明您正朝着正确的方向前进：

* 请求明确说明了目标和约束条件。
* Agent 的计划足够清晰，便于审核。
* 文字不足以说明时，添加截图、文件和 Canvas 注释。
* 更改后测试关键流程。
* 出现问题时提供有针对性的反馈。
* 使用检查点，而不是反复尝试撤销错误的更改。
...
<CardGroup cols={2}>
  <Card icon="comment-dots" href="/zh/ko/learn/effective-prompting">
    了解如何编写更清晰的指令，让 Agent 能够放心执行。
  </Card>

  <Card title="构建你的第一个应用" icon="rocket" href="/zh/ko/build/your-first-app">
    从第一条提示词到有针对性的改进，练习与 Agent 协作。
  </Card>

  <Card title="检查点与回滚" icon="rotate-left" href="/zh/ko/features/version-control/checkpoints-and-rollbacks">
    了解如何在需要时恢复到更安全的状态。
  </Card>
</CardGroup>