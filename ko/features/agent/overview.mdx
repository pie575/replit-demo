---
title: "Replit Agent"
description: "Replit Agent는 평범한 일상 언어만으로 아이디어를 앱, 디자인, 슬라이드 등으로 만들어 줍니다. 코딩 지식은 필요 없습니다."
...
<YouTubeEmbed videoId="bAUVQfz_SIY" title="Replit Agent 4 소개" />

<div id="what-is-replit-agent">
  ## Replit Agent란?
</div>

Agent는 여러분의 창작 파트너입니다. 여러분의 아이디어를 받아 함께 다듬고, 실제로 구현해 냅니다. 질문에 답하기만 하는 챗봇과 달리 Agent는 직접 실행합니다. 프로젝트를 설정하고, 애플리케이션을 만들고, 스스로 결과물을 점검하며, 그 과정에서 생기는 문제까지 해결합니다.

원하는 것을 평소 쓰는 말로 설명하기만 하면 됩니다. 코드나 기술 지식은 필요 없습니다. 계획부터 배포까지 나머지는 Agent가 알아서 처리합니다.
...
<Steps>
  <Step title="원하는 것을 설명하세요">
    [Project Editor](/ko/learn/projects-and-artifacts/project-editor)에서 편하게 대화를 시작하세요. 만들고 싶은 앱을 설명하거나, 질문을 하거나, 주제를 조사하거나, BigQuery, Slack, Notion 같은 [연결된 서비스](/ko/features/agent/general-agent)에서 데이터를 가져올 수 있습니다. 제약은 없습니다. Replit Agent는 무엇을 요청하든 처리합니다.

    <AgentInput />
  </Step>

  <Step title="무엇을 만들지 선택하세요">
    원한다면 project 유형을 선택할 수 있습니다. 웹 앱, 모바일 앱, slides, design, data visualization 등이 있습니다. 1단계에서 이미 원하는 것을 설명했다면 Agent가 알맞은 설정을 자동으로 판단합니다.

    <AgentCarousel />
  </Step>

  <Step title="Agent가 만듭니다">
    Agent가 코드를 작성하고, 인프라를 설정하고, 결과를 테스트합니다. project에 들어간 뒤에는 [Agent modes](#agent-modes)를 전환해 빌드 방식을 조절할 수 있습니다.
  </Step>

  <Step>
    Agent와 대화하며 project를 다듬은 뒤, 준비가 되면 게시하세요. 모든 아티팩트가 함께 게시됩니다.
  </Step>
</Steps>

<Note>한 가지 유형에만 묶여 있지 않습니다. 웹 앱으로 시작해 나중에 모바일 앱, slides, 영상을 추가할 수 있습니다. 모두 같은 project 안에서 가능합니다.</Note>

<div id="plan-mode">
  ### Plan mode
</div>

Agent가 코드나 데이터를 변경하기 전에 Plan mode를 켜고 아이디어를 구상하고, 질문하고, 프로젝트를 구상해 보세요. Plan mode에서 Agent는 다음과 같이 동작합니다.

* **복잡한 프로젝트를 세분화**하여 순서가 정해진 task 목록으로 정리합니다
* **다양한 접근 방식을 살펴보고** 장단점을 비교합니다
* 코드를 작성하기 전에 **검토하고 다듬습니다**

Chat 입력창에서 &quot;Plan&quot;을 클릭하거나 Agent에게 요청하면 Plan mode로 전환됩니다. 예: &quot;우리 팀을 위한 프로젝트 트래커를 만들 계획을 세워줘&quot;

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent는 검토하고 다듬을 수 있는 순서가 정해진 task 목록을 만듭니다. 계획이 마음에 들면 승인하세요. 그러면 Agent가 building을 시작합니다.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="제안된 네 개의 task와 Accept tasks, Revise plan 옵션이 표시된 task plan 생성 대화 상자" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

자세한 내용은 [Plan mode 가이드](/ko/features/agent/plan-mode)에서 확인하세요.

<div id="agent-modes">
  ### Agent modes
</div>

Agent가 project를 build하는 방식을 선택하세요:

<Frame caption="Free, Power, Max 모드가 표시된 Agent modes popover.">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Power가 선택된 Free, Power, Max 모드와 Primary 모델 목록, Effort 슬라이더가 표시된 Agent modes popover" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**: plan의 allowance 범위 내에서 무료로 사용하는 일상적인 Agent 작업입니다. Core와 Pro의 allowance는 5시간마다 초기화되며 주간 limit이 적용됩니다. Starter 등급은 기존 limit을 그대로 유지합니다. **Settings → Usage**에서 usage를 확인하세요.
* **Auto**: Core와 Pro에서 Power 또는 Max로 진행하는 유료 작업에 자동으로 모델을 선택합니다.
* **Power**: 강력한 모델을 합리적인 가격에 제공하는 유료 중간 등급 모드입니다.
* **Max**: 가장 까다로운 작업을 위해 최고 수준의 프런티어 모델을 제공하는 유료 최상위 모드입니다.

요청에 Free Mode보다 높은 성능이 필요한 경우 Agent가 유료 **Allow once** 옵션을 제안할 수 있습니다. Replit은 유료 작업을 시작하기 전에 확인을 요청하며, 작업이 끝나면 Agent는 다시 Free Mode로 돌아갑니다.

<Note>팁: **[Plan mode](/ko/features/agent/plan-mode)**를 활성화하면 building이 시작되기 전에 Agent의 계획을 검토하고 다듬을 수 있습니다.</Note>

<div id="what-you-can-build-with-the-replit-agent">
  ## Replit Agent로 만들 수 있는 것
</div>

* **웹 앱, 모바일 앱, 데이터 대시보드, AI 기반 도구**
* **비주얼 디자인과 프로토타입** — 코드로 옮기기 전에 [Design Canvas](/ko/design/canvas)에서 목업을 탐색해 보세요
* **하나의 project에서 여러 결과물** — 동일한 백엔드를 공유하는 웹 앱, 모바일 앱, 슬라이드, 동영상
* **파일과 문서** — CSV, PDF, PowerPoint 파일, Markdown 문서
* **연결된 서비스 조회** — BigQuery, Linear, Slack, Notion 등의 데이터를 채팅에서 바로 가져오기

<Card title="지금 빌드 시작하기" icon="arrow-up-right" href="https://replit.com">
  아이디어를 설명하면 Agent가 실제로 구현해 드립니다. 별도의 설정은 필요 없습니다.
</Card>
...
<Accordion title="코딩을 할 줄 알아야 하나요?">
  아니요. 코드 작성, 인프라 설정, database 구성 등 기술적인 작업은 모두 Agent가 처리합니다. 원하는 것을 평소 쓰는 말로 설명하기만 하면 Agent가 만들어 줍니다.
</Accordion>

<Accordion title="Auto는 Free, Power, Max와 어떻게 다른가요?">
  **Free Mode**는 plan에 포함된 allowance 범위 안에서 무료로 사용하는 일상적인 Agent 작업입니다. Power나 Max에서는 **Auto**가 유료 task마다 가장 적합한 모델을 선택합니다. **Power**는 강력한 모델을 합리적인 가격에 제공하는 유료 중간 등급 모드이고, **Max**는 가장 까다로운 작업을 위한 유료 최상위 모드입니다.

  유료 작업은 시작하기 전에 확인 절차를 거칩니다.
</Accordion>

<Accordion title="하나의 project에서 여러 개를 만들 수 있나요?">
  네. [동일한 project](/ko/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project)에 웹 앱, 모바일 앱, slides, 동영상, Data Visualization을 추가할 수 있으며, 모두 같은 백엔드와 데이터를 공유합니다.
</Accordion>

<Accordion title="Agent가 실수하면 어떻게 되나요?">
  Agent는 자신이 한 작업을 수시로 직접 테스트합니다. 또한 작업하는 동안 checkpoint를 생성하므로 언제든 이전 상태로 roll back할 수 있습니다. Agent와 대화하며 무엇이 잘못되었는지 설명하면 문제를 고쳐 줍니다.
</Accordion>
...
Enterprise workspace에서는 Free Mode 대신 단일 Auto 모드를 사용합니다. Auto는 각 task에 맞는 모드와 허용된 모델을 선택합니다. 계정 관리자는 [Enterprise 모델 제어](/ko/features/collaboration/enterprise-model-controls)에서 모델 접근 권한을 관리할 수 있습니다.

자세한 가격 정보는 [Agent 청구](/ko/billing/ai-billing#agent-billing)를 참조하세요.
...
<CardGroup cols={2}>
  <Card title="Agent skills" icon="puzzle-piece" href="/ko/features/agent/skills">
    Agent에게 전문 지식을 학습시키세요. 미리 만들어진 skill을 사용하거나 직접 만들 수 있습니다.
  </Card>

  <Card title="task 시스템" icon="list-check" href="/ko/core-concepts/agent/task-system">
    Kanban 방식의 계획 수립, 백그라운드 작업, 메인 버전에 반영되는 변경 사항까지.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/ko/design/canvas">
    시각적인 mockup과 직접 조작하는 Visual Editor.
  </Card>

  <Card title="Connectors" icon="plug" href="/ko/features/agent/general-agent">
    BigQuery, Linear, Slack, Notion 등을 연결하세요.
  </Card>

  <Card title="Billing" icon="credit-card" href="/ko/billing/ai-billing#agent-billing">
    Agent 요금, plan 비교, 지출 관리.
  </Card>

  <Card title="Vibe coding 가이드" icon="wand-magic-sparkles" href="/ko/learn/build-with-agent">
    효과적인 prompting과 AI를 활용한 개발 팁.
  </Card>
</CardGroup>