---
title: "Replit Agent"
description: "Replit Agent превращает ваши идеи в приложения, макеты, презентации и многое другое — с помощью естественного языка. Программировать не нужно."
---
...
Agent — ваш творческий партнёр. Он принимает ваши идеи, помогает их развить, а затем воплощает в жизнь. В отличие от чат-бота, который лишь отвечает на вопросы, Agent действует: настраивает проект, создаёт приложения, проверяет вашу работу и устраняет проблемы по ходу дела.

Опишите желаемый результат обычным языком. Код или технические знания не нужны. Agent позаботится обо всём остальном — от планирования до развёртывания.
...
<Steps>
  <Step title="Опишите, что вы хотите">
    В [Project Editor](/ru/pt/core-concepts/project-editor) просто начните диалог. Опишите приложение, которое хотите создать, задайте вопрос, изучите тему или получите данные из [подключённых сервисов](/ru/pt/core-concepts/agent/general-agent), таких как BigQuery, Slack или Notion. Никаких ограничений. Replit Agent позаботится обо всём, что вам нужно.

    <AgentInput />
  </Step>

  <Step title="Выберите, что создать">
    При желании выберите тип проекта: веб-приложение, мобильное приложение, слайды, дизайн, визуализация данных и многое другое. Если вы уже описали желаемое на шаге 1, Agent автоматически определит подходящую конфигурацию.

    <AgentCarousel />
  </Step>

  <Step title="Agent создаёт">
    Agent пишет код, настраивает инфраструктуру и тестирует результат. В проекте можно переключаться между [режимами Agent](#agent-modes), чтобы управлять процессом создания.
  </Step>

  <Step title="Дорабатывайте, тестируйте, публикуйте">
    Общайтесь с Agent, чтобы доработать проект, и публикуйте его, когда всё будет готово. Все артефакты публикуются вместе.
  </Step>
</Steps>

<Note>Вы не ограничены одним типом. Начните с веб-приложения, а затем добавьте мобильное приложение, слайды или видео — всё в рамках одного проекта.</Note>

<div id="modo-planejamento">
  ### Режим планирования
</div>

Включите режим планирования, чтобы проработать идеи, задать вопросы и наметить проект, прежде чем Agent изменит код или данные. В режиме планирования Agent:

* **Разбивает сложные проекты** на упорядоченные списки задач
* **Рассматривает разные подходы** и оценивает их плюсы и минусы
* **Проверяет и уточняет детали** до написания кода

Нажмите «Plan» в поле чата — или просто попросите Agent — чтобы перейти в режим планирования. Например: «Составь план разработки трекера проектов для моей команды»

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent создаёт упорядоченный список задач, который можно просмотреть и уточнить. Когда план вас устроит, утвердите его, и Agent начнёт работу.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="Диалоговое окно с созданным планом задач: четыре предложенные задачи и варианты «Принять задачи» и «Проверить план»" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

Подробнее — в [руководстве по режиму планирования](/ru/pt/core-concepts/agent/plan-mode).

<div id="modos-do-agent">
  ### Режимы Agent
</div>

Выберите, как Agent будет работать над вашим проектом:

<AgentModes />

* **Lite**: Быстро вносите небольшие и экономичные изменения. Режим Lite идеально подходит для визуальных доработок, исправления ошибок и чётко ограниченных задач.
* **Economy**: Используйте экономичные модели Agent для повседневных задач. Разверните раздел **Расширенные настройки**, чтобы управлять такими функциями, как тестирование App и оптимизация кода.
* **Power**: Используйте наиболее производительные модели Agent для сложных задач, крупных изменений и более длительных сборок. В разделе **Расширенные настройки** также можно включить **Turbo**, чтобы ускорить сборки до 2,5 раза за дополнительную плату (только для Pro).

<Note>Совет: Вы также можете включить **[режим планирования](/ru/pt/core-concepts/agent/plan-mode)**, чтобы просмотреть и доработать план Agent до начала сборки. Режим Max больше недоступен; для стандартных сборок с максимальными возможностями используйте Power.</Note>

<div id="o-que-voc-pode-criar-com-o-replit-agent">
  ## Что можно создать с помощью Replit Agent
</div>

* **Веб- и мобильные приложения, дашборды с данными и инструменты на базе ИИ**
* **Визуальные дизайны и прототипы** — изучайте макеты в [Design Canvas](/ru/pt/replitai/canvas), прежде чем приступать к написанию кода
* **Несколько результатов в одном проекте** — веб- и мобильные приложения, слайды и видео с общим бэкендом
...
<Accordion title="Нужно ли уметь программировать?">
  Нет. Agent берёт на себя всю техническую работу: пишет код, настраивает инфраструктуру и базы данных. Вы описываете задачу простыми словами, а Agent её выполняет.
</Accordion>

<Accordion title="В чём разница между Lite, Economy и Power?">
  **Lite** подходит для быстрых точечных изменений (10–60 секунд): визуальных правок, исправления ошибок и небольших чётко очерченных функций. **Economy** — оптимальный вариант по умолчанию для большинства сборок, когда важно сбалансировать стоимость и качество. **Power** использует самые мощные модели для сложных задач, а **Turbo** — дополнительный переключатель, доступный только в режиме Power в расширенных настройках, если вам нужна максимальная скорость выполнения.
</Accordion>

<Accordion title="Можно ли создать несколько объектов в одном проекте?">
  Да. Вы можете добавлять веб-приложения, мобильные приложения, слайды, видео и визуализации данных в [один и тот же проект](/ru/pt/replitai/artifacts#multiple-artifacts-in-one-project) — все они используют общий бэкенд и данные.
</Accordion>

<Accordion title="Что произойдёт, если Agent допустит ошибку?">
  Agent регулярно проверяет собственную работу. Кроме того, во время работы он создаёт checkpoint&#39;ы, так что вы всегда можете вернуться к любому предыдущему состоянию. А ещё можно просто написать Agent, что пошло не так, и он исправит проблему.
</Accordion>
...
<CardGroup cols={2}>
  <Card title="Навыки Agent" icon="puzzle-piece" href="/ru/pt/core-concepts/agent/skills">
    Научите Agent специализированным вещам — используйте готовые навыки или создайте свои.
  </Card>

  <Card title="Разработка в параллель" icon="list-check" href="/ru/pt/core-concepts/agent/task-system">
    Планирование в стиле канбан, фоновые задачи и изменения, которые возвращаются в основную версию.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/ru/pt/replitai/canvas">
    Визуальные макеты и удобный визуальный редактор.
  </Card>

  <Card title="Коннекторы" icon="plug" href="/ru/pt/core-concepts/agent/general-agent">
    Подключайте BigQuery, Linear, Slack, Notion и многое другое.
  </Card>

  <Card title="Оплата" icon="credit-card" href="/ru/pt/billing/ai-billing#agent-billing">
    Стоимость Agent, сравнение планов и управление расходами.
  </Card>

  <Card title="Руководство по vibe coding" icon="wand-magic-sparkles" href="/ru/pt/tutorials/how-to-vibe-code">
    Советы по составлению эффективных prompt и разработке с ИИ.
  </Card>
</CardGroup>