---
title: "Build с Agent"
description: "Пять полезных привычек при работе с Agent: будьте конкретны, составляйте план, добавляйте контекст, проверяйте и тестируйте, используйте чекпоинты."
sidebarTitle: "Build с Agent"
icon: "robot"
---
...
Agent — это ИИ-разработчик Replit и ваш помощник в превращении идей в программное обеспечение. Agent может планировать изменения, писать код, объяснять, как всё работает, отлаживать проблемы и улучшать приложения.

[Vibe coding 101](/ru/ko/learn/foundations/vibe-coding-101) объясняет этот подход, а на этой странице рассказывается, как работать с Agent на практике. [Эффективное составление промптов](/ru/ko/learn/effective-prompting) поможет вам формулировать более чёткие инструкции.

Представьте Agent квотербеком на поле: он оценивает ситуацию, выбирает маршрут и продвигает игру вперёд. Вы — тренер: разрабатываете стратегию, определяете ходы, проверяете результаты и решаете, что делать дальше.

Agent умеет многое, но лучше всего работает, когда вы управляете процессом: формулируйте задачи конкретно, составляйте план, добавляйте контекст, проверяйте и тестируйте результаты, а при необходимости используйте чекпоинты.

<div id="agent">
  ## Пять советов по сборке с Agent
</div>

Возьмите на вооружение эти привычки при работе с Agent:
...
Хороший запрос объясняет, чего вы хотите, что важно и что не следует менять. Не нужно описывать все детали реализации, но важно дать Agent достаточно информации, чтобы он мог принимать верные решения.
...
<AiPrompt>
  Улучши страницу регистрации, чтобы посетители понимали, что делает продукт, прежде чем создавать аккаунт.<br />
  Сохрани текущую цветовую палитру, не меняй поля формы и сделай основную кнопку заметнее.
</AiPrompt>

Второй промпт задаёт Agent цель, область внимания и ограничения.

Если запрос состоит из нескольких частей, укажите самое важное.

<AiPrompt>
  Добавь простую панель администратора для управления заявками на кейтеринг.<br />
  Самое важное — возможность просматривать новые заявки, отмечать их как обработанные и фильтровать по статусу.<br />
  Сделай первую версию простой.
</AiPrompt>

Agent может разбить работу на этапы, но ваши указания помогут ему понять, каким должен быть успешный результат.

Примеры более конкретных инструкций см. в разделе [Эффективные промпты](/ru/ko/learn/effective-prompting).

<div id="section-2">
  ## Составление плана
</div>

План полезен, когда есть неопределённость, несколько этапов, компромиссы или что-то, что вы хотите утвердить до внесения изменений в файлы.

В чате вы можете попросить Agent составить план:

<AiPrompt>
  Составь план добавления онлайн-заказов, прежде чем вносить изменения.<br />
  Включи пользовательские сценарии, необходимые данные, страницы и компоненты, которые изменятся, риски, а также то, как это нужно протестировать.
</AiPrompt>

Если вы хотите, чтобы Agent сначала подумал и дождался вашего одобрения, прежде чем менять файлы, используйте [режим Plan](/ru/ko/features/agent/plan-mode). Перед отправкой включите переключатель **Plan** в композере промптов.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Поле промпта на главной странице Replit с введённым промптом для запущенного приложения и включённым флажком Plan" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>

<AgentInput defaultPrompt="변경하기 전에 온라인 주문 추가 계획을 만들어줘. 사용자 흐름, 필요한 데이터, 변경될 페이지나 컴포넌트, 위험 요소, 그리고 어떻게 테스트해야 하는지 포함해줘." highlightPlan={true} showTypewriter={false} />

Когда Agent закончит обдумывание, появится баннер **План работы готов к проверке** с кнопкой **Проверить сейчас**.

<Frame>
  <img src="/assets/mirror/agent-plan-ready-banner-5fbcf58a1668.png" alt="Баннер «План работы готов к проверке» с кнопкой «Проверить сейчас» в нижней части панели Agent" width="560" height="550" data-path="images/fitstart/work-with-agent/agent-plan-ready-banner.png" />
</Frame>
...
Контекст можно добавить в виде текста, снимков экрана, набросков, файлов, данных, ошибок или аннотаций на Canvas. Выбирайте тот формат, который лучше всего описывает задачу.

Используйте текст, когда изменение касается поведения, целей или ограничений:

<AiPrompt>
  Контекст: это приложение предназначено для родителей, заказывающих торты на день рождения.<br />
  Сделай форму короткой, детали самовывоза — понятными, и избегай корпоративного стиля.
</AiPrompt>

Используйте снимок экрана или набросок, когда макет сложно описать словами:

<AiPrompt>
  Используй этот снимок экрана как ориентир для макета формы заявки.<br />
  Сохрани общую структуру, но оставь существующие поля и логику отправки без изменений.
</AiPrompt>

Используйте аннотации на [Canvas](/ru/ko/design/canvas), когда изменение визуальное и нужно указать на конкретную часть приложения.

<Frame>
  <img src="/assets/mirror/agent-canvas-annotations-c88ab5856966.png" alt="Frame фитнес-приложения на Canvas с прямоугольниками, стрелками и текстовыми подписями, указывающими на карточку прогресса, а также с набросками пером и подписями в области формы" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-canvas-annotations.png" />
</Frame>

<AiPrompt>
  Обнови главную секцию, используя мои аннотации на Canvas.<br />
  Примени пометки только к заголовку, изображению и основной кнопке.<br />
  Форму заявки на кейтеринг не меняй.
</AiPrompt>

Хороший контекст включает и то, что менять не нужно. Тогда Agent сможет улучшить одну область, случайно не переписав остальные.

<div id="section-4">
  ## Проверка и тестирование
</div>

Agent может предложить план до начала сборки, а затем подвести итог по внесённым изменениям. Прочитайте и то, и другое.

Нажмите **Просмотреть** на карточке плана работ, чтобы открыть его. В развёрнутом виде план показывает, что именно Agent собирается создать, критерии успеха, что не входит в объём работ и какие шаги Agent будет выполнять.

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="Развёрнутый план работ с разделами What and Why, критериями завершения, пунктами вне объёма работ и пронумерованными шагами сборки" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>

Проверьте план до того, как Agent начнёт сборку:

* Решает ли он нужную задачу?
* Не упущено ли что-то?
* Нет ли лишнего, выходящего за рамки задачи?
* Есть ли более простая версия, с которой стоит начать?
* Объясняется ли, как тестировать результат?

Если план слишком широкий, попросите Agent сузить его.

<AiPrompt>
  Этот план слишком широкий.<br />
  Сократи его до минимальной версии, которую можно протестировать сегодня.<br />
  Остальное перенеси в список будущих улучшений.
</AiPrompt>

После того как Agent соберёт приложение, протестируйте его сами. Откройте Preview и поработайте с приложением так, как это сделал бы ваш пользователь.

Проверьте следующее:

* Может ли человек выполнить основное действие?
* Произошли ли запрошенные изменения?
* Работает ли приложение на мобильных устройствах?
* Не сломалось ли что-то важное?
* Не изменил ли Agent что-то постороннее?
* Достаточно ли хорош результат, чтобы продолжать сборку?

Если что-то работает не так, опишите, что именно вы увидели.
...
<AiPrompt>
  При отправке формы заявки на кейтеринг появляется сообщение об успехе, но в форме по-прежнему остаётся прежний текст.<br />
  Очисти форму после отправки и сделай так, чтобы сообщение об успехе оставалось видимым.
</AiPrompt>

Конкретная обратная связь помогает Agent исправить именно ту проблему, которая есть.

<div id="section-5">
  ## Использование чекпоинтов
</div>

Чекпоинты помогают вернуться назад, если изменения пошли не в ту сторону.

Если результат уже близок к нужному, дайте точечную обратную связь:

<AiPrompt>
  Почти то, что нужно, но форма получилась слишком длинной.<br />
  Сохрани новый визуальный стиль, удали необязательное поле для заметок и сделай дату получения более читаемой.
</AiPrompt>

Используйте чекпоинты, когда приложение стало хуже, чем было, сломалась важная функциональность или Agent внёс больше изменений, чем ожидалось.

Откройте **History** на панели Agent, чтобы посмотреть чекпоинты. Для каждого чекпоинта вместе с изменениями отображаются элементы управления **Откатить сюда** и **Изменения**.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="Панель Agent History с чекпоинтом, помеченным «Переключено из режима Plan в режим Build», и кнопками «Откатить сюда» и «Изменения»" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

Если выбрать **Откатить сюда**, Agent покажет, что именно будет восстановлено, прежде чем применить откат.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-rollback-dialog-28de278b44da.png" alt="Диалог подтверждения отката к чекпоинту со списком целевого чекпоинта и того, что будет затронуто: флажки для файлов, памяти Agent, задач и базы данных" width="900" height="880" data-path="images/fitstart/work-with-agent/agent-checkpoints-rollback-dialog.png" />
</Frame>

Если вы не уверены, исправлять дальше или откатиться, попросите Agent помочь со сравнением.

<AiPrompt>
  Последнее изменение сделало приложение хуже.<br />
  Сравни текущую версию с предыдущей рабочей версией.<br />
  Скажи, что безопаснее — исправлять дальше или откатиться, и почему.
</AiPrompt>

Пойти не в ту сторону — это нормально. Чекпоинты позволяют экспериментировать, не теряя возможности вернуться к более безопасной версии.

Подробное руководство по восстановлению см. в разделе [Чекпоинты и откаты](/ru/ko/features/version-control/checkpoints-and-rollbacks).

<div id="agent-2">
  ## Как выглядит эффективная работа с Agent
</div>

Вы движетесь в верном направлении, если:

* в запросе описаны цель и ограничения;
* план Agent достаточно понятен, чтобы его проверить;
* когда слов недостаточно, вы добавляете скриншоты, файлы и заметки в Canvas;
* после изменений вы тестируете важные сценарии;
* если что-то идёт не так, вы даёте конкретную обратную связь;
* вместо бесконечной отмены неудачных изменений вы используете чекпоинты.
...
<CardGroup cols={2}>
  <Card icon="comment-dots" href="/ru/ko/learn/effective-prompting">
    Узнайте, как составлять более чёткие инструкции, чтобы Agent мог уверенно их выполнить.
  </Card>

  <Card title="Создание первого App" icon="rocket" href="/ru/ko/build/your-first-app">
    Попрактикуйтесь в работе с Agent — от первого промпта до основательной доработки.
  </Card>

  <Card title="Чекпоинты и откаты" icon="rotate-left" href="/ru/ko/features/version-control/checkpoints-and-rollbacks">
