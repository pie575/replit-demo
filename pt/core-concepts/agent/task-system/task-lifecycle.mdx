---
title: "Ciclo de vida das tarefas"
description: "Entenda os estados das tarefas, trabalho na fila, aplicação de alterações, arquivamento e cancelamento de tarefas do Agent."
---
...
Cada tarefa do Agent passa por um ciclo de vida: planejada, em execução, pronta para revisão e concluída. O ciclo de vida mantém o trabalho isolado até que você decida o que deve se tornar parte da sua versão principal.
...
Um **Rascunho** é uma tarefa que o Agent planejou, mas ainda não começou a construir. Rascunhos são seguros para revisar porque contêm apenas o trabalho proposto: o título, a descrição e o plano da tarefa.

Use os Rascunhos para decidir se o Agent entendeu o objetivo. Você pode iniciar a tarefa, pedir ao Agent para revisar o plano ou arquivar o rascunho para mais tarde.

## Ativo

Uma tarefa **Ativa** está sendo executada em segundo plano. O Agent trabalha nela em uma cópia isolada do seu projeto, para que sua versão principal permaneça inalterada enquanto a tarefa é executada.

Você pode continuar usando a thread principal enquanto as tarefas Ativas são executadas. Se uma tarefa precisar de feedback, abra sua thread e continue a conversa lá.

## Na fila

Uma tarefa **Na fila** está aprovada, mas aguardando para iniciar. Uma tarefa pode ficar na fila quando depende de outra tarefa ou quando você atingiu o limite de tarefas em segundo plano ativas do seu plano.

Tarefas na fila iniciam automaticamente quando seus pré-requisitos são concluídos e uma vaga de tarefa ativa é liberada.

## Pronto

Uma tarefa **Pronta** está concluída, mas suas alterações ainda não foram aplicadas à sua versão principal. Revise o registro de trabalho da tarefa, os resultados dos testes e a prévia antes de aplicar as alterações.

Se o trabalho estiver correto, aplique-o à versão principal. Se não for o que você queria, descarte e inicie uma nova tarefa.

## Aplicando

Uma tarefa **Aplicando** está no processo de mover suas alterações para a sua versão principal. O Agent resolve conflitos automaticamente ao aplicar alterações de múltiplas tarefas.

Aguarde a aplicação terminar antes de julgar o resultado final. Se algo parecer errado depois, peça ao Agent para corrigir na thread principal.

## Concluído

Uma tarefa **Concluída** está finalizada e não precisa mais de ação. Concluído pode significar que a tarefa foi aplicada à sua versão principal, arquivada antes de começar ou cancelada após avançar para a construção.

Use a coluna **Concluído** como um histórico de trabalho de tarefas concluídas, arquivadas e canceladas.

## Exemplos de estados

Cada tarefa passa por uma série de estágios, de uma ideia a parte do seu app. Cada estágio tem seu próprio ícone para que você possa identificar o que está acontecendo de relance.
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Rascunho</strong>: O Agent propôs esta tarefa. Ela tem um plano pronto para sua revisão.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Ativo</strong>: O Agent está construindo esta tarefa agora. Ela é executada em sua própria cópia do seu projeto para que sua versão principal fique segura.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Na fila</strong>: Esta tarefa está aprovada, mas aguardando uma vaga abrir antes de poder começar a construir.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Pronto</strong>: Esta tarefa está completa. Você pode revisar as alterações e aplicá-las à sua versão principal quando quiser.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Aplicando</strong>: As alterações desta tarefa estão sendo aplicadas à sua versão principal agora.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Concluído</strong>: Esta tarefa está finalizada e suas alterações fazem parte da sua versão principal.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Arquivado</strong>: Uma sessão de planejamento que você deixou de lado antes de começar a construir. Sessões de planejamento arquivadas podem ser restauradas posteriormente na coluna <strong>Concluído</strong>.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Cancelado</strong>: Uma tarefa que você interrompeu depois que ela havia avançado para a construção. Tarefas canceladas não podem ser restauradas. Inicie uma nova tarefa se mudar de ideia.</span>
  </div>
</div>

## Arquivar e cancelar tarefas

Você pode interromper uma tarefa em qualquer ponto do seu ciclo de vida, mas a opção disponível depende de onde a tarefa está no quadro. Tanto **Arquivar** quanto **Cancelar** movem uma tarefa para a coluna **Concluído**, mas se comportam de forma diferente:

| Ação         | Quando está disponível                                                                                              | O que acontece                                                                                                        | Reversível?                                                                                               |
| ------------ | ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| **Arquivar** | Enquanto a tarefa ainda é uma sessão de planejamento — um **Rascunho** que não começou a construir                  | A sessão de planejamento é deixada de lado sem executar. Seu plano é preservado.                                      | Sim. Abra o cartão arquivado na coluna **Concluído** e restaure-o de volta para **Rascunhos**.            |
| **Cancelar** | Depois que a tarefa passou do planejamento para a construção — qualquer tarefa **Ativa**, **Na fila** ou **Pronta** | O Agent para a tarefa e descarta qualquer trabalho em andamento. Nenhuma alteração é aplicada à sua versão principal. | Não. Tarefas canceladas não podem ser restauradas. Inicie uma nova tarefa se quiser revisitar o trabalho. |

Use **Arquivar** para sessões de planejamento que você quer revisitar mais tarde. Use **Cancelar** como o botão de parada permanente para tarefas que estão na fila, já em execução ou que terminaram de construir.

Você pode encontrar tanto sessões de planejamento arquivadas quanto tarefas canceladas na coluna **Concluído** na extremidade direita do quadro de tarefas, junto com as tarefas concluídas.
