...
description: "了解如何通过 Free Mode 更好地进行日常聊天、创作和构建。"
...
Free Mode 可让你在不使用 Power Mode 或 Max Mode 积分的情况下聊天、创建和构建。

<Note>
  Free Mode 始终使用智能模型路由，因此 Replit 会为你的请求选择模型。
  你无法在 Free Mode 中选择特定模型。
</Note>

Routine 无法在 Free Mode 中运行。它们仅在 Power 或 Max Mode 中运行，且每次运行都需要预算。详细了解 [Routine](/zh/chat/routines)。

<div id="get-more-from-your-allowance">
  ## 充分利用你的额度
</div>

Free Mode 非常适合持续推进手头的工作。你可以用它来：

* 在投入更大型的构建前，探索并验证想法。
* 创建新项目、初步实现某项功能，或处理较小的改动。
* 提出问题、制定计划，并通过后续消息完善工作。
* 在额度可用期间，无需使用积分即可继续处理日常工作。

首先，打开提示框下方的模式选择器，确保模式设为 **Free**。然后向 Replit 提出明确的请求：说明你想实现的结果，提供相关背景信息，并通过后续问题完善结果。

<div id="know-when-to-use-another-mode">
  ## 了解何时应使用其他模式
</div>

Free Mode 的限额取决于你的套餐。打开 **设置** → **Usage**，查看当前限额及重置时间。

<Frame caption="Usage 页面会显示剩余的 Free 额度及刷新时间。">
...
如果你使用的是 Core 或 Pro 套餐，并且需要更强的能力、想选择特定模型，或已用尽 Free Mode 额度，可切换到 **Power Mode** 或 **Max Mode**。这些模式会使用每月积分。Starter 套餐的构建者需先升级，才能使用 Power Mode 或 Max Mode。

<Tip>
  如果你的套餐包含 Power Mode 和 Max Mode，请先使用 Free Mode 完成第一步，
  仅当任务需要更强能力时再切换。Replit 会在请求切换到会产生使用费用的模式前
  提醒你。
</Tip>
...
| 在大多数任务中兼顾速度与能力 | Power Mode |
| 应对复杂或高难度任务     | Max Mode   |

Power Mode 以相同成本取代 Economy Mode，性能相当或更佳。在 Core 和 Pro 套餐中，你可以在 Power Mode 或 Max Mode 下选择可用模型。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/features/agent/agent-modes#free-mode">
    ...
    <div className="chat-card-title">Free Mode 参考</div>
    <p>了解 Free Mode 与 Power Mode 和 Max Mode 的区别。</p>
  ...
  <a className="chat-example has-visual" href="/zh/chat/prompting">
    ...
    <p>了解如何向 Replit 提供有用的指令和上下文。</p>
    <div className="chat-card-link">继续 →</div>
  </a>
</section>

接下来，了解[智能模型路由](/zh/chat/auto-mode)如何根据任务变化选择模型。