---
title: "Canvas"
description: "Узнайте, как использовать Canvas для изучения и доработки дизайнов с помощью Agent, а также для просмотра артефактов во фреймах."
---
...
<YouTubeEmbed videoId="Qv9cREjl9vQ" title="Обзор Canvas в Replit" />

<div id="o-que-o-canvas">
  ## Что такое Canvas?
</div>

Canvas — это пространство для проработки и развития дизайнов с помощью Agent. Здесь ваши артефакты — например, приложения, видео и слайды — отображаются в фрейме. Работайте с Agent: создавайте макеты, сравнивайте варианты дизайна и применяйте изменения в своём приложении — всё это на визуальном холсте.

<Frame>
  <img src="/assets/mirror/canvas-desktop-85e8bfec38a2.png" alt="Canvas с предпросмотром приложения рядом с Chat Agent и обновлениями задач" width="3452" height="2006" data-path="images/replitai/canvas-desktop.png" />
</Frame>
...
Canvas — это бесконечное пространство, где можно визуально просматривать всё и работать с любыми элементами.

<Frame>
  <img src="/assets/mirror/canvas-mockups-057e738a78da.png" alt="Canvas с несколькими дизайн-макетами, расположенными рядом друг с другом: тёмная тема, мобильные представления и страницы с подробностями" width="3452" height="1620" data-path="images/replitai/canvas-mockups.png" />
</Frame>

* **Ваши приложения**: Запущенные приложения отображаются как интерактивные предпросмотры в реальном времени прямо на Canvas. Вы можете нажимать на них и пользоваться ими так же, как в браузере.
* **Дизайн-макеты**: Визуальные прототипы, которые Agent создаёт для вас. Они выглядят и работают как настоящие страницы — с ними можно взаимодействовать, изменять их размер и просить Agent доработать их — но они отделены от работающего приложения, чтобы вы могли свободно экспериментировать.
* **Фигуры и рисунки**: Прямоугольники, круги, звёзды, стрелки, сердца и многое другое. Используйте их, чтобы набросать идеи, отметить элементы дизайна или показать, как связаны экраны.
* **Текст и стикеры**: Добавляйте подписи или заметки в любом месте Canvas, чтобы упорядочить мысли.
* **Изображения**: Перетаскивайте на Canvas снимки экрана или референсные изображения. Agent может воссоздать увиденное или вдохновиться им.
* **Видео**: Размещайте видеофайлы на Canvas рядом с другим содержимым.

<div id="obtendo-designs-no-canvas">
  ### Создание дизайнов в Canvas
</div>

Создать дизайн можно несколькими способами. Выберите наиболее удобный для себя.

<Steps>
  <Step title="Попросите Agent в Chat">
    Введите в поле Chat то, что хотите увидеть, и Agent добавит это в Canvas как интерактивный фрейм.

    <AgentInput defaultPrompt="Покажи макет моей целевой страницы в Canvas" showTypewriter={true} />
  </Step>

  <Step title="Выберите тип результата">
    Также можно выбрать нужный тип результата: веб, мобильное приложение, слайды, дизайн и другое. Перед отправкой prompt выберите его в карусели.

    <AgentCarousel />
  </Step>

  <Step title="Перенесите существующую страницу в Canvas">
    У вас уже есть работающее App? Попросите Agent перенести страницу в Canvas для редизайна — например: «Добавь мой dashboard в Canvas». Agent создаст копию, поэтому работающее App не будет затронуто.
  </Step>
</Steps>
...
Созданный в Canvas макет можно дорабатывать разными способами.

<div id="pea-ao-agent-atualizaes">
  ### Попросите Agent внести изменения
</div>

Выберите фрейм на Canvas и опишите нужные изменения в появившемся под ним поле. Нажмите Enter — Agent обработает запрос, внесёт изменения и обновит макет прямо на Canvas.
...
<div id="desenhe-e-anote">
  ### Рисуйте и добавляйте комментарии
</div>

Используйте инструменты рисования на нижней панели, чтобы делать наброски прямо в Canvas. Нарисуйте стрелки, указывающие на то, что вы хотите изменить, обведите проблемные области или добавьте текстовые комментарии с обратной связью. Затем попросите Agent внести изменения с учётом ваших пометок.
...
Посмотрите, как ваши дизайны выглядят на экранах разных размеров. Попросите Agent показать версию для мобильного устройства, планшета или компьютера:

* **Мобильное устройство** — 390 × 844
* **Планшет** — 768 × 1024
* **Компьютер** — 1280 × 720

Например, попросите: «Покажи мобильную версию этой страницы», и Agent отобразит её в этом размере.
...
<div id="aplique-as-alteraes-ao-seu-app">
  ### Примените изменения к своему App
</div>

Когда дизайн будет готов, примените изменения к основной версии. Agent покажет сводку внесённых изменений и предложит применить их или отклонить.
...
<div id="exporte-um-frame-como-png">
  ### Экспортируйте фрейм в формате PNG
</div>

Вы можете сохранить любой артефакт или фрейм дизайна на Canvas как изображение PNG. Это удобно, чтобы поделиться предпросмотром в документе или чате, сделать снимок перед дальнейшими изменениями или отправить скриншот коллеге.

<Steps>
  <Step title="Откройте меню фрейма">
    Щёлкните правой кнопкой мыши по артефакту или фрейму дизайна, который хотите экспортировать. Откроется меню с доступными действиями.
  </Step>

  <Step title="Выберите «Экспортировать PNG»">
    Выберите **Экспортировать** &gt; **PNG**. Изображение будет скачано на ваш компьютер, а его имя будет основано на имени фрейма.
  </Step>
</Steps>

<Frame>
  <img src="/assets/mirror/canvas-export-png-b903c6afacb8.png" alt="Контекстное меню, открытое щелчком правой кнопки мыши по фрейму на Canvas, с подменю «Экспортировать», где PNG указан как вариант экспорта" width="1740" height="1160" data-path="images/replitai/canvas-export-png.png" />
</Frame>

<Note>Экспортированный PNG соответствует тому, что сейчас отображается во фрейме на Canvas. Если ваше приложение или дизайн ещё загружается, дождитесь полной отрисовки перед экспортом.</Note>

<div id="comparando-direes-de-design">
  ## Сравнение вариантов дизайна
</div>

Если вы не уверены, какой вариант выбрать, попросите Agent создать несколько версий, чтобы сравнить их рядом.
...
* **Несколько вариантов одной страницы**: «Покажи пять вариантов этой лендинг-страницы». Agent создаст их и разместит рядом.
* **Сценарии из нескольких страниц**: «Спроектируй полный сценарий онбординга: приветствие, настройка профиля, предпочтения и подтверждение». Agent создаст каждый экран и расположит их по порядку.
* **Сочетайте разные варианты**: «Покажи лендинг-страницу и страницу с ценами в минималистичном и смелом стилях». Agent создаст все сочетания, чтобы вы могли увидеть общую картину.

<div id="mapeando-fluxos-de-usurio">
  ## Карта пользовательских сценариев
</div>

Разместите в Canvas полные пользовательские сценарии — регистрацию, онбординг, dashboard, настройки — и расположите их так, чтобы увидеть, как всё взаимосвязано.

* Разместите каждый экран в отдельном фрейме
* Попросите Agent создать полный сценарий: «Создай полный сценарий онбординга для моего App»
* Используйте стрелки и фигуры, чтобы показать переходы пользователей между экранами
* Попросите Agent превратить черновые наброски в проработанные дизайны

<div id="transformando-um-design-em-um-app-real">
  ## Превращение дизайна в полноценное App
</div>

Когда дизайн вас устроит и вы захотите воплотить его в жизнь, есть два варианта:

<div id="torne-o-um-app-completo">
  ### Превратите его в полноценное приложение
</div>

Если вашему дизайну нужна реальная функциональность — например, сохранение данных, учётные записи пользователей или интеграция с другими сервисами, — попросите Agent превратить его в полноценное приложение. Напишите, например: «Преврати это в настоящее приложение» или «Добавь сюда бэкенд». Agent создаст всё необходимое для работы полноценного приложения, готового к публикации.

<Note>
  Для преобразования дизайна в полноценное приложение требуется платный тариф.
</Note>

<div id="converta-para-um-tipo-especfico">
  ### Преобразование в определённый тип
</div>

Вы также можете преобразовать фрейм дизайна в определённый тип артефакта:

<Steps>
  <Step title="Выберите фрейм">
    Нажмите на фрейм, который хотите преобразовать.
  </Step>

  <Step title="Попросите Agent выполнить преобразование">
    Скажите Agent, что вы хотите сделать — например, «Преобразуй это в веб-приложение» или «Преврати это в слайды».
  </Step>

  <Step title="Выберите тип">
    Выберите веб-приложение, мобильное приложение, слайды, визуализацию данных и другие варианты.
  </Step>
</Steps>
...
<Accordion title="Фреймы дизайна совпадают с моим работающим приложением?">
  Нет. Фреймы дизайна — это визуальные прототипы: они выглядят и работают как настоящее приложение, но за ними нет работающих бэкенда или базы данных. Предпросмотры приложения в Canvas, напротив, показывают ваше настоящее работающее приложение.
</Accordion>

<Accordion title="Могу ли я переработать страницу, не нарушив работу опубликованного приложения?">
  Да. Попросите Agent перенести страницу в Canvas — например: «Помести мою панель управления на Canvas». Agent создаст копию, с которой вы сможете экспериментировать. Ваше опубликованное приложение не изменится, пока вы не решите применить изменения.
</Accordion>

<Accordion title="Как превратить дизайн в настоящее приложение?">
  Попросите Agent сделать из него полноценное приложение («Сделай это настоящим») или преобразовать его в определённый тип («Преобразуй это в веб-приложение»). Agent создаст всё необходимое, чтобы оно стало функциональным приложением, готовым к публикации.
</Accordion>

<Accordion title="Можно ли преобразовать в типы приложений, помимо веб-приложений?">
  Да. Вы можете превратить фреймы Canvas в веб-приложения, мобильные приложения, слайды, визуализации данных и многое другое.
</Accordion>

<Accordion title="Можно ли импортировать дизайны из Figma или других инструментов?">
  Да. Есть несколько способов перенести дизайны из Figma в Replit:

  * **Импорт из Figma**: Перейдите на [replit.com/import](https://replit.com/import), подключите свою учётную запись Figma и вставьте URL фрейма, чтобы преобразовать дизайн в работающее React-приложение. Подробнее см. в [кратком руководстве по импорту из Figma](/ru/pt/design/import-figma-designs-into-replit-design).
  * **Figma MCP в чате Agent**: Вставьте ссылку Figma прямо в чат Agent и попросите сгенерировать код, извлечь данные дизайна или создать компонент на основе дизайна. См. руководство по [интеграции Figma MCP](/ru/pt/features/mcp/figma).
  * **Справочный снимок экрана**: Перетащите снимок экрана в Canvas и попросите Agent воссоздать дизайн на его основе.
</Accordion>

<Accordion title="Что ещё можно разместить в Canvas?">
  Помимо макетов дизайна и предпросмотров приложений, вы можете добавлять фигуры, стрелки, текстовые метки, стикеры, изображения и видео — это удобно для набросков идей, аннотирования дизайнов или упорядочивания мыслей.
</Accordion>
...
Canvas доступен на всех тарифах: создание фреймов, встроенное редактирование, генерация вариантов и преобразование в артефакты. Для преобразования дизайнов в полноценные приложения требуется платный тариф.