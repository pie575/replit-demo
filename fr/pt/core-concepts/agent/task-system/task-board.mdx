...
description: "Utilisez le tableau des tâches pour examiner, lancer, suivre et appliquer les tâches de l'Agent à mesure qu'elles progressent à chaque étape."
---
...
<div id="como-funciona-o-quadro-de-tarefas">
  ## Fonctionnement du tableau de tâches
</div>

Le tableau de tâches réunit en un seul endroit tout le nécessaire pour examiner, lancer, suivre et appliquer les tâches de l&#39;Agent. Utilisez-le lorsque l&#39;Agent découpe une demande volumineuse en plusieurs parties, lorsque vous souhaitez exécuter des travaux en arrière-plan ou lorsque vous avez besoin de savoir ce qui a déjà été modifié dans votre projet.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Tableau de tâches affichant les tâches organisées dans les colonnes Brouillons, ressource, Prêt et Terminé" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

Les tâches sont organisées en colonnes selon leur statut. Le tableau indique ce que l&#39;Agent a planifié, ce qui est en cours d&#39;exécution, ce qui est prêt à être examiné et ce qui a déjà été appliqué.

Chaque tâche se déplace de gauche à droite au fil de sa progression :

* **Brouillons** : tâches que l&#39;Agent a planifiées mais pas encore lancées
* **ressource** : tâches que l&#39;Agent est en train de créer, y compris celles en file d&#39;attente en raison de la limite de tâches actives
...
Les **brouillons** sont des tâches planifiées dont l&#39;Agent n&#39;a pas encore commencé la construction. Chaque brouillon comprend un titre, une description et un plan étape par étape.

Utilisez les brouillons pour examiner l&#39;approche proposée par l&#39;Agent avant le début des travaux. Vous pouvez inspecter le plan, demander à l&#39;Agent de l&#39;ajuster ou lancer la tâche lorsque vous êtes prêt.

Les brouillons sont utiles lorsque vous souhaitez planifier une fonctionnalité d&#39;envergure avant de lancer du travail en arrière-plan. Par exemple, l&#39;Agent peut découper un projet d&#39;intégration en brouillons distincts pour l&#39;authentification, la configuration du profil, l&#39;interface du dashboard et les finitions du déploiement.

<div id="ativo">
  ## ressource
</div>

Lorsque vous acceptez une tâche, celle-ci passe dans la colonne **ressource** et Agent commence à construire. Chaque tâche s&#39;exécute dans sa propre copie isolée de votre projet : rien ne change donc dans votre version principale tant que vous n&#39;avez pas appliqué le travail.

Certaines tâches dépendent d&#39;autres. Par exemple, une tâche qui construit un dashboard peut nécessiter que la tâche relative au schéma de la base de données soit terminée au préalable. Les tâches dépendantes restent **en file d&#39;attente** jusqu&#39;à ce que leurs prérequis soient achevés. Les tâches sont également mises en file d&#39;attente si vous avez atteint la limite de tâches en arrière-plan actives de votre forfait.
...
Lorsqu&#39;une tâche a fini d&#39;être construite, elle passe au statut **prêt**. Cela signifie que le travail est terminé, mais que rien n&#39;a encore changé dans votre version principale.
...
Passez en revue les tâches prêtes avant de les appliquer. Consultez le journal de travail, la sortie des tests et l&#39;aperçu pour comprendre ce que l&#39;Agent a modifié.

<div id="concludo">
  ## terminé
</div>

Une fois appliquée, la carte de la tâche passe dans la colonne **terminée**.

La colonne terminée regroupe également les sessions de planification archivées et les tâches annulées. Vous disposez ainsi d&#39;un historique de ce qui s&#39;est passé, sans que le travail terminé ne vienne se mêler de nouveau aux Brouillons ou à la ressource.

<KanbanColumns
  columns={[
{ title: 'prêt', count: 3, cards: [
{ title: "Mode lecture du soir avec thème sombre", description: "Un mode nuit dédié, avec un thème sombre aux tons chauds et des sons d'ambiance.", status: 'ready-drop', timestamp: '35m', showMenu: true },
...
{ title: 'Personnalisation avancée des personnages', description: "Les options d'apparence rendent les illustrations vraiment personnelles.", status: 'ready-drop', timestamp: '28m', showMenu: true },
] },
...
{ title: "Liens d'histoires partageables avec aperçus OG", description: 'Les liens publics partageables, principal levier de croissance.', status: 'done', timestamp: '1h' },
{ title: 'Personnalisation avancée des personnages', description: "Des options d'apparence pour des illustrations vraiment personnalisées.", status: 'done', timestamp: '4h' },
{ title: 'Adaptation mobile et finitions pour la production', description: "Meilleurs états d'erreur, mises en page mobiles et animations de chargement.", status: 'done', timestamp: '4h' },
] }
]}
/>

L&#39;Agent résout automatiquement les conflits lorsqu&#39;il applique les modifications de plusieurs tâches. Si quelque chose ne vous semble pas correct, demandez à l&#39;Agent de le corriger.

<div id="encontrar-e-filtrar-tarefas">
  ## Rechercher et filtrer les tâches
</div>

Utilisez le tableau des tâches lorsque votre projet comporte de nombreuses tâches en cours. Vous pouvez effectuer une recherche par titre ou par numéro de tâche, filtrer les brouillons et utiliser le mode édition pour gérer des groupes de tâches en une seule fois.

Par exemple, effectuez une recherche par numéro de tâche lorsque quelqu&#39;un mentionne une tâche dans un fil de discussion, une revue ou une conversation Slack. Filtrez les brouillons lorsque vous souhaitez vous concentrer sur le travail planifié sans avoir à parcourir les tâches actives ou terminées.

Pour les suivis suggérés, consultez [Tâches de suivi](/fr/pt/core-concepts/agent/task-system/follow-up-tasks).