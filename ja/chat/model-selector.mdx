---
title: "モデルを選ぶ"
description: "手動でのモデル選択がお使いのモードとプランで利用できる場合に、利用可能なモデルを選び、その Effort を設定しましょう。"
sidebarTitle: "モデルを選ぶ"
tag: "New"
icon: "microchip"
...
<div className="chat-mkt" role="img" aria-label="モデルセレクターを表す Anthropic、OpenAI、Qimi のロゴ" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
インテリジェントなモデルルーティングを使うと、タスクに合わせて Replit がモデルを選びます。自分でモデルを選ぶには、プロンプト入力欄で**Agent Mode**を選択し、**Power**または**Max**を選んで、**Primary model**セクションでモデルを選択します。

Free Mode は常にインテリジェントなモデルルーティングを使用します。Core および Pro のビルダーは、Power Mode または Max Mode で利用可能なモデルを選択できます。利用状況によって選べるモデルは変わるため、Replit 内のモデル一覧があなたのアカウントにとっての正確な情報源になります。

<Frame caption="Agent 設定メニュー内のモデルと Effort のコントロール。">
  <img src="/assets/mirror/model-selector-gpt-5-6-sol-effort-8e4d256bf497.jpg" alt="デモワークスペースのモデルセレクター。Max Mode で GPT-5.6 Sol が選択され、Effort が Medium に設定されている" width="1821" height="864" data-path="images/chat/model-selector-gpt-5-6-sol-effort.jpg" />
</Frame>

## モデルの Effort を設定する

Effort は、選択したモデルが応答にどれだけの推論を費やすかを制御します。これは別のモードではなく、モデルごとのコントロールです。対応するレベルは**Low**から**Max**まであり、モデルごとに推奨設定が表示されます。

より高い Effort は、タスクがより深い推論を必要とする場合にのみ使いましょう。設定を高くすると、複雑な作業では時間がかかり、コストも高くなることがあります。

## さらに学ぶ

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ja/features/agent/model-selector">
    ...
    <div className="chat-card-title">モデルセレクター リファレンス</div>

    <p>
      インテリジェントなモデルルーティング、利用可能なモデル、Effort のレベル、
      Compare models を確認します。
    </p>

    <div className="chat-card-link">リファレンスを開く →</div>
  </a>

  <a className="chat-example has-visual" href="/ja/features/agent/model-selector#effort">
    ...
    <div className="chat-card-title">Effort の詳細</div>
    <p>Low、Medium、High、Extra High、Max を比較します。</p>
    <div className="chat-card-link">Effort を確認する →</div>
  </a>
</section>

モデルと Effort の設定は Replit の応答の仕方を左右します。[Voice Mode](/ja/chat/voice-mode) は、入力する代わりに話しかけることで、依頼の伝え方を変えます。
