...
description: "Выберите доступную модель и настройте её Effort, если ручной выбор модели доступен в вашем режиме и тарифном плане."
...
<div className="chat-mkt" role="img" aria-label="Логотипы Anthropic, OpenAI и Qimi, обозначающие выбор модели" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
Интеллектуальная маршрутизация моделей позволяет Replit самостоятельно подобрать модель под задачу. Чтобы выбрать модель вручную, нажмите **Agent Mode** в prompt box, выберите **Power** или **Max**, а затем укажите модель в разделе **Primary model**.

Free Mode всегда использует интеллектуальную маршрутизацию моделей. Пользователи планов Core и Pro могут выбрать доступную модель в Power Mode или Max Mode. Актуальный список моделей для вашего аккаунта — тот, что отображается в Replit, поскольку набор доступных моделей может различаться.

<Frame caption="Настройки модели и Effort в меню параметров Agent.">
  <img src="/assets/mirror/model-selector-gpt-5-6-sol-effort-8e4d256bf497.jpg" alt="Демонстрационный workspace: выбран GPT-5.6 Sol в Max Mode, для Effort задано значение Medium" width="1821" height="864" data-path="images/chat/model-selector-gpt-5-6-sol-effort.jpg" />
</Frame>

<div id="set-effort-for-the-model">
  ## Настройте Effort для модели
</div>

Effort определяет, сколько ресурсов на рассуждения выбранная модель выделяет для ответа. Это настройка отдельной модели, а не самостоятельный режим. Поддерживаемые уровни варьируются от **Low** до **Max**, при этом для каждой модели отображается рекомендуемый уровень.

Выбирайте более высокий уровень Effort, только если задача требует более глубокого анализа. На сложных задачах более высокие уровни могут потребовать больше времени и обойтись дороже.
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ru/features/agent/model-selector">
    ...
    <p>
      Узнайте об интеллектуальной маршрутизации моделей, доступных моделях, уровнях Effort и
      функции Compare models.
    </p>

    <div className="chat-card-link">Открыть справочник →</div>
  </a>

  <a className="chat-example has-visual" href="/ru/features/agent/model-selector#effort">
    ...
    <div className="chat-card-title">Подробнее об Effort</div>
    <p>Сравните Low, Medium, High, Extra High и Max.</p>
    <div className="chat-card-link">Об Effort →</div>
  </a>
</section>

Настройки модели и Effort влияют на то, как отвечает Replit. [Voice Mode](/ru/chat/voice-mode) позволяет озвучивать запросы вместо ввода текста.