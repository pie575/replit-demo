---
title: "使用智能模型路由"
description: "让 Replit 为每项 task 选择合适的模型，仅在需要自行控制时才手动选择模型。"
sidebarTitle: "智能模型路由"
...
智能模型路由让 Replit 能够在兼顾质量和成本的同时，为每项 task 选择合适的模型。当你在产品中看到 **Auto** 时，表示 Replit 正在使用智能模型路由。

<div id="enable-intelligent-model-routing">
  ## 启用智能模型路由
</div>

1. 在提示框中选择 **Agent Mode**。菜单会显示可用模式：**Free**、**Power** 和 **Max**。
2. 选择 **Power** 或 **Max**，然后找到 **主模型** 部分。其标签会根据所选模式变化，例如 **Primary Max model**。
3. 如果主模型已设置为 **Auto**，则智能模型路由已启用。如果选择了特定模型，请打开主模型菜单并选择 **Automatically choose**。
...
当你希望 Replit 随着工作内容的变化自动选择模型时，智能模型路由非常实用。简单明确的请求可以使用成本较低的模型，而复杂任务可能需要能力更强的模型。

在 Power Mode 或 Max Mode 中，Core 和 Pro 构建者也可以手动选择可用模型。

<Frame caption="Agent 设置中的智能模型路由、模型选择和 Effort。">
  <img src="/assets/mirror/auto-mode-model-and-effort-selector-fdcabef57097.jpg" alt="Agent 设置中显示“自动选择推荐项”、可用模型、Effort 控件，以及使用智能模型路由的 Max Mode" width="1839" height="855" data-path="images/chat/auto-mode-model-and-effort-selector.jpg" />
</Frame>
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/features/agent/model-selector#intelligent-model-routing">
  ...
  <a className="chat-example has-visual" href="/zh/chat/model-selector">
    ...
    <p>如需更多控制权，可选择主模型并设置其 Effort。</p>
    <div className="chat-card-link">继续 →</div>
  </a>
</section>

如果您需要比智能模型路由提供更多控制权，请继续阅读[选择模型](/zh/chat/model-selector)。