---
title: "Figmaと連携する"
description: "Figmaフレームを視覚的なコンテキストとして使用し、そこから探索、改良、構築したい場合はReplit Designのインポートフローに従いましょう。"
sidebarTitle: "Figmaと連携する"
icon: "figma"
...
ReplitはFigmaフレームのインポートに対応しています。

すでに視覚的な方向性が決まっていて、Replitにその作業から始めてもらいたい場合や、それをデザインに変えてもらいたい場合はFigmaフレームをインポートしましょう。

## Figmaが役立つ場面

* **別の方向性を探る：** 中心となる階層構造は保ちながら、異なるスタイルやレイアウトを試します。
* **プロダクトフローを続ける：** 既存の画面を、次の画面や状態のコンテキストとして使用します。
* **デザインの作業をアプリへと進める：** 承認済みの視覚的な方向性から始め、そこから改良して構築します。

## Figmaフレームをインポートする

続けたい作業を表すフレームを選びます。Replit Designはインポートしたフレームをデザインフレームに変換し、そこで変更を探ったり、提案を比較したり、次に何を構築するかを決めたりできます。

<Frame caption="Replit DesignでデザインフレームとしてインポートされたFigmaフレーム。">
  <img src="/assets/mirror/figma-import-result-052394025b8d.webp" alt="Replit DesignにデザインフレームとしてインポートされたPersonal Fitness Trainerのランディングページ" width="3440" height="2160" data-path="images/fitstart/turn-mockup-into-app/figma-import-result.webp" />
</Frame>

## Designで続ける

Designガイドでは、インポート、改良、構築までの完全なワークフローを説明しています。

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ja/design/import-figma-designs-into-replit-design">
    ...
    <div className="chat-card-title">Figmaからインポートする</div>
    <p>Figmaフレームをインポートして改良する完全なフローに沿って進めます。</p>
