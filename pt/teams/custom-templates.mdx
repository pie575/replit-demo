...
description: "Configure e gerencie templates personalizados no Replit Enterprise para padronizar como sua organização constrói com o Agent."
...
1. **[Importar](https://replit.com/import) de um provedor Git** — Importe sua stack inicial existente (ou uma que corresponda às suas preferências) do GitHub, GitLab ou Bitbucket.
2. **Fazer fork do template em branco** — Faça fork do [template em branco](https://replit.com/@replit/Nothing?v=1) para começar do zero com um ambiente de desenvolvimento completamente vazio.
...
4. As [skills](/pt/features/agent/skills) personalizadas de `.agents/skills/` são descobertas e disponibilizadas
5. O arquivo [`replit.md`](/pt/features/project-setup/replit-dot-md) fornece contexto contínuo do projeto
...
* Configuração de ambiente no [arquivo `.replit`](/replit-app/configuration)
...
Documente cada secret em `custom_instruction/instructions.md` (ou em uma skill dedicada) para que os desenvolvedores saibam o que está disponível e para que cada chave é usada.
...
**Limite de tamanho**: O conteúdo é limitado a aproximadamente 25 KB. O conteúdo além desse limite pode ser progressivamente truncado. À medida que suas instruções crescem, mova o material de referência detalhado para [skills](/pt/features/agent/skills) em `.agents/skills/` em vez de manter tudo em `instructions.md`. As skills são carregadas sob demanda quando relevantes, portanto não contam para o limite de tamanho das instruções e mantêm suas instruções principais focadas em regras de alto nível.
...
**Como as skills são carregadas**: O Agent vê apenas o `name` e a `description` em seu prompt. Ele lê o conteúdo completo do `SKILL.md` sob demanda quando a skill é relevante para a tarefa atual. Isso significa que a `description` é crítica — escreva-a para corresponder aos cenários onde o Agent deve usar a skill.
...
Para mais sobre skills, consulte [Agent Skills](/pt/features/agent/skills) e [Usar uma skill](/pt/build/use-agent-skills).

### Configurar replit.md

Pré-preencha o `replit.md` na raiz do projeto com a visão geral da arquitetura do seu template. Esse arquivo é sempre carregado no contexto do Agent e serve como memória contínua do projeto.
...
O Agent atualiza o `replit.md` à medida que o projeto evolui. Se o arquivo crescer além de aproximadamente 4.000 tokens, o Agent o resume automaticamente para mantê-lo focado e útil.

Para mais detalhes, consulte [replit.md](/pt/features/project-setup/replit-dot-md).
...
<img src="/assets/mirror/custom-templates-pin-to-agent-b733d3eb9a80.png" alt="Menu de ações do app mostrando a opção Fixar na caixa de entrada do Agent" width="984" height="856" data-path="images/teams/custom-templates-pin-to-agent.png" />

<Note>
  Apenas administradores da organização (membros da função `system_admins`) podem fixar e desafixar templates.
</Note>

Defina o acesso dos membros como **Somente leitura** para que os membros possam ver e usar o template fixado sem poder editá-lo.

<img src="/assets/mirror/custom-templates-members-read-only-cf6aa160c8ef.png" alt="Diálogo de convite mostrando o acesso dos membros definido como Somente leitura" width="954" height="1064" data-path="images/teams/custom-templates-members-read-only.png" />

O template agora aparece como uma pílula selecionável abaixo da caixa de entrada do Agent para todos os membros da sua organização. Para removê-lo, abra o mesmo menu e selecione **Desafixar da caixa de entrada do Agent**.

### Controlar o acesso ao template

Fixar torna um template visível para todos os membros da organização. Para uma distribuição mais direcionada, use o botão **Convidar** no app do template para compartilhá-lo com [Grupos ou usuários individuais](/pt/teams/identity-and-access-management/groups-and-permissions) específicos. Isso permite limitar quais equipes ou pessoas podem ver e fazer fork do template, mantendo-o desafixado da caixa de entrada global da organização.

## O que acontece quando um desenvolvedor seleciona um template

<img src="/assets/mirror/custom-templates-input-box-553384ac7dd0.png" alt="Template aparecendo como uma pílula selecionável na caixa de entrada do Agent" width="1746" height="726" data-path="images/teams/custom-templates-input-box.png" />

Quando um membro da organização escolhe um template fixado na caixa de entrada do Agent:
...
| Arquivo                              | Propósito                                   | Quando o Agent o lê                    | Mutável pelo Agent                         |
| ------------------------------------ | ------------------------------------------- | -------------------------------------- | ------------------------------------------ |
...
* [Agent Skills](/pt/features/agent/skills) — Saiba como as skills funcionam e como criá-las
* [Usar uma skill](/pt/build/use-agent-skills) — Guia prático para usar e criar skills
* [replit.md](/pt/features/project-setup/replit-dot-md) — Personalize o comportamento do Agent com contexto a nível de projeto
* [Sistemas de Design](/pt/teams/custom-design-system) — Configure o sistema de design da sua organização para o Agent
* [Configuração de apps](/replit-app/configuration) — Configure seu arquivo `.replit`
* [Frameworks de Desenvolvedor](/pt/features/project-setup/developer-frameworks) — Por que os templates iniciais de linguagem e framework foram removidos e como iniciar um projeto agora
* [Visão geral Enterprise](/pt/category/teams) — Saiba mais sobre os recursos Enterprise
