---
title: "Replit Agent"
description: "Replit Agent transforme vos idées en applications, designs, présentations et bien plus encore, le tout en langage courant. Aucune connaissance en programmation requise."
---
...
<div id="o-que-o-replit-agent">
  ## Qu&#39;est-ce que Replit Agent ?
</div>

Agent est votre partenaire créatif. Il part de vos idées, vous aide à les affiner et les concrétise. Contrairement à un chatbot qui se contente de répondre à vos questions, Agent passe à l&#39;action : il configure votre projet, crée des applications, vérifie son travail et corrige les problèmes au fur et à mesure.

Décrivez ce que vous souhaitez en langage courant. Aucune connaissance technique ni compétence en programmation n&#39;est requise. Agent s&#39;occupe du reste, de la planification au déploiement.

<div id="como-usar-o-replit-agent">
...
<Steps>
  <Step title="Décrivez ce que vous voulez">
    Dans l&#39;[éditeur de projets](/fr/pt/learn/projects-and-artifacts/project-editor), il vous suffit de lancer la conversation. Décrivez une application que vous souhaitez créer, posez une question, faites une recherche sur un sujet ou récupérez des données depuis des [services connectés](/fr/pt/features/agent/general-agent) comme BigQuery, Slack ou Notion. Il n&#39;y a aucune limite : Replit Agent gère tout ce que vous lui soumettez.

    <AgentInput />
  </Step>

  <Step title="Choisissez ce que vous voulez créer">
    Vous pouvez également sélectionner un type de projet : application web, application mobile, présentations, design, visualisation de données, et plus encore. Si vous avez déjà décrit votre idée à l&#39;étape 1, Agent détermine automatiquement la configuration adaptée.

    <AgentCarousel />
  </Step>

  <Step title="Agent construit">
    Agent écrit le code, configure l&#39;infrastructure et teste le résultat. Une fois dans votre projet, vous pouvez basculer entre les [modes d&#39;Agent](#agent-modes) pour contrôler sa façon de construire.
  </Step>

  <Step>
    Discutez avec Agent pour affiner votre projet, puis publiez-le lorsqu&#39;il est prêt. Tous les artifacts sont publiés ensemble.
  </Step>
</Steps>

<Note>Vous n&#39;êtes pas limité à un seul type. Commencez par une application web, puis ajoutez une application mobile, des présentations ou une vidéo — le tout dans le même projet.</Note>

<div id="modo-plan">
  ### Mode Plan
</div>

Activez le mode Plan pour réfléchir, poser des questions et structurer votre projet avant qu&#39;Agent ne modifie du code ou des données. En mode Plan, Agent va :

* **Décomposer les projets complexes** en listes de tâches ordonnées
* **Explorer différentes approches** et évaluer les compromis
* **Réviser et affiner** avant que la moindre ligne de code ne soit écrite

Cliquez sur « Plan » dans la zone de saisie du Chat — ou demandez-le simplement à Agent — pour passer en mode Plan. Exemple : « Crée un plan pour développer un outil de suivi de projets pour mon équipe »

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent crée une liste de tâches ordonnée que vous pouvez réviser et affiner. Lorsque le plan vous convient, approuvez-le : Agent se met alors à construire.
...
Pour en savoir plus, consultez le [guide du mode Plan](/fr/pt/features/agent/plan-mode).

<div id="modos-do-agent">
  ### Modes de l&#39;Agent
</div>

Choisissez la façon dont l&#39;Agent construit votre projet :

<Frame caption="Le popover des modes de l'Agent avec les modes Free, Power et Max.">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Popover des modes de l'Agent affichant les modes Free, Power et Max avec Power sélectionné, une liste Primary model et un curseur Effort" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode** : le travail quotidien de l&#39;Agent, sans frais pour les abonnés Core et Pro. Votre quota se réinitialise toutes les cinq heures et comporte une limite hebdomadaire. Le niveau Starter conserve ses limites actuelles. Consultez **Paramètres → Utilisation** pour suivre votre consommation.
* **Power** : un mode payant, de niveau intermédiaire, offrant des modèles puissants à un prix abordable.
* **Max** : un mode payant, de niveau supérieur, avec les meilleurs modèles de pointe pour les tâches les plus exigeantes. Dans **Paramètres avancés**, vous pouvez également activer **Turbo** pour des exécutions plus rapides.

Si une demande nécessite plus de capacité que ce que propose Free Mode, l&#39;Agent peut vous proposer une option payante **Autoriser une fois**. Replit demande une confirmation avant le lancement de toute action payante, puis l&#39;Agent revient à Free Mode.

<Note>Astuce : vous pouvez aussi activer le **[mode Plan](/fr/pt/features/agent/plan-mode)** pour examiner et affiner le plan de l&#39;Agent avant le début de la construction.</Note>

<div id="o-que-voc-pode-construir-com-o-replit-agent">
...
* **Design visuel et prototypage** — explorez des maquettes dans le [Design Canvas](/fr/pt/design/canvas) avant de vous lancer dans le code
* **Plusieurs livrables dans un même projet** — applications web, applications mobiles, présentations et vidéos partageant le même backend
* **Fichiers et documents** — CSV, PDF, fichiers PowerPoint, documents Markdown
* **Requêtes vers des services connectés** — récupérez des données depuis BigQuery, Linear, Slack, Notion et bien d&#39;autres, directement depuis le Chat

<Card title="Commencez à créer dès maintenant" icon="arrow-up-right" href="https://replit.com">
  Décrivez votre idée et laissez Agent lui donner vie — aucune configuration nécessaire.
</Card>
...
<Accordion title="Dois-je savoir programmer ?">
  Non. Agent se charge de tout le travail technique : écriture du code, mise en place de l&#39;infrastructure, configuration des bases de données. Vous décrivez ce que vous voulez en langage courant, et Agent s&#39;occupe de le créer.
</Accordion>

<Accordion title="Quelle est la différence entre Free, Power et Max ?">
  **Free Mode** couvre le travail quotidien d&#39;Agent, sans frais pour les abonnés Core et Pro, dans la limite d&#39;un quota. **Power** est un mode payant de niveau intermédiaire, qui propose des modèles puissants à un prix abordable. **Max** est le mode payant haut de gamme, conçu pour les tâches les plus exigeantes. **Turbo** est une option facultative, réservée à Max, dans les paramètres avancés.

  Les actions payantes nécessitent une confirmation avant d&#39;être lancées.
</Accordion>

<Accordion title="Puis-je créer plusieurs éléments dans un même projet ?">
  Oui. Vous pouvez ajouter des applications web, des applications mobiles, des présentations, des vidéos et des visualisations de données au [même projet](/fr/pt/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) : ils partagent tous le même backend et les mêmes données.
</Accordion>

<Accordion title="Que se passe-t-il si Agent commet une erreur ?">
  Agent teste régulièrement son propre travail. Il crée également des checkpoints au fil de l&#39;eau, ce qui vous permet de revenir à n&#39;importe quel état antérieur. Vous pouvez aussi lui expliquer par chat ce qui n&#39;a pas fonctionné, et il corrigera le problème.
</Accordion>
...
Turbo est désactivé par défaut : activez-le projet par projet, selon vos besoins, avec Max sélectionné. Le Free Mode n&#39;est pas disponible pour Enterprise au lancement. Les workspaces Enterprise bénéficieront prochainement d&#39;une expérience Agent dédiée ; votre administrateur pourra vous en communiquer les détails.

Pour connaître les tarifs détaillés, consultez la [facturation d&#39;Agent](/fr/pt/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  <Card title="Compétences Agent" icon="puzzle-piece" href="/fr/pt/features/agent/skills">
    Transmettez à Agent des connaissances spécialisées : utilisez des compétences prêtes à l&#39;emploi ou créez les vôtres.
  </Card>

  <Card title="Système de tâches" icon="list-check" href="/fr/pt/core-concepts/agent/task-system">
    Planification Kanban, tâches en arrière-plan et modifications réappliquées à la version principale.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/fr/pt/design/canvas">
    Maquettes visuelles et éditeur visuel pratique.
  </Card>

  <Card title="Connectors" icon="plug" href="/fr/pt/features/agent/general-agent">
    Connectez BigQuery, Linear, Slack, Notion et bien d&#39;autres services.
  </Card>

  <Card title="Facturation" icon="credit-card" href="/fr/pt/billing/ai-billing#agent-billing">
    Tarifs d&#39;Agent, comparaison des plans et gestion des dépenses.
  </Card>

  <Card title="Guide du vibe coding" icon="wand-magic-sparkles" href="/fr/pt/learn/build-with-agent">
