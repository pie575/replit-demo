---
title: "Replit Agent"
description: "Replit Agent превращает ваши идеи в приложения, дизайны, презентации и многое другое — достаточно описать задачу обычными словами. Программировать не нужно."
---
...
Agent — ваш творческий партнёр. Он берёт ваши идеи, помогает их доработать и воплощает в жизнь. В отличие от чат-бота, который просто отвечает на вопросы, Agent действует: настраивает проект, создаёт приложения, проверяет собственную работу и по ходу исправляет ошибки.

Опишите обычными словами, что вам нужно. Ни кода, ни технических знаний не требуется. Всё остальное Agent берёт на себя — от планирования до развёртывания.

<div id="como-usar-o-replit-agent">
  ## Как использовать Replit Agent
</div>
...
<Steps>
  <Step title="Опишите, что вам нужно">
    В [редакторе проектов](/ru/pt/learn/projects-and-artifacts/project-editor) просто начните диалог. Опишите приложение, которое хотите создать, задайте вопрос, изучите тему или получите данные из [подключённых сервисов](/ru/pt/features/agent/general-agent) — BigQuery, Slack или Notion. Ограничений нет: Replit Agent справится с любой задачей.

    <AgentInput />
  </Step>

  <Step title="Выберите, что создавать">
    При желании выберите тип проекта: веб-приложение, мобильное приложение, презентация, Design, визуализация данных и другое. Если вы уже описали задачу на шаге 1, Agent сам подберёт нужную конфигурацию.

    <AgentCarousel />
  </Step>

  <Step title="Agent создаёт">
    Agent пишет код, настраивает инфраструктуру и тестирует результат. Уже внутри проекта вы можете переключаться между [режимами Agent](#agent-modes), чтобы управлять тем, как он ведёт разработку.
  </Step>

  <Step>
    Общайтесь с Agent, чтобы дорабатывать проект, а когда всё будет готово — опубликуйте его. Все артефакты публикуются вместе.
  </Step>
</Steps>

<Note>Вы не привязаны к одному типу. Начните с веб-приложения, а позже добавьте мобильное приложение, презентацию или видео — всё в рамках одного проекта.</Note>

<div id="modo-plan">
  ### Режим Plan
</div>

Включите режим Plan, чтобы обсудить идеи, задать вопросы и продумать проект до того, как Agent изменит код или данные. В режиме Plan Agent:

* **Разбивает сложные проекты** на упорядоченные списки задач
* **Прорабатывает разные подходы** и оценивает компромиссы
* **Позволяет проверить и доработать план** до написания кода

Нажмите «Plan» в поле ввода чата — или просто попросите об этом Agent — чтобы переключиться в режим Plan. Пример: «Составь план создания трекера проектов для моей команды»

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent создаёт упорядоченный список задач, который вы можете просмотреть и доработать. Когда план вас устроит, утвердите его — и Agent приступит к работе.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="Диалог созданных планов задач с четырьмя предложенными задачами и вариантами «Принять задачи» и «Пересмотреть план»" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

Подробнее — в [руководстве по режиму Plan](/ru/pt/features/agent/plan-mode).

<div id="modos-do-agent">
  ### Режимы Agent
</div>

Выберите, как Agent будет создавать ваш проект:

<Frame caption="Всплывающее окно режимов Agent с режимами Free, Power и Max.">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Всплывающее окно режимов Agent с режимами Free, Power и Max, где выбран Power, списком Primary model и ползунком Effort" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**: повседневная работа Agent без затрат для подписчиков Core и Pro. Квота обновляется каждые пять часов, также действует недельный лимит. На тарифе Starter текущие лимиты сохраняются. Проверить расход можно в разделе **Настройки → Использование**.
* **Power**: платный режим среднего уровня с мощными моделями по доступной цене.
* **Max**: платный режим высшего уровня с лучшими передовыми моделями для самых сложных задач. В разделе **Расширенные настройки** можно также включить **Turbo** для более быстрого выполнения.

Если запросу требуется больше возможностей, чем даёт Free Mode, Agent может предложить платный вариант **Разрешить один раз**. Replit запрашивает подтверждение перед запуском платного действия, после чего Agent возвращается в Free Mode.

<Note>Совет: вы также можете включить **[режим Plan](/ru/pt/features/agent/plan-mode)**, чтобы просмотреть и доработать план Agent до начала сборки.</Note>

<div id="o-que-voc-pode-construir-com-o-replit-agent">
  ## Что можно создать с помощью Replit Agent
</div>

* **Веб-приложения, мобильные приложения, дашборды с данными и инструменты на базе ИИ**
* **Визуальный дизайн и прототипирование** — прорабатывайте макеты в [Design Canvas](/ru/pt/design/canvas), прежде чем переходить к коду
* **Несколько результатов в одном проекте** — веб-приложения, мобильные приложения, презентации и видео с общим бэкендом
* **Файлы и документы** — CSV, PDF, файлы PowerPoint, документы Markdown
* **Запросы к подключённым сервисам** — получайте данные из BigQuery, Linear, Slack, Notion и других сервисов прямо из чата

<Card title="Начните создавать прямо сейчас" icon="arrow-up-right" href="https://replit.com">
  Опишите свою идею, а Agent воплотит её — без какой-либо настройки.
</Card>
...
<Accordion title="Нужно ли уметь программировать?">
  Нет. Agent берёт на себя всю техническую работу: пишет код, настраивает инфраструктуру и базы данных. Вы простым языком описываете, что вам нужно, а Agent это создаёт.
</Accordion>

<Accordion title="В чём разница между Free, Power и Max?">
  **Free Mode** — повседневная работа с Agent без дополнительной платы для подписчиков Core и Pro в пределах квоты. **Power** — платный режим среднего уровня с мощными моделями по доступной цене. **Max** — платный режим высшего уровня для самых требовательных задач. **Turbo** — дополнительный переключатель, доступный только в режиме Max в расширенных настройках.

  Платные действия запускаются только после подтверждения.
</Accordion>

<Accordion title="Можно ли создать несколько вещей в одном проекте?">
  Да. Вы можете добавлять веб-приложения, мобильные приложения, презентации, видео и визуализации данных в [один и тот же проект](/ru/pt/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) — все они будут использовать общий бэкенд и одни и те же данные.
</Accordion>

<Accordion title="Что произойдёт, если Agent допустит ошибку?">
  Agent регулярно проверяет собственную работу. Кроме того, по ходу работы он создаёт checkpoint&#39;ы, чтобы вы могли вернуться к любому предыдущему состоянию. Вы также можете написать Agent, описав, что пошло не так, и он исправит проблему.
</Accordion>
...
| ---------------------------------- | ---- | --- |
| Chat и создание приложений с Agent | ✅    | ✅   |
| Free Mode                          | ✅    | ✅   |
| Режим Power                        | ✅    | ✅   |
| Режим Max                          | ✅    | ✅   |
| Design Canvas                      | ✅    | ✅   |
...
Turbo по умолчанию отключён — включайте его для конкретного проекта по мере необходимости при выбранном режиме Max. Free Mode на момент запуска недоступен для Enterprise. В workspace Enterprise вскоре появится отдельный вариант работы с Agent; подробности сообщит администратор.

Подробные сведения о ценах см. в разделе [оплата Agent](/ru/pt/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  <Card title="Навыки Agent" icon="puzzle-piece" href="/ru/pt/features/agent/skills">
    Научите Agent специализированным знаниям — используйте готовые навыки или создайте свои.
  </Card>

  <Card title="Система задач" icon="list-check" href="/ru/pt/core-concepts/agent/task-system">
    Планирование в Kanban, фоновые задачи и изменения, применяемые обратно к основной версии.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/ru/pt/design/canvas">
    Визуальные макеты и удобный визуальный редактор.
  </Card>

  <Card title="Connectors" icon="plug" href="/ru/pt/features/agent/general-agent">
    Подключайте BigQuery, Linear, Slack, Notion и другие сервисы.
  </Card>

  <Card title="Оплата" icon="credit-card" href="/ru/pt/billing/ai-billing#agent-billing">
    Стоимость Agent, сравнение планов и контроль расходов.
  </Card>

  <Card title="Руководство по vibe coding" icon="wand-magic-sparkles" href="/ru/pt/learn/build-with-agent">
    Советы по эффективным prompt и разработке с помощью ИИ.
  </Card>
</CardGroup>