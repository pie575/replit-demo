---
title: "Разработка с Agent"
description: "Освойте пять полезных привычек при работе с Agent: чётко формулируйте запросы, планируйте работу, добавляйте контекст, проверяйте и тестируйте, используйте чекпоинты."
sidebarTitle: "Разработка с Agent"
icon: "robot"
---
...
Agent — AI-разработчик Replit и ваш партнёр в превращении идей в программное обеспечение. Agent может планировать изменения, писать код, объяснять, как всё работает, отлаживать проблемы и улучшать App.

[Vibe coding 101](/ru/ja/learn/foundations/vibe-coding-101) рассказывает о таком подходе, а эта страница — о том, как работать с Agent на практике. [Эффективные промпты](/ru/ja/learn/effective-prompting) помогут научиться формулировать более чёткие инструкции.

Представьте Agent как квотербека на поле: он оценивает ситуацию, выбирает направление и продвигает работу вперёд. Вы — тренер: задаёте стратегию, выбираете розыгрыши, анализируете результаты и определяете следующий шаг.

Agent умеет многое, но лучше всего работает, когда вы руководите процессом: чётко формулируете задачи, планируете работу, добавляете контекст, проверяете и тестируете результат, а при необходимости восстановления используете чекпоинты.

<div id="agent5">
  ## 5 советов по созданию с Agent
</div>

Используйте эти рекомендации при работе с Agent:

1. Чётко формулируйте задачу.
2. Планируйте работу.
3. Добавляйте контекст.
4. Проверяйте и тестируйте.
5. Используйте чекпоинты.

<div id="section">
  ## Формулируйте запросы конкретно
</div>

Agent может справляться со сложными задачами, но ему нужны чёткие указания.

Хороший запрос объясняет, чего вы хотите, что важно и что нельзя менять. Необязательно описывать все детали реализации, но нужно дать Agent достаточно ориентиров, чтобы он мог принимать верные решения.

Вместо этого:

<AiPrompt>
  Сделай моё приложение лучше.
</AiPrompt>

Попробуйте так:

<AiPrompt>
  Улучши страницу регистрации, чтобы посетители понимали, что делает продукт, прежде чем создавать учётную запись.<br />
  Сохрани текущую цветовую палитру и поля формы, а основную кнопку сделай заметнее.
</AiPrompt>

Во втором промпте Agent получает цель, область внимания и ограничения.

Если ваш запрос состоит из нескольких частей, укажите, что важнее всего.

<AiPrompt>
  Добавь простой интерфейс администратора для управления запросами на кейтеринг.<br />
  Самое важное — видеть новые запросы, отмечать, что с пользователями связались, и фильтровать запросы по статусу.<br />
  Пусть первая версия будет простой.
</AiPrompt>

Agent может разбить работу на шаги, но именно ваши указания определяют, каким должен быть результат.

Дополнительные примеры конкретных инструкций см. в разделе [Эффективные промпты](/ru/ja/learn/effective-prompting).

<div id="section-2">
  ## Планирование работы
</div>

План полезен, если в работе есть неопределённость, несколько этапов, компромиссы или что-то, что вы хотите согласовать до внесения изменений в файлы.

Попросить Agent составить план можно в Conversation:

<AiPrompt>
  Перед внесением изменений составь план добавления онлайн-заказов.<br />
  Включи пользовательский сценарий, необходимые данные, страницы или компоненты, которые изменятся, риски и способы тестирования.
</AiPrompt>

Если вы хотите, чтобы Agent сначала всё обдумал и дождался вашего одобрения перед внесением изменений в файлы, используйте [режим Plan](/ru/ja/features/agent/plan-mode). Перед отправкой включите переключатель **Plan** в поле ввода промпта.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="В поле ввода промпта на главной странице Replit введён промпт для работающего App, а флажок Plan включён" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>

<AgentInput defaultPrompt="Перед внесением изменений составь план добавления онлайн-заказов. Включи пользовательский сценарий, необходимые данные, страницы или компоненты, которые изменятся, риски и способы тестирования." highlightPlan={true} showTypewriter={false} />

Когда Agent завершит анализ, появятся баннер **План задач готов к проверке** и кнопка **Проверить сейчас**.

<Frame>
  <img src="/assets/mirror/agent-plan-ready-banner-5fbcf58a1668.png" alt="Баннер «План задач готов к проверке» и кнопка «Проверить сейчас» в нижней части панели Agent" width="560" height="550" data-path="images/fitstart/work-with-agent/agent-plan-ready-banner.png" />
</Frame>
...
Контекст помогает Agent понимать, что важно.

Добавить контекст можно с помощью текста, скриншотов, эскизов, файлов, данных, сообщений об ошибках и аннотаций Canvas. Используйте формат, который лучше всего описывает задачу.

Используйте текст, если изменение касается поведения, целей или ограничений:

<AiPrompt>
  Контекст: это App для родителей, заказывающих торты на день рождения.<br />
  Сделайте форму короткой, выделите информацию о самовывозе и избегайте всего, что выглядит слишком корпоративно.
</AiPrompt>

Используйте скриншоты или эскизы, если раскладку сложно описать словами:

<AiPrompt>
  Используйте этот скриншот как ориентир для раскладки формы заявки.<br />
  Сохраните общую структуру, но оставьте существующие поля и логику отправки.
</AiPrompt>

Используйте аннотации [Canvas](/ru/ja/design/canvas), если изменение связано с визуальной частью и вы хотите указать на конкретную область App.

<Frame>
  <img src="/assets/mirror/agent-canvas-annotations-c88ab5856966.png" alt="Фрейм фитнес-App на Canvas. Прямоугольник, стрелка, текстовые метки, указывающие на карточку прогресса, а также нарисованный пером эскиз и метка в области формы" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-canvas-annotations.png" />
</Frame>

<AiPrompt>
  Используйте мои аннотации Canvas, чтобы обновить главный раздел.<br />
  Примените заметки только к заголовку, изображению и основной кнопке.<br />
  Не изменяйте форму заявки на кейтеринг.
</AiPrompt>

Хороший контекст также указывает, что не следует изменять. Это помогает Agent улучшать одну область, не переписывая случайно другую.

<div id="section-4">
  ## Проверяйте и тестируйте
</div>

Перед началом работы Agent может предложить план, а после внесения изменений — кратко описать, что изменилось. Прочитайте оба.

Выберите **Показать** на карточке плана задачи, чтобы открыть план. В развёрнутом плане указано, что Agent собирается создать, критерии успеха, что выходит за рамки задачи и каким шагам будет следовать Agent.

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="Развёрнутый план задачи. Показаны описания What и Why, критерии завершения, элементы вне рамок задачи и пронумерованные шаги реализации" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>

Проверьте план до того, как Agent начнёт работу:

* Он решает правильную проблему?
* Ничего не упущено?
* Есть ли что-то, что не должно выходить за рамки задачи?
* Можно ли сначала создать меньшую версию?
* Объясняется ли, как следует тестировать результат?

Если план слишком обширный, попросите Agent сузить его.

<AiPrompt>
  Этот план слишком обширный.<br />
  Сократи его до минимальной версии, которую я смогу протестировать сегодня.<br />
  Перенеси всё остальное в список будущих улучшений.
</AiPrompt>

После того как Agent завершит работу, протестируйте App самостоятельно. Откройте Preview и попробуйте использовать его так, как это сделал бы целевой пользователь.

Проверьте следующее:

* Может ли пользователь выполнить основное действие?
* Были ли внесены запрошенные изменения?
* Работает ли App на мобильных устройствах?
* Не сломана ли важная функциональность?
* Не изменил ли Agent что-то, не относящееся к задаче?
* Достаточно ли хорош результат, чтобы продолжить разработку?

Если что-то не так, опишите, что вы увидели.
...
<AiPrompt>
  Когда я отправляю форму запроса на кейтеринг, появляется сообщение об успехе, но в форме по-прежнему отображается старый текст.<br />
  После отправки очисть форму и оставь сообщение об успехе видимым.
</AiPrompt>

Конкретная обратная связь помогает Agent исправить нужную проблему.

<div id="section-5">
  ## Использование чекпоинтов
</div>

Чекпоинты помогают восстановиться, если изменения пошли не в том направлении.

Если результат почти устраивает, дайте конкретную обратную связь:

<AiPrompt>
  Почти то, что нужно, но форма стала слишком длинной.<br />
  Сохрани новый визуальный стиль, убери необязательное поле для заметок и сделай дату самовывоза более удобной для быстрого просмотра.
</AiPrompt>

Используйте чекпоинты, если App стала хуже, важная функция перестала работать или Agent внес больше изменений, чем вы ожидали.

Откройте History в панели Agent, чтобы просмотреть чекпоинты. Каждый чекпоинт показывает внесённые изменения и содержит элементы управления **Откатить до этого состояния** и **Изменения**.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="Панель History Agent с чекпоинтом с меткой «Переход от плана к режиму сборки». Доступны кнопки «Откатить до этого состояния» и «Изменения»" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

При выборе **Откатить до этого состояния** Agent покажет, что будет восстановлено, прежде чем применить откат.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-rollback-dialog-28de278b44da.png" alt="Диалог подтверждения отката к чекпоинту. В нём перечислены целевой чекпоинт и затрагиваемые элементы: файлы, память Agent, задачи и флажок базы данных" width="900" height="880" data-path="images/fitstart/work-with-agent/agent-checkpoints-rollback-dialog.png" />
</Frame>

Если вы не уверены, исправлять ли изменения в текущей версии или выполнить откат, попросите Agent помочь сравнить варианты.

<AiPrompt>
  Последнее изменение ухудшило App.<br />
  Сравни текущую версию с предыдущей рабочей версией.<br />
  Скажи, безопаснее ли исправить изменения в текущей версии или выполнить откат, и объясни почему.
</AiPrompt>

Ошибочные направления — это нормально. Чекпоинты позволяют исследовать варианты, не теряя возможности вернуться к более надёжной версии.

Дополнительные рекомендации по восстановлению см. в разделе [Чекпоинты и откаты](/ru/ja/features/version-control/checkpoints-and-rollbacks).

<div id="agent">
  ## Что значит эффективно работать с Agent
</div>

Вы на правильном пути, если:

* ваш запрос описывает цель и ограничения
* план Agent достаточно понятен, чтобы его можно было проверить
* вы добавляете скриншоты, файлы и аннотации Canvas, когда одних слов недостаточно
* вы тестируете важные сценарии после внесения изменений
* если что-то не так, вы даёте конкретную обратную связь
* вы используете чекпоинты вместо того, чтобы бесконечно пытаться исправить неудачные изменения
...
<CardGroup cols={2}>
  <Card icon="comment-dots" href="/ru/ja/learn/effective-prompting">
    Узнайте, как формулировать более чёткие инструкции, чтобы Agent мог действовать уверенно.
  </Card>

  <Card title="Создайте своё первое App" icon="rocket" href="/ru/ja/build/your-first-app">
    Попрактикуйтесь в работе с Agent — от первого промпта до целенаправленных улучшений.
  </Card>

  <Card title="Чекпоинты и откат" icon="rotate-left" href="/ru/ja/features/version-control/checkpoints-and-rollbacks">
    Узнайте, как при необходимости вернуться к более безопасному состоянию.
  </Card>
</CardGroup>