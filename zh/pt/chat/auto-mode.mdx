---
title: "使用智能模型路由"
description: "让 Replit 为每个任务选择合适的模型，仅在需要自行掌控时才手动指定模型。"
sidebarTitle: "智能模型路由"
...
<div className="chat-mkt" role="img" aria-label="Anthropic、OpenAI 和 Kimi 的品牌标识，代表智能模型路由" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
智能模型路由让 Replit 为每项任务选择合适的模型，在质量与成本之间取得平衡。当你在产品中看到 **Auto** 时，就说明 Replit 正在进行智能模型路由。

<div id="ativar-o-roteamento-inteligente-de-modelos">
  ## 启用智能模型路由
</div>

1. 在 prompt 输入框中选择 **Agent Mode**，菜单会列出可用模式：**Free**、**Power** 和 **Max**。
2. 选择 **Power** 或 **Max**，然后找到 **Primary model** 部分。该标签会随所选模式而变化，例如 **Primary Max model**。
3. 如果主模型已设置为 **Auto**，说明智能模型路由已启用；如果当前选中的是某个特定模型，请打开主模型菜单并选择 **Automatically choose**。

Free Mode 始终使用智能模型路由，无法手动选择模型。

<div id="deixe-a-tarefa-guiar-o-modelo">
  ## 让任务决定模型
</div>

如果你希望在工作内容变化时由 Replit 自动完成模型选择，智能模型路由就很有用。目标明确的请求可能会使用成本更低的模型，而复杂任务则可能需要能力更强的模型。

在 Power 模式或 Max 模式下，使用 Core 和 Pro 方案的构建者也可以手动选择可用的模型。

<Frame caption="Agent 设置中的智能模型路由、模型选择与 Effort。">
  <img src="/assets/mirror/auto-mode-model-and-effort-selector-fdcabef57097.jpg" alt="Agent 设置中显示自动选择推荐项、可用模型、Effort 控件以及使用智能模型路由的 Max 模式" width="1839" height="855" data-path="images/chat/auto-mode-model-and-effort-selector.jpg" />
</Frame>
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/pt/features/agent/model-selector#roteamento-inteligente-de-modelos">
  ...
  <a className="chat-example has-visual" href="/zh/pt/chat/model-selector">
    ...
    <p>如需更多控制，请选择主模型并设置其 Effort。</p>
    <div className="chat-card-link">继续 →</div>
  </a>
</section>

如果您需要比智能模型路由提供的更多控制，请前往[选择模型](/zh/pt/chat/model-selector)。