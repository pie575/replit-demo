---
title: "集成你的工具"
description: "连接受支持的服务，明确 Conversation 需要哪些信息或操作，并参照官方集成文档完成配置。"
sidebarTitle: "集成你的工具"
icon: "plug"
...
集成可为 Replit 添加受支持服务的上下文和功能。

* **Slack** 允许 Replit 读取 Slack 中的消息和会话串。
* **Google Drive** 让 Replit 可以访问文件和文件夹，Google Slides 则支持演示文稿。
* **Salesforce** 让 Replit 可以访问 CRM 数据并执行受支持的操作。
...
<Steps>
  <Step title="打开集成">
    ...
    <Frame>
      <img src="/assets/mirror/integrations-open-from-workspace-4dfe77901ec3.jpg" alt="已脱敏的 Replit workspace，左侧边栏中高亮显示集成" width="1451" height="1084" data-path="images/chat/integrations-open-from-workspace.jpg" />
    </Frame>
  </Step>
  ...
  <Step title="选择一个集成">
    ...
    <Frame>
      <img src="/assets/mirror/integrations-library-sanitized-e4b7670d4e3e.jpg" alt="已脱敏的 Replit 集成库，显示推荐和可用的集成" width="1606" height="979" data-path="images/chat/integrations-library-sanitized.jpg" />
    </Frame>
  </Step>

  <Step title="进行连接">
    查看该集成可访问的内容，然后按照服务提供方的登录和授权流程继续操作。此处以 Salesforce 为例，其他集成可能略有不同。
...
你也可以在提示词输入框中打开 **更多 (+)** 菜单，然后选择 **添加集成**。
...
<section className="chat-mkt chat-workflow-grid has-visuals catalog-three-up">
  <a className="chat-example has-visual" href="/zh/pt/features/integrations/overview#connectors">
  ...
  <a className="chat-example has-visual" href="/zh/pt/features/integrations/overview#connectors">
    ...
    <p>把相关的文件和文件夹加入 Conversation 的上下文。</p>
    <div className="chat-card-link">探索连接器 →</div>
  </a>
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/pt/use-cases/prepare-for-a-customer-call">
    ...
    <p>从 Salesforce、Slack 和 Google Drive 收集上下文。</p>
    <div className="chat-card-link">查看此使用场景 →</div>
  </a>
</section>
...
原生集成是 Replit 连接器目录中已支持的服务连接。如果某个服务尚未支持，你可以创建自定义 MCP，并将其作为集成添加到 Replit。

[了解如何通过 MCP 连接。](/zh/pt/chat/connect-through-mcp)

参阅[集成功能指南](/zh/pt/features/integrations/overview)，了解连接器类型、可用性、配置、支持的服务以及权限。

当 Conversation 已具备足够的上下文来开展实质性工作时，可了解如何[将其转为 Project](/zh/pt/chat/continue-or-create-a-project)。