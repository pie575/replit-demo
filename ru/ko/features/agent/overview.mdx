---
title: "Replit Agent"
description: "Replit Agent превращает ваши идеи в App, дизайны, слайды и многое другое — с помощью обычного языка и без написания кода."
---
...
<YouTubeEmbed videoId="bAUVQfz_SIY" title="Знакомство с Replit Agent 4" />

<div id="replit-agent">
  ## Что такое Replit Agent?
</div>

Agent — ваш творческий партнёр. Он помогает развивать ваши идеи и воплощать их в жизнь. В отличие от чат-ботов, которые лишь отвечают на вопросы, Agent действительно действует: настраивает проекты, создаёт приложения, проверяет результаты работы и решает возникающие проблемы.

Опишите, что вы хотите, обычным языком. Код или технические знания не нужны. Agent позаботится обо всём остальном — от планирования до деплоя.
...
<Steps>
  <Step title="Опишите, что хотите">
    Начните общение прямо в [Project Editor](/ru/ko/learn/projects-and-artifacts/project-editor). Расскажите, какое App хотите создать, задайте вопрос, изучите тему или получите данные из [подключённых сервисов](/ru/ko/features/agent/general-agent), таких как BigQuery, Slack и Notion. Ограничений нет. Replit Agent справится с чем угодно.

    <AgentInput />
  </Step>

  <Step title="Выберите, что создать">
    При желании можно выбрать тип проекта: веб-App, мобильное App, слайд, дизайн, визуализацию данных и многое другое. Если вы уже описали желаемое на шаге 1, Agent автоматически определит подходящие настройки.

    <AgentCarousel />
  </Step>

  <Step title="Agent приступает к разработке">
    Agent пишет код, настраивает инфраструктуру и тестирует результат. В проекте можно переключать [режимы Agent](#agent-modes), чтобы управлять процессом разработки.
  </Step>

  <Step>
    Общайтесь с Agent и дорабатывайте проект, а когда всё будет готово — опубликуйте его. Все артефакты будут опубликованы вместе.
  </Step>
</Steps>

<Note>Не ограничивайтесь одним типом. Начните с веб-App, а затем добавьте мобильное App, слайд или видео — всё в рамках одного проекта.</Note>

<div id="plan">
  ### Режим Plan
</div>

Включите режим Plan, чтобы Agent мог обдумывать идеи, задавать вопросы и планировать проект, прежде чем вносить изменения в код или данные. В режиме Plan Agent:

* **разбивает сложные проекты** на упорядоченный список задач
* **рассматривает разные подходы** и взвешивает их преимущества и недостатки
* **проверяет и улучшает** решения до написания кода

Нажмите «Plan» в поле ввода чата или напрямую попросите Agent перейти в режим Plan. Например: «Составь план создания трекера проектов для нашей команды»

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent создаст упорядоченный список задач, который вы сможете проверить и доработать. Если план вас устраивает, подтвердите его, и Agent начнёт создание приложения.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="Диалоговое окно создания плана задач с четырьмя предложенными задачами и вариантами «Принять задачи» (Accept tasks) и «Изменить план» (Revise plan)" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

Подробнее см. в [руководстве по режиму Plan](/ru/ko/features/agent/plan-mode).

<div id="agent">
  ### Режимы Agent
</div>

Выберите, как Agent будет собирать проект:

<Frame caption="Всплывающее окно режимов Agent с режимами Free, Power и Max">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Всплывающее окно режимов Agent с выбранным Power, режимами Free, Power и Max, списком моделей по умолчанию и ползунком Effort" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**: бесплатно доступен подписчикам Core и Pro для повседневных задач Agent. Лимиты использования сбрасываются каждые 5 часов, также действует недельный лимит. Для тарифа Starter сохраняются прежние лимиты. Проверяйте использование в разделе **Настройки → Usage**.
* **Power**: платный режим среднего уровня с мощными моделями по доступной цене.
* **Max**: платный режим высшего уровня с передовыми моделями максимальной производительности для самых сложных задач. В разделе **Расширенные настройки** можно включить **Turbo** для более быстрого выполнения.

Если вашему запросу требуется больше возможностей, чем предоставляет Free Mode, Agent может предложить платный вариант **Разрешить один раз**. Replit запросит подтверждение до начала платной задачи, а после ее завершения Agent вернется в Free Mode.

<Note>Совет: также можно включить **[режим Plan](/ru/ko/features/agent/plan-mode)**, чтобы просматривать и корректировать план Agent до начала сборки.</Note>

<div id="replit-agent-3">
  ## Что можно создать с Replit Agent
</div>

* **Веб-приложения, мобильные приложения, дашборды данных, инструменты на базе ИИ**
* **Визуальные дизайны и прототипы** — изучайте макеты в [Design Canvas](/ru/ko/design/canvas), прежде чем воплощать их в коде
* **Несколько результатов в одном проекте** — веб-приложения, мобильные приложения, слайды и видео с общим бэкендом
* **Файлы и документы** — файлы CSV, PDF, PowerPoint и документы Markdown
* **Работа с подключёнными сервисами** — получайте данные из BigQuery, Linear, Slack, Notion и других сервисов прямо в чате

<Card title="Начните создавать прямо сейчас" icon="arrow-up-right" href="https://replit.com">
  Опишите свою идею — и Agent воплотит её в жизнь без дополнительной настройки.
</Card>
...
<Accordion title="Нужно ли уметь программировать?">
  Нет. Agent берёт на себя всю техническую работу: написание кода, настройку инфраструктуры, конфигурирование баз данных и многое другое. Просто опишите обычным языком, что вам нужно, и Agent всё создаст.
</Accordion>

<Accordion title="Чем отличаются Free, Power и Max?">
  **Free Mode** — это режим Agent для повседневных задач, бесплатно доступный подписчикам Core и Pro в пределах установленного лимита. **Power** — платный режим среднего уровня с мощными моделями по разумной цене. **Max** — платный режим высшего уровня для самых сложных задач. **Turbo** — дополнительная опция, доступная в расширенных настройках только для Max.

  Перед началом платных задач требуется подтверждение.
</Accordion>

<Accordion title="Можно ли создавать несколько вещей в одном проекте?">
  Да. В [одном проекте](/ru/ko/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) можно добавлять веб-приложения, мобильные приложения, слайды, видео и визуализации данных — все они используют один и тот же бэкенд и данные.
</Accordion>

<Accordion title="Что произойдёт, если Agent ошибётся?">
  Agent регулярно тестирует свою работу. Во время работы Agent также создаёт чекпоинты, поэтому вы можете вернуться к предыдущему состоянию. Вы также можете обсудить с Agent возникшую проблему, и он поможет её решить.
</Accordion>
...
Turbo по умолчанию отключён — при необходимости его можно включить для каждого проекта, выбрав Max. Free Mode недоступен в Enterprise на момент запуска. Рабочие пространства Enterprise вскоре получат отдельный интерфейс Agent; подробности сообщит администратор.

Подробнее о тарифах см. в разделе [Биллинг Agent](/ru/ko/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  <Card title="Agent Skills" icon="puzzle-piece" href="/ru/ko/features/agent/skills">
    Научите Agent специализированным знаниям: используйте готовые навыки или создавайте свои.
  </Card>

  <Card title="Система задач" icon="list-check" href="/ru/ko/core-concepts/agent/task-system">
    Канбан-планирование, фоновые задачи и изменения, применяемые к основной версии.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/ru/ko/design/canvas">
    Визуальные макеты и визуальный редактор с прямым манипулированием.
  </Card>

  <Card title="Connectors" icon="plug" href="/ru/ko/features/agent/general-agent">
    Подключайте BigQuery, Linear, Slack, Notion и другие сервисы.
  </Card>

  <Card title="Биллинг" icon="credit-card" href="/ru/ko/billing/ai-billing#agent-billing">
    Тарифы Agent, сравнение тарифов и управление расходами.
  </Card>

  <Card title="Руководство по vibe coding" icon="wand-magic-sparkles" href="/ru/ko/learn/build-with-agent">
    Советы по составлению эффективных промптов и разработке с помощью ИИ.
  </Card>
</CardGroup>