---
title: "Canvas"
description: "Apprenez à utiliser le Canvas pour explorer et itérer sur des designs avec l'Agent, et voir vos artefacts affichés dans des cadres."
---
...
<YouTubeEmbed videoId="Qv9cREjl9vQ" title="Présentation du Canvas Replit" />

## Qu'est-ce que le Canvas ?

Le Canvas est un espace pour explorer et itérer sur des designs avec l'Agent. Ici, vous verrez vos artefacts — comme des applications, des vidéos et des diapositives — affichés dans un cadre. Vous pouvez travailler avec l'Agent pour créer des maquettes, comparer des directions de design et appliquer les modifications à votre application — le tout depuis un tableau visuel unique.

<Frame>
  <img src="/assets/mirror/canvas-desktop-85e8bfec38a2.png" alt="Canvas affichant un aperçu d'application à côté du chat Agent avec les mises à jour des tâches" width="3452" height="2006" data-path="images/replitai/canvas-desktop.png" />
</Frame>

## Ce que contient le Canvas

Le Canvas est un tableau infini où vous pouvez tout voir et travailler visuellement.

<Frame>
  <img src="/assets/mirror/canvas-mockups-057e738a78da.png" alt="Canvas affichant plusieurs maquettes de design disposées côte à côte — thème sombre, vues mobiles et pages de détail" width="3452" height="1620" data-path="images/replitai/canvas-mockups.png" />
</Frame>

* **Vos applications** : Vos applications en cours d'exécution apparaissent comme des aperçus interactifs en direct sur le Canvas. Vous pouvez cliquer dessus et les utiliser comme vous le feriez dans un navigateur.
* **Maquettes de design** : Des prototypes visuels créés par l'Agent. Ceux-ci ressemblent à de vraies pages — vous pouvez interagir avec eux, les redimensionner et demander à l'Agent de les modifier — mais ils sont séparés de votre application en direct pour que vous puissiez expérimenter librement.
...
* **Images** : Déposez des captures d'écran ou des images de référence sur le Canvas. L'Agent peut les recréer ou s'en inspirer.
* **Vidéos** : Placez des fichiers vidéo sur le Canvas aux côtés de votre autre contenu.

### Ajouter des designs sur le Canvas

Il existe plusieurs façons de commencer à créer des designs. Choisissez celle qui vous semble la plus naturelle.

<Steps>
  <Step title="Demander à l'Agent dans le chat">
    Tapez ce que vous souhaitez voir dans le champ de chat et l'Agent le place comme un cadre interactif sur le Canvas.

    <AgentInput defaultPrompt="Show me a mockup of my landing page on the canvas" showTypewriter={true} />
  </Step>

  <Step title="Choisir un type de sortie">
  ...
  <Step title="Amener une page existante sur le Canvas">
    Vous avez déjà une application en direct ? Demandez à l'Agent d'amener une page sur le Canvas pour une refonte — par exemple, « Mets mon tableau de bord sur le canvas. » L'Agent en fait une copie pour que votre application en direct reste intacte.
  </Step>
</Steps>
...
### Demander des mises à jour à l'Agent

Sélectionnez un cadre sur le Canvas et saisissez vos modifications dans le champ qui apparaît en dessous. Lorsque vous appuyez sur Entrée, l'Agent prend en compte la demande, effectue les mises à jour et rafraîchit la maquette directement sur le Canvas.
...
Utilisez les outils de dessin dans la barre d'outils inférieure pour esquisser directement sur le Canvas. Dessinez des flèches pointant vers ce que vous souhaitez modifier, entourez les zones problématiques ou ajoutez des notes textuelles avec vos commentaires. Demandez ensuite à l'Agent d'apporter les modifications en fonction de vos annotations.
...
Voyez à quoi ressemblent vos designs sur différentes tailles d'écran. Demandez à l'Agent de vous montrer une version mobile, tablette ou bureau :

* **Mobile** — 390 × 844
* **Tablette** — 768 × 1024
* **Bureau** — 1280 × 720

Par exemple, « Montre-moi la version mobile de cette page » et l'Agent l'affiche à cette taille.
...
Lorsque vous êtes satisfait d'un design, appliquez les modifications à votre version principale. L'Agent vous montre un résumé de ce qui a été fait et vous donne la possibilité d'appliquer ou d'ignorer.
...
Lorsque vous n'êtes pas sûr de la direction à prendre, demandez à l'Agent de générer plusieurs versions pour les comparer côte à côte.
...
* **Plusieurs versions d'une page** : « Montre-moi cinq versions de cette page d'accueil. » L'Agent crée chacune d'elles et les dispose côte à côte.
* **Flux multi-pages** : « Conçois un flux d'intégration complet : bienvenue, configuration du profil, préférences et confirmation. » L'Agent construit chaque écran et les arrange dans l'ordre.
* **Mélanger et assortir** : « Montre-moi la page d'accueil et la page de tarification dans un style minimal et un style audacieux. » L'Agent crée chaque combinaison pour que vous puissiez voir l'ensemble.
...
* Demandez à l'Agent de générer le flux complet : « Dessine le flux d'intégration complet de mon application »
* Utilisez des flèches et des formes pour montrer comment les utilisateurs se déplacent entre les écrans
* Demandez à l'Agent de transformer les esquisses brutes en designs soignés

## Transformer un design en vraie application

Lorsque vous êtes satisfait d'un design et souhaitez le rendre réel, deux options s'offrent à vous :

### En faire une application complète

Si votre design nécessite de vraies fonctionnalités — comme la sauvegarde de données, des comptes utilisateurs ou la connexion à d'autres services — demandez à l'Agent de le transformer en application complète. Dites quelque chose comme « Rends cette application réelle » ou « Ajoute un backend à ceci. » L'Agent construit tout ce dont il a besoin pour fonctionner comme une vraie application publiable.
...
<Steps>
  ...
  <Step title="Demander à l'Agent de convertir">
    Dites à l'Agent ce que vous voulez — par exemple, « Convertis ceci en application web » ou « Transforme ceci en diapositives. »
  </Step>
...
<Accordion title="Les cadres de design sont-ils identiques à mon application en cours d'exécution ?">
  Non. Les cadres de design sont des prototypes visuels — ils ressemblent à la vraie chose, mais n'ont pas de backend ou de base de données fonctionnels derrière eux. Les aperçus de votre application sur le Canvas, en revanche, sont votre vraie application en cours d'exécution.
</Accordion>

<Accordion title="Puis-je repenser une page sans casser mon application en direct ?">
  Oui. Demandez à l'Agent d'amener une page sur le Canvas — par exemple, « Mets mon tableau de bord sur le canvas. » L'Agent en fait une copie avec laquelle vous pouvez expérimenter. Votre application en direct n'est pas affectée tant que vous ne choisissez pas d'appliquer les modifications.
</Accordion>

<Accordion title="Comment transformer un design en vraie application ?">
  Demandez à l'Agent d'en faire une application complète (« Rends ceci réel ») ou convertissez-le en un type spécifique (« Convertis ceci en application web »). L'Agent construit tout ce dont il a besoin pour devenir une application fonctionnelle et publiable.
</Accordion>

<Accordion title="Puis-je convertir en types autres que les applications web ?">
...
<Accordion title="Puis-je importer des designs depuis Figma ou d'autres outils ?">
  ...
  * **Importer depuis Figma** : Rendez-vous sur [replit.com/import](https://replit.com/import), connectez votre compte Figma et collez une URL de cadre pour convertir un design en application React fonctionnelle. Voir le [guide de démarrage rapide Import depuis Figma](/fr/getting-started/quickstarts/import-from-figma) pour plus de détails.
  * **Figma MCP dans le chat Agent** : Collez un lien Figma directement dans le chat Agent et demandez-lui de générer du code, d'extraire des données de design ou de construire un composant à partir du design. Voir le guide [Intégration Figma MCP](/replitai/mcp/figma).
  * **Référence par capture d'écran** : Déposez une capture d'écran sur le Canvas et demandez à l'Agent de recréer le design à partir de celle-ci.
</Accordion>

<Accordion title="Que puis-je d'autre mettre sur le Canvas ?">
