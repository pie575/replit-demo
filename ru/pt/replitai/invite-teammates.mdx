---
title: "Приглашение соавторов"
description: "Приглашайте соавторов в свой проект и работайте над ним вместе — каждый может начинать новые треды с Agent, работая в одном проекте."
---
...
<div id="construa-com-colaboradores">
  ## Создавайте вместе с соавторами
</div>

Replit позволяет нескольким людям работать над одним проектом одновременно. Каждый может запускать новые треды планирования с Agent, а задачи всех участников отображаются на общей доске — так ваша команда работает сообща, а не по очереди.

<div id="comece-a-construir-com-colaboradores">
  ## Начните создавать вместе с соавторами
</div>
...
3. Соавторы присоединяются и начинают новые треды с Agent
...
Каждый может создавать новые треды, поэтому вы можете работать независимо и не мешать друг другу. Задачи всех участников отображаются на общей канбан-доске — так сразу видно, кто над чем работает, и легче избежать пересечений.

<Note>
  На плане Core во всём проекте одновременно может выполняться только 1 фоновая задача. На плане Pro — до 10 задач параллельно, так что несколько человек могут одновременно поручать Agent создание разных частей проекта.
</Note>
...
Когда команда работает сообща, задачи всех участников отображаются на одной доске. Вы сразу видите, кто чем занят, что находится в работе, а что готово к применению.
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Черновик</strong> — задачу предложил Agent. Для неё уже готов план, который можно просмотреть.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Активная</strong> — Agent прямо сейчас работает над этой задачей в отдельной изолированной копии проекта.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Готовая</strong> — задача выполнена. Просмотрите изменения и примените их к основной версии.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Завершённая</strong> — работа над задачей закончена, а её изменения вошли в основную версию.</span>
  </div>
</div>
...
Когда несколько человек работают одновременно, задачи каждого отображаются на общей доске. Вот как это выглядит: один соавтор работает над продолжениями историй и повторной генерацией, другой — над функцией отправки подарков, а остальная завершённая работа команды находится в столбцах «Готово» и «Завершено».
...
{ title: 'Повторная генерация историй — переработка страниц', description: 'Возможность повторно генерировать отдельные страницы даёт родителям больше контроля и уменьшает разочарование.', status: 'active', timestamp: '8m' },
{ title: 'Добавить возможность дарить истории родственникам и друзьям', description: 'Позвольте родителям создать историю для ребёнка другого человека и отправить её в подарок.', status: 'active-queued', timestamp: '1m' },
] },
...
{ title: 'Ссылки на истории, которыми можно делиться, с OG-превью', description: 'Добавление общедоступных ссылок для публикации — главный драйвер роста.', status: 'ready-drop', timestamp: '31m', showMenu: true },
] },
{ title: 'Завершено', count: 3, cards: [
{ title: 'Режим чтения вслух с выделением слов', description: 'Озвучивание текста позволяет детям самостоятельно наслаждаться историями.', status: 'done', timestamp: '4h' },
...
{ title: 'Адаптация для мобильных устройств и финальная доработка', description: 'Улучшенная обработка ошибок, макеты для мобильных устройств и анимации загрузки.', status: 'done', timestamp: '4h' },
] }
]}
/>

Когда задача переходит в статус «Готово», любой участник команды может проверить и применить изменения. Agent автоматически разрешает конфликты, когда применяется несколько задач.

<div id="o-que-voc-pode-construir-juntos">
  ## Что вы можете создавать вместе
</div>

* **Разделите разработку функции между участниками команды**: Один человек создаёт фронтенд, а другой занимается бэкендом — каждый в своём чате с Agent. Канбан-доска помогает всем оставаться в курсе.
* **Подключите нового участника**: Он может изучить проект в своём чате с Agent, задать вопросы о том, как всё устроено, и начать вносить вклад — не мешая работе других.
* **Выпускайте быстрее с Pro**: Благодаря параллельной работе два или три человека могут одновременно выполнять и отправлять задачи. Проект, на который у одного человека ушла бы целая сессия, можно завершить за гораздо меньшее время.

<div id="perguntas-frequentes">
  ## Часто задаваемые вопросы
</div>

<Accordion title="Могу ли я написать соавтору через Agent?">
  Нет. Agent — не мессенджер: он постоянно слушает и попытается выполнить всё, что вы введёте. Для общения в команде используйте Slack, Discord или другой инструмент.
</Accordion>

<Accordion title="Не перезапишем ли мы работу друг друга?">
  Система задач помогает этого избежать: каждая задача получает собственную изолированную копию, но при этом «знает» о других задачах. По завершении вы применяете задачи к основной версии и на этом этапе разрешаете возможные конфликты.
</Accordion>

<Accordion title="Как посмотреть, чем занимаются мои соавторы?">
  Откройте канбан-доску, чтобы увидеть активные задачи. Вы также можете видеть курсоры соавторов и то, какие файлы они просматривают.
</Accordion>

<Accordion title="Могу ли я удалить кого-то из проекта?">
  Да. Владельцы проекта могут управлять участниками команды и отзывать доступ в любой момент.
</Accordion>
