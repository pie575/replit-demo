...
description: "Используйте доска задач, чтобы просматривать, запускать, отслеживать и применять задачи Agent по мере их продвижения по каждому этапу."
---
...
Доска задач — это единое место, где можно просматривать, запускать и отслеживать задачи Agent, а также применять их результаты. Используйте её, когда Agent разбивает большой запрос на несколько частей, когда вы хотите выполнять работу в фоновом режиме или когда нужно разобраться, что уже изменилось в вашем проекте.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Доска задач с задачами, распределёнными по колонкам «Черновики», «Активные», «Готово» и «Завершено»" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

Задачи распределены по колонкам в зависимости от статуса. На доске видно, что Agent запланировал, что выполняется, что готово к проверке и что уже применено.

По мере выполнения каждая задача перемещается слева направо:

* **Черновики**: задачи, которые Agent запланировал, но ещё не начал
* **Активные**: задачи, над которыми Agent работает прямо сейчас, включая задачи в очереди из-за ограничения на количество активных задач
* **Готово**: задачи, которые Agent завершил и которые готовы к вашей проверке
...
**Черновики** — это запланированные задачи, к выполнению которых Agent ещё не приступил. Каждый черновик включает заголовок, описание и пошаговый план.

Используйте черновики, чтобы проверить предложенный Agent подход до начала работы. Вы можете изучить план, попросить Agent скорректировать его или приступить к задаче, когда будете готовы.

Черновики полезны, когда вы хотите спланировать более крупную функцию, прежде чем поручать работу в фоновом режиме. Например, Agent может разделить проект по интеграции на отдельные черновики для аутентификации, настройки профиля, интерфейса панели управления и финальной доработки развёртывания.

<div id="ativo">
  ## Активные
</div>

После принятия задача переходит в столбец **Активные**, и Agent приступает к работе. Каждая задача выполняется в собственной изолированной копии вашего проекта, поэтому в основной версии ничего не изменится, пока вы не примените изменения.

Некоторые задачи зависят от других. Например, задача по созданию dashboard может требовать предварительного завершения задачи со схемой базы данных. Зависимые задачи остаются **В очереди**, пока не будут выполнены необходимые условия. Задачи также становятся в очередь, если вы достигли лимита активных фоновых задач по своему тарифу.

<KanbanColumns
  columns={[
{ title: 'Черновики', count: 2, cards: [
{ title: 'Добавить возможность дарить истории семье и друзьям', description: 'Позвольте родителям создавать историю для чужого ребёнка и отправлять её в подарок. Это открывает новый канал привлечения пользователей.', status: 'draft-static', timestamp: '2m' },
{ title: 'Режим чтения перед сном с тёмной темой', description: 'Специальный режим перед сном с тёмной темой тёплых оттенков и фоновыми звуками превращает приложение в полноценный вечерний ритуал.', status: 'draft-static', timestamp: '2m' },
] },
{ title: 'Активные', count: 3, cards: [
{ title: 'Продолжения историй: сиквелы', description: 'Добавление кнопки "Продолжить приключение" создаёт продолжение истории с того места, где закончилась предыдущая.', status: 'active', timestamp: '12m' },
...
{ title: 'Добавить возможность дарить истории семье и друзьям', description: 'Позвольте родителям создавать историю для чужого ребёнка и отправлять её в подарок.', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
Когда задача завершает сборку, она переходит в статус **Готово**. Это значит, что работа выполнена, но в основной версии пока ничего не изменилось.

Применить изменения можно двумя способами:

* **Из треда**: откройте тред задачи и выберите **Применить изменения к основной версии**, чтобы перенести работу в проект, или **Отклонить**, чтобы отказаться от неё
* **С доски**: откройте меню с тремя точками на любой карточке задачи со статусом «Готово» и выберите **Применить изменения**

<TaskReviewDrawer />

Прежде чем применять готовые задачи, проверьте их. Изучите журнал работы, результаты тестов и предпросмотр, чтобы понять, что изменил Agent.
...
В колонке «Завершённые» также собираются архивированные сессии планирования и отменённые задачи. Так сохраняется история происходившего, а завершённая работа не смешивается с «Черновиками» или «Активными».

<KanbanColumns
  columns={[
{ title: 'Готово', count: 3, cards: [
...
{ title: 'Завершённые', count: 4, cards: [
...
Agent автоматически разрешает конфликты при применении изменений из нескольких задач. Если что-то выглядит не так, попросите Agent это исправить.

<div id="encontrar-e-filtrar-tarefas">
  ## Поиск и фильтрация задач
</div>

Используйте доску задач, если в проекте много задач в работе. Вы можете искать по названию или номеру задачи, фильтровать черновики и управлять группами задач одновременно в режиме редактирования.

Например, ищите по номеру задачи, если кто-то ссылается на задачу из треда, ревью или беседы в Slack. Фильтруйте черновики, если хотите сосредоточиться на запланированной работе, не просматривая активные или завершённые задачи.

О предлагаемых последующих задачах см. в разделе [Последующие задачи](/ru/pt/core-concepts/agent/task-system/follow-up-tasks).