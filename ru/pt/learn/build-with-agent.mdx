...
description: "Освойте пять практических привычек работы с Agent: формулируйте запросы конкретно, планируйте работу, добавляйте контекст, проверяйте и тестируйте результат, используйте checkpoint."
...
Agent — это ИИ-конструктор Replit и ваш напарник в превращении идей в готовые программы. Agent умеет планировать изменения, писать код, объяснять поведение приложения, устранять ошибки и улучшать ваш app.

[Vibe coding 101](/ru/pt/learn/foundations/vibe-coding-101) объясняет сам подход, а эта страница — как работать с Agent на практике. [Effective prompting](/ru/pt/learn/effective-prompting) рассказывает, как формулировать более чёткие инструкции.

Представьте, что Agent — квотербек на поле: он оценивает ситуацию, выбирает направление и продвигает работу вперёд. Вы — тренер: вы определяете стратегию, объявляете розыгрыш, разбираете результат и решаете, каким будет следующий шаг.

Agent способен на многое, но лучше всего работает, когда процессом руководите вы: формулируйте конкретно, планируйте работу, добавляйте контекст, проверяйте и тестируйте, а также используйте checkpoints, когда нужно откатиться назад.

<div id="cinco-dicas-para-construir-com-agent">
  ## Пять советов по созданию приложений с Agent
</div>

Возьмите за правило следующие приёмы при работе с Agent:

1. Формулируйте запросы конкретно.
2. Планируйте работу.
3. Добавляйте контекст.
4. Проверяйте и тестируйте.
5. Используйте checkpoint.
...
Полезный запрос объясняет, чего вы хотите, что для вас важно и что должно остаться без изменений. Описывать каждую деталь реализации не нужно, но ориентиров должно быть достаточно, чтобы Agent принимал верные решения.
...
Agent может разбить эту работу на этапы, но именно ваши указания определяют, как выглядит успешный результат.

Больше примеров конкретных инструкций смотрите в разделе [Эффективные prompt](/ru/pt/learn/effective-prompting).

<div id="planeje-o-trabalho">
  ## Планируйте работу
</div>

Планирование помогает, когда в задаче есть неопределённость, несколько этапов, компромиссы или что-то, что вы хотите утвердить до того, как файлы будут изменены.

Вы можете попросить Agent составить план прямо в чате:
...
Используйте [Plan mode](/ru/pt/features/agent/plan-mode), когда хотите, чтобы Agent сначала всё обдумал и дождался вашего одобрения, прежде чем менять файлы. Включите **Plan** в поле ввода prompt перед отправкой.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Начальное поле ввода prompt в Replit с введённым prompt для работающего приложения и включённым флажком Plan" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Когда Agent закончит обдумывать задачу, вы увидите баннер **task plan is ready for review** с кнопкой **Review now**.

<Frame>
  <img src="/assets/mirror/agent-plan-ready-banner-5fbcf58a1668.png" alt="Баннер «task plan is ready for review» с кнопкой «Review now» в нижней части панели Agent" width="560" height="550" data-path="images/fitstart/work-with-agent/agent-plan-ready-banner.png" />
</Frame>
...
Контекст можно добавить в виде текста, снимков экрана, набросков, файлов, данных, ошибок или аннотаций из Canvas. Выбирайте тот формат, который лучше всего объясняет задачу.
...
Используйте снимки экрана или наброски, когда макет сложно описать словами:
...
Используйте аннотации [Canvas](/ru/pt/design/canvas), когда изменение носит визуальный характер и вы хотите указать на конкретную часть приложения.

<Frame>
  <img src="/assets/mirror/agent-canvas-annotations-c88ab5856966.png" alt="Frame фитнес-приложения в Canvas с прямоугольником, стрелкой и текстовой подписью, указывающими на карточку прогресса, а также с наброском от руки и подписью в области формы" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-canvas-annotations.png" />
</Frame>
...
Agent может предложить план перед началом работы, а после — подвести итог внесённых изменений. Читайте и то, и другое.

Откройте план, выбрав **View** в карточке задачи. В развёрнутом виде план показывает, что Agent собирается создать, критерии успеха, что остаётся за рамками задачи и какие шаги он выполнит.

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="Развёрнутый план задачи с разделами «Что» и «Почему», критериями готовности, пунктами вне области задачи и пронумерованными шагами создания" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>

Прежде чем Agent приступит к работе, изучите план:

* Решает ли он нужную задачу?
* Не упущено ли что-нибудь?
* Есть ли лишнее, выходящее за рамки задачи?
* Можно ли сначала собрать более простую версию?
* Сказано ли, как вам протестировать результат?

Если план слишком обширный, попросите Agent сократить его.
...
Когда Agent закончит, протестируйте приложение сами. Откройте Preview и поработайте с приложением так, как это делал бы человек, для которого оно создано.
...
* Достаточно ли хорош результат, чтобы продолжать разработку?

Если что-то не так, опишите, что именно вы увидели.
...
Конкретная обратная связь помогает Agent исправить именно ту проблему, которая вас беспокоит.

<div id="use-checkpoints">
  ## Use checkpoints
</div>

Checkpoint&#39;ы помогают откатиться назад, если изменение пошло не в ту сторону.

Используйте точечную обратную связь, когда результат уже почти готов:
...
Используйте checkpoint, когда приложение стало хуже, чем было, сломалось важное поведение или Agent изменил больше, чем ожидалось.

Откройте **History** на панели Agent, чтобы увидеть checkpoint&#39;ы. Каждый checkpoint показывает, что было изменено, и содержит элементы управления **Rollback here** и **Changes**.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="Панель History в Agent показывает checkpoint с меткой 'Transitioned from Plan to Build mode' и кнопками Rollback here и Changes" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

Когда вы выбираете **Rollback here**, Agent уточняет, что именно будет восстановлено, прежде чем выполнить откат.
...
Если вы не уверены, что лучше — исправлять дальше или откатиться, попросите Agent помочь со сравнением.
...
Ошибки по ходу работы — это нормально. Checkpoint&#39;ы позволяют экспериментировать, не теряя возможности вернуться к более надёжной версии.

Больше рекомендаций по восстановлению см. в разделе [Checkpoints and rollbacks](/ru/pt/features/version-control/checkpoints-and-rollbacks).

<div id="como-uma-boa-colaborao-com-agent">
  ## Как выглядит продуктивная работа с Agent
</div>

Вы на верном пути, если:

* В ваших запросах описаны цель и ограничения
* План Agent достаточно понятен, чтобы его можно было проверить
* Вы добавляете скриншоты, файлы или заметки из Canvas, когда одних слов мало
* После изменений вы проверяете ключевые сценарии
* Вы даёте конкретную обратную связь, когда что-то пошло не так
* Вы возвращаетесь к checkpoint вместо того, чтобы бесконечно распутывать неудачное изменение
...
<CardGroup cols={2}>
  <Card icon="comment-dots" href="/ru/pt/learn/effective-prompting">
    Научитесь формулировать более чёткие инструкции, по которым Agent сможет уверенно действовать.
  </Card>

  <Card title="Создайте своё первое приложение" icon="rocket" href="/ru/pt/build/your-first-app">
    Потренируйтесь работать с Agent — от первого prompt до точечной доработки.
  </Card>

  <Card title="Checkpoints и откаты" icon="rotate-left" href="/ru/pt/features/version-control/checkpoints-and-rollbacks">
