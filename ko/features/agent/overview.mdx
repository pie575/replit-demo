---
title: "Replit Agent"
description: "Replit Agent는 여러분의 아이디어를 앱, 디자인, 슬라이드 등으로 만들어 줍니다. 모두 평범한 언어로 코딩 없이 가능합니다."
---
...
Agent는 여러분의 창작 파트너입니다. Agent는 여러분의 아이디어를 받아 다듬는 것을 돕고, 이를 현실로 만듭니다. 질문에만 답하는 챗봇과 달리 Agent는 실제로 행동합니다: 프로젝트를 설정하고, 애플리케이션을 만들고, 작업을 점검하고, 과정 중에 발생하는 문제를 해결합니다.

원하는 것을 일상적인 언어로 설명하세요. 코드나 기술 지식이 필요하지 않습니다. Agent가 계획부터 배포까지 나머지 모든 것을 처리합니다.
...
<Steps>
  <Step title="원하는 것을 설명하세요">
    [Project Editor](/ko/learn/projects-and-artifacts/project-editor)에서 바로 대화를 시작하세요. 만들고 싶은 앱을 설명하거나, 질문을 하거나, 주제를 조사하거나, BigQuery, Slack, Notion 같은 [연결된 서비스](/ko/features/agent/general-agent)에서 데이터를 가져올 수 있습니다. 제약은 없습니다. Replit Agent는 무엇이든 처리할 수 있습니다.

    <AgentInput />
  </Step>

  <Step title="무엇을 만들지 선택하세요">
    선택적으로 프로젝트 유형을 선택할 수 있습니다: 웹 앱, 모바일 앱, 슬라이드, 디자인, 데이터 시각화 등. 1단계에서 이미 원하는 것을 설명했다면, Agent가 알맞은 설정을 자동으로 파악합니다.

    <AgentCarousel />
  </Step>

  <Step title="Agent가 빌드합니다">
    Agent는 코드를 작성하고, 인프라를 설정하고, 결과를 테스트합니다. 프로젝트에 들어가면 [Agent 모드](#agent-modes)를 전환하여 빌드 방식을 제어할 수 있습니다.
  </Step>

  <Step>
    Agent와 대화하며 프로젝트를 다듬고, 준비가 되면 게시하세요. 모든 아티팩트가 함께 게시됩니다.
  </Step>
</Steps>

<Note>한 가지 유형에만 얽매이지 않습니다. 웹 앱으로 시작해서 나중에 모바일 앱, 슬라이드, 영상을 추가할 수 있습니다 — 모두 같은 프로젝트 안에서요.</Note>

### Plan 모드

Agent가 코드나 데이터를 변경하기 전에 아이디어를 구상하고, 질문하고, 프로젝트를 계획할 수 있도록 Plan 모드를 활성화하세요. Plan 모드에서 Agent는 다음을 수행합니다:

* **복잡한 프로젝트를 세분화**하여 순서가 있는 작업 목록으로 만듭니다
* **다양한 접근 방식을 탐색**하고 장단점을 따져봅니다
* 코드가 작성되기 전에 **검토하고 개선**합니다

채팅 입력창에서 "Plan"을 클릭하거나 Agent에게 직접 요청하여 Plan 모드로 전환하세요. 예시: "우리 팀을 위한 프로젝트 트래커를 만들기 위한 계획을 세워줘"

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent는 검토하고 다듬을 수 있는 순서가 있는 작업 목록을 생성합니다. 계획이 마음에 들면 승인하고 Agent가 빌드를 시작합니다.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="네 가지 제안된 작업과 작업 승인(Accept tasks), 계획 수정(Revise plan) 옵션을 보여주는 작업 계획 생성 대화상자" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

자세한 내용은 [Plan 모드 가이드](/ko/features/agent/plan-mode)를 참고하세요.

### Agent 모드

프로젝트를 빌드할 방식을 선택하세요:

<Frame caption="Free, Power, Max 모드가 있는 Agent 모드 팝업창">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Power가 선택된 Free, Power, Max 모드와 기본 모델 목록, Effort 슬라이더를 보여주는 Agent 모드 팝업창" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**: Core 및 Pro 구독자를 위한 일상적인 Agent 작업에 무료로 제공됩니다. 사용 한도는 5시간마다 초기화되며 주간 한도가 있습니다. Starter 등급은 기존 한도를 그대로 유지합니다. **설정 → 사용량**에서 사용량을 확인하세요.
* **Power**: 강력한 모델을 합리적인 가격에 제공하는 유료 중간 등급 모드입니다.
* **Max**: 가장 까다로운 작업을 위해 최고 성능의 프런티어 모델을 제공하는 유료 최상위 등급 모드입니다. **고급 설정**에서 더 빠른 실행을 위해 **Turbo**를 켤 수도 있습니다.

요청이 Free Mode보다 더 많은 성능을 필요로 하면 Agent가 유료 **한 번 허용** 옵션을 제안할 수 있습니다. Replit은 유료 작업이 시작되기 전에 확인을 요청하며, 작업이 끝나면 Agent는 다시 Free Mode로 돌아갑니다.

<Note>팁: 빌드가 시작되기 전에 Agent의 계획을 검토하고 반복할 수 있도록 \*\*[Plan 모드](/ko/features/agent/plan-mode)\*\*를 활성화할 수도 있습니다.</Note>

## Replit Agent로 만들 수 있는 것

* **웹 앱, 모바일 앱, 데이터 대시보드, AI 기반 도구**
* **시각적 디자인과 프로토타입** — 코드로 옮기기 전에 [Design Canvas](/ko/design/canvas)에서 목업을 탐색
* **하나의 프로젝트에서 여러 출력물** — 같은 백엔드를 공유하는 웹 앱, 모바일 앱, 슬라이드, 영상
* **파일과 문서** — CSV, PDF, PowerPoint 파일, Markdown 문서
* **연결된 서비스 조회** — 채팅에서 바로 BigQuery, Linear, Slack, Notion 등의 데이터를 가져오기

<Card title="지금 빌드를 시작하세요" icon="arrow-up-right" href="https://replit.com">
  아이디어를 설명하고 Agent가 이를 현실로 만들도록 하세요 — 설정이 필요 없습니다.
</Card>

## 자주 묻는 질문

<Accordion title="코딩하는 방법을 알아야 하나요?">
  아닙니다. Agent가 코드 작성, 인프라 설정, 데이터베이스 구성 등 모든 기술적인 작업을 처리합니다. 원하는 것을 평범한 언어로 설명하면 Agent가 만들어 줍니다.
</Accordion>

<Accordion title="Free, Power, Max의 차이는 무엇인가요?">
  **Free Mode**는 Core 및 Pro 구독자에게 할당량 내에서 무료로 제공되는 일상적인 Agent 작업 모드입니다. **Power**는 강력한 모델을 합리적인 가격에 제공하는 유료 중간 등급 모드입니다. **Max**는 가장 까다로운 작업을 위한 유료 최상위 등급 모드입니다. **Turbo**는 고급 설정에서 사용할 수 있는 Max 전용 선택 옵션입니다.

  유료 작업은 시작되기 전에 확인이 필요합니다.
</Accordion>

<Accordion title="한 프로젝트에서 여러 가지를 만들 수 있나요?">
  네. [같은 프로젝트](/ko/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project)에 웹 앱, 모바일 앱, 슬라이드, 영상, 데이터 시각화를 추가할 수 있습니다 — 모두 같은 백엔드와 데이터를 공유합니다.
</Accordion>

<Accordion title="Agent가 실수하면 어떻게 되나요?">
  Agent는 정기적으로 자신의 작업을 테스트합니다. Agent는 작업하는 동안 체크포인트도 생성하므로 이전 상태로 되돌릴 수 있습니다. 문제가 무엇인지 Agent에게 설명하며 대화할 수도 있고, Agent가 문제를 해결해 줄 것입니다.
</Accordion>
...
Turbo는 기본적으로 꺼져 있습니다 — Max를 선택한 상태에서 필요할 때 프로젝트별로 켤 수 있습니다. Free Mode는 출시 시점에 Enterprise에서는 사용할 수 없습니다. Enterprise 워크스페이스는 곧이어 전용 Agent 경험을 제공받게 되며, 자세한 내용은 관리자가 안내할 것입니다.

자세한 요금 정보는 [Agent 결제](/ko/billing/ai-billing#agent-billing)를 참고하세요.

## 다음 단계

<CardGroup cols={2}>
  <Card title="Agent Skills" icon="puzzle-piece" href="/ko/features/agent/skills">
    Agent에게 전문 지식을 가르치세요 — 미리 만들어진 스킬을 사용하거나 직접 만드세요.
  </Card>

  <Card title="Task system" icon="list-check" href="/ko/core-concepts/agent/task-system">
    칸반 계획, 백그라운드 작업, 그리고 메인 버전에 다시 적용되는 변경 사항.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/ko/design/canvas">
    시각적 목업과 직접 조작하는 시각 편집기.
  </Card>
  ...
  <Card title="Billing" icon="credit-card" href="/ko/billing/ai-billing#agent-billing">
    Agent 요금제, 플랜 비교, 지출 관리.
  </Card>

  <Card title="Vibe coding guide" icon="wand-magic-sparkles" href="/ko/learn/build-with-agent">
    효과적인 프롬프트 작성과 AI를 활용한 빌드를 위한 팁.
  </Card>
</CardGroup>
