---
title: "Conversation 与项目"
description: "了解在构建或设计工作较多时，应继续在 Conversation 中进行还是使用项目。"
---
...
您已调研周边、连接工具并制定了上线计划。现在该让大家了解这家
咖啡店了。为此，我们将创建一个开业预告网站。

开始之前，先了解这项工作在 Replit 中分别在何处进行会很有帮助：
Conversation 和项目。
...
咖啡店的上线始于一次 Conversation。在那里，你打磨了想法、调研了市场、连接了工具，还创建了 Routine。

现在，可以着手制作开业预告页面了。项目正是承载这类正式设计与构建工作的空间。Replit 能直接沿用 Conversation 中已有的上下文，你无需从头再来。

<div id="conversation">
  ## 什么是 Conversation？
</div>

**Conversation** 是一个会话，您可以在其中提问、探索信息、运行 Routine，并通过后续
问题继续推进工作。此会话包含有关咖啡店的
调研和上线计划。
...
**项目**是用于正式构建和设计工作的持久化、结构化空间。在本文的场景中，它就是你创建并打磨即将上线站点的地方。
...
| 领域      | Conversation       | 项目                      |
| ------- | ------------------ | ----------------------- |
| 适用场景    | 探索想法、调研竞品、执行每周市场监测 | 设计并构建即将上线的网站            |
| 结构      | 包含调研、决策与后续追问的单条会话  | 用于网站、设计及其他构建产物的结构化空间    |
| Routine | 创建并管理重复调研          | 使用回传到 Conversation 的结果  |
| 上下文     | 保留促成这一切的上线计划       | 从 Conversation 中确立的方向出发 |

想提问或探索时，请从 Conversation 开始。准备好动手构建时，
再转入项目。在这一转换过程中，你此前确立的方向会
完整保留。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/ko/chat/conversations">
    ...
    <p>
      了解 Conversation 如何帮助你提问、探索、运行 Routine 并获得结果。
    </p>
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/ko/home/design-an-opening-site">
    <div className="chat-use-case-visual">
    ...
    <div className="chat-example-tag">即将开业的网站</div>
    <div className="chat-card-title">设计即将开业的网站</div>
    <p>借助 Conversation 和上线计划，为咖啡馆打造一个网站。</p>
