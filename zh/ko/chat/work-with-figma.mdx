---
title: "使用 Figma"
description: "将 Figma 设计帧作为视觉上下文后，如需探索、完善或据此构建，请按照 Replit Design 的导入流程操作。"
...
Replit 支持导入 Figma 设计帧。

如果你已有视觉方向，并希望 Replit 基于该设计开始构建或切换到设计模式，请导入 Figma 设计帧。

<div id="figma">
  ## 适合使用 Figma 的场景
</div>

* **探索不同方案：**在保留核心结构的同时，尝试不同的风格或布局。
* **延续产品流程：**将现有界面作为后续界面或状态的上下文。
* **将设计稿发展为应用：**从已获批准的视觉方向出发，再加以完善并
  构建。
...
选择要继续处理的设计帧。Replit Design 会将导入的设计帧转换为设计
帧，方便你探索修改方案、比较建议，并决定下一步要构建什么。

<Frame caption="导入 Replit Design 后转换为设计帧的 Figma 设计帧。">
  <img src="/assets/mirror/figma-import-result-052394025b8d.webp" alt="导入的 Personal Fitness Trainer 着陆页，在 Replit Design 中显示为设计帧" width="3440" height="2160" data-path="images/fitstart/turn-mockup-into-app/figma-import-result.webp" />
</Frame>
...
Design 指南涵盖完整的导入、优化和构建工作流。

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/ko/design/import-figma-designs-into-replit-design">
