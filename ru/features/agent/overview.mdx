...
Agent — ваш творческий партнёр. Он подхватывает ваши идеи, помогает их доработать и воплощает в жизнь. В отличие от чат-бота, который просто отвечает на вопросы, Agent действует: настраивает проект, создаёт приложения, проверяет результат и попутно устраняет проблемы.

Опишите, что вам нужно, обычными словами. Знание кода или технических деталей не требуется. Остальное Agent берёт на себя — от планирования до развёртывания.
...
<Steps>
  <Step title="Опишите, что вам нужно">
    В [Project Editor](/ru/learn/projects-and-artifacts/project-editor) просто начните диалог. Опишите приложение, которое хотите создать, задайте вопрос, изучите тему или получите данные из [подключённых сервисов](/ru/features/agent/general-agent), таких как BigQuery, Slack или Notion. Ограничений нет — Replit Agent справится с любой задачей.

    <AgentInput />
  </Step>

  <Step title="Выберите, что создавать">
    При желании выберите тип проект: веб-приложение, мобильное приложение, презентация, design, визуализация данных и другое. Если вы уже описали задачу на шаге 1, Agent сам подберёт подходящую конфигурацию.

    <AgentCarousel />
  </Step>

  <Step title="Agent всё создаёт">
    Agent пишет код, настраивает инфраструктуру и тестирует результат. Открыв свой проект, вы можете переключаться между [режимами Agent](#agent-modes), чтобы управлять тем, как он ведёт разработку.
  </Step>

  <Step>
    Общайтесь с Agent, чтобы доработать проект, и опубликуйте его, когда будете готовы. Все artifact публикуются вместе.
  </Step>
</Steps>
...
* **Разобьёт сложные проекты** на упорядоченные списки задач
* **Изучит разные подходы** и взвесит их плюсы и минусы
* **Даст всё просмотреть и доработать** до написания кода

Нажмите «Plan» в поле чата — или просто попросите об этом Agent, — чтобы переключиться в режим Plan. Пример: «Составь план разработки трекера проектов для моей команды»
...
<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="Диалог с созданными планами задач: четыре предложенные задачи и варианты «Принять задачи» и «Изменить план»" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

Подробнее — в [руководстве по режиму Plan](/ru/features/agent/plan-mode).
...
<Frame caption="Всплывающее окно режимов Agent с режимами Free, Power и Max.">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Всплывающее окно режимов Agent с режимами Free, Power и Max, где выбран Power, списком основных моделей и ползунком усилий" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**: бесплатная повседневная работа с Agent в рамках лимита вашего тарифа. Лимиты на тарифах Core и Pro обновляются каждые пять часов, при этом действуют и недельные ограничения. На тарифе Starter лимиты остаются прежними. Чтобы посмотреть своё потребление, откройте **Settings → Usage**.
...
Если для запроса нужно больше возможностей, чем даёт Free Mode, Agent может предложить платный вариант **Allow once**. Перед началом платного действия Replit запрашивает подтверждение, а после его завершения Agent возвращается в Free Mode.

<Note>Совет: вы также можете включить **[режим Plan](/ru/features/agent/plan-mode)**, чтобы просмотреть и доработать план Agent до начала сборки.</Note>
...
* **Визуальные макеты и прототипы** — прорабатывайте макеты на [Design Canvas](/ru/design/canvas), прежде чем переходить к коду
...
<Accordion title="Нужно ли уметь программировать?">
  Нет. Agent берёт на себя всю техническую работу: пишет код, настраивает инфраструктуру и базы данных. Вы описываете простыми словами, что хотите получить, а Agent это создаёт.
</Accordion>

<Accordion title="Чем Auto отличается от Free, Power и Max?">
  **Free Mode** — бесплатная повседневная работа с Agent в рамках лимита вашего тарифа. В режимах Power и Max **Auto** подбирает модель для каждой платной задачи. **Power** — платный режим среднего уровня с мощными моделями по доступной цене. **Max** — платный режим высшего уровня для самых требовательных задач.

  Платные действия требуют подтверждения перед запуском.
</Accordion>

<Accordion title="Можно ли создать несколько вещей в одном проекте?">
  Да. Вы можете добавлять веб-приложения, мобильные приложения, презентации, видео и визуализации данных в [один проект](/ru/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) — все они используют общий бэкенд и данные.
</Accordion>

<Accordion title="Что произойдёт, если Agent ошибётся?">
  Agent регулярно проверяет собственную работу. Кроме того, по ходу работы он создаёт checkpoint, поэтому вы можете откатиться к любому предыдущему состоянию. Также вы можете написать Agent в Chat, описать, что пошло не так, и он исправит проблему.
</Accordion>
...
В workspace Enterprise вместо Free Mode используется единый режим Auto. Auto сам выбирает режим и разрешённую модель для каждой задачи. Администраторы аккаунта могут управлять доступом к моделям с помощью [элементов управления моделями Enterprise](/ru/features/collaboration/enterprise-model-controls).

Подробную информацию о ценах см. в разделе [Оплата Agent](/ru/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  <Card title="Навыки Agent" icon="puzzle-piece" href="/ru/features/agent/skills">
    Передайте Agent специализированные знания — используйте готовые навыки или создайте собственные.
  </Card>

  <Card title="Система задач" icon="list-check" href="/ru/core-concepts/agent/task-system">
    Планирование на канбан-доске, фоновые задачи и перенос изменений в основную версию.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/ru/design/canvas">
    Визуальные макеты и практичный визуальный редактор.
  </Card>

  <Card title="Connectors" icon="plug" href="/ru/features/agent/general-agent">
    Подключайте BigQuery, Linear, Slack, Notion и другие сервисы.
  </Card>

  <Card title="Оплата" icon="credit-card" href="/ru/billing/ai-billing#agent-billing">
    Стоимость Agent, сравнение тарифов и управление расходами.
  </Card>

  <Card title="Руководство по vibe coding" icon="wand-magic-sparkles" href="/ru/learn/build-with-agent">
    Советы по составлению эффективных prompt и разработке с помощью ИИ.
  </Card>
</CardGroup>