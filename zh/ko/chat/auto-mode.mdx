---
title: "使用智能模型路由"
description: "让 Replit 为每个任务选择合适的模型，仅在必要时手动指定模型。"
sidebarTitle: "智能模型路由"
...
<div className="chat-mkt" role="img" aria-label="表示智能模型路由的 Anthropic、OpenAI 和 Kimi 标志" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
使用智能模型路由时，Replit 会在质量与成本之间取得平衡，为每项任务选择合适的模型。如果产品中显示为 **Auto**，即表示 Replit 正在使用智能模型路由。

<div id="section">
  ## 启用智能模型路由
</div>

1. 在提示词输入框中选择 **Agent Mode**,菜单中会列出可用模式(**Free**、**Power**、**Max**)。
2. 选择 **Power** 或 **Max**,然后找到 **Primary model** 部分。该标签会随所选模式变化,例如可能显示为 **Primary Max model**。
3. 如果主模型已设置为 **Auto**,说明智能模型路由已启用;如果选择的是特定模型,请打开主模型菜单并选择 **Automatically choose**。

Free 模式始终使用智能模型路由,无法手动选择模型。

<div id="section-2">
  ## 让模型随任务自动决定
</div>

如果你希望在任务发生变化时由 Replit 来处理模型选择，智能模型路由会很有用。明确的请求可以使用低成本模型，而复杂的任务可能需要性能更强的模型。

在 Power 模式或 Max 模式下，Core 和 Pro 用户可以改为手动选择可用的模型。

<Frame caption="Agent 设置中的智能模型路由、模型选择与 Effort。">
  <img src="/assets/mirror/auto-mode-model-and-effort-selector-fdcabef57097.jpg" alt="显示 Automatically choose recommended、可用模型、Effort 控件以及使用智能模型路由的 Max 模式的 Agent 设置" width="1839" height="855" data-path="images/chat/auto-mode-model-and-effort-selector.jpg" />
</Frame>
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/ko/features/agent/model-selector#지능형-모델-라우팅">
    ...
    <p>
      了解 Replit 如何选择模型，以及何时可以手动选择。
    </p>

    <div className="chat-card-link">打开参考文档 →</div>
  </a>

  <a className="chat-example has-visual" href="/zh/ko/chat/model-selector">
    ...
    <p>需要更精细的控制时，可自行选择基础模型并设置 Effort。</p>
    <div className="chat-card-link">继续 →</div>
  </a>
</section>

如果你需要比智能模型路由更精细的控制，请前往[选择模型](/zh/ko/chat/model-selector)。