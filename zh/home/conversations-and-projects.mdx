---
title: "Conversation 与 项目"
description: "了解何时应在 Conversation 中继续工作，以及何时应使用 项目 进行大型 Build 或 Design 工作。"
sidebarTitle: "Conversation 与 项目"
icon: "objects-column"
...
你已经调研了周边社区、连接了工具，并制定了发布计划。现在该让大家了解你的咖啡店了。为此，你将创建一个开业网站。

开始之前，先了解这项工作在 Replit 中涉及的两个位置会很有帮助：Conversation 和 项目。

<div id="from-plan-to-project">
  ## 从计划到项目
</div>

咖啡店的筹备始于一次 Conversation。你可以在其中打磨创意、研究市场、连接工具，并创建 Routine。

现在，你已准备好搭建开业网站。项目是开展这类重要设计和 Build 工作的场所。Replit 可以利用 Conversation 中已有的上下文，因此你无需从头开始。

<div id="what-is-a-conversation">
  ## 什么是 Conversation？
</div>

**Conversation** 是一个单独的对话线程，你可以在其中提问、探索信息、运行 Routine，并通过后续跟进继续推进工作。在本示例中，它包含咖啡店调研和开业计划。

<div id="what-is-a-project">
  ## 什么是项目？
</div>

**项目**是一个持久化的结构化空间，用于开展较大型的构建和设计工作。在本教程中，你将在这里创建并完善开场网站。
...
| 方面      | Conversation           | 项目                    |
| ------- | ---------------------- | -------------------------- |
| 适用场景    | 探索想法、研究竞争对手，以及进行每周市场观察 | 设计和构建上线网站                  |
| 结构      | 包含研究、决策和后续跟进的单一对话线程    | 用于网站、设计及其他构建产物的结构化空间       |
| Routine | 创建和管理定期研究              | 使用返回到 Conversation 的结果     |
| 上下文     | 保留引导你走到这一步的发布计划        | 以你在 Conversation 中确定的方向为起点 |

想提问或探索时，请从 Conversation 开始。准备好构建时，再转到 项目。
交接过程会保留你已确定的方向。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/chat/conversations">
    ...
    <div className="chat-example-tag">提问与探索</div>
    <div className="chat-card-title">Conversations</div>

    <p>
      了解 Conversations 如何支持提问、探索、Routine 以及产出成果。
    </p>
  ...
  <a className="chat-example has-visual" href="/zh/features/projects-and-artifacts/projects">
    ...
    <div className="chat-example-tag">构建与整理</div>
    <div className="chat-card-title">项目</div>

    <p>
      了解 项目 如何借助多个 构建产物 组织大型构建和设计工作。
    </p>
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/home/design-an-opening-site">
    <div className="chat-use-case-visual">
    ...
    <div className="chat-example-tag">开业网站</div>
    <div className="chat-card-title">设计咖啡馆开业网站</div>
    <p>使用 Conversation 和开业计划，为你的咖啡馆创建网站。</p>
