...
description: "Glossaire des termes de sécurité utilisés dans les outils de sécurité de Replit — CVE, sévérité, SBOM, exposition, Corriger avec Agent, Republier, Pare-feu de packages et Auto-Protect."
---

Définitions des termes de sécurité utilisés dans le [Centre de sécurité du projet](/fr/features/security/project-security-center) et le [Centre de sécurité de l'espace de travail](/fr/features/security/workspace-security-center).
...
Une protection au niveau réseau qui bloque les packages malveillants et compromis au moment de l'installation, avant qu'ils ne soient téléchargés dans votre application. Activé par défaut et alimenté par Socket. Voir [Pare-feu de packages](/fr/features/security/package-firewall).

## Auto-Protect

L'ensemble des protections que Replit applique automatiquement à votre application. Les protections activées par défaut pour chaque développeur comprennent le [Pare-feu de packages](/fr/features/security/package-firewall) pour le blocage au moment de l'installation, un pare-feu d'application web (WAF) et le chiffrement SSL/TLS automatique. Les administrateurs peuvent également opter pour la remédiation automatique des CVE, où Replit Agent prépare un correctif pour les vulnérabilités de dépendances nouvellement divulguées et notifie le propriétaire du projet. Voir [Auto-Protect](/fr/features/security/project-security-center#auto-protect).
