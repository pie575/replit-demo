...
description: "Utilisez le tableau des tâches pour examiner, lancer, suivre et appliquer les tâches Agent au fil des différentes étapes."
sidebarTitle: "Tableau des tâches"
icon: "table-columns"
---
...
Le tableau des tâches réunit en un seul endroit tout ce qu&#39;il faut pour examiner, lancer, suivre et appliquer les tâches d&#39;Agent. Utilisez-le lorsqu&#39;Agent découpe une demande volumineuse en plusieurs parties, lorsque vous souhaitez exécuter des travaux en arrière-plan ou lorsque vous avez besoin de savoir ce qui a déjà changé dans votre projet.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Tableau des tâches affichant les tâches organisées dans les colonnes Brouillons, Actives, Prêtes et Terminées" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

Les tâches sont organisées en colonnes selon leur statut. Le tableau indique ce qu&#39;Agent a planifié, ce qui est en cours d&#39;exécution, ce qui est prêt à être examiné et ce qui a déjà été appliqué.

Chaque tâche progresse de gauche à droite :

* **Brouillons** : tâches qu&#39;Agent a planifiées mais pas encore commencées
* **Actives** : tâches qu&#39;Agent est en train de construire, y compris celles mises en file d&#39;attente au-delà de votre limite de tâches actives
* **Prêtes** : tâches qu&#39;Agent a terminées et qui attendent votre examen
...
Les **brouillons** sont des tâches planifiées qu&#39;Agent n&#39;a pas encore commencé à développer. Chaque brouillon comprend un titre, une description et un plan étape par étape.

Utilisez les brouillons pour examiner l&#39;approche proposée par Agent avant que le travail ne commence. Vous pouvez consulter le plan, demander à Agent de l&#39;ajuster ou lancer la tâche lorsque vous êtes prêt.

Les brouillons sont utiles lorsque vous souhaitez planifier une fonctionnalité d&#39;envergure avant de lancer un travail en arrière-plan. Par exemple, Agent peut découper un projet d&#39;onboarding en plusieurs brouillons distincts : authentification, configuration du profil, interface du tableau de bord et finitions du deployment.

<div id="active">
  ## Active
</div>

Dès que vous acceptez une tâche, elle passe dans la colonne **Active** et Agent se met au travail. Chaque tâche s&#39;exécute dans une copie isolée de votre projet : rien ne change dans votre version principale tant que vous n&#39;appliquez pas le travail réalisé.

Certaines tâches dépendent d&#39;autres. Par exemple, une tâche qui crée un tableau de bord peut exiger que la tâche de schéma de base de données soit terminée au préalable. Les tâches dépendantes restent au statut **Queued** jusqu&#39;à ce que leurs prérequis soient terminés. Les tâches sont également mises en file d&#39;attente si vous atteignez la limite de tâches d&#39;arrière-plan actives de votre plan.

<KanbanColumns
  columns={[
{ title: 'Brouillons', count: 2, cards: [
{ title: 'Ajouter un parcours cadeau pour la famille et les amis', description: 'Permettre aux parents de créer une histoire pour l\'enfant de quelqu\'un d\'autre et de l\'envoyer en cadeau. Ouvre un nouveau canal d\'acquisition.', status: 'draft-static', timestamp: '2m' },
{ title: 'Mode lecture du soir avec thème sombre', description: 'Un mode coucher dédié, avec un thème sombre aux tons chauds et des sons d\'ambiance, transforme l\'application en un véritable rituel du soir.', status: 'draft-static', timestamp: '2m' },
] },
{ title: 'Active', count: 3, cards: [
{ title: 'Suites d\'histoires : épisodes suivants', description: 'L\'ajout d\'un bouton « Continuer l\'aventure » génère une suite qui reprend là où la précédente s\'était arrêtée.', status: 'active', timestamp: '12m' },
{ title: 'Régénération d\'histoire : refaire des pages', description: 'La possibilité de régénérer des pages individuelles donne du contrôle aux parents et réduit la frustration.', status: 'active', timestamp: '8m' },
{ title: 'Ajouter un parcours cadeau pour la famille et les amis', description: 'Permettre aux parents de créer une histoire pour l\'enfant de quelqu\'un d\'autre et de l\'envoyer en cadeau.', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
Lorsqu&#39;une tâche termine sa construction, elle passe à l&#39;état **Prête**. Cela signifie que le travail est terminé, mais que rien n&#39;a encore changé dans votre version principale.

Vous pouvez appliquer les changements de deux façons :

* **Depuis le fil** : ouvrez le fil de la tâche et sélectionnez **Appliquer les changements à la version principale** pour intégrer le travail à votre projet, ou **Ignorer** pour l&#39;abandonner
* **Depuis le tableau** : ouvrez le menu à trois points d&#39;une carte de tâche Prête, puis sélectionnez **Appliquer les modifications**

<TaskReviewDrawer />

Examinez les tâches Prêtes avant de les appliquer. Consultez le journal de travail, les résultats des tests et le Preview afin de bien comprendre ce qu&#39;Agent a modifié.

<div id="done">
  ## Terminé
</div>

Une fois les modifications appliquées, la carte de tâche passe dans la colonne **Terminé**.

La colonne Terminé contient également les sessions de planification archivées et les tâches annulées. Vous disposez ainsi d&#39;un historique de ce qui s&#39;est passé, sans que le travail achevé ne revienne se mêler aux colonnes Brouillons ou Actif.

<KanbanColumns
  columns={[
{ title: 'Prêt', count: 3, cards: [
{ title: 'Mode lecture du soir avec thème sombre', description: 'Un mode lecture du soir dédié, avec un thème sombre aux tons chauds et des sons d\'ambiance.', status: 'ready-drop', timestamp: '35 min', showMenu: true },
{ title: 'Liens d\'histoires partageables avec aperçus OG', description: 'L\'ajout de liens publics partageables est le principal levier de croissance.', status: 'ready-drop', timestamp: '31 min', showMenu: true },
{ title: 'Personnalisation avancée des personnages', description: 'Les options d\'apparence rendent les illustrations vraiment personnalisées.', status: 'ready-drop', timestamp: '28 min', showMenu: true },
] },
{ title: 'Terminé', count: 4, cards: [
{ title: 'Mode lecture à voix haute avec surlignage des mots', description: 'La synthèse vocale permet aux enfants de profiter des histoires en toute autonomie.', status: 'done', timestamp: '4 h' },
{ title: 'Liens d\'histoires partageables avec aperçus OG', description: 'Des liens publics partageables comme principal levier de croissance.', status: 'done', timestamp: '1 h' },
{ title: 'Personnalisation avancée des personnages', description: 'Des options d\'apparence pour des illustrations vraiment personnalisées.', status: 'done', timestamp: '4 h' },
{ title: 'Adaptation mobile et finitions pour la production', description: 'Meilleure gestion des erreurs, mises en page mobiles et animations de chargement.', status: 'done', timestamp: '4 h' },
] }
]}
/>

Agent gère automatiquement les conflits lorsque vous appliquez les modifications de plusieurs tâches. Si le résultat ne vous convient pas, demandez à Agent de le corriger.

<div id="find-and-filter-tasks">
  ## Rechercher et filtrer les tâches
</div>

Utilisez le tableau des tâches lorsque votre projet comporte de nombreuses tâches en cours. Vous pouvez effectuer une recherche par titre ou par numéro de tâche, filtrer les brouillons et utiliser le mode édition pour gérer plusieurs tâches à la fois.

Par exemple, recherchez par numéro de tâche lorsque quelqu&#39;un fait référence à une tâche depuis un fil de discussion, une revue ou une conversation Slack. Filtrez les brouillons pour vous concentrer sur le travail planifié sans avoir à parcourir les tâches actives ou terminées.

Pour les suivis suggérés, consultez [Tâches de suivi](/fr/features/agent/follow-up-tasks).

<div id="rename-a-task">
  ## Renommer une tâche
</div>

Les longues listes de tâches peuvent devenir difficiles à parcourir au fil de l&#39;évolution du périmètre. Pour renommer une tâche, ouvrez le menu à trois points de sa carte (dans la barre latérale de la liste des tâches ou sur le tableau des tâches) et sélectionnez **Renommer**. Saisissez un titre plus clair, puis sélectionnez **Enregistrer**.

Renommer une tâche modifie uniquement la façon dont elle s&#39;affiche pour vous et vos coéquipiers. Le travail déjà accompli par Agent, l&#39;historique de la tâche et les modifications en attente restent intacts.

<div id="settings-on-each-task">
  ## Paramètres de chaque tâche
</div>

Chaque tâche dispose de son propre menu déroulant de paramètres à côté de son titre. Ouvrez-le pour définir la façon dont Agent termine la tâche et dont le résultat s&#39;intègre à votre projet :

* **Appliquer les modifications automatiquement** (Brouillons et Actives) : dès que la tâche est prête, l&#39;appliquer immédiatement à la version principale au lieu d&#39;attendre une relecture.
* **Approuver le plan automatiquement** (Brouillons) : lorsque Agent propose un plan pour la tâche, l&#39;accepter automatiquement et lancer la construction.
* **Appliquer les modifications** (Prêtes) : appliquer le travail terminé à votre version principale.
* **Renommer** : modifier le titre affiché sur la carte.
* **Relire les modifications** (Prêtes) : accéder directement au diff d&#39;une tâche prête.
* **Annuler** : arrêter la tâche ou supprimer un brouillon.

L&#39;application et l&#39;approbation automatiques sont des actions ponctuelles, et non des options qui restent activées. Le menu conserve une largeur constante afin que les libellés ne se décalent pas d&#39;une tâche à l&#39;autre.

<Frame>
  <img src="/assets/mirror/task-settings-dropdown-7ea7868303b4.png" alt="Menu déroulant des paramètres ouvert sur une carte de tâche affichant les actions Appliquer les modifications, Renommer, Relire les modifications et Annuler dans une fenêtre contextuelle de largeur fixe" width="858" height="536" data-path="images/replitai/task-settings-dropdown.png" />
</Frame>