...
description: "Invitez des coéquipiers à rejoindre votre projet et créez ensemble : chacun peut démarrer de nouveaux fils de discussion avec Agent tout en travaillant sur le même projet."
---
...
<div id="construa-com-colegas-de-equipe">
  ## Créez avec vos coéquipiers
</div>

Replit permet à plusieurs personnes de travailler simultanément sur le même projet. Chacun peut lancer de nouveaux fils de discussion de planification avec Agent, et les tâches de tous s’affichent sur le tableau partagé : l’équipe peut ainsi développer ensemble plutôt que de travailler à tour de rôle.

<div id="comece-a-construir-com-colegas-de-equipe">
  ## Commencez à créer avec votre équipe
</div>
...
3. Vos collègues rejoignent le projet et lancent de nouveaux fils de discussion avec Agent
...
Chaque personne peut créer de nouveaux fils de discussion afin de travailler de manière indépendante, sans se gêner mutuellement. Les tâches de chacun apparaissent sur le tableau Kanban partagé, ce qui permet de voir facilement qui travaille sur quoi et d’éviter les conflits.

<Note>
  Avec Core, une seule tâche en arrière-plan peut être exécutée à la fois dans l’ensemble du projet. Avec Pro, jusqu’à 10 tâches peuvent être exécutées en parallèle : plusieurs personnes peuvent ainsi faire travailler Agent simultanément sur différentes parties du projet.
</Note>
...
Lorsque l&#39;équipe travaille ensemble, les tâches de chacun apparaissent sur un même tableau. Vous voyez en un coup d&#39;œil qui travaille sur quoi, ce qui est en cours et ce qui est prêt à être appliqué.
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Actif</strong> — Agent réalise cette tâche en ce moment, dans sa propre copie isolée du projet.</span>
  </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Terminé</strong> — Cette tâche est finalisée et ses modifications font désormais partie de la version principale.</span>
  </div>
</div>
...
Lorsque plusieurs personnes développent simultanément, les tâches de chacun apparaissent sur le tableau partagé. Voici un exemple : un collègue travaille sur les suites et la régénération d’histoires, tandis qu’un autre s’occupe du parcours cadeau, et les tâches terminées du reste de la Team se trouvent dans Ready et Done.

<KanbanColumns
  columns={[
{ title: 'Active', count: 3, cards: [
{ title: 'Continuations d’histoires — suites', description: 'L’ajout d’un bouton « Continuer l’aventure » génère une suite qui reprend là où la précédente s’est arrêtée.', status: 'active', timestamp: '12m' },
{ title: 'Régénération d’histoires — refaire des pages', description: 'La possibilité de régénérer des pages individuelles donne aux parents plus de contrôle et réduit la frustration.', status: 'active', timestamp: '8m' },
{ title: 'Ajouter un parcours cadeau pour la famille et les amis', description: 'Permettez aux parents de créer une histoire pour l’enfant d’une autre personne et de l’envoyer en cadeau.', status: 'active-queued', timestamp: '1m' },
] },
{ title: 'Ready', count: 2, cards: [
{ title: 'Mode de lecture au coucher avec thème sombre', description: 'Un mode dédié au coucher avec un thème sombre aux tons chauds et des sons d’ambiance.', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: 'Liens partageables vers des histoires avec aperçus OG', description: 'L’ajout de liens publics partageables est le principal levier de croissance.', status: 'ready-drop', timestamp: '31m', showMenu: true },
] },
...
{ title: 'Personnalisation améliorée des personnages', description: 'Des options d’apparence pour des illustrations réellement personnalisées.', status: 'done', timestamp: '4h' },
{ title: 'Optimisation mobile et finitions de production', description: 'De meilleurs états d’erreur, dispositions mobiles et animations de chargement.', status: 'done', timestamp: '4h' },
] }
]}
/>

Lorsqu’une tâche est déplacée vers Ready, n’importe quel membre de la Team peut examiner et appliquer les modifications. L’Agent gère automatiquement les conflits lorsque plusieurs tâches sont appliquées.
...
* **Intégrer un nouveau collègue** : il peut explorer le projet depuis son propre Chat avec Agent, poser des questions sur le fonctionnement du produit et commencer à contribuer, le tout sans bloquer personne.
* **Livrer plus vite avec Pro** : grâce à l&#39;exécution parallèle, deux ou trois personnes peuvent chacune lancer des tâches qui s&#39;exécutent simultanément. Un projet qui occuperait une session entière à une seule personne peut être bouclé en une fraction du temps.

<div id="perguntas-frequentes">
  ## Questions fréquentes
</div>

<Accordion title="Puis-je envoyer des messages à mon collègue via Agent ?">
  Non. Agent n&#39;est pas un outil de messagerie : il est en permanence à l&#39;écoute et tentera de donner suite à tout ce que vous saisissez. Utilisez Slack, Discord ou un autre outil pour la communication d&#39;équipe.
</Accordion>

<Accordion title="Risquons-nous d'écraser le travail les uns des autres ?">
  Le système de tâches permet d&#39;éviter cela en attribuant à chaque tâche sa propre copie isolée, tout en tenant compte des autres tâches existantes. Une fois les tâches terminées, vous les appliquez à la version principale et résolvez les éventuels conflits à cette étape.
</Accordion>

<Accordion title="Comment voir ce que font mes collègues ?">
  Consultez le tableau Kanban pour voir les tâches actives. Vous pouvez également voir la présence du curseur et les fichiers que vos collègues consultent.
</Accordion>
