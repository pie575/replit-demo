...
description: "Apprenez à utiliser la base de données SQL intégrée de Replit pour stocker et récupérer des données pour votre application."
---
...
* **Voyage dans le temps** : Restaurez votre base de données à n'importe quel point de contrôle **Agent**. Consultez [Récupération de données](/fr/features/data-and-storage/data-recovery)
...
<Info>
  ...
  Consultez [Développement et production](/fr/features/data-and-storage/development-and-production#base-de-données-de-développement-legacy) pour plus de détails.
</Info>
...
<Accordion title="Comment accéder à l'outil Database">
  ...
  2. La vue d'accueil liste vos bases de données. Sélectionnez-en une, comme **Development Database**, pour l'ouvrir.
</Accordion>

L'outil Database est organisé autour de votre liste de bases de données. La vue d'accueil affiche chaque base de données avec son utilisation de stockage, ainsi que votre **Billing Period** et vos **Hours of Compute Used** actuels. Ouvrir une base de données affiche trois onglets :
...
| **Capacité de restauration**       | Restaurez votre base de données à n'importe quel point de contrôle grâce à la [fonctionnalité de rollback](/fr/features/version-control/checkpoints-and-rollbacks/)                               | Restauration à un point dans le temps (PITR) — récupérez à n'importe quel point dans la fenêtre de rétention des sauvegardes.                                                          |
...
| **Comportement de remix**          | Le [remix](/fr/features/projects-and-artifacts/remix-an-app) crée une nouvelle base de données de développement avec des données copiées pour l'isolation et la sécurité.                         | Le remix réutilisait la même base de données pour toutes les copies, donc les modifications dans les remixes pouvaient affecter l'application d'origine.                               |
...
| **Flux de publication**            | Le déploiement ou la publication nécessite la création d'une [base de données de production](/fr/features/data-and-storage/development-and-production) pour l'isolation entre les environnements. | Le développement et la production partageaient la même base de données, donc des modifications de développement non intentionnelles pouvaient affecter les applications en production. |

<Warning>
  Si votre application a été forkée ou remixée lors de l'utilisation de la base de données Neon legacy, votre application publiée peut encore référencer l'URL de la base de données de l'application d'origine. Après la [mise à niveau de la base de données](/fr/features/data-and-storage/development-and-production), cette connexion partagée cessera de fonctionner. Consultez [Corriger une application publiée utilisant une base de données partagée](/fr/features/data-and-storage/shared-database-migration) pour savoir comment mettre à jour votre application publiée.
</Warning>
