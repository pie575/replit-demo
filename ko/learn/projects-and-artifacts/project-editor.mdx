...
description: "Project Editor가 무엇인지 알아보세요. Agent와 대화하고, 앱이 완성되어 가는 모습을 확인하고, 모든 것을 관리할 수 있는 Replit의 작업 거점입니다."
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="만들고 싶은 것을 설명하는 prompt 입력란이 있는 Replit Project Editor 홈 화면" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>
...
Project Editor는 Replit에서의 활동 거점입니다. 개발을 시작하고, project를 관리하고, app이 실제로 구현되는 모습을 확인하는 환경입니다. 여기에서 만들고 싶은 것을 설명하면 [Agent](/ko/features/agent/overview)가 나머지를 처리하여 코드를 작성하고, project를 설정하고, 실시간 Preview를 보여줍니다.

<div id="getting-started">
  ## 시작하기
</div>

만들고 싶은 것을 prompt box에 입력하세요. &quot;내 지출을 차트로 보여주는 예산 관리 앱&quot;, &quot;우리 커피숍 landing page&quot;, &quot;아이들을 위한 AI 만화책 생성기&quot;처럼 일상적인 표현으로 아이디어를 설명하면 나머지는 Agent가 알아서 처리합니다.

<AgentInput />

캐러셀에서 project 유형을 선택해 어떤 결과물을 원하는지 Agent에게 알려줄 수도 있습니다. web app, mobile app, slides, design 등 다양한 유형이 준비되어 있습니다. 선택을 건너뛰고 아이디어만 설명해도 됩니다. Agent가 알맞은 설정을 자동으로 판단합니다.

<AgentCarousel />

바로 시작하려면 [첫 번째 앱 만들기](/ko/build/your-first-app)를 따라 해보세요.

<div id="plan-mode">
  ## Plan mode
</div>

Plan mode에서는 아이디어를 구상하고, 질문하고, project의 방향을 설계할 수 있습니다. Agent가 순서대로 정리된 task list를 만들어 주므로 이를 검토하고 다듬을 수 있습니다. 계획이 마음에 들면 승인하세요. 그러면 Agent가 building을 시작합니다. Plan mode에서 질문을 하거나 아이디어를 탐색만 할 수도 있으며, 모드를 종료하기 전까지 Agent는 어떤 changes도 적용하지 않습니다.

<AgentInput highlightPlan={true} showTypewriter={false} />

입력창 하단의 버튼으로 Plan mode를 켜고 끌 수 있습니다. 자세한 내용은 [Plan mode 가이드](/ko/features/agent/plan-mode)에서 확인하세요.

<div id="what-you-can-build">
  ## 무엇을 만들 수 있나요
</div>

project 유형을 선택해 Agent에게 원하는 것을 알려주거나, 아이디어를 설명하고 Agent가 알맞은 구성을 고르도록 하세요.

* **웹 앱** — dashboard, 내부 도구, SaaS 제품, landing page
* **모바일 앱** — 하나의 prompt로 만드는 iOS 및 Android 앱
* **슬라이드와 프레젠테이션** — 풍부한 layout의 deck 생성
* **애니메이션** — 인터랙티브 워크스루와 motion graphics
* **디자인** — [Design Canvas](/ko/design/canvas)에서 만드는 비주얼 mockup과 프로토타입
* **데이터 시각화** — chart, dashboard, database 기반 도구
* **문서** — CSV, PDF, PowerPoint 파일, Markdown 문서
* **스프레드시트** — 데이터 표와 구조화된 결과물

<div id="the-project-editor">
  ## Project Editor
</div>

Agent가 빌드를 시작하면 Project Editor에서 프로젝트가 완성되어 가는 과정을 확인할 수 있습니다. 화면은 패널로 나뉘어 있어 한쪽에는 Agent와의 대화가, 다른 쪽에는 앱의 실시간 미리보기가 표시됩니다.

<div id="threads">
  ### Threads
</div>

Agent와의 모든 대화는 thread에서 이루어집니다. main thread는 원하는 바를 설명하고 Agent가 이를 만들어 가는 과정을 지켜보는 공간입니다. [task system](/ko/core-concepts/agent/task-system)을 사용하면 각 백그라운드 task마다 별도의 thread가 생성되므로, Agent가 대화를 뒤섞지 않고 task 작업을 분리된 상태로 유지할 수 있습니다.

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="사용자의 prompt와 Agent가 project를 만드는 과정을 보여주는 thread" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>
...
Task board는 작업을 **Drafts**, **Active**, **Ready**, **Done** 열로 정리합니다. 각 task는 진행 상황에 따라 왼쪽에서 오른쪽으로 이동하므로, 예정된 작업, Agent가 진행 중인 작업, 완료된 작업을 한눈에 파악할 수 있습니다. 자세한 내용은 [Task system](/ko/core-concepts/agent/task-system)을 참고하세요.

<Frame>
  <img src="/assets/mirror/task-board-active-355ecd390c49.png" alt="Drafts, Active, Ready, Done 열로 정리된 task를 보여주는 task board" width="3430" height="1986" data-path="images/replitai/task-board-active.png" />
</Frame>
...
Design Canvas는 코드로 옮기기 전에 목업을 만들고 다듬을 수 있는 시각적 편집 환경입니다. 요소를 끌어다 놓고 레이아웃을 조정하며 디자인을 다듬은 뒤, Agent가 이를 실제로 작동하는 App으로 만들도록 하세요. 자세한 내용은 [Design Canvas](/ko/design/canvas)에서 확인하세요.

<div id="collaboration">
  ## 협업
</div>

Team Workspace를 만들어 함께 빌드하세요. 팀원을 초대하고 프로젝트를 공유하며 같은 앱을 실시간으로 함께 작업할 수 있습니다. 팀 설정은 [Team workspaces](/ko/features/collaboration/team-workspaces), 프로젝트 단위 협업은 [팀원 초대하기](/ko/build/invite-teammates)를 참고하세요.

<div id="import-code-and-design">
  ## 코드와 디자인 가져오기
</div>

이미 만들어 둔 project가 있나요? GitHub, Figma에서 가져오거나 파일을 직접 업로드해 Replit으로 옮겨보세요. Agent가 중단된 지점부터 이어받아 기존 코드나 디자인을 바탕으로 계속 빌드할 수 있습니다. 자세한 내용은 [Replit으로 가져오기](/ko/build/import-from-providers)를 참고하세요.
...
<CardGroup cols={2}>
  <Card title="에디터 및 도구" href="/ko/features/editor/editor-and-tools" icon="window-restore">
    앱의 Preview, Console 출력을 비롯한 여러 도구를 확인할 수 있는 주요 패널
  </Card>

  <Card title="앱 설정" href="/ko/features/project-setup/configuration" icon="gear">
    앱의 Settings, 종속성, Secrets, 환경을 구성하세요
  </Card>

  <Card title="버전 관리" href="/ko/learn/projects-and-artifacts/version-control" icon="clock-rotate-left">
    변경 사항을 추적하고 이전 버전으로 되돌리며 앱의 변경 이력을 관리하세요
  </Card>

  <Card title="Publishing" href="/ko/learn/projects-and-artifacts/replit-deployments" icon="rocket">
    앱을 실제 URL에 배포해 누구나 사용할 수 있도록 만드세요
  </Card>
</CardGroup>
...
<Accordion title="코딩을 할 줄 알아야 하나요?">
  아니요. 원하는 내용을 평소 쓰는 말로 설명하면 Agent가 기술적인 세부 사항을
  처리합니다. 물론 직접 수정할 수도 있습니다.
</Accordion>

<Accordion title="브라우저를 닫으면 어떻게 되나요?">
  project는 클라우드에 저장됩니다. 언제든 다시 돌아와 하던 작업을 그대로 이어갈
  수 있으며, 파일과 기록, 실행 중인 앱이 모두 그대로 유지됩니다.
</Accordion>

<Accordion title="다른 사람과 함께 작업할 수 있나요?">
  네. 특정 project에 협업자를 초대하거나, 구성원이 모든 project에 접근할 수 있는
  Team Workspace를 만들 수 있습니다. [팀
  workspace](/ko/features/collaboration/team-workspaces)와 [팀원
  초대](/ko/build/invite-teammates)를 참고하세요.
</Accordion>

<Accordion title="앱은 어떻게 게시하나요?">
  앱이 준비되면 Project Editor에서 바로 라이브 URL로 게시할 수 있습니다.
  [Deployments](/ko/learn/projects-and-artifacts/replit-deployments)를 참고하세요.
</Accordion>
...
Project Editor는 모든 plan에서 사용할 수 있습니다. 단, 컴퓨팅 성능 확장이나 상시 실행 deployment 같은 일부 기능은 paid plan에서만 이용할 수 있습니다. 자세한 내용은 [Billing](/ko/billing/ai-billing)을 참조하세요.