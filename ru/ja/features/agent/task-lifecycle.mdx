...
description: "Узнайте о статусах задач, работе в очереди, применении изменений, архивации и отмене задач Agent."
---
...
Каждая задача Agent проходит жизненный цикл: запланирована, выполняется, ожидает проверки, завершена. Благодаря этому работа остаётся изолированной до тех пор, пока вы не решите, что именно включить в основную версию.
...
**Черновики** — это задачи, которые Agent запланировал, но ещё не начал выполнять. Они содержат только предлагаемую работу — название задачи, описание и план, — поэтому их можно безопасно просмотреть.

Используйте черновики, чтобы убедиться, что Agent правильно понял вашу цель. Вы можете начать задачу, попросить Agent изменить план или архивировать черновик на будущее.

<div id="section-2">
  ## Активные
</div>

**Активные** задачи выполняются в фоновом режиме. Agent работает с отдельной копией проекта, поэтому основная версия не изменяется, пока выполняется задача.

Вы можете продолжать работать в основном потоке, пока выполняется активная задача. Если задаче понадобится обратная связь, откройте её поток и продолжите общение.

<div id="section-3">
  ## В очереди
</div>

Задачи **в очереди** одобрены, но ожидают запуска. Задача может попасть в очередь, если она зависит от другой задачи или достигнут лимит активных фоновых задач, предусмотренный вашим тарифным планом.

Задачи в очереди запускаются автоматически, когда выполнены все предварительные условия и освобождается слот для активной задачи.

<div id="section-4">
  ## Готово
</div>

Задача **Готово** выполнена, но изменения ещё не применены к основной версии. Перед применением изменений проверьте журнал выполнения задачи, результаты тестов и предварительный просмотр.

Если всё выполнено правильно, примените изменения к основной версии. Если результат не соответствует ожиданиям, отклоните задачу и начните новую.

<div id="section-5">
  ## Применение
</div>

Задачи в состоянии **Применение** находятся в процессе переноса изменений в основную версию. Конфликты при применении изменений из нескольких задач Agent разрешает автоматически.

Дождитесь завершения применения, прежде чем оценивать итоговый результат. Если после этого возникнут проблемы, попросите Agent исправить их в основном потоке.

<div id="section-6">
  ## Завершено
</div>

Задачи в разделе **Завершено** закрыты, и никаких действий от вас не требуется. Сюда попадают задачи, изменения которых применены к основной версии, задачи, архивированные до начала работы, а также отменённые после перехода к сборке.

Используйте столбец **Завершено** как историю работы по завершённым, архивированным и отменённым задачам.

<div id="section-7">
  ## Примеры состояний
</div>

Каждая задача проходит ряд этапов — от идеи до готовой части приложения. У каждого этапа свой значок, так что происходящее понятно с первого взгляда.

<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Добавить возможность дарить истории семье и друзьям" description="Родители смогут создавать истории для других детей и отправлять их в подарок." status="draft-static" timestamp="2m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Черновик</strong>: Agent предложил эту задачу. План готов к проверке.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Продолжение истории: сиквел" description="Добавьте кнопку «Продолжить приключение», и будет создан сиквел, продолжающий предыдущую часть." status="active" timestamp="12m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Активно</strong>: Agent сейчас работает над этой задачей. Работа выполняется в отдельной копии проекта, поэтому основная версия остаётся в безопасности.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Добавить возможность дарить истории родным и друзьям" description="Позволяет родителям создавать истории для других детей и отправлять их в подарок." status="active-queued" timestamp="1m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>В очереди</strong>: Эта задача одобрена, но ожидает свободного слота для начала сборки.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Ночной режим с тёмной темой" description="Отдельный ночной режим с тёплой тёмной темой и фоновыми звуками." status="ready-drop" timestamp="35m" width="100%" />
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Ссылка для общего доступа с предпросмотром OG" description="Добавление публичной ссылки для общего доступа — самый мощный драйвер роста." status="applying" timestamp="31m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Применяется</strong>: изменения из этой задачи сейчас применяются к основной версии.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Чтение вслух с подсветкой слов" description="Функция преобразования текста в речь позволяет детям самостоятельно читать истории, следя за подсвеченными словами." status="done" timestamp="4h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Завершено</strong>: задача закрыта, а изменения вошли в основную версию.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Онбординг для новых читателей" description="Краткое пошаговое руководство, знакомящее родителей с основным процессом чтения при первом запуске." status="draft-static" timestamp="1h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>В архиве</strong>: сеанс планирования, отложенный до начала разработки. Позже его можно восстановить из столбца <strong>Завершено</strong>.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Экспериментальный генератор наборов стикеров" description="Создаёт уникальные иллюстрированные стикеры на основе персонажей или сцен из истории." status="done" timestamp="2h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Отменено</strong>: задача была остановлена после начала сборки. Отменённые задачи нельзя восстановить. Если вы передумали, начните новую задачу.</span>
  </div>
</div>
...
Задачу можно остановить на любом этапе её жизненного цикла, но доступные варианты зависят от её положения на доске. И **архивирование**, и **отмена** перемещают задачу в столбец **Завершено**, но работают по-разному:

| Действие         | Когда доступно                                                                                                     | Что происходит                                                                                             | Можно ли отменить?                                                                                |
| ---------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **Архивировать** | Пока задача находится в сеансе планирования: это **черновик**, для которого ещё не началась сборка                 | Сеанс планирования приостанавливается без выполнения. План сохраняется.                                    | Да. Откройте архивированную карточку в столбце **Завершено** и восстановите её как **Черновик**.  |
| **Отменить**     | После перехода задачи от планирования к сборке: для **активных**, **поставленных в очередь** или задач со статусом **Готово** | Agent останавливает задачу и отбрасывает незавершённую работу. Изменения не применяются к основной версии. | Нет. Отменённые задачи нельзя восстановить. Если хотите возобновить работу, начните новую задачу. |

Используйте **архивирование** для сеансов планирования, к которым хотите вернуться позже. Используйте **отмену** для окончательной остановки задач в очереди, выполняющихся или с завершённой сборкой.

Архивированные сеансы планирования и отменённые задачи можно найти вместе с завершёнными задачами в столбце **Завершено** в правой части доски задач.