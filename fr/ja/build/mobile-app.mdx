---
title: "Créer des applications mobiles"
description: "Utilisez Agent pour créer des applications optimisées pour les smartphones, les tester sur des simulateurs et des appareils réels, puis les partager afin de recueillir des retours."
---
...
Les applications mobiles doivent être naturelles à utiliser sur smartphone : elles doivent s’ouvrir rapidement, être faciles à utiliser au toucher et confortables d’une seule main.

Créez **Pace Mobile**, un tracker de course pensé avant tout pour les smartphones, inspiré de l’exemple de tracker de course présenté dans [Qu’est-ce que Replit ?](/fr/ja/build/welcome). L’objectif est de tester un parcours mobile ciblé avant de vous lancer dans le processus complet de publication sur l’App Store.

<Frame>
  <img src="/assets/mirror/mobile-app-4203ff45ecd5.png" alt="Exemple d’une application mobile soignée avec un écran de suivi de l’activité physique conçu pour une utilisation sur smartphone" width="1440" height="850" data-path="images/build-examples/mobile-app.png" />
</Frame>
...
* Une première version centrée sur un seul parcours adapté aux smartphones.
* Une application fonctionnant dans un simulateur ou un émulateur.
* Une application fonctionnant sur votre smartphone avec Expo Go.
* Une version partageable à envoyer à quelqu’un pour recueillir ses commentaires.
...
* définir le périmètre de la première version en tenant compte des contraintes des smartphones.
* tester votre application dans un simulateur ou un émulateur.
* tester votre application sur un appareil réel avec Expo Go.
* déterminer quand poursuivre les itérations avant d’entamer les démarches pour l’App Store ou TestFlight.

<div id="section-3">
  ## Public concerné
</div>

Utilisez ce guide si votre idée est destinée à être utilisée sur smartphone.

Il est utile pour les outils de suivi, les parcours de réservation, les outils de terrain, les applications événementielles, les outils d’apprentissage, les applications de suivi d’habitudes et tout ce que vous utilisez loin de votre ordinateur.

<Note>
  La publication sur TestFlight et l’App Store est distincte de ce guide. Lorsque vous serez prêt à passer à cette étape, consultez [Créer et publier une application mobile](/fr/ja/build/mobile-app).
</Note>
...
* Un smartphone sur lequel [Expo Go](https://expo.dev/go) est installé.
* Un flux mobile à tester en quelques minutes.

Pour connaître les détails concernant les simulateurs, les émulateurs et l’aperçu sur appareil, consultez [Native Mobile Apps](/fr/ja/features/artifact-types/building-mobile-apps).

<div id="section-5">
  ## Définir le flux mobile
</div>

Avant de commencer à développer, déterminez ce que les utilisateurs feront sur un smartphone.

Pour Pace Mobile :

* **Public cible** : les coureurs qui souhaitent enregistrer rapidement une course.
* **Flux principal** : sélectionner le type de course → saisir la distance et la durée → enregistrer la course → consulter le récapitulatif du jour.
* **Contraintes mobiles** : les principales actions doivent être facilement accessibles d’une seule main, avec le pouce.
* **Première version** : pas de compte, pas de fil social, pas d’analyses avancées.

Cela permet de garder la première tranche mobile suffisamment réduite pour être testée.

<div id="section-6">
  ## Créer la première version
</div>

Ouvrez Replit et créez une nouvelle App. Décrivez ce que vous souhaitez créer, puis sélectionnez **Mobile app** comme type d&#39;application.

<Frame>
  <img src="/assets/mirror/prompt-5cb144699b45.png" alt="Prompt de la page d'accueil de Replit avec Mobile app sélectionné comme type d'application" width="1440" height="900" data-path="images/native-mobile-apps/prompt.png" />
</Frame>

Utilisez ce prompt :

<AiPrompt>
  Créez Pace Mobile, une application de suivi de course à pied conçue pour les téléphones.<br />
  ...
  Concevez l&#39;application pour une utilisation à une main, avec de grandes zones tactiles, une navigation simple et une interface soignée digne d&#39;une application de fitness.<br />
  Gardez la première version suffisamment simple pour pouvoir la tester dès aujourd&#39;hui dans un simulateur et sur un téléphone réel.
</AiPrompt>

Si vous souhaitez qu&#39;Agent réfléchisse avant de commencer le Build, utilisez le [mode Plan](/fr/ja/features/agent/plan-mode). Avant de lancer le Build, demandez à Agent d&#39;examiner les écrans, la navigation, les zones tactiles et les contraintes propres aux smartphones.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Composeur de prompts Replit avec le mode Plan sélectionné avant qu'Agent définisse le périmètre du Build d'une application mobile" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Une fois qu’Agent a terminé le premier build, ouvrez [Preview](/fr/ja/features/editor/preview). Dans le sélecteur d’appareil, choisissez **iOS Simulator** ou **Android Emulator**.

Essayez d’utiliser l’application comme le ferait une personne sur son smartphone.

<Frame>
  <img src="/assets/mirror/workspace-dda0f81e6b83.png" alt="Replit Project Editor affichant Preview d’une application mobile et l’option de prévisualisation sur un appareil mobile" width="1440" height="900" data-path="images/native-mobile-apps/workspace.png" />
</Frame>

Vérifiez les points suivants :

* L’application démarre sans erreur.
* Vous pouvez sélectionner le type de course.
* Vous pouvez saisir la distance et la durée.
* Vous pouvez enregistrer une course.
* Le résumé du jour est mis à jour.
* Vous pouvez naviguer sans rencontrer de blocage.
...
Les tests sur simulateur sont utiles, mais seul un appareil réel permet de vérifier si votre application est vraiment facile à utiliser.

Ouvrez le panneau Preview sur appareil mobile et scannez le code QR avec votre smartphone. Votre application s’ouvrira alors dans Expo Go.

<Frame>
  <img src="/assets/mirror/mobile-screen-e5685fc26793.png" alt="Application mobile exécutée dans le Preview du Replit Project Editor. Le panneau Preview et un code QR pour Expo Go sont affichés sur un appareil mobile." width="1440" height="900" data-path="images/native-mobile-apps/mobile-screen.png" />
</Frame>

Vérifiez sur votre smartphone :

* Pouvez-vous effectuer le flux principal d’une seule main ?
* Les boutons et champs de saisie sont-ils suffisamment grands pour être facilement touchés ?
* Les principales actions sont-elles faciles à atteindre ?
* Le clavier masque-t-il des champs importants ?
* L’application reste-t-elle facile à utiliser sur un petit écran ?

Si un élément est difficile à utiliser, demandez à Agent de corriger cette partie précise plutôt que de repenser toute l’application.

<AiPrompt>
  Améliorez Pace Mobile pour une utilisation du téléphone à une main.<br />
  Rendez le bouton Save Run plus facile à atteindre, augmentez les zones tactiles si nécessaire et gardez le formulaire de course simple.<br />
...
Une fois le parcours sur smartphone opérationnel, publiez l’application afin de créer une version Expo Go partageable pour recueillir des retours précoces.

Partagez-la en proposant une tâche précise :

> Ouvrez Pace Mobile sur votre smartphone, enregistrez une course et indiquez-nous ce qui vous a semblé difficile à toucher, à lire ou à comprendre.

Les retours ciblés sont plus exploitables que les avis généraux. Lorsqu’un retour signale un problème précis, demandez à Agent d’apporter une correction minimale, puis testez à nouveau l’application sur smartphone.

<div id="app-store">
  ## Publier sur l’App Store
</div>

Après avoir testé Pace Mobile sur un simulateur et un appareil réel avec Expo Go, vous pouvez promouvoir cette même app sur TestFlight et l’App Store. Ce processus nécessite d’être membre de l’**Apple Developer Program** et d’utiliser l’authentification à deux facteurs. Pour en savoir plus sur l’inscription, les comptes individuels et d’organisation, les processus de révision de TestFlight et de l’App Store, ainsi que les tests externes, consultez [Créer et publier une application mobile](/fr/ja/build/mobile-app).

<Steps>
  <Step title="Ouvrez l’onglet Publishing pour commencer à publier sur l’App Store">
    Une fois l’aperçu Expo Go publié, ouvrez l’onglet **Publishing**. Sous **Production**, la carte **Ready to publish Pace to the App Store?** s’affiche. Sélectionnez **Start publishing to the App Store**.

    <Frame>
      <img src="/assets/mirror/app-store-02-start-publishing-190eb7841bcf.png" alt="Onglet Publishing du Project Editor affichant le statut Production, la carte « Ready to publish Pace to the App Store? » et le bouton « Start publishing to the App Store »" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-02-start-publishing.png" />
    </Frame>
  </Step>

  <Step title="Sélectionner un projet Expo">
    Replit ouvre **Launch**, le flux Expo intégré à Replit. Saisissez ou vérifiez le nom et le slug du projet Expo, puis sélectionnez **Continue**. Si vous disposez déjà d’un projet Expo, sélectionnez plutôt **Use existing**.

    <Frame>
      <img src="/assets/mirror/app-store-03-choose-expo-project-4707fafffafe.png" alt="Boîte de dialogue Launch avec l’étape Choose Expo project active : Pace est prérempli dans les champs Project name et Project slug, et le bouton Continue ainsi que l’option Use existing sont visibles" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-03-choose-expo-project.png" />
    </Frame>
  </Step>

  <Step title="Se connecter avec Apple">
    Connectez le compte Apple Developer propriétaire de la fiche App Store. Ce compte doit être inscrit à l’Apple Developer Program. Apple applique l’authentification à deux facteurs : préparez-vous à valider un code de vérification sur un appareil de confiance.

    <Frame>
      <img src="/assets/mirror/app-store-04-sign-in-apple-459dfc9b84ba.png" alt="Boîte de dialogue Launch de l’étape Sign in with Apple — champs d’adresse e-mail ou de numéro de téléphone Apple Developer et de mot de passe. Une note indique que le mot de passe ne quitte jamais l’appareil et que le compte doit être inscrit à l’Apple Developer Program" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-04-sign-in-apple.png" />
    </Frame>
  </Step>

  <Step title="Sélectionner ou créer une app Apple">
    Saisissez le **nom de l’app** et le **Bundle ID** (par exemple : `app.replit.pace`) qui s’afficheront dans l’App Store. L’identifiant de bundle ne peut plus être modifié après la première compilation ; choisissez-le donc avec soin.

    <Frame>
      <img src="/assets/mirror/app-store-05-choose-apple-app-3971aea62f14.png" alt="Boîte de dialogue Launch de l’étape Choisir l’app Apple — le nom de l’app est défini sur Pace, le Bundle ID sur app.replit.pace, avec une note indiquant que l’identifiant de bundle ne peut pas être modifié ultérieurement et le bouton Création de l’app…" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-05-choose-apple-app.png" />
    </Frame>
  </Step>

  <Step title="Publier sur l’App Store">
    Replit configure les certificats Apple pour vous. Lorsque l’étape **Ready for launch** est activée, sélectionnez **Launch to App Store**. Replit crée l’application native dans le cloud et envoie la build à App Store Connect pour l’examen bêta dans TestFlight.

    <Frame>
      <img src="/assets/mirror/app-store-06-ready-for-launch-ab02659ee0bb.png" alt="Boîte de dialogue Launch montrant Choose Apple app et Configure Apple certificates comme terminés, l’étape Ready for launch, le bouton Launch to App Store et des paramètres Launch facultatifs" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-06-ready-for-launch.png" />
    </Frame>

    Une fois le lancement terminé, une confirmation indiquant que l’application est destinée à l’App Store s’affiche. La boîte de dialogue se ferme ensuite et vous revenez au Project Editor.

    <Frame>
      <img src="/assets/mirror/app-store-targeting-f8797071c9e0.png" alt="Boîte de dialogue Launch montrant Targeting App Store comme terminé et le message « Redirecting back to Replit » — état de confirmation après avoir sélectionné Launch to App Store" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-targeting.png" />
    </Frame>
  </Step>
</Steps>

<Note>
  Le premier build d’une version nécessite généralement un examen bêta par TestFlight avant que les testeurs puissent l’installer. Une fois approuvés, les builds suivants sont généralement disponibles plus rapidement pour les testeurs. Pour les partager avec des testeurs externes ou les soumettre en vue d’une publication sur l’App Store, complétez la fiche dans **App Store Connect** — consultez [Créer et publier des applications mobiles](/fr/ja/build/mobile-app) pour la checklist complète d’App Store Connect.
</Note>
...
* L’App s’ouvre dans iOS Simulator ou Android Emulator.
* L’App s’ouvre sur un smartphone avec Expo Go.
* Vous pouvez effectuer les principaux flux mobiles.
* Le texte, les boutons et la navigation sont faciles à utiliser sur smartphone.
* Vous avez publié ou partagé une version mobile testable.
* Au moins une personne peut essayer l’App et fournir des commentaires.
* (Facultatif) Vous avez envoyé l’App à TestFlight via le processus de l’App Store.
...
<CardGroup cols={2}>
  <Card title="Ajouter une connexion" icon="user-lock" href="/fr/ja/build/add-login">
    Permettez à chaque utilisateur de consulter sa progression.
  </Card>

  <Card title="Ajouter une base de données" icon="database" href="/fr/ja/build/add-database">
    Enregistrez l&#39;historique des courses au-delà de la première session de test.
  </Card>

  <Card title="Ajouter des intégrations" icon="plug" href="/fr/ja/build/add-integrations">
    Connectez votre application aux notifications, aux données de santé et aux services externes.
  </Card>

  <Card title="Créer et publier une application mobile" icon="mobile-screen" href="/fr/ja/build/mobile-app">
    Préparez votre application mobile fonctionnelle pour TestFlight et l&#39;App Store.
  </Card>
</CardGroup>
...
* **Agent crée une application web :** recommencez et sélectionnez **Mobile app** comme type d’application avant de lancer le build.
* **L’aperçu Expo Go semble obsolète :** secouez votre téléphone, ouvrez le menu développeur et sélectionnez **Refresh**.
* **Les cibles tactiles sont trop petites :** demandez à Agent d’agrandir les zones tactiles et de garder les actions principales à portée du pouce.
* **La navigation est encombrée :** demandez à Agent de limiter l’application aux écrans essentiels au parcours principal.
* **Le simulateur fonctionne, mais pas sur un appareil réel :** testez les comportements spécifiques au matériel sur un appareil réel et consultez [Native Mobile Apps](/fr/ja/features/artifact-types/building-mobile-apps).
* **La publication échoue :** consultez le [dépannage des applications mobiles](/fr/ja/features/troubleshooting/mobile-app).

<div id="section-13">
  ## Essayez dès maintenant
</div>

<AgentInput highlightStart defaultPrompt="Créez Pace Mobile, une application de suivi de course à pied pensée d’abord pour les téléphones. Les coureurs doivent pouvoir choisir un type de course, saisir la distance et le temps, enregistrer leur course et consulter le résumé du jour. Concevez-la pour une utilisation à une main, avec de grandes zones tactiles, une navigation simple et l’aspect soigné d’une application de fitness. Gardez la première version suffisamment simple pour la tester aujourd’hui dans un simulateur et sur un vrai téléphone." showTypewriter={false} />

<div id="section-14">
  ## Articles associés
</div>

* [Applications mobiles natives](/fr/ja/features/artifact-types/building-mobile-apps)
* [Créer et publier une application mobile](/fr/ja/build/mobile-app)
* [Introduction au vibe coding](/fr/ja/learn/foundations/vibe-coding-101)
* [Dépannage des applications mobiles](/fr/ja/features/troubleshooting/mobile-app)