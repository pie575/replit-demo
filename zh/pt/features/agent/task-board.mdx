---
title: "任务面板"
description: "使用任务面板审阅、启动、监控并应用 Agent 任务，随时掌握任务在各个阶段的进展。"
---
...
任务面板将 Agent 任务的查看、启动、监控和应用集中在一处。当 Agent 把一个较大的请求拆分成多个部分时、当你希望在后台执行工作时，或者当你需要了解项目中已经发生了哪些更改时，都可以使用它。

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="任务面板中的任务按草稿、进行中、就绪和已完成列进行组织" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

任务按状态分列排列。面板会显示 Agent 规划了哪些内容、哪些正在执行、哪些已可供查看，以及哪些已经应用。

每个任务会随着进展从左向右移动：

* **草稿**：Agent 已规划但尚未开始的任务
* **进行中**：Agent 正在构建的任务，包括因超出活动任务上限而排队等待的任务
* **就绪**：Agent 已完成、等待你查看的任务
* **已完成**：已完结、已归档或已取消的任务
...
**草稿**是 Agent 已规划但尚未开始构建的任务。每个草稿都包含标题、描述和分步计划。

在动手之前，可以借助草稿查看 Agent 提出的实现方案。你可以检查计划、让 Agent 进行调整，或在准备就绪时启动该任务。

如果你想在投入后台工作之前先规划一项较大的功能，草稿会很有用。例如，Agent 可以把一个集成项目拆分成多个独立草稿，分别对应身份验证、个人资料设置、仪表板 UI 和部署优化。

<div id="ativo">
  ## 进行中
</div>

接受任务后，它会移入 **进行中** 一栏，Agent 随即开始构建。每个任务都在项目的独立副本中运行，因此在你应用这些改动之前，主版本不会有任何变化。

有些任务依赖于其他任务。例如，构建仪表板的任务可能需要先完成数据库架构任务。存在依赖关系的任务会一直处于 **排队中** 状态，直到其前置任务完成。如果你达到了所属方案的活跃后台任务上限，任务同样会进入队列。

<KanbanColumns
  columns={[
{ title: '草稿', count: 2, cards: [
{ title: '为家人和朋友添加赠送流程', description: '让家长为他人的孩子创建故事并作为礼物赠送，开辟新的获客渠道。', status: 'draft-static', timestamp: '2m' },
{ title: '搭配深色主题的睡前阅读模式', description: '专属睡前模式配上暖色调深色主题和环境音效，让应用成为完整的睡前仪式。', status: 'draft-static', timestamp: '2m' },
] },
{ title: '进行中', count: 3, cards: [
{ title: '故事续写：续集', description: '添加“继续冒险”按钮，生成承接上一部结尾的续集。', status: 'active', timestamp: '12m' },
{ title: '故事重新生成：重做页面', description: '增加重新生成单页的功能，让家长掌握主动权，减少挫败感。', status: 'active', timestamp: '8m' },
{ title: '为家人和朋友添加赠送流程', description: '让家长为他人的孩子创建故事并作为礼物赠送。', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
任务构建完成后，会进入**就绪**状态。这表示工作已完成，但主版本尚未发生任何改动。

你可以通过两种方式应用更改：

* **通过会话线程**：打开该任务的会话线程，选择**将更改应用到主版本**，把成果合入你的项目；或选择**放弃**将其丢弃
* **通过看板**：在任意就绪状态的任务卡片上打开三点菜单，然后选择**应用更改**

<TaskReviewDrawer />

应用前请先审查处于就绪状态的任务。查看工作日志、测试输出和 Preview，了解 Agent 做了哪些改动。
...
已完成列还会收录已归档的规划会话和已取消的任务，从而保留一份完整的历史记录，又不会让已结束的工作重新混入草稿或进行中。
...
当你应用多个任务的更改时，Agent 会自动处理冲突。如果发现哪里不对劲，直接让 Agent 修复即可。

<div id="encontrar-e-filtrar-tarefas">
  ## 查找和筛选任务
</div>

当项目中同时进行的任务较多时，可以使用任务面板。你可以按标题或任务编号搜索、筛选草稿，还可以使用编辑模式批量管理多个任务。

例如，当有人在讨论串、评审或 Slack 对话中提到某个任务时，可以按任务编号搜索。想专注于计划中的工作、不想翻看进行中或已完成的任务时，可以筛选草稿。

有关后续跟进建议，请参阅[后续任务](/zh/pt/features/agent/follow-up-tasks)。

<div id="renomear-uma-tarefa">
  ## 重命名任务
</div>

随着任务范围不断变化，过长的任务列表会变得难以查找。要重命名任务，请在对应卡片上 (任务列表侧边栏或任务面板中) 打开三点菜单，然后选择 **重命名**。输入更清晰的标题，再选择 **Save**。

重命名任务只会改变该任务对你和同事的显示方式，不会影响 Agent 已完成的工作、任务历史记录以及任何待处理的更改。

<div id="configuraes-em-cada-tarefa">
  ## 每个任务的设置
</div>

每个任务的标题旁都有专属的设置下拉菜单。打开它即可控制 Agent 如何完成任务，以及结果如何进入你的项目：

* **自动应用更改** (草稿和进行中) ：任务完成后立即应用到主版本，无需等待审查。
* **自动批准计划** (草稿) ：当 Agent 为任务提出计划时，自动接受该计划并开始构建。
* **应用更改** (就绪) ：将已完成的工作应用到你的主版本。
* **重命名**：更改卡片上显示的标题。
* **审查更改** (就绪) ：直接查看就绪任务的差异对比。
* **取消**：停止任务或删除草稿。

自动应用和自动批准是选择一次即生效的快捷操作，而不是可以一直保持开启的开关。菜单宽度保持一致，因此在不同任务之间切换时，标签不会发生位移。

<Frame>
  <img src="/assets/mirror/task-settings-dropdown-7ea7868303b4.png" alt="任务卡片上打开的任务设置下拉菜单，在固定宽度的浮层中显示应用更改、重命名、审查更改和取消操作" width="858" height="536" data-path="images/replitai/task-settings-dropdown.png" />
</Frame>