---
title: "Projects"
description: "了解 Project 如何将代码、数据和产出物汇聚在一起，助力规模较大的构建与设计工作。"
---
...
项目是你在 Replit 上构建内容的容器，包含代码、数据以及创建的所有 artifact——web 应用、移动应用、幻灯片演示文稿、数据仪表板和视频。告诉 Agent 你想要什么后，它会创建一个项目，并在其中构建 artifact。

你可以把项目想象成桌面上的工作空间：所有内容都集中在一个地方，可以共享相同的信息，但每个部分都有不同的用途。

例如，如果你让 Agent 构建一个健身追踪应用，它会创建一个项目，其中包含移动应用 artifact，以及让它正常运行所需的 backend、数据库和存储。

<ArtifactGrid />

关键概念：

* **项目**：容纳所有内容的容器——代码、数据和所有 artifact
* **Artifact**：项目中可发布的成果 (如 web 应用、幻灯片演示文稿或视频)
* **资源**：每个 artifact 都会自动获得 backend、数据库和文件存储，无需任何设置

<Note>
  Publishing 会一次性发布整个项目。所有 artifact 都会同时上线，
  因此始终保持同步。
</Note>
...
| **[幻灯片演示文稿](/zh/features/artifact-types/slide-decks)**              | 可以演示或导出的演示文稿                  | 融资演示文稿、团队进展汇报、产品概览 |
| **[动画](/zh/features/artifact-types/animated-videos)**           | 可导出为 MP4 的 motion graphics 视频 | 产品发布视频、讲解视频、社交媒体内容 |
| **3D 游戏**                                                    | 3D 游戏、仿真或交互式体验                | 益智游戏、物理仿真、交互式体验    |
| **[Design](/zh/design/canvas)**                                 | 在设计 Canvas 上探索的视觉方向与原型        | 情绪板、原型图、设计探索       |

一个项目可以包含多个 artifact。例如，你可以在同一个项目中为客户构建 web 应用、为团队构建移动应用，再为投资人制作一份幻灯片演示文稿。当多个 artifact 位于同一个项目中时，它们会自动共享相同的数据和 backend 逻辑。了解更多，请参阅 [Artifacts](/zh/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project)。

<div id="managing-your-projects">
  ## 管理你的项目
</div>

项目页面会列出你创建的以及受邀加入的所有项目。当 workspace 中项目众多时，这里是你回到近期工作、快速找到目标项目的最快途径。

<div id="see-at-a-glance-whats-inside-each-project">
  ### 一眼看清每个项目里有什么
</div>

每张项目卡片都会以小图标形式展示它所包含的 artifact，涵盖项目中的每个 web 应用、移动应用、幻灯片演示文稿、数据可视化或动画。将鼠标悬停在任意图标上，即可直接在卡片上预览该 artifact 的截图，无需打开项目即可区分各个项目。

<Frame>
  <img src="/assets/mirror/projects-page-artifacts-a9eb175ffbaf.png" alt="Projects 页面上的项目卡片，显示 artifact 图标以及列出项目内所有 artifact 的下拉菜单" width="721" height="1022" data-path="images/replitai/projects-page-artifacts.png" />
</Frame>
...
使用 Projects 页面顶部的 **Build type** 下拉菜单，按 artifact 筛选列表。选择某一项后，将只显示包含该类 artifact 的项目：
...
选择 **Any build type** 即可清除筛选条件。

<Frame>
  <img src="/assets/mirror/projects-page-build-type-filter-9e23df1b365a.png" alt="Projects 页面上的 Build type 下拉菜单，可按 artifact 筛选，包含 Design、Web、Data、Mobile、3D Game 和 Slides 选项" width="472" height="622" data-path="images/replitai/projects-page-build-type-filter.png" />
</Frame>
...
项目页面默认按**你最近打开的时间**排序——你最近使用过的项目会排在最上方。这在团队 Workspace 中尤其有用，否则其他成员的近期编辑会把你当前的工作淹没在下方。

你也可以点击其他列标题来切换排序方式 (例如**最近更新**或**名称**) 。

<div id="pin-the-projects-you-use-most">
  ### 置顶最常用的项目
</div>

置顶项目后，它会始终显示在列表顶部。置顶**仅对你自己可见**——在团队 workspace 中置顶项目不会改变其他成员看到的内容。

要置顶项目，请打开项目卡片上的更多菜单 (三个点) ，然后选择**置顶**。在同一菜单中选择**取消置顶**即可取消置顶。
...
<CardGroup cols={3}>
  ...
  <Card title="幻灯片演示文稿" href="/zh/features/artifact-types/slide-decks" icon="presentation-screen">
