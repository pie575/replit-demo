---
title: "Crie um site com Notion"
description: "Aprenda a criar um site que usa o Notion como Sistema de Gerenciamento de Conteúdo (CMS) com o Replit Agent."
---
...
O Notion é uma ferramenta poderosa para organizar informações e se destaca como um Sistema de Gerenciamento de Conteúdo (CMS). Ao integrar o Notion com o Replit, você pode gerenciar o conteúdo do seu site — como posts de blog, itens de portfólio ou listagens de produtos — diretamente do seu espaço de trabalho no Notion.

O Replit, com o suporte do [Replit Agent](/pt/features/agent/overview), cuida da programação, hospedagem e publicação, permitindo que você vá da ideia a um aplicativo publicado rapidamente. Criar prompts eficazes é fundamental para guiar o Agent; para um guia completo, consulte [Prompting eficaz com o Replit AI](/pt/learn/effective-prompting) e [Como fazer vibe coding de forma eficaz](/pt/learn/build-with-agent).

<YouTubeEmbed videoId="AOYqTONf07o" title="Vibe coding de um site personalizado com o Notion como CMS" />

Este tutorial orienta você na criação de um blog minimalista que busca seus posts de uma tabela do Notion. Você irá:
...
<Frame caption="Resultado final: um blog minimalista com Notion">
  <img src="/assets/mirror/final-app-0a4ae75ee231.png" alt="Captura de tela do aplicativo de blog final com Notion, com design minimalista" width="3840" height="2160" data-path="images/tutorials/notion-cms/final-app.png" />
</Frame>
...
* Uma conta no Replit
* Uma conta no Notion
* Familiaridade com as interações básicas do Replit Agent. Se você é novo no Agent, confira a [documentação do Replit Agent](/pt/features/agent/overview).
...
<Steps>
  <Step title="Crie uma integração no Notion">
    ...
    <Frame caption="Criando uma nova integração no Notion">
      <img src="/assets/mirror/new-integration-1-dcb1779aa5d5.png" alt="Portal do desenvolvedor do Notion mostrando o botão Nova integração" width="3794" height="2168" data-path="images/tutorials/notion-cms/new-integration-1.png" />
    </Frame>

    5. Nomeie sua integração (ex.: "Minha Integração de Blog no Replit").
    6. Associe-a ao espaço de trabalho desejado no Notion.

    <Frame caption="Configurando sua integração no Notion">
      <img src="/assets/mirror/new-integration-2-95484e5e5ce5.png" alt="Tela de configuração de integração do Notion com nome e seleção de espaço de trabalho" width="3796" height="2168" data-path="images/tutorials/notion-cms/new-integration-2.png" />
    </Frame>

    7. Para "Tipo de integração", escolha **Integração interna**.
    8. Selecione **Enviar**.
    9. Copie seu **Segredo de integração interna** (token) e salve-o com segurança. Esta é sua chave de API do Notion.

    <Frame caption="Obtendo seu Segredo de integração">
      <img src="/assets/mirror/new-integration-3-6ddc518a34f7.png" alt="Página de segredos de integração do Notion mostrando o token de integração" width="3796" height="2168" data-path="images/tutorials/notion-cms/new-integration-3.png" />
    </Frame>

    10. Em **Recursos**, certifique-se de que "Ler conteúdo" está habilitado. Para este tutorial, apenas leitura é suficiente. Se posteriormente quiser gravar dados no Notion, habilite "Inserir conteúdo" e "Atualizar conteúdo".
    ...
    Para instruções mais detalhadas, consulte a [documentação oficial do Notion sobre como criar uma integração](https://developers.notion.com/docs/create-a-notion-integration).

    Você pode acessar diretamente o painel de integrações do Notion em [notion.so/my-integrations](https://notion.so/my-integrations).
  </Step>

  <Step title="Crie uma página do Notion com um banco de dados">
    ...
    <Frame caption="Exemplo de configuração do banco de dados do Notion para posts de blog">
      <img src="/assets/mirror/blog-posts-8e330155e945.png" alt="Banco de dados do Notion configurado com colunas para posts de blog" width="3840" height="2160" data-path="images/tutorials/notion-cms/blog-posts.png" />
    </Frame>

    5. Adicione alguns posts de exemplo. Você pode usar os recursos de IA do Notion para ajudar a gerar conteúdo!
  </Step>

  <Step title="Conecte sua integração à página">
    1. Abra a página do Notion que contém seu banco de dados.
    2. Clique no menu **•••** (três pontos) no canto superior direito.

    <Frame caption="Acesse o menu de integração no Notion">
      <img src="/assets/mirror/add-integration-89d788bbd7b7.png" alt="Menu de conexões do Notion mostrando como adicionar sua integração à página" width="3840" height="2160" data-path="images/tutorials/notion-cms/add-integration.png" />
    </Frame>

    3. Selecione **+ Adicionar conexões**.
    4. Pesquise e selecione a integração que você criou (ex.: "Minha Integração de Blog no Replit").

    <Frame caption="Adicionando sua integração à página do Notion">
      <img src="/assets/mirror/add-integration-menu-f6b013e70a48.png" alt="Página do Notion mostrando o menu de três pontos para adicionar integrações" width="3840" height="2160" data-path="images/tutorials/notion-cms/add-integration-menu.png" />
    </Frame>

    5. Confirme a conexão. Isso permite que o Replit (via token de integração) acesse esta página e seu banco de dados.
  </Step>
</Steps>
...
<Steps>
  <Step title="Abra o Replit Agent">
  ...
  <Step title="Escreva seu prompt">
    ...
    <Tip>
      Para mais dicas sobre como criar prompts eficazes, consulte nosso guia sobre [Prompting eficaz com o Replit AI](/pt/learn/effective-prompting).
      Você também pode **mostrar** ao Agent o que você quer fornecendo uma URL para raspar para ideias iniciais de estilo ou conteúdo (ex.: seu portfólio pessoal) adicionando: `Raspe o conteúdo de [URL] para inspiração de design inicial e texto de espaço reservado.`
    </Tip>
  ...
  <Step title="Revise a pré-visualização inicial">
...
<Steps>
  <Step title="Adicione Secrets no Replit">
    ...
    * Chave: `NOTION_DATABASE_ID`, Valor: `[Seu_ID_do_Banco_de_Dados_do_Notion]`
    ...
    <Frame caption="Adicionando seus secrets do Notion ao Replit">
      <img src="/assets/mirror/add-secrets-0cf2e788b372.png" alt="Ferramenta Secrets do Replit com chave API do Notion e ID do banco de dados adicionados" width="3840" height="2160" data-path="images/tutorials/notion-cms/add-secrets.png" />
    </Frame>
...
O aplicativo executa mas não exibe posts. Erros no Console podem mencionar "Não foi possível encontrar propriedade de classificação com o nome ID created\_time" ou outras incompatibilidades de colunas.
...
<AccordionGroup>
  <Accordion title="Implementar cache e pré-busca">
  ...
  <Accordion title="Melhorar o estilo">
  ...
  <Accordion title="Configurar estratégia de atualização de dados">
...
2. Revise as configurações de publicação (nome do projeto, nível). Para mais detalhes, consulte [Implantações](/pt/learn/projects-and-artifacts/replit-deployments).
3. Selecione **Publicar**.

<Frame caption="Publicando seu site com Notion">
  <img src="/assets/mirror/deploy-13a5233cd675.png" alt="Interface de publicação do Replit mostrando como publicar seu site com Notion" width="3840" height="2160" data-path="images/tutorials/notion-cms/deploy.png" />
</Frame>
...
Para informações detalhadas sobre checkpoints e reversões, consulte [Checkpoints e Reversões](/pt/features/version-control/checkpoints-and-rollbacks).

Construir com IA como o Replit Agent é colaborativo. Pensamento procedural, instruções claras e depuração metódica são fundamentais para transformar ideias em realidade rapidamente.
...
<AccordionGroup>
  <Accordion title="Experimente com tipos de conteúdo">
    Tente adicionar conteúdo diverso do Notion: galerias de imagens, vídeos incorporados ou itens categorizados.
  </Accordion>

  <Accordion title="Aproveite os recursos avançados do Notion">
  ...
  <Accordion title="Combine com outras integrações do Replit">
    Aprimore seu aplicativo mesclando dados do Notion com outras ferramentas do [Replit AI](/pt/build/welcome) ou [integrações do Agent](/pt/features/integrations/overview). Por exemplo, use o Replit Auth para conteúdo privado ou o OpenAI para resumos gerados por IA a partir de dados do Notion no seu aplicativo.
  </Accordion>
</AccordionGroup>
