---
title: "Создайте мобильное приложение"
description: "Создайте приложение для смартфона с помощью Agent, протестируйте его в симуляторе и на своём телефоне, а затем поделитесь им, чтобы получить отзывы."
---
...
Мобильное приложение должно ощущаться естественно на телефоне: быстро запускаться, иметь удобные элементы управления и быть комфортным для использования одной рукой.

Вы создадите **Pace Mobile** — трекер пробежек для телефона, вдохновлённый примером трекера пробежек из статьи [Что такое Replit?](/ru/pt/build/welcome). Цель — протестировать мобильный сценарий, прежде чем переходить к полному процессу публикации в App Store.

<Frame>
  <img src="/assets/mirror/mobile-app-4203ff45ecd5.png" alt="Пример хорошо оформленного мобильного приложения с экранами отслеживания физической активности, разработанными для использования на телефоне" width="1440" height="850" data-path="images/build-examples/mobile-app.png" />
</Frame>
...
* Первая версия, ограниченная одним удобным для телефона сценарием.
* Приложение, запущенное в симуляторе или эмуляторе.
* Приложение, запущенное на вашем телефоне через Expo Go.
* Версия, которой можно поделиться с кем-то, чтобы получить feedback.
...
* Создать мобильное приложение по prompt.
* Определить объём первой версии с учётом ограничений телефона.
* Протестировать приложение в симуляторе или эмуляторе.
* Протестировать приложение на реальном устройстве через Expo Go.
* Понять, когда стоит продолжить доработку, прежде чем переходить к App Store или TestFlight.

<div id="para-quem-este-guia">
  ## Для кого это руководство
</div>

Используйте это руководство, если ваша идея предназначена для использования на телефоне.

Оно пригодится для трекеров, сервисов бронирования, инструментов для полевых работ, приложений для мероприятий, инструментов обучения, приложений для формирования привычек и всего, чем люди должны пользоваться не за компьютером.

<Note>
  TestFlight и публикация в App Store не рассматриваются в этом руководстве. Когда будете готовы к этому этапу, ознакомьтесь с [руководством по созданию и запуску мобильного приложения](/ru/pt/build/mobile-app).
</Note>
...
* Мобильный рабочий процесс, который можно протестировать за несколько минут.

Подробную информацию о симуляторе, эмуляторе и предпросмотре на устройстве см. в разделе [Нативные мобильные приложения](/ru/pt/features/artifact-types/building-mobile-apps).

<div id="defina-o-fluxo-mobile">
  ## Определите мобильный сценарий
</div>

Прежде чем приступать к разработке, решите, что пользователь должен делать на телефоне.

Для Pace Mobile:

* **Аудитория**: бегуны, которым нужно быстро записать пробежку.
* **Основной сценарий**: выбрать тип пробежки → ввести дистанцию и время → сохранить пробежку → посмотреть сводку за день.
* **Мобильное ограничение**: до основного действия должно быть легко дотянуться одним большим пальцем.
* **Первая версия**: без учётных записей, без социальной ленты, без продвинутой аналитики.

Так первая мобильная версия останется достаточно компактной, чтобы её можно было протестировать.

<div id="construa-a-primeira-verso">
  ## Создайте первую версию
</div>

Откройте Replit и создайте новый app. Опишите, что вы хотите создать, и выберите **Мобильное приложение** в качестве типа app.

<Frame>
  <img src="/assets/mirror/prompt-5cb144699b45.png" alt="Стартовый экран Replit с выбранным типом app «Мобильное приложение»" width="1440" height="900" data-path="images/native-mobile-apps/prompt.png" />
</Frame>

Используйте такой prompt:

<AiPrompt>
  Создай Pace Mobile — трекер пробежек, рассчитанный на телефон.<br />
  Бегун должен иметь возможность выбрать тип пробежки, ввести дистанцию и время, сохранить пробежку и увидеть сводку за сегодня.<br />
  Спроектируй его для управления одной рукой: с крупными областями нажатия, простой навигацией и ощущением отточенного фитнес-приложения.<br />
  Сделай первую версию достаточно компактной, чтобы протестировать её сегодня в симуляторе и на реальном телефоне.
</AiPrompt>

Если вы хотите, чтобы Agent сначала всё продумал, используйте [режим Plan](/ru/pt/features/agent/plan-mode). Попросите Agent проверить экраны, навигацию, области нажатия и ограничения, специфичные для телефона, прежде чем приступать к разработке.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Поле ввода prompt в Replit с выбранным режимом Plan перед тем, как Agent определит объём работ по созданию мобильного приложения" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Когда Agent завершит первую сборку, откройте [Preview](/ru/pt/features/editor/preview). В списке устройств выберите **Симулятор iOS** или **Эмулятор Android**.

Протестируйте приложение так, как если бы вы пользовались им на телефоне.

<Frame>
  <img src="/assets/mirror/workspace-dda0f81e6b83.png" alt="Редактор проекта Replit с предварительным просмотром мобильного приложения и возможностью просмотреть его на мобильном устройстве" width="1440" height="900" data-path="images/native-mobile-apps/workspace.png" />
</Frame>

Убедитесь, что вы можете:

* Запустить приложение без ошибок.
* Выбрать тип пробежки.
* Ввести расстояние и время.
* Сохранить пробежку.
* Увидеть обновление сводки за сегодня.
* Перемещаться по приложению, не застревая.
* Читать текст без масштабирования.

<div id="teste-no-seu-telefone">
  ## Тестирование на телефоне
</div>

Тестировать в симуляторе полезно, но понять, действительно ли app выглядит как надо, можно только на телефоне.

Откройте панель предпросмотра мобильного устройства и отсканируйте QR-код телефоном. App откроется в Expo Go.

<Frame>
  <img src="/assets/mirror/mobile-screen-e5685fc26793.png" alt="Мобильное приложение, запущенное в предпросмотре редактора проекта Replit, с панелью Preview на устройстве и QR-кодом для Expo Go" width="1440" height="900" data-path="images/native-mobile-apps/mobile-screen.png" />
</Frame>

На телефоне проверьте:

* Получается ли пройти основной сценарий одной рукой?
* Достаточно ли крупные кнопки и поля ввода, чтобы по ним попадать?
* Легко ли дотянуться до основного действия?
* Не перекрывает ли клавиатура важные поля?
* Остаётся ли app понятным на маленьком экране?

Если что-то выглядит неудачно, попросите Agent исправить именно эту часть, а не переделывать весь app целиком.

<AiPrompt>
  Улучши Pace Mobile для использования одной рукой на телефоне.<br />
  Сделай кнопку «Сохранить пробежку» доступнее, увеличь области нажатия там, где это нужно, и оставь форму пробежки простой.<br />
  Не добавляй новые функции.<br />
  После изменения проверь, что сохранение пробежки по-прежнему обновляет сводку за сегодня.
</AiPrompt>

<div id="compartilhe-para-obter-feedback">
  ## Поделитесь приложением для получения обратной связи
</div>

Когда всё будет работать на телефоне, опубликуйте App, чтобы создать версию для Expo Go, которой можно поделиться для получения первоначальной обратной связи.

Поделитесь им, поставив конкретную задачу:

> Откройте Pace Mobile на телефоне, сохраните пробежку и сообщите, что было неудобно нажимать, читать или понимать.

Целенаправленную обратную связь проще использовать, чем общие мнения. Если обратная связь указывает на конкретную проблему, попросите Agent внести минимальное исправление и снова протестируйте App на телефоне.

<div id="publique-na-app-store">
  ## Публикация в App Store
</div>

Протестировав Pace Mobile в симуляторе и на телефоне с Expo Go, вы можете отправить это же приложение в TestFlight и App Store. Для этого требуется учётная запись участника **Apple Developer Program** с включённой двухфакторной аутентификацией. Подробнее о регистрации, индивидуальных и корпоративных учётных записях, TestFlight, проверке в App Store и внешнем тестировании см. в разделе [Создание и выпуск мобильного приложения](/ru/pt/build/mobile-app).

<Steps>
  <Step title="Откройте вкладку «Публикация» и начните публикацию в App Store">
    После запуска предварительного просмотра в Expo Go откройте вкладку **Publishing**. В разделе **Production** появится карточка **Ready to publish Pace to the App Store?**. Нажмите **Start publishing to the App Store**.

    <Frame>
      <img src="/assets/mirror/app-store-02-start-publishing-190eb7841bcf.png" alt="Вкладка Publishing в редакторе проекта со статусом Production и карточкой «Ready to publish Pace to the App Store?» с кнопкой Start publishing to the App Store" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-02-start-publishing.png" />
    </Frame>
  </Step>

  <Step title="Выберите проект Expo">
    Replit откроет **Launch** — процесс Expo, встроенный в Replit. Введите или подтвердите имя и слаг вашего проекта Expo и нажмите **Продолжить**. Если у вас уже есть проект Expo, выберите **Использовать существующий**.

    <Frame>
      <img src="/assets/mirror/app-store-03-choose-expo-project-4707fafffafe.png" alt="Диалоговое окно Launch с активным шагом «Выбрать проект Expo» — поля «Имя проекта» и «Слаг проекта», предзаполненные значением Pace, кнопка «Продолжить» и вариант «Использовать существующий»" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-03-choose-expo-project.png" />
    </Frame>
  </Step>

  <Step title="Войдите с помощью Apple">
    Подключите учётную запись Apple Developer, которой будет принадлежать страница в App Store. Учётная запись должна быть зарегистрирована в Apple Developer Program; Apple также требует двухфакторную аутентификацию — будьте готовы подтвердить код на доверенном устройстве.

    <Frame>
      <img src="/assets/mirror/app-store-04-sign-in-apple-459dfc9b84ba.png" alt="Диалоговое окно Launch на этапе «Войти с Apple» — поля для адреса электронной почты или номера телефона и пароля Apple Developer, а также примечание о том, что пароль никогда не покидает устройство и что учётная запись должна быть зарегистрирована в Apple Developer Program" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-04-sign-in-apple.png" />
    </Frame>
  </Step>

  <Step title="Выберите или создайте своё приложение Apple">
    Введите **Название приложения** и **Bundle ID** (например, `app.replit.pace`), которые будут отображаться в App Store. Bundle ID нельзя изменить после первой сборки, поэтому выбирайте его внимательно.

    <Frame>
      <img src="/assets/mirror/app-store-05-choose-apple-app-3971aea62f14.png" alt="Диалоговое окно Launch на шаге выбора приложения Apple — в поле «Название приложения» указано Pace, Bundle ID app.replit.pace, примечание о том, что Bundle ID нельзя изменить позже, и кнопка «Создание приложения…»" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-05-choose-apple-app.png" />
    </Frame>
  </Step>

  <Step title="Опубликуйте в App Store">
    Replit настраивает сертификаты Apple от вашего имени. Когда загорится этап **«Готово к запуску»**, выберите **«Запустить в App Store»**. Replit собирает нативное приложение в облаке и отправляет сборку в App Store Connect для бета-проверки в TestFlight.

    <Frame>
      <img src="/assets/mirror/app-store-06-ready-for-launch-ab02659ee0bb.png" alt="Диалоговое окно Launch на этапе «Готово к запуску»: пункты «Выбрать приложение Apple» и «Настроить сертификаты Apple» отмечены как выполненные, есть кнопка «Запустить в App Store» и необязательные параметры запуска" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-06-ready-for-launch.png" />
    </Frame>

    Когда Launch завершится, появится подтверждение, что приложение отправлено в App Store. Затем диалоговое окно закроется, и вы вернётесь в Редактор проекта.

    <Frame>
      <img src="/assets/mirror/app-store-targeting-f8797071c9e0.png" alt="Диалоговое окно Launch: пункт «Отправка в App Store» отмечен как выполненный, отображается сообщение «Перенаправление обратно в Replit» — экран подтверждения после выбора «Запустить в App Store»" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-targeting.png" />
    </Frame>
  </Step>
</Steps>

<Note>
  Первая сборка версии обычно должна пройти бета-проверку TestFlight, прежде чем тестировщики смогут её установить. После одобрения последующие сборки, как правило, доходят до тестировщиков быстрее. Чтобы предоставить доступ внешним тестировщикам или отправить приложение на публикацию в App Store, заполните карточку приложения в **App Store Connect** — полный контрольный список для App Store Connect см. в разделе [Создание и запуск мобильного приложения](/ru/pt/build/mobile-app).
</Note>
...
* App открывается в симуляторе iOS или эмуляторе Android.
* App открывается на вашем телефоне через Expo Go.
* Вам удаётся пройти основной мобильный сценарий целиком.
* Текст, кнопки и навигация выглядят удобно на телефоне.
* Вы опубликовали мобильную версию или поделились ею для тестирования.
* Хотя бы один человек может её попробовать и оставить отзыв.
* (Необязательно) Вы отправили App в TestFlight через процесс публикации в App Store.
...
<CardGroup cols={2}>
  <Card title="Добавить вход" icon="user-lock" href="/ru/pt/build/add-login">
    Позвольте каждому пользователю видеть свой прогресс.
  </Card>

  <Card title="Добавить базу данных" icon="database" href="/ru/pt/build/add-database">
    Сохраняйте историю пробежек после первой тестовой сессии.
  </Card>
  ...
  <Card title="Создать и опубликовать мобильное приложение" icon="mobile-screen" href="/ru/pt/build/mobile-app">
    Подготовьте работающее мобильное приложение к публикации в TestFlight и App Store.
  </Card>
</CardGroup>
...
* **Agent создаёт веб-приложение:** начните заново и перед началом разработки выберите тип приложения **Мобильное приложение**.
* **Предпросмотр в Expo Go выглядит устаревшим:** встряхните телефон, откройте меню разработчика и выберите **Обновить**.
* **Области нажатия кажутся слишком маленькими:** попросите Agent увеличить области нажатия и разместить основные действия в пределах досягаемости большого пальца.
* **Навигация выглядит перегруженной:** попросите Agent сократить количество экранов до минимума, необходимого для основного сценария.
* **Симулятор работает, а телефон — нет:** проверьте поведение, зависящее от аппаратного обеспечения, на реальном устройстве и ознакомьтесь с разделом [Нативные мобильные приложения](/ru/pt/features/artifact-types/building-mobile-apps).
* **Публикация завершается ошибкой:** ознакомьтесь с разделом [Устранение неполадок мобильного приложения](/ru/pt/features/troubleshooting/mobile-app).

<div id="experimente-agora">
  ## Попробуйте сейчас
</div>

<AgentInput highlightStart defaultPrompt="Создайте Pace Mobile — мобильный трекер пробежек. Пользователь должен иметь возможность выбрать тип пробежки, указать дистанцию и время, сохранить пробежку и посмотреть сводку за сегодня. Проектируйте его для использования одной рукой: с крупными сенсорными элементами, простой навигацией и стильным интерфейсом фитнес-приложения. Сделайте первую версию достаточно компактной, чтобы протестировать её уже сегодня в симуляторе и на реальном телефоне." showTypewriter={false} />

<div id="relacionado">
  ## Связанные материалы
</div>

* [Нативные мобильные приложения](/ru/pt/features/artifact-types/building-mobile-apps)
* [Создайте и запустите мобильное приложение](/ru/pt/build/mobile-app)
* [Vibe coding 101](/ru/pt/learn/foundations/vibe-coding-101)
* [Устранение неполадок в мобильном приложении](/ru/pt/features/troubleshooting/mobile-app)