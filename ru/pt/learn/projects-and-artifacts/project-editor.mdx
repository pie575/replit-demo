...
description: "Узнайте, что такое редактор проекта: это ваша основная площадка в Replit, где вы общаетесь с Agent, видите, как App обретает форму, и управляете всем процессом."
sidebarTitle: "Редактор проекта"
icon: "window"
---
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="Главный экран редактора проекта Replit с полем промпта, где вы описываете, что хотите создать" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>

<div id="o-que-o-editor-de-projeto">
  ## Что такое редактор проекта?
</div>

Редактор проекта — это ваша отправная точка в Replit: среда, где вы начинаете разработку, управляете проектами и наблюдаете, как ваши App оживают. Здесь достаточно описать, что вы хотите создать, — и [Agent](/ru/pt/features/agent/overview) сделает всё остальное: напишет код, настроит проект и покажет живой предпросмотр в реальном времени.

<div id="primeiros-passos">
  ## Начало работы
</div>

Введите в поле промпта описание того, что хотите создать. Опишите идею простыми словами — «трекер бюджета, который отображает мои расходы на графике», «лендинг для моей кофейни», «генератор комиксов с ИИ для детей» — а Agent позаботится обо всём остальном.

<AgentInput />

Также можно выбрать тип проекта в карусели, чтобы указать Agent нужный тип результата: веб-App, мобильное App, презентация, дизайн и многое другое. Или пропустите выбор и просто опишите свою идею — Agent автоматически определит подходящую конфигурацию.

<AgentCarousel />

Чтобы начать прямо сейчас, ознакомьтесь с руководством [Создание первого App](/ru/pt/build/your-first-app).

<div id="modo-de-plano">
  ## Режим «План»
</div>

Режим «План» позволяет проводить мозговой штурм, задавать вопросы и продумывать структуру проекта. Agent создаёт упорядоченный список задач, который можно просмотреть и доработать: когда план вас устроит, утвердите его — и Agent приступит к разработке. Режим «План» также подходит для вопросов и обсуждения идей: Agent не внесёт никаких изменений, пока вы из него не выйдете.

<AgentInput highlightPlan={true} showTypewriter={false} />

Включите режим «План» с помощью кнопки в нижней части поля ввода. Подробнее — в [руководстве по режиму «План»](/ru/pt/features/agent/plan-mode).

<div id="o-que-voc-pode-construir">
  ## Что можно создать
</div>

Выберите тип проекта, чтобы объяснить Agent, что вам нужно, или просто опишите свою идею — и Agent сам подберёт подходящую конфигурацию.

* **Веб-App** — дашборды, внутренние инструменты, SaaS-продукты, лендинги
* **Мобильные App** — приложения для iOS и Android по одному промпту
* **Слайды и презентации** — генерация презентаций с продуманными раскладками
* **Анимации** — интерактивные пошаговые обзоры и моушн-графика
* **Дизайны** — визуальные макеты и прототипы в [Design Canvas](/ru/pt/design/canvas)
* **Визуализация данных** — графики, дашборды и инструменты на основе базы данных
...
Когда Agent начнёт создавать App, в Редакторе проекта вы увидите, как оно постепенно обретает форму. Он разделён на панели: с одной стороны — ваш диалог с Agent, с другой — интерактивный предпросмотр App.

<div id="threads">
  ### Ветки
</div>

Каждый разговор с Agent ведётся в отдельной ветке. В основной ветке вы описываете желаемый результат и наблюдаете, как Agent его создаёт. При использовании [системы задач](/ru/pt/core-concepts/agent/task-system) каждая фоновая задача получает собственную ветку, чтобы Agent мог изолировать работу над задачами и не смешивать разговоры.

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="Ветка, в которой показаны промпт пользователя и процесс создания проекта Agent" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>
...
Доска задач распределяет вашу работу по столбцам: **Черновики**, **Активные**, **Готовые** и **Завершённые**. По мере выполнения каждая задача перемещается слева направо, поэтому вы сразу видите, что запланировано, над чем работает Agent и что уже закончено. Подробнее — в разделе [Система задач](/ru/pt/core-concepts/agent/task-system).

<Frame>
  <img src="/assets/mirror/task-board-active-355ecd390c49.png" alt="Доска задач с задачами, распределёнными по столбцам «Черновики», «Активные», «Готовые» и «Завершённые»" width="3430" height="1986" data-path="images/replitai/task-board-active.png" />
</Frame>
...
Design Canvas — это визуальный редактор, в котором можно создавать и дорабатывать макеты, прежде чем переходить к коду. Перетаскивайте элементы, настраивайте раскладку и улучшайте дизайн, а затем поручите Agent превратить его в работающий app. Подробнее — в разделе [Design Canvas](/ru/pt/design/canvas).

<div id="colaborao">
  ## Совместная работа
</div>

Создайте командный Workspace, чтобы работать вместе. Приглашайте коллег, делитесь проектами и работайте над одним App в реальном времени. О настройке команды см. [Командные workspaces](/ru/pt/features/collaboration/team-workspaces), а о совместной работе на уровне проекта — [Приглашение коллег](/ru/pt/build/invite-teammates).

<div id="importe-cdigo-e-design">
  ## Импорт кода и дизайна
</div>

Уже есть проект? Перенесите его в Replit из GitHub или Figma либо загрузите файлы напрямую. Agent продолжит с того места, где вы остановились, и будет развивать ваш существующий код или дизайн. См. [Импорт в Replit](/ru/pt/build/import-from-providers).
...
<CardGroup cols={2}>
  <Card title="Редактор и инструменты" href="/ru/pt/features/editor/editor-and-tools" icon="window-restore">
    Основные панели для просмотра предпросмотра App, вывода Console и работы с другими инструментами
  </Card>

  <Card title="Настройка App" href="/ru/pt/features/project-setup/configuration" icon="gear">
    Настройте параметры, зависимости, Secrets и среду App
  </Card>

  <Card title="Контроль версий" href="/ru/pt/learn/projects-and-artifacts/version-control" icon="clock-rotate-left">
    Отслеживайте изменения, возвращайтесь к предыдущим версиям и управляйте историей App
  </Card>

  <Card title="Публикация" href="/ru/pt/learn/projects-and-artifacts/replit-deployments" icon="rocket">
    Разверните App по общедоступному URL, чтобы им мог пользоваться любой
  </Card>
</CardGroup>
...
<Accordion title="Нужно ли уметь программировать?">
  Нет. Опишите, что вам нужно, простыми словами — Agent возьмёт на себя технические детали. При желании вы также можете вносить правки вручную.
</Accordion>

<Accordion title="Что произойдёт, когда я закрою браузер?">
  Ваш проект сохраняется в облаке. Возвращайтесь в любой момент и продолжайте с того места, где остановились: файлы, история и запущенный app — всё сохранится.
</Accordion>

<Accordion title="Могу ли я работать вместе с другими?">
  Да. Вы можете пригласить соавторов в конкретный проект или создать командный Workspace, где участники получают доступ ко всем проектам. См. [Командные workspaces](/ru/pt/features/collaboration/team-workspaces) и [Приглашение коллег](/ru/pt/build/invite-teammates).
</Accordion>

<Accordion title="Как опубликовать мой app?">
  Когда app будет готов, разверните его на рабочем URL прямо из редактора проекта. См. [Deployments](/ru/pt/learn/projects-and-artifacts/replit-deployments).
</Accordion>
...
Редактор проекта доступен на всех тарифах. Некоторые возможности — например, увеличенные вычислительные ресурсы и постоянно活 работающие деплойменты — требуют платного тарифа. Подробности см. в разделе [Биллинг](/ru/pt/billing/ai-billing).