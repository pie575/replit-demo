...
description: "Créez une application conçue d’abord pour les téléphones avec Agent, testez-la dans le simulateur et sur un téléphone réel, puis partagez-la pour recueillir des retours."
---
...
Une application mobile doit offrir une expérience naturelle sur téléphone : s’ouvrir rapidement, être facile à utiliser et confortable d’une seule main.

Vous allez créer **Pace Mobile**, un tracker de course à pied conçu en priorité pour les téléphones, inspiré de l’exemple de tracker de course de [Qu’est-ce que Replit ?](/fr/ko/welcome). L’objectif est de tester un parcours mobile ciblé avant de vous engager dans le processus complet de publication sur l’App Store.

<Frame>
  <img src="/assets/mirror/mobile-app-4203ff45ecd5.png" alt="Exemple d’application mobile élégante avec un écran de suivi d’activité physique conçu pour une utilisation sur téléphone" width="1440" height="850" data-path="images/build-examples/mobile-app.png" />
</Frame>
...
* Une première version centrée sur un seul parcours optimisé pour téléphone.
* Une application exécutée dans un simulateur ou un émulateur.
* Une application exécutée sur votre téléphone avec Expo Go.
* Une version partageable que vous pourrez envoyer à quelqu’un pour recueillir ses retours.
...
* démarrer une application mobile à partir d’un prompt.
* définir le périmètre de votre première version en tenant compte des contraintes des téléphones.
* tester votre application dans un simulateur ou un émulateur.
* tester votre application sur un appareil réel avec Expo Go.
* déterminer à quel moment continuer à itérer avant de passer aux étapes liées à l’App Store ou à TestFlight.

<div id="section-3">
  ## À qui s&#39;adresse ce guide ?
</div>

Utilisez ce guide si votre idée doit être utilisée sur un téléphone.

Il est utile pour les outils de suivi, les parcours de réservation, les outils de terrain, les applications événementielles, les outils d&#39;apprentissage, les applications de suivi d&#39;habitudes et tout ce que les utilisateurs doivent pouvoir utiliser ailleurs que sur un ordinateur de bureau.

<Note>
  TestFlight et la publication sur l&#39;App Store ne sont pas couverts dans ce guide. Lorsque vous serez prêt à suivre cette voie, consultez [Créer et lancer une application mobile](/fr/ko/build/mobile-app).
</Note>
...
* Un flux mobile à tester en quelques minutes.

Pour en savoir plus sur les simulateurs, les émulateurs et les aperçus sur appareil, consultez la page [Applications mobiles natives](/fr/ko/features/artifact-types/building-mobile-apps).

<div id="section-5">
  ## Définir le parcours mobile
</div>

Avant de créer l’application, déterminez ce que les utilisateurs doivent pouvoir faire sur leur téléphone.

Pour Pace Mobile :

* **Public cible **: les coureurs qui souhaitent enregistrer rapidement leurs courses.
* **Parcours principal **: choisir le type de course → saisir la distance et le temps → enregistrer la course → consulter le récapitulatif du jour.
* **Contraintes mobiles **: les actions principales doivent être facilement accessibles avec un seul pouce.
* **Première version **: aucun compte, aucun fil social, aucune analyse avancée.

Cela permet de garder la première version mobile suffisamment restreinte pour être testée.

<div id="section-6">
  ## Créer votre première version
</div>

Ouvrez Replit et créez une nouvelle application. Décrivez ce que vous souhaitez créer, puis sélectionnez **application mobile** comme type d’application.

<Frame>
  <img src="/assets/mirror/prompt-5cb144699b45.png" alt="Prompt de la page d’accueil de Replit avec « application mobile » sélectionnée comme type d’application" width="1440" height="900" data-path="images/native-mobile-apps/prompt.png" />
</Frame>

Utilisez ce prompt :

<AiPrompt>
  Crée Pace Mobile, une application de suivi de course à pied conçue avant tout pour les téléphones.<br />
  Un coureur doit pouvoir choisir un type de course, saisir la distance et le temps, enregistrer sa course et consulter le récapitulatif du jour.<br />
  Conçois l’application pour une utilisation à une main, avec de grandes zones tactiles, une navigation simple et une interface soignée d’application de fitness.<br />
  Limite la première version afin de pouvoir la tester dès aujourd’hui dans un simulateur et sur un téléphone réel.
</AiPrompt>

Utilisez le [mode Plan](/fr/ko/features/agent/plan-mode) pour permettre à Agent de réfléchir avant de commencer le build. Avant de lancer le build, demandez à Agent de vérifier les écrans, la navigation, les zones tactiles et les contraintes propres aux téléphones.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Éditeur de prompt Replit avec le mode Plan sélectionné avant qu’Agent définisse le périmètre de l’application mobile à créer" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Une fois qu’Agent a terminé la première version, ouvrez [Preview](/fr/ko/features/editor/preview). Dans le sélecteur d’appareil, choisissez **iOS Simulator** ou **Android Emulator**.

Testez l’application comme le ferait quelqu’un sur son téléphone.

<Frame>
  <img src="/assets/mirror/workspace-dda0f81e6b83.png" alt="Project Editor de Replit affichant l’aperçu de l’application mobile et les options pour l’afficher sur un appareil mobile" width="1440" height="900" data-path="images/native-mobile-apps/workspace.png" />
</Frame>

Vérifiez les points suivants :

* L’application démarre sans erreur.
* Vous pouvez sélectionner un type de course.
* Vous pouvez saisir la distance et la durée.
* Vous pouvez enregistrer une course.
* Le récapitulatif du jour se met à jour.
* Vous pouvez naviguer sans blocage.
* Vous pouvez lire le texte sans zoomer.

<div id="section-8">
  ## Tester sur votre téléphone
</div>

Les tests sur simulateur sont utiles, mais seul un téléphone permet de savoir si l’application est réellement agréable à utiliser.
...
<Frame>
  <img src="/assets/mirror/mobile-screen-e5685fc26793.png" alt="Application mobile en cours d’exécution dans l’aperçu du Replit Project Editor, avec le panneau Preview on device pour Expo Go et un code QR" width="1440" height="900" data-path="images/native-mobile-apps/mobile-screen.png" />
</Frame>

Vérifiez sur votre téléphone :

* si vous pouvez effectuer les principaux parcours d’une seule main ;
* si les boutons et les champs de saisie sont suffisamment grands pour être touchés ;
* si les actions principales sont faciles à atteindre ;
* si le clavier ne masque pas de champs importants ;
* si l’application reste pertinente sur un petit écran.

Si un élément vous semble peu pratique, demandez à Agent de corriger cette partie précise plutôt que de repenser toute l’application.

<AiPrompt>
  Améliorez Pace Mobile pour une utilisation à une main sur téléphone.<br />
  Rendez le bouton Save Run plus facile à atteindre, agrandissez les zones tactiles si nécessaire et gardez le formulaire de course simple.<br />
  N’ajoutez pas de nouvelles fonctionnalités.<br />
  Après la modification, vérifiez que l’enregistrement d’une course met toujours à jour le résumé du jour.
</AiPrompt>

<div id="section-9">
  ## Partager pour recueillir des retours
</div>

Une fois le parcours mobile fonctionnel, publiez l’application pour créer une version partageable dans Expo Go afin de recueillir les premiers retours.

Partagez-la en donnant une consigne précise :

> Ouvrez Pace Mobile sur votre téléphone, enregistrez une course, puis indiquez-nous si quelque chose vous semble difficile à toucher, à lire ou à comprendre.

Les retours ciblés sont plus faciles à exploiter que les avis généraux. Si les retours mettent en évidence un problème précis, demandez à Agent d’apporter la correction la plus limitée possible, puis testez à nouveau l’application sur votre téléphone.

<div id="section-10">
  ## Publier sur l’App Store
</div>

Après avoir testé Pace Mobile sur un simulateur et un téléphone avec Expo Go, vous pouvez promouvoir la même application sur TestFlight et l’App Store. Ce processus nécessite une adhésion à l’**Apple Developer Program** avec authentification à deux facteurs. Pour en savoir plus sur l’inscription, les comptes individuels ou d’organisation, l’examen par TestFlight ou l’App Store, et les tests externes, consultez [Créer et publier des applications mobiles](/fr/ko/build/mobile-app).

<Steps>
  <Step title="Ouvrez l’onglet Publishing et commencez à publier sur l’App Store">
    Lorsque l’aperçu Expo Go est disponible, ouvrez l’onglet **Publishing**. Sous **Production**, la carte **Ready to publish Pace to the App Store?** s’affiche. Sélectionnez **Start publishing to the App Store**.

    <Frame>
      <img src="/assets/mirror/app-store-02-start-publishing-190eb7841bcf.png" alt="Onglet Publishing du Project Editor affichant la carte Ready to publish Pace to the App Store? avec l’état Production et le bouton Start publishing to the App Store" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-02-start-publishing.png" />
    </Frame>
  </Step>

  <Step title="Sélectionner un projet Expo">
    Replit ouvre **Launch**, le flux Expo intégré à Replit. Saisissez ou vérifiez le nom et le slug du projet Expo, puis sélectionnez **Continue**. Si vous avez déjà un projet Expo, sélectionnez **Use existing**.

    <Frame>
      <img src="/assets/mirror/app-store-03-choose-expo-project-4707fafffafe.png" alt="Boîte de dialogue Launch avec l’étape Choose Expo project activée : champs du nom et du slug du projet préremplis avec Pace, bouton Continue et option Use existing" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-03-choose-expo-project.png" />
    </Frame>
  </Step>

  <Step title="Se connecter avec Apple">
    Connectez le compte Apple Developer propriétaire de la fiche App Store. Ce compte doit être inscrit au programme Apple Developer, et Apple impose l’authentification à deux facteurs — préparez-vous à valider le code de vérification sur un appareil de confiance.

    <Frame>
      <img src="/assets/mirror/app-store-04-sign-in-apple-459dfc9b84ba.png" alt="Boîte de dialogue Launch à l’étape Se connecter avec Apple — champs d’adresse e-mail ou de numéro de téléphone Apple Developer et de mot de passe, avec une note indiquant que le mot de passe ne quitte pas l’appareil et que le compte doit être inscrit au programme Apple Developer" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-04-sign-in-apple.png" />
    </Frame>
  </Step>

  <Step title="Sélectionner ou créer une application Apple">
    Saisissez le **App name** et le **Bundle ID** (par exemple, `app.replit.pace`) qui seront affichés dans l’App Store. L’identifiant du bundle ne peut pas être modifié après la première compilation. Choisissez-le donc avec soin.

    <Frame>
      <img src="/assets/mirror/app-store-05-choose-apple-app-3971aea62f14.png" alt="Boîte de dialogue Launch à l’étape Choose Apple app — nom de l’application défini sur Pace, Bundle ID défini sur app.replit.pace, note indiquant que l’identifiant du bundle ne peut pas être modifié ultérieurement, bouton Creating app…" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-05-choose-apple-app.png" />
    </Frame>
  </Step>

  <Step title="Publier sur l’App Store">
    Replit configure les certificats Apple pour vous. Lorsque l’étape **Ready for launch** est active, sélectionnez **Launch to App Store**. Replit crée l’application native dans le cloud et soumet une build à App Store Connect pour l’examen bêta de TestFlight.

    <Frame>
      <img src="/assets/mirror/app-store-06-ready-for-launch-ab02659ee0bb.png" alt="Boîte de dialogue Launch à l’étape Ready for launch, avec Choose Apple app et Configure Apple certificates indiqués comme terminés, le bouton Launch to App Store et des paramètres de lancement facultatifs" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-06-ready-for-launch.png" />
    </Frame>

    Une fois le lancement terminé, un message de confirmation indiquant que l’application est destinée à l’App Store s’affiche. La boîte de dialogue se ferme alors et vous revenez au Project Editor.

    <Frame>
      <img src="/assets/mirror/app-store-targeting-f8797071c9e0.png" alt="Boîte de dialogue Launch avec Targeting App Store indiqué comme terminé et le message Redirecting back to Replit — état de confirmation après la sélection de Launch to App Store" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-targeting.png" />
    </Frame>
  </Step>
</Steps>

<Note>
  Le premier Build d’une version doit généralement passer par l’examen bêta de TestFlight avant que les testeurs puissent l’installer. Après approbation, les Builds suivants sont généralement envoyés plus rapidement aux testeurs. Pour partager votre application avec des testeurs externes ou la soumettre pour publication sur l’App Store, complétez sa fiche dans **App Store Connect** — consultez [Créer et publier des applications mobiles](/fr/ko/build/mobile-app) pour la liste de contrôle complète d’App Store Connect.
</Note>
...
* L’application s’ouvre dans l’iOS Simulator ou l’Android Emulator.
* L’application s’ouvre sur votre téléphone avec Expo Go.
* Vous pouvez effectuer les principaux parcours utilisateur sur mobile.
* Le texte, les boutons et la navigation sont agréables à utiliser sur téléphone.
* Vous avez publié ou partagé une version mobile testable.
* Au moins une personne peut l’essayer et vous faire part de ses retours.
* (Facultatif) Vous avez soumis l’application à TestFlight via le processus de l’App Store.
...
<CardGroup cols={2}>
  <Card title="Ajouter une authentification" icon="user-lock" href="/fr/ko/build/add-login">
    Permettez à chacun de consulter sa progression.
  </Card>

  <Card title="Ajouter une base de données" icon="database" href="/fr/ko/build/add-database">
    Conservez vos données de course après votre première séance d’essai.
  </Card>

  <Card title="Ajouter des intégrations" icon="plug" href="/fr/ko/build/add-integrations">
    Connectez votre application à des notifications, à des données de santé ou à des services externes.
  </Card>

  <Card title="Créer et publier une application mobile" icon="mobile-screen" href="/fr/ko/build/mobile-app">
    Transférez votre application mobile fonctionnelle vers TestFlight et l’App Store.
  </Card>
</CardGroup>
...
* **Si Agent crée une application web :** recommencez et sélectionnez **application mobile** comme type d&#39;application avant de lancer le développement.
* **Si l&#39;aperçu Expo Go n&#39;est pas à jour :** secouez votre téléphone, ouvrez le menu développeur et sélectionnez **Refresh**.
* **Si les zones tactiles semblent trop petites :** demandez à Agent d&#39;agrandir les zones tactiles et de garder les actions principales à portée du pouce.
* **Si la navigation semble complexe :** demandez à Agent de réduire votre application au nombre minimal d&#39;écrans nécessaires aux principaux parcours.
* **Si le simulateur fonctionne, mais pas le téléphone :** testez les comportements spécifiques au matériel sur un appareil réel et consultez les [applications mobiles natives](/fr/ko/features/artifact-types/building-mobile-apps).
* **Si la publication échoue :** consultez le [dépannage des applications mobiles](/fr/ko/features/troubleshooting/mobile-app).

<div id="section-14">
  ## Essayez dès maintenant
</div>

<AgentInput highlightStart defaultPrompt="Créez Pace Mobile, une application de suivi de course à pied pensée d’abord pour les téléphones. Le coureur doit pouvoir choisir un type de course, saisir la distance et le temps, enregistrer sa course et consulter le résumé de la journée. Concevez-la pour une utilisation à une main, avec de grandes zones tactiles, une navigation simple et l’aspect soigné d’une application de fitness. Veillez à ce que la première version soit suffisamment simple pour être testée dès aujourd’hui dans un simulateur et sur un vrai téléphone." showTypewriter={false} />

<div id="section-15">
  ## Articles associés
</div>

* [Applications mobiles natives](/fr/ko/features/artifact-types/building-mobile-apps)
* [Créer et publier des applications mobiles](/fr/ko/build/mobile-app)
* [Vibe coding 101](/fr/ko/learn/foundations/vibe-coding-101)
* [Résoudre les problèmes liés aux applications mobiles](/fr/ko/features/troubleshooting/mobile-app)