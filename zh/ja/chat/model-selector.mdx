---
title: "选择模型"
description: "如果你当前的模式和套餐支持手动选择模型，可选择可用模型并设置其 Effort。"
sidebarTitle: "选择模型"
tag: "New"
icon: "microchip"
...
<div className="chat-mkt" role="img" aria-label="表示模型选择器的 Anthropic、OpenAI 和 Qimi 标志" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
使用智能模型路由时，Replit 会根据任务自动选择模型。如需手动选择模型，请在提示词输入框中选择**Agent Mode**，再选择**Power**或**Max**，然后在**Primary model**部分选择所需模型。

Free Mode 始终使用智能模型路由。Core 和 Pro 用户可以选择 Power Mode 或 Max Mode 中可用的模型。可选模型会因你的使用情况而异，因此 Replit 中的模型列表是最准确的账号适用信息来源。

<Frame caption="Agent 设置菜单中的模型和 Effort 控件。">
  <img src="/assets/mirror/model-selector-gpt-5-6-sol-effort-8e4d256bf497.jpg" alt="演示工作区中的模型选择器。Max Mode 已选择 GPT-5.6 Sol，Effort 设置为 Medium" width="1821" height="864" data-path="images/chat/model-selector-gpt-5-6-sol-effort.jpg" />
</Frame>
...
Effort 可控制所选模型生成回复时投入的推理程度。这不是另一种模式，而是针对各个模型的控制选项。支持的级别从**Low**到**Max**，并会为每个模型显示推荐设置。

仅当任务需要更深入的推理时，才使用更高的 Effort。提高设置后，复杂任务可能需要更长时间，成本也可能更高。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/ja/features/agent/model-selector">
    ...
    <p>
      了解智能模型路由、可用模型、Effort 级别以及 Compare models。
    </p>

    <div className="chat-card-link">打开参考文档 →</div>
  </a>

  <a className="chat-example has-visual" href="/zh/ja/features/agent/model-selector#effort">
    ...
    <div className="chat-card-title">Effort 详解</div>
    <p>比较 Low、Medium、High、Extra High 和 Max。</p>
    <div className="chat-card-link">查看 Effort →</div>
  </a>
</section>

模型和 Effort 设置会影响 Replit 的响应方式。[语音模式](/zh/ja/chat/voice-mode) 让你可以通过说话而非输入文字来传达请求。