---
title: "Replit Agent"
description: "Replit Agent 只需简单的自然语言，就能将你的想法变成应用、设计、演示文稿等，无需编写代码。"
---
...
Agent 是你的创意伙伴。它接收你的想法，帮你打磨完善，并将其变为现实。与只会回答问题的聊天机器人不同，Agent 会真正动手：配置项目、创建应用、检查成果，并在此过程中修复问题。

用日常语言描述你想要的东西即可，无需编写代码或掌握技术知识。从规划到部署，其余的都交给 Agent。
...
<Steps>
  <Step title="描述你想要什么">
    在[项目编辑器](/zh/pt/learn/projects-and-artifacts/project-editor)中，直接开始对话即可。描述你想构建的应用、提出问题、研究某个主题，或从 BigQuery、Slack、Notion 等[已连接的服务](/zh/pt/features/agent/general-agent)获取数据。没有任何限制，无论你提出什么需求，Replit Agent 都能应对。

    <AgentInput />
  </Step>

  <Step title="选择要构建的内容">
    你也可以选择一种 项目 类型：Web 应用、移动应用、演示文稿、design、数据可视化等等。如果你已经在第 1 步中描述了需求，Agent 会自动判断出合适的配置。

    <AgentCarousel />
  </Step>

  <Step title="Agent 开始构建">
    Agent 会编写代码、配置基础设施并测试结果。进入 项目 后，你可以在 [Agent 模式](#agent-modes)之间切换，以控制它的构建方式。
  </Step>

  <Step>
    与 Agent 对话来完善你的 项目，准备就绪后即可发布。所有 artifact 会一起发布。
  </Step>
</Steps>

<Note>你并不会被局限于某一种类型。可以先从网页应用做起，之后再添加移动应用、幻灯片或视频——全部都在同一个 项目 中。</Note>

<div id="modo-plan">
  ### Plan 模式
</div>

启用 Plan 模式，在 Agent 改动任何代码或数据之前，先进行头脑风暴、提问并梳理项目思路。在 Plan 模式下，Agent 会：

* **拆解复杂项目**，形成有序的任务列表
* **探索不同方案**并权衡利弊
* **在动手写代码前**先审阅并完善计划

在聊天输入框中点击 &quot;Plan&quot;，或直接告诉 Agent，即可切换到 Plan 模式。例如：&quot;为我的团队制定一个构建项目跟踪器的计划&quot;

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent 会创建一个有序的任务列表，供你审阅和完善。对计划满意后，批准该计划，Agent 便会开始构建。

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="已创建任务计划的对话框，显示四项建议任务以及「接受任务」和「修改计划」选项" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

更多内容请参阅 [Plan 模式指南](/zh/pt/features/agent/plan-mode)。
...
<Frame caption="包含 Free、Power 和 Max 模式的 Agent 模式弹出框。">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Agent 模式弹出框，显示 Free、Power 和 Max 模式，其中已选中 Power，并带有 Primary model 列表和 Effort 滑块" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**：面向 Core 和 Pro 订阅用户的免费日常 Agent 工作。额度每五小时重置一次，并设有每周上限。Starter 层级的限额保持不变。可在 **设置 → 用量** 中查看你的使用情况。
* **Power**：付费的中间层级模式，以实惠的价格提供强大的模型。
* **Max**：付费的顶级模式，配备最出色的前沿模型，适合最具挑战性的工作。在 **高级设置** 中，你还可以开启 **Turbo** 以加快运行速度。

如果某个请求所需的能力超出 Free Mode 的范围，Agent 可能会提供 **允许一次** 的付费选项。付费操作开始前，Replit 会先请你确认，完成后 Agent 会回到 Free Mode。

<Note>提示：你也可以开启 **[Plan 模式](/zh/pt/features/agent/plan-mode)**，在构建开始前审阅并迭代 Agent 的计划。</Note>

<div id="o-que-voc-pode-construir-com-o-replit-agent">
  ## 你可以用 Replit Agent 构建什么
</div>

* **Web 应用、移动应用、数据仪表板和 AI 工具**
* **视觉设计与原型** —— 在正式编写代码之前，先在 [Design Canvas](/zh/pt/design/canvas) 中探索设计稿
* **一个项目，多种产出** —— Web 应用、移动应用、演示文稿和视频共用同一个后端
* **文件与文档** —— CSV、PDF、PowerPoint 文件、Markdown 文档
* **查询已连接的服务** —— 直接在 Chat 中获取来自 BigQuery、Linear、Slack、Notion 等服务的数据
...
<Accordion title="我需要会编程吗？">
  不需要。Agent 会包办所有技术工作——编写代码、配置基础设施、搭建数据库。你只需用日常语言描述想要的效果，Agent 就会帮你构建出来。
</Accordion>

<Accordion title="Free、Power 和 Max 有什么区别？">
  **Free Mode** 供 Core 和 Pro 订阅用户在配额范围内免费处理 Agent 的日常工作。**Power** 是中间层的付费模式，以实惠的价格提供强大的模型。**Max** 是顶级付费模式，适合最具挑战性的工作。**Turbo** 是高级设置中 Max 专属的可选开关。

  付费操作在开始前需要确认。
</Accordion>

<Accordion title="我可以在一个 项目 中构建多个内容吗？">
  可以。你可以将 Web 应用、移动应用、演示文稿、视频和数据可视化都添加到[同一个 项目](/zh/pt/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) 中——它们共享同一个后端和同一份数据。
</Accordion>

<Accordion title="如果 Agent 出错了怎么办？">
  Agent 会定期测试自己的工作成果，并在工作过程中创建 checkpoint，方便你回滚到之前的任意状态。你也可以直接与 Agent 对话，描述出了什么问题，它会把问题修复。
</Accordion>
...
Turbo 默认关闭——选择 Max 后，可在需要时按项目单独启用。Free Mode 在首发时暂不支持 Enterprise。Enterprise workspace 稍后将获得专属的 Agent 体验，具体信息请咨询管理员。

详细定价请参阅 [Agent 计费](/zh/pt/billing/ai-billing#agent-billing)。
...
<CardGroup cols={2}>
  <Card title="Agent Skills" icon="puzzle-piece" href="/zh/pt/features/agent/skills">
    让 Agent 掌握专业知识——使用现成的 skills，或创建你自己的。
  </Card>

  <Card title="任务系统" icon="list-check" href="/zh/pt/core-concepts/agent/task-system">
    看板式规划、后台任务，以及合并回主版本的更改。
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/zh/pt/design/canvas">
    可视化原型稿与好用的可视化编辑器。
  </Card>

  <Card title="Connectors" icon="plug" href="/zh/pt/features/agent/general-agent">
    连接 BigQuery、Linear、Slack、Notion 等服务。
  </Card>

  <Card title="计费" icon="credit-card" href="/zh/pt/billing/ai-billing#agent-billing">
    Agent 定价、套餐对比与支出管理。
  </Card>

  <Card title="Vibe coding 指南" icon="wand-magic-sparkles" href="/zh/pt/learn/build-with-agent">
    高效编写 prompt 与借助 AI 构建应用的技巧。
  </Card>
</CardGroup>