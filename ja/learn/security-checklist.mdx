---
title: "セキュリティチェックリスト"
description: "バイブコーディングアプリがセキュリティのベストプラクティスに従っているかを確認するための包括的なセキュリティチェックリスト。"
sidebarTitle: "セキュリティチェックリスト"
icon: "clipboard-check"
---

このガイドでは、バイブコーディングアプリがセキュリティのベストプラクティスに従っているかを確認するための包括的なセキュリティチェックリストを提供します。

Replitは多くのセキュリティ機能を[標準で](/ja/features/security/overview)提供していますが、特定のアプリケーションのニーズに合わせてより多くのセキュリティ対策を理解・実装することが重要です。

<iframe src="https://www.youtube.com/embed/0D9FMFyNBWo" title="バイブコードセキュリティチェックリスト" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen />
...
<AccordionGroup>
  ...
  <Accordion title="入力バリデーションとサニタイズ" icon="shield-check">
  ...
  <Accordion title="機密データをブラウザに置かない" icon="eye-slash">
    APIキーなどの機密情報の保存にはReplit Secretsを使用してください。

    シークレットをクライアントサイドに渡したり、以下の場所に置かないようにしてください：
  ...
  <Accordion title="CSRF対策" icon="shield">
...
<AccordionGroup>
  <Accordion title="認証の基本" icon="key">
    認証を実装する際：
  ...
  <Accordion title="認可チェック" icon="user-lock">
  ...
  <Accordion title="APIエンドポイントの保護" icon="shield">
    APIエンドポイントを安全にする：
  ...
  <Accordion title="SQLインジェクション対策" icon="database">
  ...
  <Accordion title="セキュリティヘッダー" icon="code">
    ...
    [securityheaders.com](https://securityheaders.com)でサイトをスキャンして推奨事項を確認できます。
...
<AccordionGroup>
  <Accordion title="依存関係を最新の状態に保つ" icon="arrow-rotate-right">
  ...
  <Accordion title="適切なエラー処理" icon="exclamation">
  ...
  <Accordion title="Cookieのセキュリティ" icon="cookie">
    Cookieを使用する際：

    * JavaScriptアクセスを防ぐためにHttpOnlyフラグを設定する
    * HTTPSを必須にするためにSecure属性を使用する
    * CSRFを防ぐためにSameSite属性を実装する
  ...
  <Accordion title="ファイルアップロードのセキュリティ" icon="upload">
  ...
  <Accordion title="レート制限" icon="clock">
