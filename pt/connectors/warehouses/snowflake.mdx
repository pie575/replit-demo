---
title: "Conectores Snowflake"
description: "Configure um conector Snowflake para criar aplicativos orientados a dados com o Replit Agent."
...
Os Conectores de Warehouse permitem que o Replit Agent consulte os data warehouses da sua organização. Com o conector Snowflake, os desenvolvedores podem criar dashboards, ferramentas de relatórios e aplicativos de dados usando linguagem natural — sem necessidade de conhecimento em SQL.

A configuração é um processo único que requer um administrador do Snowflake e um administrador do workspace Replit. Uma vez configurado, qualquer membro da organização pode fazer login e começar a criar com os dados do Snowflake.

Este guia orienta você pela configuração completa. Três funções estão envolvidas:
...
<Steps>
  <Step title="Fazer login no Snowflake">
  ...
  <Step title="Abrir uma planilha SQL">
    Clique em **Planilhas** na barra lateral esquerda e crie uma nova planilha.
  </Step>

  <Step title="Executar o script de integração OAuth">
    Copie e cole o seguinte script SQL e execute-o:

    ```sql Criar integração OAuth theme={null}
  ...
  <Step title="Recuperar suas credenciais OAuth">
    Execute esta consulta para obter o Client ID e o Client Secret:

    ```sql Obter credenciais OAuth theme={null}
...
<Steps>
  <Step title="Encontrar o conector Snowflake">
  ...
  <Step title="Inserir seu ID de conta Snowflake">
    O prompt OAuth solicita seu ID de conta do Snowflake.

    Para encontrar seu ID de conta, observe a URL do seu Snowflake:
...
<AccordionGroup>
  <Accordion title="Erro de solicitação de consentimento inválida ou função bloqueada">
    Sua integração Snowflake pode ter funções bloqueadas por padrão.

    Verifique se a integração foi criada com `BLOCKED_ROLES_LIST = ()`. Se você já a criou sem essa configuração, execute:
  ...
  <Accordion title="Erro 404 na autenticação">
  ...
  <Accordion title="Erro de escopo inválido">
  ...
  <Accordion title="Erro de falha na conexão">
...
* [Visão geral dos conectores de warehouse](/pt/connectors/warehouses/overview) — Visão geral de todos os conectores de warehouse suportados
...
* [Snowflake — Configurar OAuth para clientes personalizados](https://docs.snowflake.com/en/user-guide/oauth-custom) — Documentação OAuth do Snowflake
