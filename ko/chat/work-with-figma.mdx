---
title: "Figma로 작업하기"
description: "Figma 프레임을 시각적 컨텍스트로 사용한 다음, 이를 탐구하거나 다듬거나 그로부터 빌드하고 싶을 때 Replit Design의 가져오기 플로우를 따르세요."
...
Replit은 Figma 프레임 가져오기를 지원합니다.

이미 시각적 방향이 있고 그 작업을 바탕으로 Replit이 시작하거나 디자인으로 전환하기를
원할 때 Figma 프레임을 가져오세요.

## Figma가 유용한 경우

* **다른 방향 탐구하기:** 핵심 구조는 유지하면서 다른 스타일이나 레이아웃을 시도합니다.
* **제품 플로우 이어가기:** 기존 화면을 다음 화면이나 상태의 컨텍스트로 사용합니다.
* **디자인 작업을 앱으로 발전시키기:** 승인된 시각적 방향에서 시작한 다음 이를 다듬고
  빌드합니다.

## Figma 프레임 가져오기

이어서 작업하고 싶은 프레임을 선택하세요. Replit Design은 가져온 프레임을 디자인
프레임으로 전환해서, 변경 사항을 탐구하고, 제안을 비교하고, 다음에 무엇을 빌드할지
결정할 수 있게 해줍니다.

<Frame caption="Replit Design에서 디자인 프레임으로 가져온 Figma 프레임.">
  <img src="/assets/mirror/figma-import-result-052394025b8d.webp" alt="Replit Design에서 디자인 프레임으로 표시된, 가져온 Personal Fitness Trainer 랜딩 페이지" width="3440" height="2160" data-path="images/fitstart/turn-mockup-into-app/figma-import-result.webp" />
</Frame>

## Design에서 계속 진행하기

Design 가이드는 전체 가져오기, 다듬기, 빌드 워크플로를 다룹니다.
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ko/design/import-figma-designs-into-replit-design">
    ...
    <p>Figma 프레임을 가져오고 다듬는 전체 플로우를 따라 해보세요.</p>
