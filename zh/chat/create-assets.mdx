---
title: "创建资源"
description: "创建图像、视频、矢量图形、动画和音频资源，并参考各工作流程的权威指南。"
sidebarTitle: "创建资源"
icon: "photo-film"
...
让 Replit 创建工作所需的媒体资源，然后在 Conversation 或承载该工作流程的产品界面中进一步优化结果。选择符合预期成果的资源路径。
...
<section className="chat-mkt chat-workflow-grid has-visuals catalog-three-up">
  <a className="chat-example has-visual" href="/zh/features/agent/image-generation">
    ...
    <p>
      为 Project 创建自定义图像、插画、背景、icon 及其他视觉内容。
    </p>

    <div className="chat-card-link">打开图像生成 →</div>
  </a>

  <a className="chat-example has-visual" href="/zh/design/toolbar#generate">
    ...
    <p>
      使用 Replit Design 的媒体生成工具创建短动态片段和动画。
    </p>
  ...
  <a className="chat-example has-visual" href="/zh/design/toolbar#generate">
    ...
    <p>
      在 Replit Design 中创建 SVG icon、徽标、图表和风格化插画。
    </p>

    <div className="chat-card-link">打开 Design 指南 →</div>
  </a>

  <a className="chat-example has-visual" href="/zh/features/artifact-types/animated-videos">
    ...
    <p>
      通过 Conversation 制作程序化 motion graphics，并将成果导出为视频。
    </p>
  ...
  <a className="chat-example has-visual" href="/zh/features/agent/audio-generation">
    ...
    <p>为应用创建音乐、音效或语音旁白。</p>
    <div className="chat-card-link">打开音频生成 →</div>
  </a>
</section>

资源创建会立即产出结果。如果某项工作需要按 schedule 重复执行，请继续阅读[运行例行 task](/zh/chat/routines)。

生成的视频片段和动画视频采用不同的工作流程。动画视频是可导出为 MP4 的编程式动态图形。有关视觉资源示例，请参阅[设计你的第一个界面](/zh/design/design-your-first-screen)。

<div id="bring-in-current-information">
  ## 获取最新信息
</div>

网页搜索不会创建资源。当需要根据最新信息生成图像、音频片段、视频或图形时，请先使用此功能，再让 Replit 根据这些信息创建相应资源。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/features/agent/web-search">
    ...
    <div className="chat-card-title">搜索网页</div>
