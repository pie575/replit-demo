...
URL에 다음 파라미터를 반드시 포함하세요.

| 파라미터       | 유형     | 설명                                                                                                                                                                       | 필수/선택 |
| ---------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| `stack`    | string | 사용할 빌드 모드입니다. `Design` 또는 `Build` 중 하나여야 합니다. `Design`은 시각적 프로토타이핑을 위한 [Canvas](/ko/design/canvas)를 엽니다. `Build`는 전체 앱 개발을 위한 [Agent](/ko/features/agent/overview)를 엽니다. | 필수    |
| `prompt`   | string | 빌드할 애플리케이션을 설명하는 텍스트로, **[LZ-string](https://pieroxy.net/blog/pages/lz-string/index.html)으로 압축되어** 있습니다. 압축되지 않은 프롬프트는 최대 50,000자까지 가능합니다.                               | 필수    |
| `referrer` | string | 이 링크가 포함된 페이지의 이름 또는 URL입니다. 어트리뷰션에 사용됩니다.                                                                                                                               | 선택    |

<Note>
  `prompt` 파라미터는 URL에 추가하기 전에 LZ-string 압축으로 압축해야
  합니다. 이렇게 하면 URL이 브라우저의 길이 제한 내에 유지되고 올바르게
  구문 분석됩니다.
</Note>

### 대화형 링크 빌더

아래 도구를 사용해 **Open in Replit** 링크를 생성하세요. 프롬프트를 입력하고 스택 모드를 선택하면 LZ-string 압축이 적용된 링크가 자동으로 생성됩니다.

<CreateUrlLinkBuilder />

### 프롬프트 압축하기

유효한 **Open in Replit** 링크를 만들려면 LZ-string 압축으로 프롬프트를 압축해야 합니다. 방법은 다음과 같습니다.
...
<Tip>
  압축된 문자열이 URL에 안전하도록 `compressToEncodedURIComponent`를
  사용하세요.
</Tip>
...
`stack=Design`을 사용해 [Canvas](/ko/design/canvas)에서 프롬프트를 여세요. 다음에 최적화되어 있습니다.
...
`stack=Build`를 사용해 [Agent](/ko/features/agent/overview)에서 프롬프트를 여세요. 다음에 최적화되어 있습니다.
...
* 최상의 생성 결과를 위해 프롬프트를 간결하게 유지하고 핵심 기능에 집중하세요.
* 빌드하고 싶은 것을 명확하게 설명하는 언어를 사용하세요.
* 사용 사례에 맞는 `stack` 모드를 선택하세요(UI/UX에는 Design, 전체 앱에는 Build).
* 공유하기 전에 압축된 URL이 올바르게 구문 분석되는지 테스트하세요.
* 프롬프트를 작성할 때 대상 독자의 기술 수준을 고려하세요.

### 사용자 인증

Replit 인증 상태와 관계없이, 앱을 만들기 전에 입력을 제공해야 합니다. 보통 이는 프롬프트를 제출하는 것을 의미합니다.

### 오류 처리

URL 형식이 잘못되었거나 파라미터가 유효하지 않으면 Replit이 프롬프트를 채우지 않습니다. 가능한 원인은 다음과 같습니다.

* 필수 파라미터(`stack` 또는 `prompt`) 누락
* 유효하지 않은 `stack` 값(`Design` 또는 `Build`여야 함)
* `prompt` 파라미터의 압축 해제 실패
* 브라우저의 "URL too long" 오류(압축이 이를 완화하는 데 도움이 되지만)

### 보안 고려사항

<Warning>
  **Open in Replit** 링크는 의도치 않은 동작이나 보안 취약점이 있는 앱을 만드는 데
  사용될 수 있습니다. 신뢰할 수 있는 소스의 링크만 클릭하세요.
</Warning>
