---
title: "도구 연동하기"
description: "Replit이 커피숍 출시 계획을 세우는 데 필요한 컨텍스트를 제공하는 파일과 도구를 연결하세요."
...
integrations에 대해 알아봤으니, 이제 커피숍 launch에 적용해 봅시다.

방향은 정해졌습니다. 역 앞 출퇴근길 매장이 아니라 **동네 카페**를 만드는 것입니다. Replit이 그 direction을 실행 가능한 plan으로 바꿀 수 있도록 필요한 자료를 제공하세요.

Integrations는 이미 사용 중인 도구와 파일을 Replit에 연결해 줍니다. 덕분에 Replit은 관련 정보를 가져오거나 task에 필요한 작업을 수행할 수 있습니다.
**[Slack](/ko/features/integrations/overview#connectors)**,
**[Google Drive](/ko/features/integrations/overview#connectors)**,
**[Notion](/ko/features/integrations/overview#connectors)**,
**[Salesforce](/ko/features/integrations/overview#connectors)** 같은 서비스는 물론 450개가 넘는 서비스를 연결할 수 있습니다.

커피숍의 경우, Google Drive를 통해 Replit이 예산, 입지 메모, 메뉴 아이디어, 사진, 시각 자료에 접근할 수 있습니다. Notion은 launch plan과 결정 사항, 미해결 질문을 기록할 공간이 되어 줍니다.

[도구 연동](/ko/chat/integrate-your-tools)에 대해 자세히 알아보세요.

<Steps>
  <Step title="Replit에 자료를 모아달라고 요청하기">
    아래 요청으로 시작하세요. Replit은 plan을 완성하는 데 무엇이 필요한지 확인하고, 접근 권한이 필요하면 서비스 연결을 요청합니다.

    <AiPrompt>
      내 카페 파일을 사용해 동네 카페의 첫 달 launch plan을 만들어 줘. 관련 가정을 업데이트한 다음, 콘셉트, 주요 결정 사항, 미해결 질문, launch 체크리스트를 Notion에 정리해 줘.
    </AiPrompt>
  </Step>

  <Step title="Replit이 요청하면 Google Drive 연결하기">
    Google Drive 연결을 승인하면 Replit이 사용하길 원하는 자료를 살펴볼 수 있습니다. 연결 요청은 Conversation 안에서 바로 열립니다.

    <CoffeeShopScreenshotFrame src="/assets/mirror/connect-google-drive-f7951617b078.png" alt="카페 파일을 확인하기 위해 Google Drive 연결을 요청하는 Replit" />
  </Step>

  <Step title="Replit이 plan을 문서화해야 할 때 Notion 연결하기">
    Replit이 launch brief를 만들기 위해 요청하면 Notion을 승인하세요. 그러면 콘셉트, 결정 사항, 미해결 질문, 체크리스트를 한곳에 기록할 수 있습니다.
  </Step>
</Steps>

Google Drive는 Replit이 근거로 삼을 자료를 제공합니다. Notion은 새로운 질문에 답하고 결정을 내리면서 계속 바뀌는 plan을 한곳에 모아 둡니다.

[context 추가](/ko/chat/add-context), [파일 업로드](/ko/chat/upload-and-use-files), [도구 연동](/ko/chat/integrate-your-tools)에 대해 자세히 알아보세요.
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ko/home/watch-the-competition">
    ...
    <div className="chat-card-title">경쟁사 동향 주시하기</div>
    <p>출시를 준비하는 동안 변화를 감지하도록 Routine을 설정해 보세요.</p>
