---
title: "添加上下文"
description: "为 Replit 提供理解 Chat 对话所需的相关文件、来源、已连接的工具和已有成果。"
sidebarTitle: "添加上下文"
icon: "paperclip"
...
如果相关上下文和工具有助于 Replit 理解你的需求，就把它们添加进来。有效的上下文能让 Replit 更准确地把握你的意图，从而提升输出质量。

<div id="como-adiciono-contexto">
  ## 如何添加上下文？
</div>

在 prompt 输入框中点击 **加号 (+)** 菜单，即可上传文件、导入已有工作、使用 skill 或设计系统，或添加集成。

<Frame>
  <img src="/assets/mirror/prompt-box-plus-menu-8c1e2d807e0b.png" alt="prompt 输入框的加号 (+) 菜单，包含上传文件、新建内容、导入现有项目、使用 skill、使用设计系统和添加集成" width="1558" height="556" data-path="images/chat/prompt-box-plus-menu.png" />
</Frame>
...
你可以添加文件、屏幕截图和图片、网页来源与搜索结果，以及已连接的工具。请选择对当前任务有帮助的来源：

* **[文件、屏幕截图和图片](/zh/pt/chat/upload-and-use-files)：** 添加包含有用依据或需求的文档、电子表格、导出文件、屏幕截图或视觉参考。
* **[网页来源与搜索](/zh/pt/features/agent/web-search)：** 当最新信息或外部视角有助于改善结果时，让 Replit 在网上搜索。
* **[集成](/zh/pt/chat/integrate-your-tools)：** 连接你所需的工具，然后指明 Replit 应当参考的服务、频道、日志或其他信息。
* **[设计上下文](/zh/pt/chat/work-with-figma)：** 当布局、层级或视觉方向较为重要时，附上一个 Figma frame。
* **[已有工作](/zh/pt/chat/import-a-project)：** 当你想在别处创建的代码、设计或成果基础上继续构建时，导入一个 Project。导入 Project 与把文件作为依据附加上去并不相同。

你也可以在请求中粘贴相关链接，或让 Replit 在网上搜索。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/pt/chat/upload-and-use-files">
    <FilesCardVisual />
    ...
    <p>针对上传的文件提问，或将其作为依据。</p>
    <div className="chat-card-link">探索 →</div>
  </a>

  <a className="chat-example has-visual" href="/zh/pt/chat/work-with-figma">
    <FigmaCardVisual />
    ...
    <p>当视觉设计方向很重要时，使用特定 frame。</p>
    <div className="chat-card-link">探索 →</div>
  </a>

  <a className="chat-example has-visual" href="/zh/pt/chat/integrate-your-tools">
...
如需获取最新公开信息，请参阅[网页搜索指南](/zh/pt/features/agent/web-search)。[MCP](/zh/pt/chat/connect-through-mcp) 则是用于连接兼容工具和客户端的另一种扩展方式。