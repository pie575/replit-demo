---
title: "Agentと一緒に構築する"
description: "Agentと働く5つの実践的な習慣を学ぶ：具体的に伝える、作業を計画する、コンテキストを追加する、確認してテストする、チェックポイントを使う。"
sidebarTitle: "Agentと一緒に構築する"
icon: "robot"
---
...
AgentはReplitのAIビルダーで、アイデアをソフトウェアに変えるあなたのパートナーです。Agentは変更を計画し、コードを書き、動作を説明し、問題をデバッグし、アプリを改善できます。

[Vibe coding 101](/ja/learn/foundations/vibe-coding-101)はマインドセットを説明し、このページはAgentと実際にどのように取り組むかを説明します。[効果的なプロンプト](/ja/learn/effective-prompting)はより明確な指示の書き方を説明します。

Agentをフィールドのクォーターバックと考えてください：状況を読み取り、進む道を選び、作業を前進させます。あなたはコーチです：戦略を設定し、プレーを呼び出し、何が起こったかをレビューし、次の動きを決定します。

Agentは多くのことができますが、あなたがプロセスをリードするときに最もうまく機能します：具体的に伝える、作業を計画する、コンテキストを追加する、確認してテストする、回復が必要なときはチェックポイントを使う。

## Agentと構築するための5つのヒント

Agentと作業するときにこれらの習慣を使ってください：
...
Agentは複雑な作業を処理できますが、明確な方向性が必要です。

有用なリクエストは、何が欲しいか、何が重要か、何を変えてはいけないかを説明します。すべての実装の詳細を説明する必要はありませんが、Agentが良い決断を下せるだけの方向性を与える必要があります。
...
2番目のプロンプトはAgentに目標、フォーカスエリア、制約を与えています。

リクエストに複数の部分がある場合は、最も重要なものを伝えます。
...
Agentはその作業をステップに分解できますが、あなたの方向性が成功の形を伝えます。

具体的な指示のさらなる例については、[効果的なプロンプト](/ja/learn/effective-prompting)を参照してください。

## 作業を計画する

計画は、作業に不確実性、複数のステップ、トレードオフ、またはファイルが変更される前に承認したいものがある場合に役立ちます。

会話の中でAgentに計画を作るよう依頼できます：
...
Agentに先に考えて、ファイルを変更する前に承認を待ってほしい場合は、[Planモード](/ja/features/agent/plan-mode)を使います。プロンプトコンポーザーで送信する前に**Plan**をトグルします。

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="実行中のアプリのプロンプトが入力され、PlanチェックボックスがオンになっているReplitホームプロンプトボックス" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Agentが考え終わると、**タスクプランのレビューが準備できました**バナーと**今すぐレビュー**ボタンが表示されます。

<Frame>
  <img src="/assets/mirror/agent-plan-ready-banner-5fbcf58a1668.png" alt="Agentパネルの下部に「タスクプランのレビューが準備できました」バナーと「今すぐレビュー」ボタン" width="560" height="550" data-path="images/fitstart/work-with-agent/agent-plan-ready-banner.png" />
</Frame>

## コンテキストを追加する

コンテキストはAgentが何が重要かを理解するのに役立ちます。

テキスト、スクリーンショット、スケッチ、ファイル、データ、エラー、Canvas注釈でコンテキストを追加できます。問題を最もよく説明する形式を使います。

変更が動作、目標、制約に関するものの場合はテキストを使います：
...
レイアウトを言葉で説明するのが難しい場合はスクリーンショットやスケッチを使います：
...
変更がビジュアルで、アプリの特定の部分を指し示したい場合は[Canvas](/ja/design/canvas)注釈を使います。

<Frame>
  <img src="/assets/mirror/agent-canvas-annotations-c88ab5856966.png" alt="Canvas上のフィットネスアプリフレーム。長方形、矢印、プログレスカードを指し示すテキストラベル、フォームエリアにペンのスケッチとラベルがある" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-canvas-annotations.png" />
</Frame>
...
良いコンテキストには、変更すべきでないものも含まれます。これにより、Agentが1つの領域を改善しながら、誤って別の領域を書き直すのを防げます。

## 確認してテストする

Agentは構築前にプランを提案し、変更後に何が変わったかをサマリーする場合があります。両方を読んでください。

タスクプランカードの**表示**を選択してプランを開きます。展開されたプランは、Agentが構築しようとしているもの、成功の基準、スコープ外のもの、Agentが従う手順を示しています。

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="展開されたタスクプラン。What と Whyの説明、完了の基準、スコープ外の項目、番号付きの構築ステップが表示されている" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>

Agentが構築する前に、プランをレビューします：

* これは正しい問題を解決していますか？
* 何か欠けているものはありますか？
* スコープ外になっているものはありますか？
* まず構築できるより小さいバージョンはありますか？
* 結果をどのようにテストすべきかが説明されていますか？

プランが広すぎる場合は、Agentに縮小するよう依頼します。
...
Agentが構築した後、アプリを自分でテストします。Previewを開いて、それが対象とする人のように使ってみます。

確認すること：

* 誰かがメインのアクションを完了できますか？
* 要求した変更は行われましたか？
* アプリはモバイルでも機能しますか？
* 重要な動作が壊れていませんか？
* Agentが無関係なものを変更していませんか？
* 結果は続けて構築するのに十分ですか？
...
具体的なフィードバックはAgentが正しい問題を修正するのに役立ちます。

## チェックポイントを使う

チェックポイントは、変更が間違った方向に進んだときに回復するのに役立ちます。

結果が近い場合は、フォーカスしたフィードバックを使います：
...
アプリが以前より悪くなった、重要な動作が壊れた、またはAgentが予想以上に変更した場合はチェックポイントを使います。

AgentパネルでHistoryを開いてチェックポイントを確認します。各チェックポイントは何が変わったかを示し、**ここにロールバック**と**変更**のコントロールがあります。

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="「プランからビルドモードに移行」というラベルのチェックポイントが表示されているAgentHistoryパネル。「ここにロールバック」と「変更」ボタンがある" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

**ここにロールバック**を選択すると、Agentはロールバックを適用する前に何が復元されるかを確認します。

<Frame>
  <img src="/assets/mirror/agent-checkpoints-rollback-dialog-28de278b44da.png" alt="チェックポイントへのロールバック確認ダイアログ。対象チェックポイントと影響を受けるもの（ファイル、Agentメモリ、タスク、データベースチェックボックス）がリストされている" width="900" height="880" data-path="images/fitstart/work-with-agent/agent-checkpoints-rollback-dialog.png" />
</Frame>

前進修正とロールバックのどちらにすべきか迷っている場合は、Agentに比較を手伝うよう依頼します。
...
間違った方向への進行は正常です。チェックポイントにより、より安全なバージョンに戻る能力を失わずに探索できます。

さらなる回復のガイダンスについては、[チェックポイントとロールバック](/ja/features/version-control/checkpoints-and-rollbacks)を参照してください。

## 良いAgentとのコラボレーションとは

以下の状態であれば、正しい軌道に乗っています：

* リクエストが目標と制約を説明している
* Agentのプランがレビューできるほど明確
* 言葉では不十分な場合はスクリーンショット、ファイル、Canvas注釈を追加している
* 変更後に重要なフローをテストしている
* 何かがおかしい場合はフォーカスしたフィードバックを提供している
* 悪い変更を永遠にほぐそうとする代わりにチェックポイントを使っている

## 次のステップ

<CardGroup cols={2}>
  <Card icon="comment-dots" href="/ja/learn/effective-prompting">
    Agentが自信を持って行動できるより明確な指示の書き方を学びます。
  </Card>

  <Card title="最初のアプリを構築する" icon="rocket" href="/ja/build/your-first-app">
    最初のプロンプトから焦点を絞った改善まで、Agentとの作業を練習します。
  </Card>

  <Card title="チェックポイントとロールバック" icon="rotate-left" href="/ja/features/version-control/checkpoints-and-rollbacks">
    必要なときにより安全な状態に戻る方法を学びます。
  </Card>
</CardGroup>
