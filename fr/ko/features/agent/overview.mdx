---
title: "Replit Agent"
description: "Replit Agent transforme vos idées en applications, designs, diapositives et bien plus encore. Le tout en langage courant, sans écrire une seule ligne de code."
---
...
<YouTubeEmbed videoId="bAUVQfz_SIY" title="Présentation de Replit Agent 4" />

<div id="replit-agent">
  ## Qu&#39;est-ce que Replit Agent ?
</div>

Agent est votre partenaire de création. Il vous aide à préciser et à affiner vos idées, puis les concrétise. Contrairement à un chatbot qui se contente de répondre à vos questions, Agent passe réellement à l&#39;action : il configure votre projet, crée l&#39;application, vérifie son travail et résout les problèmes rencontrés en cours de route.

Décrivez ce que vous voulez avec vos mots de tous les jours. Aucune compétence en code ni connaissance technique n&#39;est nécessaire. Agent se charge de tout le reste, de la planification au déploiement.

<div id="replit-agent-2">
  ## Comment utiliser Replit Agent
</div>
...
<Steps>
  <Step title="Décrivez ce que vous voulez">
    Lancez une conversation directement dans le [Project Editor](/fr/ko/learn/projects-and-artifacts/project-editor). Décrivez l&#39;application que vous souhaitez créer, posez une question, explorez un sujet ou récupérez des données depuis des [services connectés](/fr/ko/features/agent/general-agent) comme BigQuery, Slack ou Notion. Il n&#39;y a aucune limite : Replit Agent peut tout gérer.

    <AgentInput />
  </Step>

  <Step title="Choisissez ce que vous voulez créer">
    Vous pouvez aussi choisir un type de projet : application web, application mobile, diapositives, design, visualisation de données, etc. Si vous avez déjà décrit ce que vous vouliez à l&#39;étape 1, Agent détermine automatiquement la configuration adaptée.

    <AgentCarousel />
  </Step>

  <Step title="Agent se charge du build">
    Agent écrit le code, configure l&#39;infrastructure et teste le résultat. Une fois dans le projet, vous pouvez changer de [mode Agent](#agent-modes) pour contrôler sa façon de construire.
  </Step>

  <Step>
    Échangez avec Agent pour peaufiner votre projet, puis publiez-le lorsque vous êtes prêt. Tous les artifacts sont publiés ensemble.
  </Step>
</Steps>

<Note>Vous n&#39;êtes pas limité à un seul type. Vous pouvez commencer par une application web et ajouter plus tard une application mobile, des diapositives ou une vidéo — le tout au sein du même projet.</Note>

<div id="plan">
  ### Mode Plan
</div>

Activez le mode Plan pour permettre à Agent de réfléchir à des idées, de poser des questions et de planifier votre projet avant de modifier le code ou les données. En mode Plan, Agent :

* **décompose les projets complexes** en une liste de tâches ordonnée
* **explore différentes approches** et en évalue les avantages et les inconvénients
* **révise et améliore** le projet avant d’écrire du code

Cliquez sur « Plan » dans le champ de saisie du chat ou demandez directement à Agent de passer en mode Plan. Exemple : « Établis un plan pour créer un outil de suivi de projet pour notre équipe »

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent génère une liste de tâches ordonnée que vous pouvez examiner et affiner. Si le plan vous convient, approuvez-le pour qu’Agent commence la création.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="Boîte de dialogue de génération d’un plan de tâches affichant quatre tâches proposées ainsi que les options d’acceptation des tâches (Accept tasks) et de révision du plan (Revise plan)" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

Pour en savoir plus, consultez le [guide du mode Plan](/fr/ko/features/agent/plan-mode).

<div id="agent">
  ### mode Agent
</div>

Choisissez la façon dont vous souhaitez créer votre projet :

<Frame caption="Fenêtre contextuelle du mode Agent avec les modes Free, Power et Max">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Fenêtre contextuelle du mode Agent affichant les modes Free, Power et Max avec Power sélectionné, la liste des modèles par défaut et le curseur Effort" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode** : gratuit pour les tâches Agent quotidiennes des abonnés Core et Pro. Les limites d&#39;utilisation sont réinitialisées toutes les 5 heures et une limite hebdomadaire s&#39;applique. Le niveau Starter conserve ses limites existantes. Consultez votre consommation dans **Paramètres → Utilisation**.
* **Power** : mode payant de niveau intermédiaire offrant des modèles puissants à un prix raisonnable.
* **Max** : mode payant de niveau supérieur offrant les modèles de pointe les plus performants pour les tâches les plus exigeantes. Vous pouvez également activer **Turbo** dans les **Paramètres avancés** pour une exécution plus rapide.

Si votre demande nécessite plus de puissance que Free Mode, Agent peut vous proposer l&#39;option payante **Autoriser une fois**. Replit vous demande confirmation avant le démarrage de toute tâche payante, puis Agent revient à Free Mode une fois la tâche terminée.

<Note>Astuce : vous pouvez également activer le **[mode Plan](/fr/ko/features/agent/plan-mode)** pour examiner le plan d&#39;Agent et l&#39;affiner avant le lancement du build.</Note>

<div id="replit-agent-3">
  ## Ce que vous pouvez créer avec Replit Agent
</div>

* **Applications web, applications mobiles, tableaux de bord de données et outils basés sur l&#39;IA**
* **Designs visuels et prototypes** — explorez des maquettes dans le [Canvas Design](/fr/ko/design/canvas) avant de passer au code
* **Plusieurs livrables à partir d&#39;un seul projet** — applications web, applications mobiles, diapositives et vidéos partageant le même backend
* **Fichiers et documents** — fichiers CSV, PDF, PowerPoint et documents Markdown
* **Consultation de services connectés** — récupérez des données depuis BigQuery, Linear, Slack, Notion et bien d&#39;autres, directement dans le chat

<Card title="Commencez à créer dès maintenant" icon="arrow-up-right" href="https://replit.com">
  Décrivez votre idée et laissez Agent la concrétiser — aucune configuration nécessaire.
</Card>
...
<Accordion title="Dois-je savoir coder ?">
  Non. Agent se charge de tout l&#39;aspect technique : écriture du code, mise en place de l&#39;infrastructure, configuration de la base de données, etc. Décrivez ce que vous souhaitez en langage courant, et Agent le crée pour vous.
</Accordion>

<Accordion title="Quelle est la différence entre Free, Power et Max ?">
  **Free Mode** est le mode Agent destiné aux tâches courantes, offert gratuitement aux abonnés Core et Pro dans la limite de leur allocation. **Power** est un mode payant de gamme intermédiaire qui propose des modèles puissants à un prix raisonnable. **Max** est le mode payant haut de gamme, conçu pour les tâches les plus exigeantes. **Turbo** est une option réservée à Max, disponible dans les paramètres avancés.

  Les tâches payantes nécessitent une confirmation avant de démarrer.
</Accordion>

<Accordion title="Puis-je créer plusieurs éléments dans un même projet ?">
  Oui. Vous pouvez ajouter une application web, une application mobile, des diapositives, des vidéos et des visualisations de données [au sein d&#39;un même projet](/fr/ko/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) — le tout partageant le même backend et les mêmes données.
</Accordion>

<Accordion title="Que se passe-t-il si Agent commet une erreur ?">
  Agent teste régulièrement son propre travail. Il crée également des points de contrôle au fil de sa progression, ce qui vous permet de revenir à un état antérieur. Vous pouvez aussi dialoguer avec Agent en lui expliquant le problème : il se chargera de le corriger.
</Accordion>
...
Turbo est désactivé par défaut — vous pouvez l&#39;activer projet par projet, selon vos besoins, lorsque Max est sélectionné. Free Mode n&#39;est pas disponible sur Enterprise au lancement. Les espaces de travail Enterprise bénéficieront prochainement d&#39;une expérience Agent dédiée : vos administrateurs vous en communiqueront les détails.

Pour en savoir plus sur la tarification, consultez [Facturation d&#39;Agent](/fr/ko/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  <Card title="Compétences Agent" icon="puzzle-piece" href="/fr/ko/features/agent/skills">
    Enseignez à Agent des connaissances spécialisées : utilisez des compétences prédéfinies ou créez les vôtres.
  </Card>

  <Card title="Système de tâches" icon="list-check" href="/fr/ko/core-concepts/agent/task-system">
    Planification Kanban, tâches en arrière-plan et modifications réintégrées à la branche principale.
  </Card>

  <Card title="Canvas Design" icon="paintbrush" href="/fr/ko/design/canvas">
    Maquettes visuelles et éditeur graphique à manipulation directe.
  </Card>

  <Card title="Connectors" icon="plug" href="/fr/ko/features/agent/general-agent">
    Connectez BigQuery, Linear, Slack, Notion, etc.
  </Card>

  <Card title="Facturation" icon="credit-card" href="/fr/ko/billing/ai-billing#agent-billing">
    Tarification d’Agent, comparaison des forfaits et gestion des dépenses.
  </Card>

  <Card title="Guide du vibe coding" icon="wand-magic-sparkles" href="/fr/ko/learn/build-with-agent">
    Conseils pour rédiger des prompts efficaces et créer avec l’IA.
  </Card>
</CardGroup>