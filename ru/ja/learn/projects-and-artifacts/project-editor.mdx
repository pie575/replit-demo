...
description: "Что такое редактор проекта: ваша основная рабочая среда в Replit. Общайтесь с Agent, наблюдайте, как приложение обретает форму, и управляйте всем в одном месте."
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="Главный экран редактора проектов Replit. Показано поле ввода запроса с предложением описать, что вы хотите создать" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>
...
Редактор проектов — это ваша основная площадка в Replit: среда, где вы начинаете разработку, управляете проектами и наблюдаете, как приложение обретает форму. Опишите здесь, что хотите создать, и [Agent](/ru/ja/features/agent/overview) возьмёт дело на себя: напишет код, настроит проект и покажет живой предпросмотр в реальном времени.

<div id="section-2">
  ## С чего начать
</div>

Введите в поле запроса то, что хотите создать. Опишите идею простыми словами — «трекер бюджета с графиком расходов», «лендинг для кофейни», «генератор ИИ-комиксов для детей», — а остальное сделает Agent.

<AgentInput />

Вы также можете выбрать тип проекта в карусели, чтобы указать Agent, какой результат вам нужен: веб-приложение, мобильное приложение, слайды, дизайн и так далее. Либо пропустите выбор и просто опишите идею — Agent сам подберёт подходящую конфигурацию.

<AgentCarousel />

Чтобы начать прямо сейчас, следуйте руководству [Создайте своё первое приложение](/ru/ja/build/your-first-app).

<div id="section-3">
  ## Режим планирования
</div>

В режиме планирования можно проводить мозговой штурм, задавать вопросы и продумывать структуру проекта. Agent создаёт упорядоченный список задач, который вы можете просмотреть и доработать — как только план вас устроит, подтвердите его, и Agent приступит к разработке. Режим планирования подходит и для того, чтобы задать вопросы или обдумать идеи: пока вы из него не выйдете, Agent не вносит изменений.

<AgentInput highlightPlan={true} showTypewriter={false} />

Режим планирования включается кнопкой в нижней части поля ввода. Подробнее — в [руководстве по режиму планирования](/ru/ja/features/agent/plan-mode).

<div id="section-4">
  ## Что можно создать
</div>

Выберите тип проекта и расскажите Agent, что вам нужно, либо просто опишите идею — и Agent сам подберёт подходящую конфигурацию.

* **Веб-приложения** — дашборды, внутренние инструменты, SaaS-продукты, лендинги
* **Мобильные приложения** — приложения для iOS и Android по одному запросу
* **Слайды и презентации** — презентации с насыщенными макетами
* **Анимации** — интерактивные пошаговые обзоры и моушн-графика
* **Дизайн** — визуальные макеты и прототипы на [Canvas](/ru/ja/design/canvas)
* **Визуализация данных** — графики, дашборды, инструменты на базе баз данных
* **Документы** — файлы CSV, PDF, PowerPoint, документы в Markdown
* **Таблицы** — таблицы данных и структурированный вывод

<div id="section-5">
  ## Редактор проекта
</div>

Как только Agent приступает к работе, вы можете наблюдать в редакторе проекта, как приложение обретает форму. Редактор разделён на две панели: в одной — диалог с Agent, в другой — живой предпросмотр приложения.

<div id="section-6">
  ### Треды
</div>

Всё общение с Agent происходит в тредах. Главный тред — это место, где вы описываете, что хотите получить, и наблюдаете, как Agent это создаёт. При использовании [системы задач](/ru/ja/core-concepts/agent/task-system) у каждой фоновой задачи есть собственный тред — так Agent работает над задачами независимо, не смешивая переписки.

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="Тред с запросом пользователя и Agent, создающим проект" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>
...
Design Canvas — это визуальный редактор, в котором можно создавать и дорабатывать макеты, прежде чем переходить к коду. Перетаскивайте элементы, настраивайте компоновку и совершенствуйте дизайн шаг за шагом, а затем поручите Agent превратить его в работающее приложение. Подробнее см. в разделе [Design Canvas](/ru/ja/design/canvas).

<div id="section-8">
  ## Совместная работа
</div>

Создайте командный workspace и создавайте приложения вместе. Приглашайте коллег, делитесь проектами и работайте над одним приложением в реальном времени. О настройке команды читайте в разделе [Командные workspace](/ru/ja/features/collaboration/team-workspaces), а о совместной работе на уровне проекта — в разделе [Приглашение коллег](/ru/ja/build/invite-teammates).

<div id="section-9">
  ## Импорт кода и дизайна
</div>

Уже есть готовый проект? Импортируйте его из GitHub или Figma либо просто загрузите файлы — и перенесите всё в Replit. Agent продолжит с того места, где вы остановились, и продолжит разработку на основе существующего кода и дизайна. Подробнее см. в разделе [Импорт в Replit](/ru/ja/build/import-from-providers).
...
<CardGroup cols={2}>
  <Card title="Редактор и инструменты" href="/ru/ja/features/editor/editor-and-tools" icon="window-restore">
    Основная панель, где отображаются предпросмотр приложения, вывод консоли и другие инструменты
  </Card>

  <Card title="Настройка приложения" href="/ru/ja/features/project-setup/configuration" icon="gear">
    Задайте параметры приложения, зависимости, секреты и окружение
  </Card>

  <Card title="Контроль версий" href="/ru/ja/learn/projects-and-artifacts/version-control" icon="clock-rotate-left">
    Отслеживайте изменения, откатывайтесь к предыдущим версиям и управляйте историей приложения
  </Card>

  <Card title="Публикация" href="/ru/ja/learn/projects-and-artifacts/replit-deployments" icon="rocket">
    Разверните приложение по рабочему URL, чтобы им мог пользоваться любой желающий
  </Card>
</CardGroup>
...
<Accordion title="Нужно ли уметь программировать?">
  Нет. Просто опишите простыми словами, что вам нужно, — технические детали возьмёт на себя Agent. При желании вы можете вносить правки и вручную.
</Accordion>

<Accordion title="Что будет, если закрыть браузер?">
  Проект хранится в облаке. Вы можете вернуться в любой момент и продолжить с того места, где остановились: файлы, история и запущенное приложение — всё сохранится.
</Accordion>

<Accordion title="Можно ли работать вместе с другими?">
  Да. Вы можете пригласить соавторов в конкретный проект или создать командный workspace, в котором участники получат доступ ко всем проектам. См. [Командные workspace](/ru/ja/features/collaboration/team-workspaces) и [Приглашение участников команды](/ru/ja/build/invite-teammates).
</Accordion>

<Accordion title="Как опубликовать приложение?">
  Когда приложение готово, разверните его на рабочий URL прямо из редактора проекта. См. [Deployments](/ru/ja/learn/projects-and-artifacts/replit-deployments).
</Accordion>
...
Редактор проектов доступен на всех тарифах. Для некоторых возможностей — например, увеличенных вычислительных ресурсов и постоянно работающих развёртываний — требуется платный тариф. Подробнее см. в разделе [Оплата](/ru/ja/billing/ai-billing).