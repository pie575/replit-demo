---
title: "项目编辑器"
description: "什么是项目编辑器：你在 Replit 中的工作中心。与 Agent 对话，见证应用成形，并管理一切。"
sidebarTitle: "项目编辑器"
icon: "window"
---
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="Replit 项目编辑器的主页，显示用于描述您想构建内容的提示词输入框" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>
...
项目编辑器是你在 Replit 的工作中心：你可以在这里开始构建、管理项目，并见证应用逐渐成形。只需描述你想构建的内容，[Agent](/zh/ja/features/agent/overview)就会接手后续工作，编写代码、设置项目，并实时显示实时预览。

<div id="section-2">
  ## 开始使用
</div>

在提示词输入框中输入你想构建的内容。用通俗易懂的语言描述你的想法，例如“将支出绘制成图表的预算追踪器”“咖啡店落地页”或“面向儿童的 AI 漫画书生成器”，Agent 会处理其余工作。

<AgentInput />

你也可以从轮播栏中选择项目类型，告诉 Agent 你希望生成什么内容，例如 Web 应用、移动应用、幻灯片或设计。或者跳过选择，直接描述你的想法，Agent 会自动确定合适的设置。

<AgentCarousel />

想立即开始，请按照[构建你的第一个 App](/zh/ja/build/your-first-app)操作。

<div id="section-3">
  ## 规划模式
</div>

在规划模式下，您可以进行头脑风暴、提问并梳理项目。Agent 会创建按顺序排列的任务清单，供您审核和完善。对规划满意后，批准即可让 Agent 开始构建。您也可以使用规划模式提问或探索想法；在您退出该模式前，Agent 不会做出任何更改。

<AgentInput highlightPlan={true} showTypewriter={false} />

通过输入框底部的按钮切换规划模式。有关详细信息，请参阅[规划模式指南](/zh/ja/features/agent/plan-mode)。

<div id="section-4">
  ## 可以构建的内容
</div>

选择项目类型并告诉 Agent 你想构建什么，或者描述你的想法，让 Agent 为你选择合适的设置。

* **Web 应用** — 控制台、内部工具、SaaS 产品和落地页
* **移动应用** — 根据一条提示词创建 iOS 和 Android 应用
* **幻灯片和演示文稿** — 生成布局丰富的演示文稿
* **动画** — 交互式演示和动态图形
* **设计** — 在[设计 Canvas](/zh/ja/design/canvas)上创建视觉模型和原型
* **数据可视化** — 图表、控制台和数据库后端工具
...
Agent 开始构建后，你可以在项目编辑器中看到项目逐渐成形。界面分为多个面板：一侧是与 Agent 的对话，另一侧是应用的实时预览。

<div id="section-6">
  ### 线程
</div>

与 Agent 的所有对话都在线程中进行。主线程用于描述你的需求，并查看 Agent 如何构建应用。使用[任务系统](/zh/ja/core-concepts/agent/task-system)后，每个后台 task 都有自己的线程，让 Agent 能够独立处理各项 task，而不会混淆对话。

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="用户提示词以及 Agent 构建项目过程所在的线程" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>
...
任务看板会将你的工作整理为**草稿**、**进行中**、**准备就绪**和**已完成**几个列。每个 task 会随进度从左向右移动，让你一目了然地了解已计划的内容、Agent 正在处理的内容以及已完成的内容。有关详细信息，请参阅[任务系统](/zh/ja/core-concepts/agent/task-system)。

<Frame>
  <img src="/assets/mirror/task-board-active-355ecd390c49.png" alt="任务看板中的任务按草稿、进行中、准备就绪和已完成列整理" width="3430" height="1986" data-path="images/replitai/task-board-active.png" />
</Frame>
...
设计 Canvas 是一款可让你在提交代码前创建和完善模型稿的可视化编辑器。拖动元素、调整布局、反复打磨设计，然后让 Agent 将其变成可运行的应用。有关详细信息，请参阅[设计 Canvas](/zh/ja/design/canvas)。

<div id="section-8">
  ## 协作
</div>

创建团队工作区，携手构建。邀请团队成员、共享项目，并可在同一应用中实时协作。有关团队设置，请参阅[团队工作区](/zh/ja/features/collaboration/team-workspaces)；有关项目级协作，请参阅[邀请团队成员](/zh/ja/build/invite-teammates)。

<div id="section-9">
  ## 导入代码和设计
</div>

已有项目？可从 GitHub、Figma 导入，或直接上传文件到 Replit。Agent 可以从上次中断的地方继续，并基于现有代码或设计继续构建。请参阅[导入到 Replit](/zh/ja/build/import-from-providers)。
...
<CardGroup cols={2}>
  <Card title="编辑器和工具" href="/zh/ja/features/editor/editor-and-tools" icon="window-restore">
    用于查看应用预览、控制台输出和其他工具的主面板
  </Card>

  <Card title="应用设置" href="/zh/ja/features/project-setup/configuration" icon="gear">
    配置应用设置、依赖项、Secrets 和环境
  </Card>

  <Card title="版本控制" href="/zh/ja/learn/projects-and-artifacts/version-control" icon="clock-rotate-left">
    跟踪更改、回滚到历史版本，并管理应用历史记录
  </Card>

  <Card title="发布" href="/zh/ja/learn/projects-and-artifacts/replit-deployments" icon="rocket">
    将应用部署到线上 URL，供所有人使用
  </Card>
</CardGroup>
...
<Accordion title="我需要懂编程吗？">
  不需要。只需用通俗的语言描述你的需求，Agent 会处理技术细节。如果需要，你也可以直接编辑。
</Accordion>

<Accordion title="关闭浏览器后会怎样？">
  项目会保存在云端。你可以随时回来，从上次中断的地方继续：文件、历史记录和正在运行的应用都会保留。
</Accordion>

<Accordion title="我可以与他人协作吗？">
  可以。你可以邀请协作者加入特定项目，或者创建团队工作区，让成员访问所有项目。请参阅[团队工作区](/zh/ja/features/collaboration/team-workspaces)和[邀请团队成员](/zh/ja/build/invite-teammates)。
</Accordion>

<Accordion title="如何发布我的应用？">
  应用准备就绪后，可直接从项目编辑器部署到线上 URL。请参阅[部署](/zh/ja/learn/projects-and-artifacts/replit-deployments)。
</Accordion>
...
所有套餐均可使用项目编辑器。更多计算资源和始终在线的部署等部分功能需要付费套餐。详情请参阅[计费](/zh/ja/billing/ai-billing)。