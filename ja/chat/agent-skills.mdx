---
title: "Skillsを使う"
description: "再利用可能な指示と補足資料を使って、Replitが繰り返し行う作業への取り組み方をカスタマイズします。"
sidebarTitle: "Skillsを使う"
icon: "puzzle-piece"
canonical: "https://docs.replit.com/chat/agent-skills"
---

<div className="chat-mkt" role="img" aria-label="Agent Skillsを表す幾何学模様" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
Skillとは、再利用可能な指示と補足ファイルをまとめたフォルダです。繰り返し行うタスクや慣習、ワークフローへのReplitの取り組み方をカスタマイズするために使用します。

プロンプトボックスで**プラス**メニューを選択し、**スキルを使う**を選びます。タスクに合ったSkillを選び、必要な結果をReplitに伝えます。

## 適切な連携方法を選ぶ

* 再利用可能な指示、例、補足資料には**Skill**を使います。
* サポートされているサービスとその認可された情報やアクションには[インテグレーション](/ja/chat/integrate-your-tools)を使います。
* 対応するMCPサーバーやクライアントを通じて公開されるツールには[MCP](/ja/chat/connect-through-mcp)を使います。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ja/features/agent/skills">
    ...
    <p>Skillがどのように構成され、発見され、使用されるかを学びます。</p>
    <div className="chat-card-link">機能ガイドを開く →</div>
  </a>

  <a className="chat-example has-visual" href="/ja/build/use-agent-skills">
    ...
    <div className="chat-card-title">プロジェクトでSkillsを使う</div>
    <p>Skillを追加して使用する詳細なワークフローに沿って進めます。</p>
    <div className="chat-card-link">ガイドを開く →</div>
  </a>
</section>

Skillsは、Replit内で再利用可能なアプローチを提供します。互換性のあるクライアントやサーバー向けにプロトコルベースの拡張機能が必要な場合は、[MCPで接続する](/ja/chat/connect-through-mcp)に進んでください。
