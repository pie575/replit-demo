---
title: "제공업체에서 가져오기"
description: "다른 도구에서 Replit으로 프로젝트, 코드, 디자인을 가져오고 Agent로 계속 빌드하세요."
---
...
Figma 디자인의 경우 [Replit Design으로 Figma 디자인 가져오기](/ko/design/import-figma-designs-into-replit-design)를 참조하세요.

Claude에서 전송된 디자인의 경우 [Replit Design으로 Claude 디자인 가져오기](/ko/design/import-claude-designs-into-replit-design)를 참조하세요.
...
<Tabs>
  ...
  <Tab title="ZIP">
    ...
    * 프로젝트가 포함된 `.zip` 파일 (200MB 이하)
    * 아카이브에 명확한 프로젝트 루트 폴더
    * Replit 계정

    zip 파일이 200MB보다 크면 가져오기 양식이 거부합니다. 크기를 줄이려면 `node_modules`, `.venv`, `build`와 같은 의존성 및 빌드 폴더를 제외하세요. Replit은 가져오기 중에 의존성을 다시 설치합니다.
