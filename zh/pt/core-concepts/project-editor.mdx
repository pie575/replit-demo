---
title: "项目编辑器"
description: "了解项目编辑器：这是您在 Replit 中的工作中心，可在此与 Agent 对话、跟踪 App 开发进度并管理一切。"
---
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="Replit 项目编辑器首页，显示可在其中描述要创建内容的提示词输入框" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>
...
项目编辑器是您在 Replit 上的工作中心：您可以在这里开始创建、管理项目，并见证 App 从构想到成型。您只需描述想要创建的内容，[Agent](/zh/pt/core-concepts/agent) 就会完成其余工作，包括编写代码、配置项目，并实时显示交互式预览。

<div id="primeiros-passos">
  ## 快速开始
</div>

在提示框中输入你想创建的内容。用日常语言描述你的想法——“一个用图表展示我支出情况的预算记账工具”“为我的咖啡馆做一个落地页”“一个面向儿童的 AI 漫画故事生成器”——剩下的交给 Agent。

<AgentInput />

你也可以在轮播选择器中选择项目类型，告诉 Agent 你想要什么样的成果——Web 应用、移动应用、演示文稿、Design 等等。或者跳过这一步，直接描述想法，Agent 会自动确定合适的配置。

<AgentCarousel />

想立即上手，请参阅[用 AI 创建快速入门](/zh/pt/getting-started/quickstarts/ask-ai/)。

<div id="modo-plan">
  ## Plan 模式
</div>

Plan 模式可让你进行头脑风暴、解答疑问并规划项目。Agent 会创建一份按顺序排列的任务清单，供你审阅和完善。对方案满意后，批准即可让 Agent 开始构建。你也可以使用 Plan 模式提问或探索想法；在退出该模式前，Agent 不会做出任何更改。

<AgentInput highlightPlan={true} showTypewriter={false} />

通过输入框底部的按钮启用 Plan 模式。有关详细信息，请参阅 [Plan 模式指南](/zh/pt/core-concepts/agent/plan-mode)。

<div id="o-que-voc-pode-criar">
  ## 你可以创建什么
</div>

选择一种项目类型来告诉 Agent 你想要什么，或者直接描述你的想法，让 Agent 来选择合适的配置。

* **Web 应用** — 仪表板、内部工具、SaaS 产品、落地页
* **移动应用** — 一条 prompt 即可生成 iOS 和 Android 应用
* **演示文稿与幻灯片** — 生成布局丰富的演示文稿
* **动画** — 交互式教程和动态图形
* **设计稿** — 在 [Design Canvas](/zh/pt/replitai/canvas) 中制作视觉稿和原型
* **数据可视化** — 图表、仪表板以及基于数据库的工具
...
Agent 开始构建后，您可以在项目编辑器中跟踪项目的开发进度。它由多个面板组成：一侧是您与 Agent 的对话，另一侧是 App 的实时预览。

<div id="threads">
  ### 线程
</div>

你与 Agent 的每一次对话都在一个线程中进行。主线程是你描述需求、并跟进 Agent 构建过程的地方。使用[任务系统](/zh/pt/core-concepts/agent/task-system)时，每个后台任务都有自己的线程——这样 Agent 就能让各项任务的工作彼此隔离，不会让对话混在一起。

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="一个线程，显示用户的 prompt 以及 Agent 正在构建项目" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>
...
任务面板将你的工作分为四列——**草稿**、**进行中**、**就绪**和**已完成**。每个任务会随着进展从左向右推进，让你一眼就能看清哪些已在计划中、Agent 正在处理什么，以及哪些已经完成。了解更多请参阅[并行构建](/zh/pt/core-concepts/agent/task-system)。

<Frame>
  <img src="/assets/mirror/task-board-active-355ecd390c49.png" alt="任务面板显示按草稿、进行中、就绪和已完成分列组织的任务" width="3430" height="1986" data-path="images/replitai/task-board-active.png" />
</Frame>
...
Design Canvas 是一款可视化编辑器，让你在正式动手写代码之前先创建并打磨模型稿。拖动元素、调整布局、不断迭代设计，然后交给 Agent 将其变成可运行的 App。了解更多，请参阅 [Design Canvas](/zh/pt/replitai/canvas)。

<div id="colaborao">
  ## 协作
</div>

创建团队 Workspace，与团队共同构建。邀请团队成员、共享项目，并实时在同一应用中协作。有关团队设置，请参阅[团队 Workspace](/zh/pt/core-concepts/workspaces/team-workspaces)；有关项目级协作，请参阅[邀请团队成员](/zh/pt/replitai/invite-teammates)。

<div id="importar-cdigo-e-design">
  ## 导入代码和设计
</div>

已有项目？您可以从 GitHub 或 Figma 导入项目，也可以直接上传文件。Agent 可以从您中断的地方继续，基于现有代码或设计继续构建。请参阅[导入到 Replit](/zh/pt/core-concepts/project-editor/app-setup/import)。
...
<CardGroup cols={2}>
  <Card title="编辑器和工具" href="/zh/pt/core-concepts/project-editor/editor-and-tools" icon="window-restore">
    用于查看应用预览、控制台输出及使用其他工具的主要面板
  </Card>

  <Card title="应用设置" href="/zh/pt/core-concepts/project-editor/app-setup/configuration" icon="gear">
    配置应用的设置、依赖项、Secrets 和环境
  </Card>

  <Card title="版本控制" href="/zh/pt/core-concepts/project-editor/version-control" icon="clock-rotate-left">
    跟踪更改、还原到早期版本，并管理应用历史记录
  </Card>

  <Card title="部署" href="/zh/category/replit-deployments" icon="rocket">
    将应用部署到公开 URL，供所有人使用
  </Card>
</CardGroup>
...
<Accordion title="我需要会编程吗？">
  不需要。只需用简单的语言描述你的需求，Agent 会处理技术细节。你也可以按需直接编辑。
</Accordion>

<Accordion title="关闭浏览器后会怎样？">
  你的项目会保存到云端。你可以随时回来并从上次中断处继续：文件、历史记录和正在运行的应用都会保留。
</Accordion>

<Accordion title="我可以与他人协作吗？">
  可以。你可以邀请协作者加入特定项目，或创建团队 Workspace，让成员访问所有项目。请参阅[团队 Workspace](/zh/pt/core-concepts/workspaces/team-workspaces)和[邀请团队成员](/zh/pt/replitai/invite-teammates)。
</Accordion>

<Accordion title="如何发布我的应用？">
  应用就绪后，可直接通过项目编辑器将其部署到公开 URL。请参阅[Deployments](/zh/category/replit-deployments)。
</Accordion>
...
所有方案均可使用项目编辑器。部分功能 (例如增强算力和始终在线的部署) 需要付费计划。详情请参阅[计费](/zh/pt/billing/ai-billing)。