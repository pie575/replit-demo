---
title: "Canvas"
description: "Découvrez comment utiliser Canvas pour explorer et affiner des designs avec Agent, et afficher vos artefacts dans des frames."
---
...
<YouTubeEmbed videoId="Qv9cREjl9vQ" title="Présentation de Canvas de Replit" />

<div id="o-que-o-canvas">
  ## Qu’est-ce que Canvas ?
</div>

Canvas est un espace où vous pouvez explorer et itérer sur des designs avec Agent. Vous y verrez vos artefacts — comme des apps, des vidéos et des diapositives — affichés dans un frame. Vous pouvez travailler avec Agent pour créer des maquettes, comparer différentes orientations de design et appliquer les modifications à votre app, le tout depuis un espace visuel.

<Frame>
  <img src="/assets/mirror/canvas-desktop-85e8bfec38a2.png" alt="Canvas affichant un aperçu de l’app à côté du Chat d’Agent avec les mises à jour des tâches" width="3452" height="2006" data-path="images/replitai/canvas-desktop.png" />
</Frame>
...
Le Canvas est un espace infini où vous pouvez visualiser et travailler sur tous vos éléments.

<Frame>
  <img src="/assets/mirror/canvas-mockups-057e738a78da.png" alt="Canvas affichant plusieurs maquettes de design disposées côte à côte — thème sombre, aperçus mobiles et pages de détail" width="3452" height="1620" data-path="images/replitai/canvas-mockups.png" />
</Frame>

* **Vos apps** : Vos apps en cours d&#39;exécution apparaissent sous forme d&#39;aperçus interactifs en direct sur le Canvas. Vous pouvez cliquer dessus et les utiliser exactement comme dans un navigateur.
* **Maquettes de design** : Des prototypes visuels que l&#39;Agent crée pour vous. Ils ressemblent à de vraies pages et fonctionnent comme telles : vous pouvez interagir avec eux, les redimensionner et demander à l&#39;Agent de les ajuster. Ils restent toutefois distincts de votre application en ligne, afin que vous puissiez expérimenter librement.
* **Formes et dessins** : Rectangles, cercles, étoiles, flèches, cœurs et bien plus encore. Utilisez-les pour esquisser des idées, annoter des designs ou représenter les connexions entre les écrans.
* **Texte et notes autocollantes** : Ajoutez des étiquettes ou des notes n&#39;importe où sur le Canvas pour organiser vos idées.
* **Images** : Déposez des captures d&#39;écran ou des images de référence sur le Canvas. L&#39;Agent peut recréer ce qu&#39;il voit ou s&#39;en inspirer.
* **Vidéos** : Placez des fichiers vidéo sur le Canvas à côté de vos autres contenus.

<div id="obtendo-designs-no-canvas">
  ### Créer des designs dans Canvas
</div>

Il existe plusieurs façons de commencer à créer des designs. Choisissez celle qui vous semble la plus naturelle.

<Steps>
  <Step title="Demandez à Agent dans le chat">
    Saisissez ce que vous souhaitez voir dans le champ de chat, et Agent le transforme en frame interactive dans Canvas.

    <AgentInput defaultPrompt="Montre-moi une maquette de ma page de destination dans Canvas" showTypewriter={true} />
  </Step>

  <Step title="Choisissez un type de résultat">
    Vous pouvez également choisir le type de résultat souhaité — web, mobile, diapositives, design, etc. Utilisez le carrousel pour faire votre choix avant d’envoyer votre prompt.

    <AgentCarousel />
  </Step>

  <Step title="Ajoutez une page existante à Canvas">
    Vous avez déjà une application en ligne ? Demandez à Agent d’ajouter une page à Canvas pour la refondre — par exemple, « Ajoute mon tableau de bord à Canvas. » Agent crée une copie afin que votre application en ligne ne soit pas affectée.
  </Step>
</Steps>
...
Une fois que vous avez un design sur le Canvas, vous pouvez le faire évoluer de plusieurs façons.

<div id="pea-ao-agent-atualizaes">
  ### Demandez des modifications à Agent
</div>

Sélectionnez un frame dans Canvas et saisissez vos modifications dans le champ qui s’affiche en dessous. Lorsque vous appuyez sur Entrée, Agent reçoit votre demande, effectue les modifications et met à jour la maquette directement dans Canvas.
...
Utilisez les outils de dessin de la barre d’outils inférieure pour dessiner directement sur le Canvas. Tracez des flèches vers les éléments que vous souhaitez modifier, entourez les zones problématiques ou ajoutez des notes pour partager votre retour. Demandez ensuite à Agent d’effectuer les modifications en fonction de vos annotations.
...
<div id="visualize-em-diferentes-dispositivos">
  ### Visualisez sur différents appareils
</div>

Voyez comment vos designs s’affichent selon la taille de l’écran. Demandez à Agent d’afficher une version mobile, tablette ou ordinateur :
...
<div id="aplique-as-alteraes-ao-seu-app">
  ### Appliquez les modifications à votre app
</div>

Une fois satisfait d’un design, appliquez les modifications à votre version principale. Agent affiche un résumé des modifications effectuées et vous permet de les appliquer ou de les ignorer.
...
Vous pouvez enregistrer n’importe quel artefact ou frame de design du Canvas au format PNG. Cette option est utile pour partager un aperçu dans un document ou un chat, prendre un instantané avant de poursuivre les itérations ou envoyer une capture d’écran à un collègue.

<Steps>
  <Step title="Ouvrez le menu du frame">
    Faites un clic droit sur l’artefact ou le frame de design que vous souhaitez exporter. Un menu s’affiche avec les actions disponibles.
  </Step>

  <Step title="Choisissez Exporter au format PNG">
    Sélectionnez **Exporter** &gt; **PNG**. L’image est téléchargée sur votre ordinateur sous un nom de fichier basé sur celui du frame.
  </Step>
</Steps>

<Frame>
  <img src="/assets/mirror/canvas-export-png-b903c6afacb8.png" alt="Menu contextuel obtenu par clic droit sur un frame du Canvas, avec le sous-menu Exporter affichant PNG comme option d’exportation" width="1740" height="1160" data-path="images/replitai/canvas-export-png.png" />
</Frame>

<Note>Le PNG exporté correspond à ce qui est actuellement affiché dans le frame sur le Canvas. Si votre app ou votre design est encore en cours de chargement, attendez qu’il soit entièrement affiché avant de l’exporter.</Note>

<div id="comparando-direes-de-design">
  ## Comparer des pistes de conception
</div>

Lorsque vous hésitez sur la direction à suivre, demandez à Agent de générer plusieurs versions afin de pouvoir les comparer côte à côte.
...
* **Plusieurs versions d’une page** : « Montre-moi cinq versions de cette page de destination. » Agent les crée et les dispose côte à côte.
* **Parcours sur plusieurs pages** : « Conçois un parcours d’intégration complet : écran de bienvenue, configuration du profil, préférences et confirmation. » Agent crée chaque écran et les organise dans l’ordre.
* **Mélangez les styles** : « Montre-moi la page de destination et la page des tarifs dans un style minimaliste et un style audacieux. » Agent crée chaque combinaison afin que vous puissiez avoir une vue d’ensemble.

<div id="mapeando-fluxos-de-usurio">
  ## Cartographier les parcours utilisateur
</div>

Disposez des parcours utilisateur complets sur le Canvas — inscription, onboarding, tableau de bord, paramètres — et organisez-les pour visualiser les liens entre les différentes étapes.

* Placez chaque écran dans un frame distinct
* Demandez à l’Agent de générer le parcours complet : &quot;Conçois le parcours d’onboarding complet de mon app&quot;
* Utilisez des flèches et des formes pour montrer comment les utilisateurs passent d’un écran à l’autre
* Demandez à l’Agent de transformer des croquis bruts en designs soignés

<div id="transformando-um-design-em-um-app-real">
  ## Transformer un design en app fonctionnelle
</div>

Lorsque vous êtes satisfait d’un design et souhaitez le concrétiser, deux possibilités s’offrent à vous :

<div id="torne-o-um-app-completo">
  ### Transformez-le en application complète
</div>

Si votre design nécessite de véritables fonctionnalités — comme l’enregistrement de données, des comptes utilisateur ou la connexion à d’autres services — demandez à Agent de le transformer en application complète. Dites, par exemple, « Transforme ceci en véritable application » ou « Ajoute un backend à ceci. » Agent crée tout ce qui est nécessaire pour en faire une véritable application prête à être publiée.

<Note>
  La transformation d’un design en application complète nécessite un plan payant.
</Note>
...
<Steps>
  <Step title="Sélectionner un frame">
    Cliquez sur le frame que vous souhaitez convertir.
  </Step>

  <Step title="Demander à Agent de convertir">
    Indiquez à Agent ce que vous souhaitez — par exemple : « Convertis ceci en web app » ou « Transforme ceci en diapositives. »
  </Step>

  <Step title="Choisir un type">
    Choisissez parmi les options suivantes : web app, app mobile, diapositives, visualisation de données, etc.
  </Step>
</Steps>
...
<Accordion title="Les frames de design sont-elles identiques à mon application en cours d’exécution ?">
  Non. Les frames de design sont des prototypes visuels : elles ressemblent à l’application réelle et fonctionnent comme elle, mais elles ne disposent ni de backend ni de base de données fonctionnels. Les aperçus de votre application dans le Canvas, en revanche, correspondent à votre véritable application en cours d’exécution.
</Accordion>

<Accordion title="Puis-je repenser une page sans perturber mon application en ligne ?">
  Oui. Demandez à Agent d’ajouter une page au Canvas — par exemple, « Place mon tableau de bord sur le Canvas. » Agent crée une copie avec laquelle vous pouvez expérimenter. Votre application en ligne n’est pas affectée tant que vous ne choisissez pas d’appliquer les modifications.
</Accordion>

<Accordion title="Comment transformer un design en véritable application ?">
  Demandez à Agent d’en faire une application complète (« Transforme ceci en une vraie application ») ou de le convertir vers un type spécifique (« Convertis ceci en web app »). Agent crée tout ce qui est nécessaire pour en faire une application fonctionnelle et publiable.
</Accordion>

<Accordion title="Puis-je convertir vers d’autres types que les web apps ?">
  Oui. Vous pouvez transformer des frames du Canvas en web apps, applications mobiles, présentations, visualisations de données et bien plus encore.
</Accordion>

<Accordion title="Puis-je importer des designs depuis Figma ou d’autres outils ?">
  Oui. Il existe plusieurs façons d’importer des designs de Figma dans Replit :

  * **Importer depuis Figma** : Accédez à [replit.com/import](https://replit.com/import), connectez votre compte Figma et collez l’URL d’une frame pour convertir un design en application React fonctionnelle. Consultez le [guide de démarrage rapide Importer depuis Figma](/fr/pt/design/import-figma-designs-into-replit-design) pour plus de détails.
  * **Figma MCP dans le Chat d’Agent** : Collez un lien Figma directement dans le Chat d’Agent et demandez-lui de générer du code, d’extraire des données de design ou de créer un composant à partir du design. Consultez le guide d’[intégration Figma MCP](/fr/pt/features/mcp/figma).
  * **Référence par capture d’écran** : Déposez une capture d’écran dans le Canvas et demandez à Agent de recréer le design à partir de celle-ci.
</Accordion>

<Accordion title="Que puis-je ajouter d’autre au Canvas ?">
...
Canvas est disponible dans tous les plans, y compris pour créer des frames, les modifier en ligne, générer des variantes et les convertir en artefacts. La transformation de designs en apps complètes nécessite un plan payant.