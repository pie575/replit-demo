---
title: "Replit Agent"
description: "Replit Agent はアイデアをアプリ、デザイン、スライドなどに変換します。プログラミングの知識は不要です。"
---
...
<YouTubeEmbed videoId="bAUVQfz_SIY" title="Replit Agent 4 のご紹介" />

## Replit Agent とは？

Agent はあなたのクリエイティブパートナーです。Agent はあなたのアイデアを受け取り、それを磨き上げ、実現させます。質問に答えるだけのチャットボットとは異なり、Agent は行動します：プロジェクトを設定し、アプリケーションを作成し、その作業を確認し、途中で問題を修正します。

日常的な言葉で望むものを説明するだけです。コードや技術的な知識は不要です。Agent が計画からデプロイまですべてを処理します。
...
<Steps>
  <Step title="望むものを説明する">
    [プロジェクトエディター](/ja/learn/projects-and-artifacts/project-editor)で、チャットを始めましょう。作りたいアプリを説明したり、質問したり、トピックを調査したり、BigQuery、Slack、Notion などの[接続サービス](/ja/features/agent/general-agent)からデータを引き出したりできます。制約はありません。Replit Agent は何でも対応します。

    <AgentInput />
  </Step>

  <Step title="作るものを選ぶ">
    オプションで、プロジェクトタイプを選択します：ウェブアプリ、モバイルアプリ、スライド、デザイン、データビジュアライゼーションなど。ステップ1で望むものをすでに説明している場合、Agent が自動的に適切な設定を把握します。

    <AgentCarousel />
  </Step>

  <Step title="Agent が構築する">
    Agent がコードを書き、インフラを設定し、結果をテストします。プロジェクトに入ったら、[Agent モード](#agent-modes)を切り替えて構築方法を制御できます。
  </Step>

  <Step>
    Agent とチャットしてプロジェクトを改善し、準備ができたら公開します。すべてのアーティファクトがまとめてデプロイされます。
  </Step>
</Steps>

<Note>1つのタイプに縛られる必要はありません。ウェブアプリから始めて、後でモバイルアプリ、スライド、動画を追加できます—すべて同じプロジェクト内で。</Note>

### Plan モード

Plan モードを有効にして、Agent がコードやデータを変更する前にブレインストーミング、質問、プロジェクトのマッピングを行いましょう。Plan モードでは、Agent が：

* **複雑なプロジェクトを分解**して順序付きタスクリストを作成します
* **さまざまなアプローチを検討**してトレードオフを評価します
* **コードが書かれる前に確認と改善**を行います

チャット入力の「Plan」をクリックするか、単に Agent に依頼して Plan モードに切り替えます。例：「チーム用のプロジェクトトラッカーを構築する計画を立ててください」

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent は確認と改善ができる順序付きタスクリストを作成します。計画に満足したら承認して、Agent が構築を開始します。

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="4つの提案されたタスクと「タスクを承認」と「計画を修正」オプションを表示するタスク計画作成ダイアログ" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

詳細は[Plan モードガイド](/ja/features/agent/plan-mode)をご覧ください。

### Agent モード

Agent がプロジェクトを構築する方法を選択します：

<Frame caption="Free、Power、Max モードを表示する Agent モードのポップオーバー。">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Power が選択され、プライマリモデルリストと Effort スライダーを表示する、Free、Power、Max モードの Agent モードポップオーバー" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode**：CoreおよびProサブスクライバー向けの日常のAgent作業を無料で提供します。許容量は5時間ごとにリセットされ、週次の上限があります。Starterティアは既存の上限を維持します。**設定 → 使用量**で使用状況を確認できます。
* **Power**：手頃な価格で強力なモデルを利用できる有料のミッドティアモードです。
* **Max**：最も要求の厳しい作業向けに最高峰のフロンティアモデルを利用できる有料のトップティアモードです。**詳細設定**では、より高速な実行のために **Turbo** をオンにすることもできます。

Free Modeを超える機能が必要なリクエストの場合、Agent は有料の **一度だけ許可** オプションを提示することがあります。Replit は有料アクションが開始する前に確認を求め、その後Agent はFree Modeに戻ります。

<Note>ヒント：**[Plan モード](/ja/features/agent/plan-mode)** を有効にして、構築が始まる前に Agent の計画を確認・改善することもできます。</Note>

## Replit Agent で構築できるもの

* **ウェブアプリ、モバイルアプリ、データダッシュボード、AIを活用したツール**
* **ビジュアルデザインとプロトタイプ** — コードにコミットする前に[デザイン Canvas](/ja/design/canvas)でモックアップを探索します
* **1つのプロジェクト内の複数のアウトプット** — 同じバックエンドを共有するウェブアプリ、モバイルアプリ、スライド、動画
* **ファイルとドキュメント** — CSV、PDF、PowerPointファイル、Markdownドキュメント
* **接続サービスのクエリ** — チャットから直接 BigQuery、Linear、Slack、Notion などのデータを引き出します

<Card title="今すぐ構築を始める" icon="arrow-up-right" href="https://replit.com">
  アイデアを説明して Agent に実現させましょう — セットアップ不要。
</Card>

## よくある質問

<Accordion title="コードを書く方法を知っている必要がありますか？">
  いいえ。Agent がすべての技術的な作業を処理します：コードの記述、インフラの設定、データベースの設定。望むものを日常的な言葉で説明するだけで、Agent が構築します。
</Accordion>

<Accordion title="Free、Power、Max の違いは何ですか？">
  **Free Mode** は、CoreおよびProサブスクライバー向けに許容量の範囲内で日常のAgent作業を無料で提供します。**Power** は手頃な価格で強力なモデルを利用できる有料のミッドティアモードです。**Max** は最も要求の厳しい作業向けの有料のトップティアモードです。**Turbo** は詳細設定にあるMax専用のオプションのトグルです。

  有料アクションは開始前に確認が必要です。
</Accordion>

<Accordion title="プロジェクトで複数のものを構築できますか？">
  はい。ウェブアプリ、モバイルアプリ、スライド、動画、データビジュアライゼーションを[同じプロジェクト](/ja/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project)に追加できます — すべて同じバックエンドとデータを共有します。
</Accordion>

<Accordion title="Agent が間違いを犯した場合はどうなりますか？">
  Agent は定期的に自分の作業をテストします。また、作業中にチェックポイントを作成するため、任意の以前の状態にロールバックできます。Agent とチャットして何が問題だったかを説明すると、問題を修正します。
</Accordion>
...
Turbo はデフォルトでオフです — Max を選択した状態で、必要なときにプロジェクトごとにオンにしてください。Free Mode はローンチ時点ではEnterpriseでは利用できません。Enterprise のワークスペースには、その後間もなく専用のAgent体験が提供されます。詳細は管理者にお問い合わせください。

詳細な料金については、[Agent の課金](/ja/billing/ai-billing#agent-billing)を参照してください。

## 次のステップ

<CardGroup cols={2}>
  <Card title="Agent スキル" icon="puzzle-piece" href="/ja/features/agent/skills">
    Agent に専門知識を教えます — 事前構築済みのスキルを使用するか、独自のものを作成します。
  </Card>

  <Card title="タスクシステム" icon="list-check" href="/ja/core-concepts/agent/task-system">
    カンバン計画、バックグラウンドタスク、メインバージョンへの変更の適用。
  </Card>

  <Card title="デザイン Canvas" icon="paintbrush" href="/ja/design/canvas">
    ビジュアルモックアップと実践的なビジュアルエディター。
  </Card>

  <Card title="コネクター" icon="plug" href="/ja/features/agent/general-agent">
    BigQuery、Linear、Slack、Notion などを接続します。
  </Card>

  <Card title="課金" icon="credit-card" href="/ja/billing/ai-billing#agent-billing">
    Agent の料金、プランの比較、支出管理。
  </Card>

  <Card title="バイブコーディングガイド" icon="wand-magic-sparkles" href="/ja/learn/build-with-agent">
    AIを使った効果的なプロンプトと構築のヒント。
  </Card>
</CardGroup>
