...
description: "Utilisez le tableau des tâches pour examiner, lancer, suivre et appliquer les tâches de l'Agent au fil de leur progression à chaque étape."
---
...
<div id="como-o-quadro-de-tarefas-funciona">
  ## Fonctionnement du tableau de tâches
</div>

Le tableau de tâches réunit en un seul endroit tout ce qu&#39;il faut pour examiner, lancer, suivre et appliquer les tâches de l&#39;Agent. Utilisez-le lorsque l&#39;Agent découpe une demande volumineuse en plusieurs parties, lorsque vous souhaitez exécuter du travail en arrière-plan ou lorsque vous avez besoin de savoir ce qui a déjà changé dans votre projet.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Tableau de tâches affichant les tâches organisées dans les colonnes Brouillons, Actif, Prêt et Terminé" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

Les tâches sont réparties en colonnes selon leur statut. Le tableau indique ce que l&#39;Agent a planifié, ce qui est en cours d&#39;exécution, ce qui est prêt à être examiné et ce qui a déjà été appliqué.

Chaque tâche se déplace de gauche à droite au fil de son avancement :

* **Brouillons** : tâches que l&#39;Agent a planifiées, mais pas encore démarrées
* **Actif** : tâches que l&#39;Agent est en train de construire, y compris celles mises en file d&#39;attente en raison de votre limite de tâches actives
* **Prêt** : tâches que l&#39;Agent a terminées et qui attendent votre relecture
* **Terminé** : tâches finalisées, archivées ou annulées

<div id="rascunhos">
  ## Brouillons
</div>

Les **brouillons** sont des tâches planifiées dont l&#39;Agent n&#39;a pas encore commencé la réalisation. Chaque brouillon comprend un titre, une description et un plan étape par étape.

Utilisez les brouillons pour examiner l&#39;approche proposée par l&#39;Agent avant le démarrage des travaux. Vous pouvez inspecter le plan, demander à l&#39;Agent de l&#39;ajuster ou lancer la tâche lorsque vous êtes prêt.

Les brouillons sont utiles lorsque vous souhaitez planifier une fonctionnalité d&#39;envergure avant de lancer un travail en arrière-plan. Par exemple, l&#39;Agent peut découper un projet d&#39;intégration en brouillons distincts pour l&#39;authentification, la configuration du profil, l&#39;interface du tableau de bord et les finitions du déploiement.

<div id="ativo">
  ## Actif
</div>

Une fois que vous avez accepté une tâche, elle passe dans la colonne **Actif** et Agent se met au travail. Chaque tâche s&#39;exécute dans sa propre copie isolée de votre projet : rien ne change donc dans votre version principale tant que vous n&#39;appliquez pas le travail réalisé.

Certaines tâches dépendent d&#39;autres. Par exemple, une tâche qui crée un tableau de bord peut nécessiter que la tâche de schéma de base de données se termine au préalable. Les tâches dépendantes restent **En file d&#39;attente** jusqu&#39;à ce que leurs prérequis soient terminés. Les tâches sont également mises en file d&#39;attente si vous atteignez la limite de tâches en arrière-plan actives de votre plan.
...
Lorsqu&#39;une tâche a terminé sa construction, elle passe à l&#39;état **Prêt**. Cela signifie que le travail est terminé, mais que rien n&#39;a encore changé dans votre version principale.

Vous pouvez appliquer les modifications de deux façons :

* **Depuis le fil de discussion** : ouvrez le fil de discussion de la tâche et sélectionnez **Appliquer les modifications à la version principale** pour intégrer le travail à votre projet, ou **Ignorer** pour l&#39;abandonner
...
Passez en revue les tâches Prêtes avant de les appliquer. Consultez le journal de travail, la sortie des tests et le Preview pour comprendre ce qu&#39;Agent a modifié.

<div id="concludo">
  ## Terminé
</div>

Une fois appliquée, la carte de la tâche passe dans la colonne **Terminé**.

La colonne Terminé regroupe également les sessions de planification archivées et les tâches annulées. Vous disposez ainsi d&#39;un historique de ce qui s&#39;est passé, sans que le travail achevé ne revienne se mêler aux Brouillons ou à Actif.
...
Agent gère automatiquement les conflits lorsque vous appliquez les modifications de plusieurs tâches. Si quelque chose vous semble incorrect, demandez à Agent de le corriger.

<div id="encontrar-e-filtrar-tarefas">
  ## Rechercher et filtrer des tâches
</div>

Utilisez le tableau des tâches lorsque votre projet comporte de nombreuses tâches en cours. Vous pouvez lancer une recherche par titre ou par numéro de tâche, filtrer les brouillons et utiliser le mode édition pour gérer plusieurs tâches à la fois.

Par exemple, recherchez le numéro de la tâche lorsque quelqu&#39;un mentionne une tâche dans un fil de discussion, une revue ou une conversation Slack. Filtrez les brouillons lorsque vous souhaitez vous concentrer sur le travail planifié sans passer en revue les tâches actives ou terminées.

Pour des suggestions de suivi, consultez [Tâches de suivi](/fr/pt/features/agent/follow-up-tasks).

<div id="renomear-uma-tarefa">
  ## Renommer une tâche
</div>

Les longues listes de tâches peuvent devenir difficiles à parcourir à mesure que le périmètre évolue. Pour renommer une tâche, ouvrez le menu à trois points de sa carte (dans la barre latérale de la liste de tâches ou dans le tableau des tâches) et sélectionnez **Renommer**. Saisissez un titre plus clair, puis sélectionnez **Enregistrer**.

Renommer une tâche modifie uniquement la façon dont elle s&#39;affiche pour vous et vos collègues. Le travail déjà effectué par Agent, l&#39;historique de la tâche et les modifications en attente ne sont pas affectés.

<div id="configuraes-em-cada-tarefa">
  ## Paramètres de chaque tâche
</div>

Chaque tâche possède son propre menu déroulant de paramètres, à côté du titre. Ouvrez-le pour définir la façon dont l&#39;Agent termine la tâche et dont le résultat arrive dans votre projet :

* **Appliquer les modifications automatiquement** (Brouillons et Actif) : dès que la tâche est prête, appliquez-la immédiatement à la version principale au lieu d&#39;attendre la revue.
* **Approuver le plan automatiquement** (Brouillons) : lorsque l&#39;Agent propose un plan pour la tâche, acceptez-le automatiquement et lancez la construction.
* **Appliquer les modifications** (Prêt) : appliquez le travail terminé à votre version principale.
* **Renommer** : modifiez le titre affiché sur la carte.
* **Examiner les modifications** (Prêt) : accédez directement au diff d&#39;une tâche prête.
* **Annuler** : arrêtez la tâche ou supprimez un brouillon.

L&#39;application automatique et l&#39;approbation automatique sont des actions ponctuelles, que vous sélectionnez une seule fois, et non des interrupteurs que vous laissez activés. Le menu conserve une largeur constante afin que les libellés ne se déplacent pas lorsque vous passez d&#39;une tâche à l&#39;autre.
