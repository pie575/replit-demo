...
description: "Создавайте приложения с приоритетом для смартфонов с помощью Agent, тестируйте их в симуляторе и на реальных устройствах и делитесь ими для сбора отзывов."
---
...
Мобильные приложения должны быть удобны на смартфоне: быстро открываться, иметь удобные элементы для касаний и комфортно использоваться одной рукой.

Мы создадим трекер пробежек **Pace Mobile**, ориентированный на смартфоны и вдохновлённый примером из [Что такое Replit?](/ru/ja/build/welcome). Цель — протестировать целенаправленный мобильный сценарий, прежде чем переходить к полноценной публикации в App Store.

<Frame>
  <img src="/assets/mirror/mobile-app-4203ff45ecd5.png" alt="Пример современного мобильного приложения с экраном отслеживания фитнеса, разработанным для использования на смартфоне" width="1440" height="850" data-path="images/build-examples/mobile-app.png" />
</Frame>
...
После прохождения этого руководства у вас будет:

* Проект мобильного приложения, созданный с помощью Agent.
* Первая версия, ориентированная на один сценарий использования на смартфоне.
* Приложение, работающее в симуляторе или эмуляторе.
* Приложение, работающее на смартфоне через Expo Go.
* Версия, которой можно поделиться для получения обратной связи.
...
* начать создавать мобильное приложение с помощью промпта;
* определить объем первой версии с учетом ограничений смартфонов;
* протестировать приложение в симуляторе или эмуляторе;
* протестировать приложение на физическом устройстве с помощью Expo Go;
* определить, когда следует продолжать итерации, прежде чем приступать к работе с App Store или TestFlight.

<div id="section-3">
  ## Для кого
</div>

Используйте это руководство, если ваша идея предназначена для использования на смартфоне.

Оно пригодится для создания трекеров, сервисов бронирования, полевых инструментов, приложений для мероприятий и обучения, приложений для формирования привычек и всего, чем пользуются вдали от компьютера.

<Note>
  Публикация в TestFlight и App Store рассматривается отдельно от этого руководства. Когда будете готовы к этому этапу, ознакомьтесь с разделом [Создание и публикация мобильного приложения](/ru/ja/build/mobile-app).
</Note>
...
* Смартфон с установленным [Expo Go](https://expo.dev/go).
* Один мобильный сценарий, который можно протестировать за несколько минут.

Подробнее о симуляторах, эмуляторах и предварительном просмотре на устройстве см. в разделе [Нативные мобильные приложения](/ru/ja/features/artifact-types/building-mobile-apps).

<div id="section-5">
  ## Определите мобильный сценарий
</div>

Прежде чем приступать к разработке, определите, что пользователь будет делать на смартфоне.

Для Pace Mobile:

* **Целевая аудитория**: бегуны, которым нужно быстро записать пробежку.
* **Основной сценарий**: выбрать тип пробежки → ввести дистанцию и время → сохранить пробежку → посмотреть сводку за сегодня.
* **Ограничения мобильного интерфейса**: разместите основные действия так, чтобы до них было легко дотянуться большим пальцем одной руки.
* **Первая версия**: без аккаунтов, социальной ленты и расширенной аналитики.

Так вы сохраните первую мобильную версию достаточно простой для тестирования.

<div id="section-6">
  ## Создание первой версии
</div>

Откройте Replit и создайте новое приложение. Опишите, что хотите создать, и выберите **Mobile app** в качестве типа приложения.
...
<AiPrompt>
  ...
  Пользователь должен иметь возможность выбрать тип пробежки, указать расстояние и время, сохранить пробежку и просмотреть сводку за сегодня.<br />
  Сделай его удобным для использования одной рукой: с крупными зонами нажатия, простой навигацией и продуманным стилем фитнес-приложения.<br />
  Сделай первую версию достаточно небольшой, чтобы протестировать её уже сегодня в симуляторе и на реальном телефоне.
</AiPrompt>

Если хотите, чтобы Agent продумал приложение перед сборкой, используйте [режим Plan](/ru/ja/features/agent/plan-mode). Перед началом сборки попросите Agent проверить экраны, навигацию, зоны нажатия и ограничения, характерные для смартфонов.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Компоновщик промптов Replit с выбранным режимом Plan перед тем, как Agent определит объём работ по созданию мобильного приложения" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Когда Agent завершит первую сборку, откройте [Preview](/ru/ja/features/editor/preview). В списке устройств выберите **iOS Simulator** или **Android Emulator**.

Попробуйте пользоваться приложением так, как это делал бы пользователь на смартфоне.

<Frame>
  <img src="/assets/mirror/workspace-dda0f81e6b83.png" alt="Replit Project Editor с Preview мобильного приложения и вариантами предварительного просмотра на мобильных устройствах" width="1440" height="900" data-path="images/native-mobile-apps/workspace.png" />
</Frame>

Проверьте следующее:

* Приложение запускается без ошибок.
* Можно выбрать тип пробежки.
* Можно ввести расстояние и время.
* Можно сохранить пробежку.
* Сводка за сегодня обновляется.
* По приложению можно перемещаться без затруднений.
* Текст можно читать без масштабирования.

<div id="section-8">
  ## Тестирование на смартфоне
</div>

Тестирование в симуляторе полезно, но только на реальном устройстве можно понять, действительно ли приложением удобно пользоваться.

Откройте панель Preview для мобильного устройства и отсканируйте QR-код смартфоном. Приложение откроется в Expo Go.

<Frame>
  <img src="/assets/mirror/mobile-screen-e5685fc26793.png" alt="Мобильное приложение, запущенное в Preview Replit Project Editor. Отображаются панель Preview и QR-код для открытия в Expo Go" width="1440" height="900" data-path="images/native-mobile-apps/mobile-screen.png" />
</Frame>
...
* Легко ли дотянуться до основных действий?
* Не закрывает ли клавиатура важные поля?
* Удобно ли пользоваться приложением на небольшом экране?

Если что-то неудобно, попросите Agent исправить именно эту часть, а не переделывать дизайн всего приложения.

<AiPrompt>
  Улучши Pace Mobile для использования одной рукой.<br />
...
Когда мобильный сценарий будет готов, опубликуйте приложение и создайте версию для Expo Go, которой можно поделиться, чтобы собрать первые отзывы.

Поделитесь приложением, предложив выполнить одну конкретную задачу:

> Откройте Pace Mobile на смартфоне, сохраните одну пробежку и расскажите, что было сложно нажать, прочитать или понять.

Целенаправленная обратная связь полезнее общих впечатлений. Если отзыв указывает на одну конкретную проблему, попросите Agent внести минимальные исправления и снова протестируйте приложение на смартфоне.

<div id="app-store">
  ## Публикация в App Store
</div>

Протестировав Pace Mobile в симуляторе и на реальном устройстве с помощью Expo Go, вы можете отправить это же приложение в TestFlight и App Store. Для этого необходимы членство в **Apple Developer Program** и двухфакторная аутентификация. Подробнее о регистрации, личных и корпоративных аккаунтах, проверке в TestFlight и App Store, а также внешнем тестировании см. в разделе [Создание и публикация мобильного приложения](/ru/ja/build/mobile-app).

<Steps>
  <Step title="Откройте вкладку Publishing и начните публикацию в App Store">
    После публикации предварительной версии в Expo Go откройте вкладку **Publishing**. В разделе **Production** появится карточка **Ready to publish Pace to the App Store?**. Выберите **Start publishing to the App Store**.

    <Frame>
      <img src="/assets/mirror/app-store-02-start-publishing-190eb7841bcf.png" alt="Вкладка Publishing в Project Editor со статусом Production, карточкой «Ready to publish Pace to the App Store?» и кнопкой «Start publishing to the App Store»" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-02-start-publishing.png" />
    </Frame>
  </Step>

  <Step title="Выберите проект Expo">
    Replit открывает **Launch** — встроенный в Replit поток Expo. Введите или проверьте название и слаг Expo-проекта, затем нажмите **Continue**. Если у вас уже есть Expo-проект, выберите **Use existing**.

    <Frame>
      <img src="/assets/mirror/app-store-03-choose-expo-project-4707fafffafe.png" alt="Диалоговое окно Launch с активным шагом Choose Expo project: в полях Project name и Project slug уже указано Pace, отображаются кнопка Continue и вариант Use existing" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-03-choose-expo-project.png" />
    </Frame>
  </Step>

  <Step title="Войти с помощью Apple">
    Подключите учётную запись Apple Developer, которой принадлежит листинг в App Store. Учётная запись должна быть зарегистрирована в Apple Developer Program. Apple использует двухфакторную аутентификацию — подготовьтесь подтвердить код на доверенном устройстве.

    <Frame>
      <img src="/assets/mirror/app-store-04-sign-in-apple-459dfc9b84ba.png" alt="Диалог Launch на этапе «Войти с Apple» — поля для адреса электронной почты или номера телефона Apple Developer и пароля. Примечание: пароль не покидает ваше устройство, а учётная запись должна быть зарегистрирована в Apple Developer Program" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-04-sign-in-apple.png" />
    </Frame>
  </Step>
  ...
  <Step title="Выберите или создайте приложение Apple">
    ...
    <Frame>
      <img src="/assets/mirror/app-store-05-choose-apple-app-3971aea62f14.png" alt="Диалог Launch на шаге Choose Apple app — для App name указано Pace, для Bundle ID — app.replit.pace; также показаны примечание о том, что идентификатор пакета нельзя изменить позже, и кнопка Creating app…" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-05-choose-apple-app.png" />
    </Frame>
  </Step>

  <Step title="Публикация в App Store">
    Replit настроит сертификаты Apple от вашего имени. Когда станет доступен шаг **Ready for launch**, выберите **Launch to App Store**. Replit соберёт нативное приложение в облаке и отправит сборку в App Store Connect для бета-проверки в TestFlight.

    <Frame>
      <img src="/assets/mirror/app-store-06-ready-for-launch-ab02659ee0bb.png" alt="Диалог Launch на шаге Ready for launch: Choose Apple app и Configure Apple certificates отмечены как завершённые, отображаются кнопка Launch to App Store и необязательные параметры Launch" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-06-ready-for-launch.png" />
    </Frame>

    После завершения Launch появится подтверждение, что приложение готовится для App Store, диалог закроется, и вы вернётесь в Project Editor.

    <Frame>
      <img src="/assets/mirror/app-store-targeting-f8797071c9e0.png" alt="Диалог Launch: Targeting App Store отмечен как завершённый и отображается сообщение «Redirecting back to Replit» — состояние подтверждения после выбора Launch to App Store" width="3452" height="1988" data-path="images/native-mobile-apps/app-store-targeting.png" />
    </Frame>
  </Step>
</Steps>

<Note>
  Первая сборка версии обычно проходит бета-проверку TestFlight, прежде чем тестировщики смогут её установить. После одобрения последующие сборки обычно становятся доступны тестировщикам быстрее. Чтобы поделиться приложением с внешними тестировщиками или отправить его на публикацию в App Store, заполните страницу приложения в **App Store Connect** — полный контрольный список App Store Connect см. в разделе [Создание и публикация мобильного приложения](/ru/ja/build/mobile-app).
</Note>
...
* Приложение открывается на смартфоне через Expo Go.
* Вы можете пройти основной мобильный сценарий на мобильном устройстве.
* Текст, кнопки и навигация удобны на смартфоне.
* Вы опубликовали тестовую мобильную версию или предоставили к ней доступ.
* Как минимум один человек может протестировать приложение и оставить отзыв.
...
<CardGroup cols={2}>
  <Card title="Добавить вход" icon="user-lock" href="/ru/ja/build/add-login">
    Позвольте каждому пользователю отслеживать свой прогресс.
  </Card>

  <Card title="Добавить базу данных" icon="database" href="/ru/ja/build/add-database">
    Сохраняйте историю пробежек не только в рамках первого тестового сеанса.
  </Card>

  <Card title="Добавить интеграции" icon="plug" href="/ru/ja/build/add-integrations">
    Подключите App к уведомлениям, данным о здоровье и внешним сервисам.
  </Card>

  <Card title="Создание и публикация мобильного приложения" icon="mobile-screen" href="/ru/ja/build/mobile-app">
...
* **Agent создаёт веб-приложение:** начните заново и перед сборкой выберите тип приложения **Mobile app**.
* **Предпросмотр в Expo Go выглядит устаревшим:** встряхните телефон, откройте меню разработчика и выберите **Refresh**.
* **Области нажатия слишком маленькие:** попросите Agent увеличить области касания и разместить основные действия в пределах досягаемости большого пальца.
* **Навигация перегружена:** попросите Agent сократить приложение до минимального набора экранов, необходимых для основного сценария.
* **Симулятор работает, а физическое устройство — нет:** протестируйте на физическом устройстве поведение, зависящее от аппаратных особенностей, и ознакомьтесь с [Нативными мобильными приложениями](/ru/ja/features/artifact-types/building-mobile-apps).
* **Не удаётся опубликовать:** ознакомьтесь с [устранением неполадок мобильных приложений](/ru/ja/features/troubleshooting/mobile-app).

<div id="section-13">
  ## Попробуйте прямо сейчас
</div>

<AgentInput highlightStart defaultPrompt="Создай Pace Mobile — мобильный трекер для бега. Пользователь должен иметь возможность выбрать тип пробежки, указать дистанцию и время, сохранить пробежку и посмотреть сводку за сегодня. Сделай интерфейс удобным для управления одной рукой: с крупными областями нажатия, простой навигацией и проработанным стилем фитнес-приложения. Первая версия должна быть достаточно компактной, чтобы протестировать её уже сегодня в симуляторе и на реальном телефоне." showTypewriter={false} />

<div id="section-14">
  ## Связанные материалы
</div>

* [Нативные мобильные приложения](/ru/ja/features/artifact-types/building-mobile-apps)
* [Создание и публикация мобильного приложения](/ru/ja/build/mobile-app)
* [Введение в вайб-кодинг](/ru/ja/learn/foundations/vibe-coding-101)
* [Устранение неполадок мобильных приложений](/ru/ja/features/troubleshooting/mobile-app)