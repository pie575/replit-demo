---
title: "Доска задач"
description: "Используйте доску задач, чтобы просматривать, запускать, отслеживать и применять задачи Agent по мере их прохождения по всем этапам."
---
...
Доска задач — это единое место, где можно просматривать, запускать и отслеживать задачи Agent, а также применять их результаты. Используйте её, когда Agent разбивает крупный запрос на несколько частей, когда вы хотите выполнять работу в фоновом режиме или когда нужно понять, что уже изменилось в проекте.
...
Задачи распределяются по столбцам в зависимости от статуса. Доска показывает, что запланировал Agent, что выполняется сейчас, что готово к проверке и что уже применено.

По мере выполнения каждая задача перемещается слева направо:

* **Черновики**: задачи, которые Agent запланировал, но ещё не начал
* **Активные**: задачи, над которыми Agent работает прямо сейчас, включая задачи, ожидающие в очереди из-за лимита активных задач
* **Готово**: задачи, которые Agent завершил и которые ждут вашей проверки
* **Завершено**: задачи, которые закрыты, помещены в архив или отменены
...
Черновики удобны, когда крупную функцию нужно спланировать до того, как запускать фоновую работу. Например, Agent может разбить проект интеграции на отдельные черновики для аутентификации, настройки профиля, интерфейса панели управления и финальной доработки деплоя.

<div id="ativo">
  ## Активные
</div>

После того как вы примете задачу, она перейдёт в колонку **Активные**, и Agent приступит к работе. Каждая задача выполняется в собственной изолированной копии проекта, поэтому в основной версии ничего не меняется, пока вы не примените результат.

Одни задачи зависят от других. Например, задаче по созданию панели управления может потребоваться, чтобы сначала завершилась задача по схеме базы данных. Зависимые задачи ожидают со статусом **В очереди**, пока не будут выполнены их предварительные условия. Задачи также попадают в очередь, если вы достигли предусмотренного вашим планом лимита активных фоновых задач.
...
Когда сборка задачи завершается, она переходит в статус **Готово**. Это значит, что работа выполнена, но в вашей основной версии пока ничего не изменилось.

Применить изменения можно двумя способами:

* **Через тред**: откройте тред задачи и выберите **Применить изменения к основной версии**, чтобы перенести работу в проект, или **Отклонить**, чтобы отказаться от неё
* **Через доску**: нажмите на меню из трёх точек на любой карточке готовой задачи, затем выберите **Применить изменения**

<TaskReviewDrawer />

Проверяйте готовые задачи перед применением. Изучите журнал работы, вывод тестов и Preview, чтобы понять, что именно изменил Agent.
...
В колонку «Завершённые» также попадают архивированные сессии планирования и отменённые задачи. Так сохраняется история происходившего, а завершённая работа не возвращается в «Черновики» или «Активные».
...
Используйте доску задач, когда в проекте много задач в работе. Вы можете искать по названию или номеру задачи, отфильтровывать черновики и с помощью режима редактирования управлять сразу группами задач.

Например, ищите по номеру задачи, если кто-то упомянул её в треде, при ревью или в переписке в Slack. Фильтруйте черновики, когда нужно сосредоточиться на запланированной работе и не просматривать активные или завершённые задачи.

Рекомендации по дальнейшим действиям см. в разделе [Последующие задачи](/ru/pt/features/agent/follow-up-tasks).

<div id="renomear-uma-tarefa">
  ## Переименование задачи
</div>

По мере изменения объёма работ длинные списки задач становится трудно просматривать. Чтобы переименовать задачу, откройте меню с тремя точками на её карточке (на боковой панели со списком задач или на доске задач) и выберите **Переименовать**. Введите более понятное название и нажмите **Сохранить**.

Переименование меняет только то, как задача отображается для вас и ваших коллег. На уже выполненную Agent работу, историю задачи и незавершённые изменения это не влияет.

<div id="configuraes-em-cada-tarefa">
  ## Настройки каждой задачи
</div>

У каждой задачи есть собственное выпадающее меню настроек рядом с заголовком. Откройте его, чтобы задать, как Agent завершает задачу и как результат попадает в ваш проект:

* **Применять изменения автоматически** (Черновики и Активные): когда задача будет готова, сразу применить её к основной версии, не дожидаясь проверки.
* **Утверждать план автоматически** (Черновики): когда Agent предложит план для задачи, принять его автоматически и приступить к сборке.
* **Применить изменения** (Готово): применить выполненную работу к основной версии.
* **Переименовать**: изменить заголовок на карточке.
* **Просмотреть изменения** (Готово): сразу перейти к сравнению изменений готовой задачи.
* **Отменить**: остановить задачу или удалить черновик.

Автоприменение и автоутверждение — это разовые быстрые действия, а не переключатели, которые остаются включёнными. Ширина меню всегда одинакова, поэтому подписи не смещаются при переходе между задачами.

<Frame>
  <img src="/assets/mirror/task-settings-dropdown-7ea7868303b4.png" alt="Открытое выпадающее меню настроек задачи на карточке задачи с действиями «Применить изменения», «Переименовать», «Просмотреть изменения» и «Отменить» во всплывающем окне фиксированной ширины" width="858" height="536" data-path="images/replitai/task-settings-dropdown.png" />
</Frame>