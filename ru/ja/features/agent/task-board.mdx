---
title: "Доска задач"
description: "Используйте доску задач, чтобы просматривать, запускать, отслеживать и применять задачи Agent на каждом этапе."
---
...
Доска задач — это единое место для просмотра, запуска, отслеживания и применения задач Agent. Используйте её, когда Agent разбивает большой запрос на несколько частей, когда нужно выполнять работу в фоновом режиме или узнать, какие изменения уже внесены в проект.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Доска задач с задачами, упорядоченными по столбцам «Черновики», «Активные», «Готово к проверке» и «Завершено»" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

Задачи упорядочены по столбцам в зависимости от статуса. На доске отображаются задачи, запланированные Agent, выполняемые задачи, задачи, ожидающие проверки, и уже применённые задачи.

Каждая задача перемещается слева направо по мере выполнения:

* **Черновики**: задачи, запланированные Agent, но ещё не начатые
* **Активные**: задачи, над которыми Agent сейчас работает, включая задачи, поставленные в очередь сверх лимита активных задач
* **Готово к проверке**: задачи, завершённые Agent и готовые к проверке
* **Завершено**: завершённые, архивированные или отменённые задачи

<div id="section-2">
  ## Черновики
</div>

**Черновики** — это запланированные задачи, к выполнению которых Agent ещё не приступил. Каждый черновик содержит заголовок, описание и пошаговый план.

Используйте черновики, чтобы ознакомиться с предложенным Agent подходом до начала работы. Вы можете просмотреть план, попросить Agent внести изменения или начать задачу, когда будете готовы.

Черновики полезны, когда нужно спланировать крупные функции до подтверждения выполнения фоновых задач. Например, Agent может разделить проект адаптации на отдельные черновики для аутентификации, настройки профиля, UI панели мониторинга и завершения деплоя.

<div id="section-3">
  ## Активные
</div>

Когда вы одобряете задачу, она перемещается в столбец **Активные**, и Agent начинает её выполнять. Каждая задача выполняется в отдельной копии проекта, поэтому основная версия не изменяется, пока вы не примените изменения.

Некоторые задачи могут зависеть от других. Например, задача по созданию панели мониторинга может требовать, чтобы задача по схеме базы данных была завершена первой. Зависимые задачи остаются в статусе **В очереди**, пока не будут выполнены предварительные условия. Задачи также попадают в Queue, если достигнут лимит активных фоновых задач вашего тарифа.

<KanbanColumns
  columns={[
{ title: 'Черновик', count: 2, cards: [
{ title: 'Добавить возможность дарить истории семье и друзьям', description: 'Позволяет родителям создавать истории для других детей и отправлять их в подарок. Это открывает новый канал привлечения.', status: 'draft-static', timestamp: '2m' },
{ title: 'Режим чтения перед сном с тёмной темой', description: 'Превращает App в полноценный ритуал перед сном благодаря специальному режиму с тёплой тёмной темой и атмосферными звуками.', status: 'draft-static', timestamp: '2m' },
] },
{ title: 'Активные', count: 3, cards: [
{ title: 'Продолжение истории', description: 'Добавляет кнопку «Продолжить приключение», которая создаёт продолжение предыдущей истории.', status: 'active', timestamp: '12m' },
{ title: 'Повторная генерация истории: переделать страницу', description: 'Возможность повторно генерировать отдельные страницы даёт родителям больше контроля и снижает разочарование.', status: 'active', timestamp: '8m' },
{ title: 'Добавить возможность дарить истории семье и друзьям', description: 'Позволяет родителям создавать истории для других детей и отправлять их в подарок. Это открывает новый канал привлечения.', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
После завершения работы над задачей она переходит в статус **Готово к проверке**. Это означает, что работа завершена, но изменения ещё не внесены в основную версию.

Применить изменения можно двумя способами:

* **Из треда**: откройте тред задачи и выберите **Применить изменения к основной версии**, чтобы внести изменения в проект, или **Отклонить**, чтобы отменить их
* **С доски**: откройте меню с тремя точками на карточке задачи в статусе «Готово к проверке» и выберите **Применить изменения**

<TaskReviewDrawer />

Перед применением изменений проверьте задачу в статусе «Готово к проверке». Просмотрите журнал работы, результаты тестов и предпросмотр, чтобы понять, какие изменения внёс Agent.

<div id="section-5">
  ## Завершено
</div>

После применения карточка задачи перемещается в столбец **Завершено**.

В столбце «Завершено» также отображаются архивированные сессии планирования и отменённые задачи. Так вы можете просмотреть историю выполненной работы, не смешивая её с черновиками и активными задачами.

<KanbanColumns
  columns={[
{ title: 'Готово к проверке', count: 3, cards: [
{ title: 'Режим чтения перед сном с тёмной темой', description: 'Специальный режим перед сном с тёплой тёмной темой и фоновыми звуками.', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: 'Ссылки на истории для общего доступа с OG-превью', description: 'Добавление общедоступных ссылок — самый эффективный рычаг роста.', status: 'ready-drop', timestamp: '31m', showMenu: true },
{ title: 'Расширенная настройка персонажей', description: 'Дополнительные параметры внешности сделают иллюстрации по-настоящему персонализированными.', status: 'ready-drop', timestamp: '28m', showMenu: true },
] },
{ title: 'Завершено', count: 4, cards: [
{ title: 'Режим чтения вслух с подсветкой слов', description: 'Озвучивание текста позволяет детям самостоятельно наслаждаться историями.', status: 'done', timestamp: '4h' },
{ title: 'Ссылки на истории для общего доступа с OG-превью', description: 'Общедоступные ссылки — самый эффективный рычаг роста.', status: 'done', timestamp: '1h' },
...
{ title: 'Адаптация для мобильных устройств и подготовка к продакшену', description: 'Улучшенная обработка ошибок, мобильная раскладка и анимации загрузки.', status: 'done', timestamp: '4h' },
] }
]}
/>

При применении изменений из нескольких задач Agent автоматически разрешает конфликты. Если что-то выглядит неправильно, попросите Agent это исправить.

<div id="section-6">
  ## Поиск и фильтрация задач
</div>

Если в проекте выполняется много задач, используйте доску задач. Ищите задачи по названию или номеру, фильтруйте черновики и используйте режим редактирования для одновременного управления группами задач.

Например, ищите по номеру задачи, если она упоминается в треде, при проверке или в беседе Slack. Фильтруйте черновики, если хотите сосредоточиться на запланированной работе, не просматривая активные или завершённые задачи.

Подробнее о рекомендуемых дополнительных задачах см. в разделе [Дополнительные задачи](/ru/ja/features/agent/follow-up-tasks).

<div id="section-7">
  ## Переименование задач
</div>

При изменении объёма работы список долгосрочных задач может стать менее удобным для просмотра. Чтобы переименовать задачу, откройте меню с тремя точками на карточке задачи (на боковой панели списка задач или на доске задач) и выберите **Переименовать**. Введите более понятное название и выберите **Сохранить**.

Переименование задачи меняет только её отображение для вас и ваших коллег по команде. Оно не влияет на работу, уже выполненную Agent, историю задачи или ожидающие изменения.

<div id="section-8">
  ## Настройки каждой задачи
</div>

У каждой задачи есть собственное выпадающее меню настроек рядом с заголовком. В нём можно указать, как Agent будет выполнять задачу и как результаты будут применяться к проекту:

* **Автоматически применить изменения** (черновик и активная): применяет изменения к основной версии сразу после готовности задачи, не дожидаясь проверки.
* **Автоматически одобрить план** (черновик): автоматически одобряет предложенный Agent план задачи и начинает работу.
* **Применить изменения** (готовая к проверке): применяет завершённую работу к основной версии.
* **Переименовать**: изменяет заголовок, отображаемый на карточке.
* **Просмотреть изменения** (готовая к проверке): сразу переходит к различиям готовой задачи.
* **Отменить**: останавливает задачу или удаляет черновик.

Автоматическое применение и автоматическое одобрение — это быстрые одноразовые действия, а не переключатели, которые остаются включёнными. Меню сохраняет фиксированную ширину, поэтому подписи не смещаются при открытии разных задач.

<Frame>
  <img src="/assets/mirror/task-settings-dropdown-7ea7868303b4.png" alt="Открыто выпадающее меню настроек карточки задачи: действия «Применить изменения», «Переименовать», «Просмотреть изменения» и «Отменить» отображаются во всплывающем окне фиксированной ширины" width="858" height="536" data-path="images/replitai/task-settings-dropdown.png" />
</Frame>