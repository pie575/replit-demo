---
title: "Replit Agent"
description: "Replit Agent transforme vos idées en applications, designs, présentations et bien plus encore, simplement à partir de vos mots. Aucune compétence en programmation requise."
sidebarTitle: "Replit Agent"
icon: "robot"
---
...
<div id="what-is-replit-agent">
  ## Qu&#39;est-ce que Replit Agent ?
</div>

Agent est votre partenaire créatif. Il part de vos idées, vous aide à les affiner, puis les concrétise. Contrairement à un chatbot qui se contente de répondre à vos questions, Agent passe à l&#39;action : il configure votre projet, crée des applications, vérifie son travail et corrige les problèmes en cours de route.

Décrivez ce que vous voulez avec vos mots de tous les jours. Aucune connaissance technique ni compétence en programmation n&#39;est nécessaire. Agent s&#39;occupe du reste, de la planification au déploiement.

<div id="how-to-use-replit-agent">
...
<Steps>
  <Step title="Décrivez ce que vous voulez">
    Dans le [Project Editor](/fr/learn/projects-and-artifacts/project-editor), il suffit de commencer à discuter. Décrivez l&#39;application que vous souhaitez créer, posez une question, explorez un sujet ou récupérez des données depuis des [services connectés](/fr/features/agent/general-agent) comme BigQuery, Slack ou Notion. Il n&#39;y a aucune contrainte : Replit Agent gère tout ce que vous lui soumettez.

    <AgentInput />
  </Step>

  <Step title="Choisissez ce que vous voulez créer">
    Vous pouvez également sélectionner un type de projet : web app, application mobile, diaporama, design, visualisation de données, et bien plus encore. Si vous avez déjà décrit ce que vous vouliez à l&#39;étape 1, Agent détermine automatiquement la configuration adaptée.

    <AgentCarousel />
  </Step>

  <Step title="Agent s'occupe de la création">
    Agent écrit le code, met en place l&#39;infrastructure et teste le résultat. Une fois dans votre projet, vous pouvez basculer entre les [modes Agent](#agent-modes) pour contrôler sa façon de construire.
  </Step>

  <Step>
    Discutez avec Agent pour affiner votre projet, puis publiez-le quand vous êtes prêt. Tous les artifacts sont publiés ensemble.
  </Step>
</Steps>

<Note>Vous n&#39;êtes pas limité à un seul type. Commencez par une web app, puis ajoutez une application mobile, un diaporama ou une vidéo — le tout dans le même projet.</Note>

<div id="plan-mode">
  ### Mode Plan
</div>

Activez le mode Plan pour réfléchir, poser des questions et structurer votre projet avant qu&#39;Agent ne modifie du code ou des données. En mode Plan, Agent peut :

* **Décomposer les projets complexes** en listes de tâches ordonnées
* **Explorer différentes approches** et en évaluer les compromis
* **Examiner et affiner** avant d&#39;écrire la moindre ligne de code

Cliquez sur « Plan » dans la zone de saisie du chat — ou demandez-le simplement à Agent — pour passer en mode Plan. Exemple : « Crée un plan pour construire un outil de suivi de projet pour mon équipe »

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent crée une liste de tâches ordonnée que vous pouvez examiner et affiner. Lorsque le plan vous convient, approuvez-le : Agent se met alors à construire.
...
Pour en savoir plus, consultez le [guide du mode Plan](/fr/features/agent/plan-mode).

<div id="agent-modes">
  ### Modes d&#39;Agent
</div>

Choisissez la façon dont Agent développe votre projet :

<Frame caption="La fenêtre contextuelle des modes d'Agent avec les modes Free, Power et Max.">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Fenêtre contextuelle des modes d'Agent affichant les modes Free, Power et Max avec Power sélectionné, une liste de modèles principaux et un curseur d'effort" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode** : le travail quotidien d&#39;Agent sans frais, dans la limite du quota de votre plan. Les quotas Core et Pro se réinitialisent toutes les cinq heures et sont soumis à des limites hebdomadaires. Le niveau Starter conserve ses limites actuelles. Consultez **Paramètres → Utilisation** pour suivre votre consommation.
* **Auto** : sélection automatique du modèle pour les tâches payantes en mode Power ou Max sur Core et Pro.
* **Power** : un mode payant de milieu de gamme, avec des modèles puissants à un prix abordable.
* **Max** : un mode payant haut de gamme, avec les meilleurs modèles de pointe pour les tâches les plus exigeantes.

Si une demande nécessite plus de capacité que Free Mode, Agent peut proposer une option payante **Autoriser une fois**. Replit demande une confirmation avant le lancement d&#39;une action payante, puis Agent revient à Free Mode.

<Note>Astuce : vous pouvez aussi activer le **[mode Plan](/fr/features/agent/plan-mode)** pour examiner et affiner le plan d&#39;Agent avant le début du développement.</Note>

<div id="what-you-can-build-with-the-replit-agent">
  ## Ce que vous pouvez créer avec Replit Agent
</div>

* **Applications web, applications mobiles, tableaux de bord de données et outils propulsés par l&#39;IA**
* **Designs visuels et prototypes** — explorez des maquettes sur le [Design Canvas](/fr/design/canvas) avant de passer au code
* **Plusieurs résultats dans un même projet** — applications web, applications mobiles, diapositives et vidéos partageant le même backend
...
<Card title="Commencez à créer dès maintenant" icon="arrow-up-right" href="https://replit.com">
  Décrivez votre idée et laissez Agent lui donner vie — aucune configuration requise.
</Card>
...
<Accordion title="Faut-il savoir coder ?">
  Non. Agent se charge de toute la partie technique : écriture du code, mise en place de l&#39;infrastructure, configuration des bases de données. Vous décrivez ce que vous voulez en langage courant et Agent le crée.
</Accordion>

<Accordion title="En quoi Auto se distingue-t-il de Free, Power et Max ?">
  **Free Mode** couvre le travail quotidien avec Agent, sans frais, dans la limite de l&#39;allocation de votre plan. Dans Power ou Max, **Auto** sélectionne un modèle pour chaque tâche payante. **Power** est un mode payant intermédiaire, avec des modèles puissants à un prix abordable. **Max** est le mode payant haut de gamme, pour les travaux les plus exigeants.

  Les actions payantes nécessitent une confirmation avant de démarrer.
</Accordion>

<Accordion title="Puis-je créer plusieurs éléments dans un même projet ?">
  Oui. Vous pouvez ajouter des web apps, des applications mobiles, des diaporamas, des vidéos et des visualisations de données au [même projet](/fr/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) — le tout partageant le même backend et les mêmes données.
</Accordion>

<Accordion title="Que se passe-t-il si Agent commet une erreur ?">
  Agent teste régulièrement son propre travail. Il crée également des checkpoints au fil de son travail, ce qui vous permet de revenir à n&#39;importe quel état antérieur. Vous pouvez aussi lui décrire dans le Chat ce qui n&#39;a pas fonctionné : il corrigera le problème.
</Accordion>
...
Les workspaces Enterprise utilisent un mode Auto unique au lieu de Free Mode. Auto sélectionne le mode et un modèle autorisé pour chaque tâche. Les administrateurs de compte peuvent gérer l&#39;accès aux modèles grâce aux [contrôles de modèles Enterprise](/fr/features/collaboration/enterprise-model-controls).

Pour connaître les tarifs détaillés, consultez [Facturation Agent](/fr/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  ...
  <Card title="Système de tâches" icon="list-check" href="/fr/core-concepts/agent/task-system">
    Planification Kanban, tâches en arrière-plan et modifications répercutées sur la version principale.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/fr/design/canvas">
    Maquettes visuelles et éditeur visuel interactif.
  </Card>

  <Card title="Connectors" icon="plug" href="/fr/features/agent/general-agent">
    Connectez BigQuery, Linear, Slack, Notion et bien d&#39;autres services.
  </Card>

  <Card title="Facturation" icon="credit-card" href="/fr/billing/ai-billing#agent-billing">
    Tarifs d&#39;Agent, comparaison des forfaits et gestion des dépenses.
  </Card>

  <Card title="Guide du vibe coding" icon="wand-magic-sparkles" href="/fr/learn/build-with-agent">
