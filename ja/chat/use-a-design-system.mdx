---
title: "デザインシステムを使う"
description: "Replitに一貫性のある視覚的な成果物を作成してほしいときは、保存済みのデザインシステムを色、タイポグラフィ、間隔、コンポーネントのコンテキストとして使用しましょう。"
sidebarTitle: "デザインシステムを使う"
icon: "palette"
...
デザインシステムは、Replitに再利用可能な視覚的方向性を与えます。カラートークン、タイポグラフィ、間隔、サンプルコンポーネント、ロゴ、ブランドアセットを取り込むことで、新しいデザインを一貫性のあるものに保てます。

プロンプトボックスから**プラス**メニューを選択し、保存済みのシステムに沿って作成したい場合は**デザインシステムを使う**を選びます。

## デザインシステムが役立つ場面

* **一貫して作成する：** 新しい作業全体で色、タイポグラフィ、間隔、コンポーネントを揃えます。
* **チームに共通の方向性を与える：** 1つの保存済みシステムを、複数のプロジェクトの視覚的な基準として使用します。
* **既存の作業を作り直す：** 成果物を別の視覚言語に合わせる必要があるときにシステムを適用します。

保存済みのシステム名と、必要な結果を伝えましょう。ブリーフ、Figmaフレーム、スクリーンショット、コンテンツ要件など、作業が達成すべき内容を定義する他の関連コンテキストがあれば追加してください。

例：「保存済みのAcmeデザインシステムを使って、モバイルのオンボーディング画面を作成してください。既存のタイプスケールとボタンスタイルを維持しつつ、2つのレイアウトの方向性を見せてください。」

デザインシステムは視覚的な作業の方向性を決めます。[Skill](/ja/chat/agent-skills)は、タスクのための再利用可能な指示やアプローチを提供します。

<Frame caption="色、タイポグラフィ、間隔、サンプルコンポーネントを備えた保存済みのデザインシステム。">
  <img src="/assets/mirror/ds-review-518570f07ef8.png" alt="カラーパレット、タイプスケール、間隔、サンプルコンポーネントを示すAPEX X1デザインシステム" width="2000" height="1254" data-path="images/design/replit-design/ds-review.png" />
</Frame>

## Designでデザインシステムのワークフローに沿って進める

Designガイドでは、システムに含まれる内容、システムの作成や抽出方法、既存の作業への適用方法を説明しています。

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ja/design/design-systems-explained">
    ...
    <div className="chat-card-title">デザインシステムを解説する</div>
    <p>デザインシステムに含まれる内容と、それが役立つ場面を学びましょう。</p>
    <div className="chat-card-link">詳しく見る →</div>
  </a>

  <a className="chat-example has-visual" href="/ja/design/create-a-design-system">
    ...
    <div className="chat-card-title">デザインシステムを作成する</div>
    <p>プロンプトから作成するか、既存のデザインから抽出します。</p>
    <div className="chat-card-link">Designガイドを開く →</div>
  </a>
</section>

既存の作業については、[デザインシステムを適用する](/ja/design/apply-design-system)を参照してください。

プロジェクトのコンテキストと視覚的な方向性が整ったら、画像、音声、動画、SVGといった[アセットを作成する](/ja/chat/create-assets)方法を確認しましょう。
