---
title: "결제 추가"
description: "Velocity 차량 예약 시 환불 가능한 보증금을 받고, 픽업 시 잔금을 청구합니다."
---
...
이 작업이 끝나면 Velocity 고객이 환불 가능한 보증금을 내고 차량을 예약한 다음, 픽업 시 잔금을 지불하게 됩니다.

로그인과 데이터베이스를 Velocity에 추가했습니다. 이제 실제 예약 비즈니스로 전환하세요. **예약 시 환불 가능한 보증금**으로 각 차량을 확보하고, **픽업 시 잔금을 청구합니다**. Replit은 Stripe를 통해 결제를 처리합니다 — Stripe 코드를 직접 작성할 필요가 없습니다.
...
<AiPrompt>
  테스트 모드에서 Stripe를 사용해 Velocity에 보증금-잔금 결제를 추가하세요.<br />
  예약 시 \$500 환불 가능 보증금을 받고 고객의 결제 수단을 파일에 저장하세요.<br />
  픽업 시 저장된 결제 수단으로 나머지 잔금을 청구하세요.<br />
  픽업 48시간 이상 전에 취소하면 보증금을 전액 환불하세요. 48시간 이내에는 보유합니다.<br />
  고객이 예약에서 보증금 상태와 저장된 카드를 모두 볼 수 있도록 데이터베이스에 저장하세요.<br />
  Stripe에서 어떤 시크릿 키를 가져와야 하는지, 어디에 붙여넣어야 하는지 정확히 알려주세요.<br />
  보증금 성공, 보증금 취소, 잔금 청구, 취소 환불 케이스에 대한 테스트 체크리스트를 제공해 주세요.
</AiPrompt>
...
<Frame>
  <img src="/assets/mirror/stripe-secrets-b04a3e238f51.png" alt="Agent가 빌더에게 붙여넣도록 지시한 Stripe API 키(STRIPE_SECRET_KEY, STRIPE_PUBLISHABLE_KEY, STRIPE_WEBHOOK_SECRET)가 표시된 Replit Secrets 패널" width="1280" height="720" data-path="images/velocity/add-payments/stripe-secrets.png" />
</Frame>
...
2. 체크아웃 시 "오늘 보증금: $500"과 "픽업 시 잔금: $1,900" (또는 계산된 금액)이 표시되어야 합니다.
...
<AiPrompt>
  Velocity의 Stripe 테스트 보증금은 성공했지만 예약이 확인된 것으로 표시되지 않습니다.<br />
  보증금 성공 이벤트를 데이터베이스의 예약 상태에 연결하세요.<br />
  다른 테스트 예약으로 확인하는 방법을 알려주세요.
</AiPrompt>
