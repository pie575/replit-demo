---
title: "结合 Figma 使用"
description: "将 Figma frame 用作视觉上下文，当你想要探索、优化或基于它进行构建时，按照 Replit Design 的导入流程操作。"
sidebarTitle: "结合 Figma 使用"
icon: "figma"
...
如果你已经确定了视觉方向，希望 Replit 在此基础上继续推进，或将其转化为 design，就可以导入 Figma frame。

<div id="when-figma-helps">
  ## Figma 在哪些场景下有帮助
</div>

* **探索另一种方向：** 保留核心层级结构，尝试不同的风格或布局。
* **延续产品流程：** 将已有界面作为上下文，生成下一个界面或状态。
* **推动设计成果落地为应用：** 从已确认的视觉方向出发，在此基础上细化并构建。

<div id="import-a-figma-frame">
  ## 导入 Figma frame
</div>

选择能代表你想继续推进的工作的那个 frame。Replit Design 会把导入的 frame 转换为 design frame，你可以在其中尝试各种修改、对比 suggestions，并决定下一步构建什么。

<Frame caption="作为 design frame 导入到 Replit Design 中的 Figma frame。">
  <img src="/assets/mirror/figma-import-result-052394025b8d.webp" alt="在 Replit Design 中作为 design frame 导入的 Personal Fitness Trainer landing page" width="3440" height="2160" data-path="images/fitstart/turn-mockup-into-app/figma-import-result.webp" />
</Frame>
...
Design 指南完整介绍了导入、优化到构建的整个工作流。

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/design/import-figma-designs-into-replit-design">
