...
description: "Comprenez les états des tâches, les travaux en file d'attente, l'application des modifications, l'archivage et l'annulation des tâches Agent."
---
...
Chaque tâche Agent suit un cycle de vie : planifiée, en cours d&#39;exécution, en attente de revue, terminée. Ce cycle de vie garantit que le travail reste isolé jusqu&#39;à ce que vous décidiez de ce qui sera intégré à la version principale.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Tableau des tâches affichant les tâches organisées dans les colonnes Brouillon, Actives, Prêtes et Terminées" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>
...
Un **brouillon** est une tâche qu&#39;Agent a planifiée, mais dont il n&#39;a pas encore commencé la réalisation. Un brouillon ne contient que le travail proposé (titre de la tâche, description et plan) : vous pouvez donc le consulter en toute sécurité.

Utilisez les brouillons pour vérifier qu&#39;Agent a bien compris votre objectif. Vous pouvez lancer la tâche, demander à Agent de corriger le plan ou archiver le brouillon pour plus tard.

<div id="section-2">
  ## Actives
</div>

Les tâches **actives** s’exécutent en arrière-plan. Agent travaille sur une copie indépendante du projet ; la version principale n’est donc pas modifiée pendant l’exécution de la tâche.

Vous pouvez continuer à utiliser le fil principal pendant l’exécution des tâches actives. Si une tâche a besoin de votre retour, ouvrez son fil et poursuivez la conversation.

<div id="section-3">
  ## En attente
</div>

Les tâches **en attente** sont approuvées, mais n&#39;ont pas encore démarré. Une tâche peut être mise en attente si elle dépend d&#39;une autre tâche ou si la limite de tâches d&#39;arrière-plan actives de votre forfait est atteinte.

Les tâches en attente démarrent automatiquement lorsque leurs prérequis sont remplis et qu&#39;un créneau de tâche active se libère.

<div id="section-4">
  ## Prêt
</div>

La tâche **Prêt** est terminée, mais les modifications n’ont pas encore été appliquées à la version principale. Avant de les appliquer, consultez le journal de travail de la tâche, les résultats des tests et l’aperçu.

Si le travail est correct, appliquez les modifications à la version principale. Sinon, rejetez-les et démarrez une nouvelle tâche.

<div id="section-5">
  ## En cours d’application
</div>

Les tâches **En cours d’application** transfèrent les modifications vers la version principale. Agent gère automatiquement les conflits lors de l’application de modifications issues de plusieurs tâches.

Attendez la fin de l’application avant d’évaluer le résultat final. En cas de problème, demandez ensuite à Agent de le corriger dans le fil principal.

<div id="section-6">
  ## Terminé
</div>

Les tâches **Terminées** sont closes et ne nécessitent aucune action. Elles incluent les tâches appliquées à la version principale, archivées avant leur démarrage ou annulées après le début de leur création.

Utilisez la colonne **Terminé** comme historique des tâches terminées, archivées ou annulées.

<div id="section-7">
  ## Exemples de statuts
</div>

Toutes les tâches passent par différentes étapes, de l’idée à leur intégration dans votre application. Chaque étape a sa propre icône, ce qui vous permet de voir en un coup d’œil ce qui se passe.

<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Ajouter un parcours d’envoi de cadeaux à la famille et aux amis" description="Permettre aux parents de créer des histoires pour d’autres enfants et de les envoyer en cadeau." status="draft-static" timestamp="2m" width="100%" />
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="La suite de l’histoire : le prochain épisode" description="Ajoutez un bouton « Continuer l’aventure » pour générer une suite qui reprend là où l’histoire précédente s’est arrêtée." status="active" timestamp="12m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Actif</strong> : Agent est en train de réaliser cette tâche. Il travaille sur sa propre copie du projet afin que la version principale reste intacte.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Ajouter un flux de cadeaux pour la famille et les amis" description="Permet aux parents de créer des histoires pour d’autres enfants et de les envoyer en cadeau." status="active-queued" timestamp="1m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>En file d’attente</strong> : cette tâche est approuvée, mais attend qu’un créneau se libère pour lancer la création.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Mode nuit avec thème sombre" description="Mode nuit dédié avec un thème sombre chaleureux et des sons d’ambiance." status="ready-drop" timestamp="35m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Prêt</strong> : cette tâche est terminée. Examinez les modifications et, lorsque vous êtes prêt, appliquez-les à la version principale.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Lien partagé avec aperçu OG" description="L’ajout de liens de partage publics est le principal levier de croissance." status="applying" timestamp="31m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>En cours d’application</strong> : les modifications de cette tâche sont en cours d’application à la version principale.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Lecture à voix haute avec surlignage des mots" description="Grâce à la synthèse vocale, les enfants peuvent suivre les mots surlignés et profiter des histoires en toute autonomie." status="done" timestamp="4h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Terminé</strong> : cette tâche est terminée et les modifications sont intégrées à la version principale.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Visite d'intégration pour les nouveaux lecteurs" description="Une courte visite guidée qui présente aux parents le parcours de lecture principal au premier lancement." status="draft-static" timestamp="1h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Archivé</strong> : sessions de planification mises en attente avant le début du développement. Les sessions de planification archivées peuvent être restaurées ultérieurement depuis la colonne <strong>Terminé</strong>.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Générateur expérimental de packs d'autocollants" description="Générez des autocollants illustrés personnalisés à partir des personnages et des scènes de votre histoire." status="done" timestamp="2h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Annulée</strong> : tâche passée en phase de construction puis interrompue. Les tâches annulées ne peuvent pas être restaurées. Si vous changez d&#39;avis, lancez une nouvelle tâche.</span>
  </div>
</div>
...
Vous pouvez arrêter une tâche à tout moment de son cycle de vie, mais les options proposées dépendent de la position de la tâche sur le tableau. **Archiver** et **Annuler** déplacent tous deux la tâche vers la colonne **Terminé**, mais leur comportement diffère :

| Action       | Disponible quand                                                                                                      | Ce qui se passe                                                                                                          | Réversible ?                                                                                                                 |
| ------------ | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| **Archiver** | La tâche est encore en session de planification : un **brouillon** dont la construction n&#39;a pas commencé          | La session de planification est mise en attente sans être exécutée. Le plan est conservé.                                | Oui. Ouvrez la carte archivée dans la colonne **Terminé** pour la restaurer à l&#39;état de **brouillon**.                   |
| **Annuler**  | La tâche est passée de la planification à la construction : tâches **active**, **en attente** ou **prête** | Agent arrête la tâche et abandonne le travail en cours. Aucune modification n&#39;est appliquée à la version principale. | Non. Les tâches annulées ne peuvent pas être restaurées. Démarrez une nouvelle tâche si vous souhaitez reprendre le travail. |

Utilisez **Archiver** pour les sessions de planification que vous souhaitez reprendre plus tard. Utilisez **Annuler** comme bouton d&#39;arrêt définitif pour les tâches en attente, en cours d&#39;exécution ou dont la construction est terminée.

Les sessions de planification archivées et les tâches annulées apparaissent, avec les tâches terminées, dans la colonne **Terminé**, à l&#39;extrémité droite du tableau des tâches.