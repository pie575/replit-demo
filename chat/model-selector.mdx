...
description: "Choose an available model and set its Effort when manual model selection is available for your mode and plan."
...
<div className="chat-mkt" role="img" aria-label="Anthropic, OpenAI, and Qimi marks representing Model selector" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
Intelligent model routing lets Replit choose a model for the task. To choose one yourself, select **Agent Mode** in the prompt box, choose **Power** or **Max**, then select a model in the **Primary model** section.

Free Mode always uses intelligent model routing. Builders on Core and Pro can choose an available model in Power Mode or Max Mode. The model list in Replit is the source of truth for your account because availability can vary.

<Frame caption="Model and Effort controls in the Agent settings menu.">
  <img src="/assets/mirror/model-selector-gpt-5-6-sol-effort-8e4d256bf497.jpg" alt="Demo workspace model selector with GPT-5.6 Sol selected in Max Mode and Effort set to Medium" width="1821" height="864" data-path="images/chat/model-selector-gpt-5-6-sol-effort.jpg" />
</Frame>

## Set Effort for the model

Effort controls how much reasoning the selected model spends on a response. It is a per-model control, not a separate mode. The supported levels run from **Low** through **Max**, and each model shows a recommended setting.

Use a higher Effort only when the task calls for deeper reasoning. Higher settings can take longer and cost more on complex work.

## Learn more

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/features/agent/model-selector">
    ...
    <div className="chat-card-title">Model selector reference</div>

    <p>
      Review intelligent model routing, available models, Effort levels, and
      Compare models.
    </p>
  ...
  <a className="chat-example has-visual" href="/features/agent/model-selector#effort">
    ...
    <div className="chat-card-title">Effort details</div>
    <p>Compare Low, Medium, High, Extra High, and Max.</p>
    <div className="chat-card-link">Review Effort →</div>
  </a>
</section>

Model and Effort settings shape how Replit responds. [Voice Mode](/chat/voice-mode) changes how you provide the request by letting you speak instead of type.
