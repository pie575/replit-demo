---
title: "Replit Agent"
description: "Replit Agent transforme vos idées en applications, designs, présentations et bien plus encore. Aucune connaissance en programmation n'est requise."
---
...
Agent est votre partenaire créatif : il prend vos idées, les affine et les concrétise. Contrairement à un chatbot qui se contente de répondre à vos questions, Agent passe à l&#39;action : il configure vos projets, crée vos applications, vérifie son propre travail et corrige les problèmes en cours de route.

Décrivez simplement ce que vous voulez, avec vos mots de tous les jours. Aucune connaissance en programmation ni compétence technique n&#39;est nécessaire : Agent se charge de tout, de la planification au déploiement.
...
<Steps>
  <Step title="Décrivez ce que vous voulez">
    Dans l&#39;[éditeur de projet](/fr/ja/learn/projects-and-artifacts/project-editor), lancez une conversation. Vous pouvez décrire l&#39;application que vous souhaitez créer, poser des questions, explorer un sujet ou récupérer des données depuis des [services connectés](/fr/ja/features/agent/general-agent) comme BigQuery, Slack ou Notion. Aucune limite : Replit Agent s&#39;adapte à tout.

    <AgentInput />
  </Step>

  <Step title="Choisissez ce que vous voulez créer">
    Vous pouvez éventuellement sélectionner un type de projet : application web, application mobile, diapositives, design, visualisation de données, etc. Si vous avez déjà décrit ce que vous vouliez à l&#39;étape 1, Agent détermine automatiquement la configuration appropriée.

    <AgentCarousel />
  </Step>

  <Step title="Agent construit">
    Agent écrit le code, configure l&#39;infrastructure et teste les résultats. Une fois dans le projet, vous pouvez changer de [mode Agent](#agent-modes) pour contrôler sa façon de construire.
  </Step>

  <Step>
    Discutez avec Agent pour améliorer votre projet, puis publiez-le lorsque vous êtes prêt. Tous les artefacts sont déployés ensemble.
  </Step>
</Steps>

<Note>Vous n&#39;êtes pas limité à un seul type. Commencez par une application web, puis ajoutez plus tard une application mobile, des diapositives ou une vidéo — le tout au sein du même projet.</Note>

<div id="plan">
  ### Mode Plan
</div>

Activez le mode Plan pour réfléchir, poser des questions et cartographier votre projet avant qu&#39;Agent ne modifie du code ou des données. En mode Plan, Agent :

* **décompose les projets complexes** en une liste de tâches ordonnées ;
* **explore différentes approches** et en évalue les compromis ;
* **vous permet de vérifier et d&#39;affiner** avant qu&#39;une seule ligne de code ne soit écrite.

Cliquez sur « Plan » dans le champ de saisie du chat, ou demandez simplement à Agent de passer en mode Plan. Par exemple : « Établis un plan pour créer un outil de suivi de projet pour mon équipe ».

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent crée une liste de tâches ordonnées que vous pouvez vérifier et affiner. Une fois le plan satisfaisant, approuvez-le pour qu&#39;Agent commence à construire.

<Frame>
  <img src="/assets/mirror/task-accept-plans-d70b85ce3421.png" alt="Boîte de dialogue de création de plan de tâches affichant quatre tâches proposées ainsi que les options « Approuver les tâches » et « Modifier le plan »" width="3430" height="1986" data-path="images/replitai/task-accept-plans.png" />
</Frame>

Pour en savoir plus, consultez le [guide du mode Plan](/fr/ja/features/agent/plan-mode).

<div id="agent">
  ### Mode Agent
</div>

Choisissez la manière dont Agent construit votre projet :

<Frame caption="Popover des modes Agent affichant les modes Free, Power et Max.">
  <img src="/assets/mirror/model-selector-power-mode-52facbd1e206.png" alt="Popover des modes Agent avec Free, Power et Max, où Power est sélectionné, affichant la liste des modèles principaux et le curseur Effort" width="1452" height="966" data-path="images/replitai/model-selector-power-mode.png" />
</Frame>

* **Free Mode** : permet aux abonnés Core et Pro d&#39;utiliser Agent gratuitement au quotidien. L&#39;allocation se réinitialise toutes les 5 heures, avec un plafond hebdomadaire. Le palier Starter conserve ses limites actuelles. Vous pouvez suivre votre consommation dans **Paramètres → Utilisation**.
* **Power** : mode payant intermédiaire qui donne accès à des modèles puissants à un tarif abordable.
* **Max** : mode payant haut de gamme qui donne accès aux meilleurs modèles de pointe pour les tâches les plus exigeantes. Dans les **Paramètres avancés**, vous pouvez également activer **Turbo** pour une exécution plus rapide.

Pour les demandes qui dépassent les capacités de Free Mode, Agent peut proposer une option payante **Autoriser une seule fois**. Replit demande une confirmation avant le démarrage de toute action payante, puis Agent repasse en Free Mode.

<Note>Astuce : vous pouvez également activer le **[mode Plan](/fr/ja/features/agent/plan-mode)** pour examiner et affiner le plan d&#39;Agent avant le début de la construction.</Note>

<div id="replit-agent-3">
  ## Ce que vous pouvez créer avec Replit Agent
</div>

* **Applications web et mobiles, tableaux de bord de données et outils alimentés par l’IA**
* **Designs visuels et prototypes** — explorez des maquettes dans le [Canvas Design](/fr/ja/design/canvas) avant de passer au code
* **Plusieurs livrables dans un même projet** — applications web et mobiles, diapositives et vidéos partageant le même backend
* **Fichiers et documents** — CSV, PDF, fichiers PowerPoint et documents Markdown
* **Interrogez des services connectés** — récupérez directement dans le chat des données depuis BigQuery, Linear, Slack, Notion et bien plus encore
...
<Accordion title="Dois-je savoir coder ?">
  Non. Agent se charge de tout l&#39;aspect technique : écriture du code, configuration de l&#39;infrastructure et de la base de données. Décrivez simplement ce que vous souhaitez, en langage courant, et Agent le construit pour vous.
</Accordion>

<Accordion title="Quelle est la différence entre Free, Power et Max ?">
  **Free Mode** permet aux abonnés Core et Pro d&#39;effectuer gratuitement leur travail quotidien avec Agent, dans la limite de l&#39;allocation incluse. **Power** est un mode payant intermédiaire qui donne accès à des modèles puissants à un tarif abordable. **Max** est le mode payant haut de gamme, conçu pour les tâches les plus exigeantes. **Turbo** est une option facultative réservée à Max, disponible dans les paramètres avancés.

  Les actions payantes doivent être confirmées avant leur lancement.
</Accordion>

<Accordion title="Puis-je créer plusieurs éléments dans un projet ?">
  Oui. Vous pouvez ajouter des applications web, des applications mobiles, des diapositives, des vidéos et des visualisations de données au sein d&#39;un [même projet](/fr/ja/features/projects-and-artifacts/artifacts#multiple-artifacts-in-one-project) — le tout partageant le même backend et les mêmes données.
</Accordion>

<Accordion title="Que se passe-t-il si Agent commet une erreur ?">
  Agent teste régulièrement son propre travail. Il crée également des points de contrôle au fil de l&#39;eau, ce qui vous permet de revenir à n&#39;importe quel état antérieur. Expliquez-lui dans le chat ce qui n&#39;a pas fonctionné, et il corrigera le problème.
</Accordion>
...
Turbo est désactivé par défaut : activez-le projet par projet, selon vos besoins, en ayant sélectionné Max. Free Mode n&#39;est pas disponible sur Enterprise au lancement. Les espaces de travail Enterprise bénéficieront peu après d&#39;une expérience Agent dédiée. Pour en savoir plus, contactez votre administrateur.

Pour connaître les tarifs détaillés, consultez [Facturation d&#39;Agent](/fr/ja/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  <Card title="Compétences Agent" icon="puzzle-piece" href="/fr/ja/features/agent/skills">
    Transmettez des connaissances spécialisées à Agent — utilisez des compétences prédéfinies ou créez les vôtres.
  </Card>

  <Card title="Système de tâches" icon="list-check" href="/fr/ja/core-concepts/agent/task-system">
    Planification kanban, tâches en arrière-plan et application des modifications à la version principale.
  </Card>

  <Card title="Canvas Design" icon="paintbrush" href="/fr/ja/design/canvas">
    Maquettes visuelles et éditeur visuel interactif.
  </Card>

  <Card title="Connecteurs" icon="plug" href="/fr/ja/features/agent/general-agent">
    Connectez BigQuery, Linear, Slack, Notion et bien d&#39;autres services.
  </Card>

  <Card title="Facturation" icon="credit-card" href="/fr/ja/billing/ai-billing#agent-billing">
    Tarifs d&#39;Agent, comparaison des plans et gestion des dépenses.
  </Card>

  <Card title="Guide du vibe coding" icon="wand-magic-sparkles" href="/fr/ja/learn/build-with-agent">
