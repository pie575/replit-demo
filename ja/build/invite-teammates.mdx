---
title: "チームメイトを招待する"
description: "チームメイトをプロジェクトに招待して一緒に作る — 全員がAgentで新しいスレッドを開始しながら同じプロジェクトで作業できます。"
---
...
Replitでは複数人が同時に同じプロジェクトで作業できます。それぞれがAgentで新しいプランニングスレッドを開始でき、全員のタスクが共有ボードに表示されます — チームが交代制でなく一緒に構築できます。
...
1. プロジェクトを開く
2. **招待**をクリックするかプロジェクトリンクを共有する
3. チームメイトが参加してAgentで新しいスレッドを開始する
...
それぞれが新しいスレッドを作成できるため、互いに邪魔することなく独立して作業できます。全員のタスクが共有カンバンボードに表示されるため、誰が何をやっているか把握し、作業の重複を避けることが簡単にできます。

<Note>
  Coreでは、プロジェクト全体で同時に実行できるバックグラウンドタスクは1つです。Proでは最大10個のタスクを並行して実行できます — 複数人が同時にAgentでプロジェクトのさまざまな部分を構築できます。
</Note>

## 共有タスクボード

チームが一緒に作業すると、全員のタスクが同じボードに表示されます。誰が何をやっているか、何が進行中か、何が適用可能かをひと目で確認できます。

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="下書き、アクティブ、準備完了、完了の列にタスクが整理されたタスクボード" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="家族や友人のためのギフトフローを追加する" description="親が他の子供のためにストーリーを作成し、ギフトとして送れるようにします。" status="draft-static" timestamp="2m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>下書き</strong> — Agentがこのタスクを提案しました。レビュー待ちのプランがあります。</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="ストーリーの続き — 続編" description="「冒険を続ける」ボタンを追加すると、前回の続きから始まる続編が生成されます。" status="active" timestamp="12m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>アクティブ</strong> — Agentがプロジェクトの独立したコピーでこのタスクを構築中です。</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="ダークテーマの就寝モード" description="温かみのあるダークテーマとアンビエントサウンドを備えた専用の就寝モード。" status="ready-drop" timestamp="35m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>準備完了</strong> — タスクが完了しました。変更を確認してメインバージョンに適用します。</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="単語ハイライト付きの読み上げ" description="テキスト読み上げ機能で、子供たちがハイライトされた単語を追いながら独立してストーリーを楽しめます。" status="done" timestamp="4h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>完了</strong> — このタスクが終わり、変更がメインバージョンに含まれています。</span>
  </div>
</div>

### チームでのボードの使い方

複数人が同時に構築している場合、それぞれのタスクが共有ボードに表示されます。これがその様子です — あるチームメイトが続編とストーリー再生成に取り組み、別の人がギフトフローに取り組み、チームの残りのメンバーの完成した作業が準備完了と完了に並んでいます。

<KanbanColumns
  columns={[
{ title: 'アクティブ', count: 3, cards: [
{ title: 'ストーリーの続き — 続編', description: '「冒険を続ける」ボタンを追加すると、前回の続きから始まる続編が生成されます。', status: 'active', timestamp: '12m' },
{ title: 'ストーリー再生成 — ページのやり直し', description: '個別のページを再生成する機能を追加すると、親がコントロールでき不満が減ります。', status: 'active', timestamp: '8m' },
{ title: '家族や友人のためのギフトフローを追加する', description: '親が他の子供のためにストーリーを作成し、ギフトとして送れるようにします。', status: 'active-queued', timestamp: '1m' },
] },
{ title: '準備完了', count: 2, cards: [
{ title: 'ダークテーマの就寝読書モード', description: '温かみのあるダークテーマとアンビエントサウンドを備えた専用の就寝モード。', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: 'OGプレビュー付き共有可能なストーリーリンク', description: '公開共有リンクを追加することが単一最大の成長レバーです。', status: 'ready-drop', timestamp: '31m', showMenu: true },
] },
{ title: '完了', count: 3, cards: [
{ title: '単語ハイライト付きの読み上げモード', description: 'テキスト読み上げ機能で、子供たちがストーリーを独立して楽しめます。', status: 'done', timestamp: '4h' },
{ title: 'キャラクターカスタマイズの強化', description: '本当にパーソナライズされたイラストのための外観オプション。', status: 'done', timestamp: '4h' },
{ title: 'モバイル対応と本番環境の整備', description: 'より良いエラー状態、モバイルレイアウト、ローディングアニメーション。', status: 'done', timestamp: '4h' },
] }
]}
/>

タスクが準備完了に移動したら、チームの誰でも変更を確認して適用できます。Agentは複数のタスクが適用される際の競合を自動的に処理します。

## 一緒に構築できること

* **機能をチームで分担する**: 1人がフロントエンドを構築し、別の人がバックエンドを担当します。それぞれがAgentとのチャットを通じて、カンバンボードで全員の調整を保ちながら作業します。
* **新しいチームメイトをオンボードする**: 自分のAgentチャットでプロジェクトを探索し、仕組みについて質問し、貢献を始めることができます — 他の人をブロックすることなく。
* **Proでより速く出荷する**: 並行実行により、2〜3人がそれぞれ同時に実行されるタスクをディスパッチできます。1人が完全なセッションを費やすプロジェクトを、一部の時間で完了できます。

## よくある質問

<Accordion title="Agentを通じてチームメイトにメッセージを送れますか？">
  いいえ。Agentはメッセージングツールではありません — 入力したものに対して常に行動しようとします。チームのコミュニケーションにはSlack、Discordや他のツールを使用してください。
</Accordion>

<Accordion title="互いの作業を上書きしてしまいますか？">
  タスクシステムはそれぞれのタスクに独立したコピーを与えることでこれを防ぎますが、他のタスクの存在を認識しています。タスクが完了したら、メインバージョンに適用してそのステップで競合を解決します。
</Accordion>

<Accordion title="チームメイトが何をしているか確認できますか？">
  カンバンボードでアクティブなタスクを確認できます。カーソルのプレゼンスやチームメイトが表示しているファイルも確認できます。
</Accordion>

<Accordion title="プロジェクトから誰かを削除できますか？">
  はい。プロジェクトオーナーはいつでもチームメンバーを管理してアクセスを取り消すことができます。
</Accordion>
