...
description: "了解 Project Editor：它是你在 Replit 上的大本营，你可以在这里与 Agent 对话、看着应用逐步成型，并管理一切。"
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="Replit Project Editor 主页界面，其中的 prompt 输入框可用于描述你想构建的内容" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>
...
Project Editor 是你在 Replit 上的大本营：在这里，你可以开始构建、管理项目，并看着应用逐步成型。你只需描述想要构建的内容，[Agent](/zh/features/agent/overview) 便会接手后续工作——编写代码、搭建项目，并实时向你展示预览效果。

<div id="getting-started">
  ## 快速开始
</div>

在提示框中输入你想构建的内容。用日常语言描述你的想法——&quot;一个能把我的开支绘成图表的预算跟踪器&quot;、&quot;我的咖啡店落地页&quot;、&quot;给孩子用的 AI 漫画书生成器&quot;——剩下的交给 Agent。

<AgentInput />

你也可以从轮播中选择一种 项目 类型，告诉 Agent 你想要什么样的产出——web app、移动应用、slides、Design 等等。或者干脆跳过选择，直接描述你的想法，Agent 会自动确定合适的配置。

<AgentCarousel />

想立即上手，请参阅[构建你的第一个 app](/zh/build/your-first-app)。

<div id="plan-mode">
  ## Plan 模式
</div>

Plan 模式让你可以头脑风暴、提出问题并梳理 项目的思路。Agent 会生成一份有序的任务清单，供你查看和完善——确认计划无误后，批准即可，Agent 随即开始构建。你也可以用 Plan 模式提问或探索想法；在你退出之前，Agent 不会做任何更改。

<AgentInput highlightPlan={true} showTypewriter={false} />

点击输入框底部的按钮即可切换 Plan 模式。更多内容请参阅 [Plan 模式指南](/zh/features/agent/plan-mode)。

<div id="what-you-can-build">
  ## 你可以构建什么
</div>

选择一种项目类型来告诉 Agent 你想要什么，或者直接描述你的想法，让 Agent 为你选择合适的配置。

* **Web 应用** — 仪表板、内部工具、SaaS 产品、落地页
* **移动应用** — 一条 prompt 即可生成 iOS 和 Android 应用
* **幻灯片与演示文稿** — 生成布局丰富的演示文档
* **动画** — 交互式演示流程和动态图形
* **设计** — 在 [Design Canvas](/zh/design/canvas) 上创建视觉稿和原型
...
Agent 开始构建后，你就能在 Project Editor 中看到项目逐步成型。界面分为两个面板：一侧是你与 Agent 的对话，另一侧是应用的实时预览。

<div id="threads">
  ### 线程
</div>

与 Agent 的每一次对话都发生在某个线程中。主线程是你描述需求、并实时查看 Agent 构建过程的地方。使用 [任务系统](/zh/core-concepts/agent/task-system)时，每个后台 task 都有各自独立的线程 —— 这样 Agent 可以将 task 的工作彼此隔离，不会让多个对话相互干扰。

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="展示用户 prompt 以及 Agent 构建 项目的线程" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>
...
任务看板将你的工作按列组织：**草稿**、**进行中**、**已就绪** 和 **Done**。每个 task 随着进展从左向右移动，让你一眼就能看到哪些已规划、Agent 正在处理哪些，以及哪些已完成。详见[任务系统](/zh/core-concepts/agent/task-system)。

<Frame>
  <img src="/assets/mirror/task-board-active-355ecd390c49.png" alt="任务看板中的 task 按草稿、进行中、已就绪和 Done 列组织展示" width="3430" height="1986" data-path="images/replitai/task-board-active.png" />
</Frame>
...
Design Canvas 是一个可视化编辑器，让你在动手写代码之前先创建并打磨设计稿。拖拽元素、调整布局、反复迭代设计，然后交给 Agent 将其变成可运行的应用。详见 [Design Canvas](/zh/design/canvas)。

<div id="collaboration">
  ## 协作
</div>

创建 Team Workspace，与他人共同构建。邀请队友、共享项目，实时协作开发同一个 App。团队设置请参阅[团队 Workspace](/zh/features/collaboration/team-workspaces)，项目级协作请参阅[邀请队友](/zh/build/invite-teammates)。

<div id="import-code-and-design">
  ## 导入代码与设计
</div>

已经有项目了？可以从 GitHub、Figma 导入到 Replit，也可以直接上传文件。Agent 能接着你上次的进度，基于现有代码或设计继续构建。请参阅[导入到 Replit](/zh/build/import-from-providers)。
...
<CardGroup cols={2}>
  <Card title="编辑器与工具" href="/zh/features/editor/editor-and-tools" icon="window-restore">
    用于查看应用 Preview、控制台输出及其他工具的主要面板
  </Card>

  <Card title="应用设置" href="/zh/features/project-setup/configuration" icon="gear">
    配置应用的设置、依赖项、Secrets 和环境
  </Card>

  <Card title="版本控制" href="/zh/learn/projects-and-artifacts/version-control" icon="clock-rotate-left">
    跟踪更改、回滚到早期版本，并管理应用的历史记录
  </Card>

  <Card title="Publishing" href="/zh/learn/projects-and-artifacts/replit-deployments" icon="rocket">
...
<Accordion title="我需要会编程吗？">
  不需要。用日常语言描述你想要的效果，Agent 会处理技术细节。如果你愿意，也可以自己直接修改。
</Accordion>

<Accordion title="关闭浏览器后会怎样？">
  你的项目保存在云端。随时回来都能从上次中断的地方继续：文件、历史记录和正在运行的应用都会保留。
</Accordion>

<Accordion title="我可以和其他人一起协作吗？">
  可以。你可以邀请协作者加入某个具体项目，也可以创建团队 Workspace，让成员访问其中的所有项目。参阅[团队
  Workspace](/zh/features/collaboration/team-workspaces) 和[邀请队友](/zh/build/invite-teammates)。
</Accordion>

<Accordion title="如何发布我的应用？">
  应用准备就绪后，可直接在 Project Editor
  中将其发布到在线 URL。参阅[部署](/zh/learn/projects-and-artifacts/replit-deployments)。
</Accordion>
...
所有套餐均可使用 Project Editor。部分功能 (如更高的计算资源和常驻运行的部署) 需要付费套餐。详情请参阅[计费](/zh/billing/ai-billing)。