---
title: "앱 SEO 개선"
description: "Replit의 SEO 점수와 Agent를 사용해 배포된 앱이 검색 엔진에서 더 높은 순위를 차지하도록 합니다."
---
...
점수 계산 방법(Lighthouse 감사, 각 점수의 의미, 지원되는 배포 유형)에 대한 자세한 내용은 [SEO 점수](/ko/features/publishing/seo-rating)를 참조하세요.

## SEO Agent로 최적화

[SEO Agent](/ko/features/publishing/seo-agent)는 배포된 앱에 대한 기술적 SEO 감사를 실행하고 각 발견 사항을 원클릭 수정으로 전환합니다. Project Editor의 **Growth** 패널에 위치하며 유료 플랜 빌더에게 제공됩니다. 다음 단계에 따라 앱을 스캔하고 수정 사항을 적용하세요.
...
<Steps>
  <Step title="앱을 배포하여 Growth 잠금 해제">
    ...
    <Frame caption="배포 전에는 Go to Publishing 프롬프트와 함께 Growth 패널이 잠겨 있습니다">
      <img src="/assets/mirror/01-growth-locked-b05c24711f10.png" alt="'앱을 배포하여 Growth 도구 잠금 해제'라는 제목과 Go to Publishing 버튼이 있는 잠긴 상태의 Growth 패널" width="1657" height="947" data-path="images/deployments/seo-agent/01-growth-locked.png" />
    </Frame>
  </Step>
  ...
  <Step title="Agent로 스캔 실행">
    **Growth** 패널을 열고 **SEO 점수** 카드를 찾으세요. **Run scan with Agent**를 선택하여 프로젝트에 대한 기술적 SEO 감사를 시작합니다. 스캔은 보통 몇 분 정도 걸립니다.
    ...
    <Frame caption="SEO 점수 카드에서 Run scan with Agent를 선택하여 감사를 시작하세요">
      <img src="/assets/mirror/02-run-scan-94a2b8918fe9.png" alt="Run scan with Agent 버튼을 빨간 사각형으로 강조한 SEO 점수 카드의 확대 화면" width="1656" height="946" data-path="images/deployments/seo-agent/02-run-scan.png" />
    </Frame>
  </Step>
  ...
  <Step title="Agent로 수정 또는 Agent로 모두 수정">
    ...
    * **Fix all with Agent**를 선택하면 모든 문제를 한 번에 Agent에 전달합니다. 채팅의 **Fix all issues** 버튼도 동일한 기능을 합니다.
    ...
    <Frame caption="스캔 후 Fix all with Agent로 개별, 카테고리별, 또는 한 번에 모든 문제를 수정하세요">
      <img src="/assets/mirror/03-fix-with-agent-c5b9e642818e.png" alt="카테고리별로 분류되고 severity 및 status 열이 있는 스캔 결과 표 — Fix all with Agent 버튼과 카테고리별 Fix with Agent 버튼을 빨간 사각형으로 강조" width="1657" height="947" data-path="images/deployments/seo-agent/03-fix-with-agent.png" />
    </Frame>
  </Step>
  ...
  <Step title="Agent가 수정 작업 진행">
    ...
    <Frame caption="Agent가 수정 작업을 생성하면 문제 행에 Fix in progress 상태와 Go to task 링크가 표시됩니다">
      <img src="/assets/mirror/04-fixes-in-progress-e05dd10c05b3.png" alt="Agent가 작업 중인 동안 Fix in progress 상태와 Go to task 링크가 표시된 스캔 결과 표" width="1657" height="947" data-path="images/deployments/seo-agent/04-fixes-in-progress.png" />
    </Frame>
  </Step>
  ...
  <Step title="메인 버전에 변경 사항 적용">
    ...
    <Frame caption="각 수정 사항을 검토하고 Apply changes to main version을 선택하여 병합하세요">
      <img src="/assets/mirror/05-apply-changes-7d167b6bb93a.png" alt="초록색 Apply changes to main version 버튼을 빨간 사각형으로 강조한 Ready for review 작업 푸터의 확대 화면" width="1656" height="946" data-path="images/deployments/seo-agent/05-apply-changes.png" />
    </Frame>
  </Step>
</Steps>
...
* **커스텀 도메인을 사용합니다.** 브랜드 도메인은 방문자와 검색 엔진 모두에게 신뢰를 구축합니다. [커스텀 도메인 추가](/ko/build/add-custom-domain)를 참조하세요.
* **올바른 배포 유형을 선택합니다.** 콘텐츠가 많은 사이트의 경우 [Static 배포](/ko/features/publishing/deployment-types#static)가 검색 엔진이 즉시 파싱하는 미리 렌더링된 HTML을 제공합니다.
...
<CardGroup cols={2}>
  ...
  <Card title="SEO 점수" icon="magnifying-glass-chart" href="/ko/features/publishing/seo-rating">
  ...
  <Card title="Static 배포" icon="file-code" href="/ko/features/publishing/deployment-types#static">
    콘텐츠가 많고 SEO가 중요한 사이트에 가장 적합한 배포 유형입니다.
  </Card>

  <Card title="배포 지역" icon="globe-pointer" href="/ko/features/publishing/project-geography">
