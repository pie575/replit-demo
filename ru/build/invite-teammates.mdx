---
title: "Пригласите участников команды"
description: "Пригласите участников команды в свой проект и создавайте вместе — каждый сможет начинать новые ветки с Agent, работая над одним проектом."
sidebarTitle: "Пригласите участников команды"
icon: "user-plus"
---
...
Replit позволяет нескольким людям одновременно работать над одним проектом. Каждый может начать новую ветку планирования с Agent, а задачи всех участников отображаются на общей доске — так команда работает вместе, а не по очереди.

<div id="start-building-with-teammates">
  ## Начните разработку вместе с командой
</div>
...
3. Участники команды смогут присоединиться и начать новые ветки с Agent
...
Каждый участник может создавать новые ветки, поэтому вы можете работать независимо, не мешая друг другу. Задачи всех участников отображаются на общей Канбан-доске, поэтому легко увидеть, кто над чем работает, и избежать дублирования работы.

<Note>
  В Core в рамках проекта одновременно может выполняться только 1 фоновая задача. В Pro параллельно могут выполняться до 10 задач — поэтому несколько человек могут одновременно поручить Agent создавать разные части проекта.
</Note>
...
Когда команда работает вместе, задачи всех участников отображаются на одной доске. Вы сразу видите, кто над чем работает, что уже выполняется и что готово к применению.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Доска задач с задачами, распределёнными по столбцам Drafts, В работе, Готово и Выполнено" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Добавить возможность дарить истории семье и друзьям" description="Позволяет родителям создать историю для чужого ребёнка и отправить её в подарок." status="draft-static" timestamp="2m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Draft</strong> — Agent предложил эту задачу. План готов к проверке.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Продолжения историй" description="При нажатии кнопки «Продолжить приключение» создаётся продолжение с того места, где закончилась предыдущая история." status="active" timestamp="12m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>В работе</strong> — Agent прямо сейчас выполняет эту задачу в отдельной изолированной копии проекта.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Режим сна с тёмной темой" description="Отдельный режим сна с тёмной темой тёплых оттенков и фоновыми звуками." status="ready-drop" timestamp="35m" width="100%" />
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Чтение вслух с подсветкой слов" description="Преобразование текста в речь позволяет детям самостоятельно слушать истории, следя за подсвечиваемыми словами." status="done" timestamp="4h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Выполнено</strong> — Эта задача завершена, и её изменения стали частью основной версии.</span>
  </div>
</div>

<div id="how-teams-use-the-board">
  ### Как команды используют доску
</div>

Когда несколько человек работают одновременно, задачи каждого отображаются на общей доске. Вот как это выглядит: один участник команды работает над продолжениями и перегенерацией историй, другой — над оформлением подарка, а завершённые задачи остальных находятся в статусах «Готово» и «Выполнено».
...
{ title: 'Перегенерация историй — повторная обработка страниц', description: 'Возможность повторно генерировать отдельные страницы даёт родителям больше контроля и снижает разочарование.', status: 'active', timestamp: '8m' },
...
{ title: 'Ссылки на истории для общего доступа с OG-превью', description: 'Добавление публичных ссылок для общего доступа — самый мощный рычаг роста.', status: 'ready-drop', timestamp: '31m', showMenu: true },
] },
...
{ title: 'Расширенная настройка персонажей', description: 'Параметры внешнего вида для по-настоящему персонализированных иллюстраций.', status: 'done', timestamp: '4h' },
{ title: 'Адаптация для мобильных устройств и финальная доработка', description: 'Улучшенная обработка ошибок, раскладки для мобильных устройств и анимации загрузки.', status: 'done', timestamp: '4h' },
] }
]}
/>

Когда задача переходит в статус «Готово», любой участник команды может проверить и применить изменения. Agent автоматически разрешает конфликты при применении нескольких задач.

<div id="what-you-can-build-together">
  ## Что можно создавать вместе
</div>

* **Распределите работу над функцией между участниками команды**: Один человек разрабатывает фронтенд, а другой занимается бэкендом — каждый в своём чате с Agent. Канбан-доска помогает всем синхронизироваться.
* **Введите нового участника команды в курс дела**: Он сможет изучить проект в собственном чате с Agent, задать вопросы о том, как всё устроено, и начать вносить свой вклад — не мешая работе остальных.
* **Выпускайте обновления быстрее с Pro**: Благодаря параллельному выполнению два или три человека могут запускать задачи одновременно. Проект, на который у одного человека ушла бы целая сессия, можно завершить за гораздо меньшее время.

<div id="frequently-asked-questions">
  ## Часто задаваемые вопросы
</div>

<Accordion title="Могу ли я написать сообщение товарищу по команде через Agent?">
  Нет. Agent — не средство обмена сообщениями: он всегда слушает и попытается выполнить всё, что вы введёте. Для общения в команде используйте Slack, Discord или другой инструмент.
</Accordion>

<Accordion title="Можем ли мы перезаписать работу друг друга?">
  Система задач помогает избежать этого: для каждой задачи создаётся собственная изолированная копия, при этом она учитывает другие существующие задачи. После завершения задач вы применяете изменения к основной версии и на этом этапе разрешаете все конфликты.
</Accordion>

<Accordion title="Как узнать, чем занимаются мои товарищи по команде?">
  Откройте Канбан-доску, чтобы посмотреть активные задачи. Также можно увидеть положение курсоров и файлы, которые просматривают товарищи по команде.
</Accordion>

<Accordion title="Могу ли я удалить кого-то из своего проекта?">
  Да. Владельцы Project могут управлять участниками команды и отзывать доступ в любое время.
</Accordion>
