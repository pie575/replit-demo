---
title: "Replit Agent"
description: "Replit Agent transforme vos idées en applications, designs, présentations et bien plus, à partir d'un langage simple. Aucune compétence en programmation requise."
---
...
Agent est votre partenaire créatif. Agent prend vos idées, vous aide à les affiner, puis les concrétise. Contrairement à un chatbot qui se contente de répondre à des questions, Agent passe à l'action : il configure votre projet, crée des applications, vérifie son travail et résout les problèmes au fur et à mesure.

Décrivez ce que vous souhaitez en langage courant. Aucune connaissance en code ni technique requise. Agent s'occupe du reste, de la planification au déploiement.
...
<Steps>
  <Step title="Décrivez ce que vous voulez">
    Dans le [Project Editor](/fr/core-concepts/project-editor), commencez simplement à discuter. Décrivez une application que vous souhaitez créer, posez une question, explorez un sujet, ou récupérez des données depuis des [services connectés](/fr/core-concepts/agent/general-agent) comme BigQuery, Slack ou Notion. Il n'y a aucune contrainte. Replit Agent gère tout ce que vous lui soumettez.

    <AgentInput />
  </Step>

  <Step title="Choisissez ce à quoi construire">
    Optionnellement, sélectionnez un type de projet : application web, application mobile, diaporama, design, visualisation de données, et plus encore. Si vous avez déjà décrit ce que vous souhaitez à l'étape 1, Agent détermine automatiquement la configuration appropriée.
  ...
  <Step title="Itérer, tester, déployer">
    Discutez avec Agent pour affiner votre projet, puis publiez quand vous êtes prêt. Tous les artefacts sont déployés ensemble.
  </Step>
</Steps>

<Note>Vous n'êtes pas limité à un seul type. Commencez avec une application web et ajoutez une application mobile, un diaporama ou une vidéo plus tard — tout dans le même projet.</Note>

### Plan mode

Activez Plan mode pour réfléchir, poser des questions et cartographier votre projet avant qu'Agent ne modifie du code ou des données. En Plan mode, Agent va :

* **Décomposer les projets complexes** en listes de tâches ordonnées
* **Explorer différentes approches** et peser les compromis
* **Réviser et affiner** avant que le moindre code ne soit écrit

Cliquez sur « Plan » dans le champ de saisie du chat — ou demandez simplement à Agent — pour passer en Plan mode. Exemple : « Créer un plan pour construire un suivi de projet pour mon équipe »

<AgentInput highlightPlan={true} showTypewriter={false} />

Agent crée une liste de tâches ordonnée que vous pouvez réviser et affiner. Lorsque vous êtes satisfait du plan, approuvez-le et Agent commence à construire.
...
En savoir plus dans le [guide Plan mode](/fr/core-concepts/agent/plan-mode).

### Modes Agent

Choisissez comment Agent construit votre projet :

<AgentModes />

* **Lite** : Effectuez des modifications légères et peu coûteuses rapidement. Le mode Lite est idéal pour les ajustements visuels, les corrections de bugs et les fonctionnalités ciblées.
* **Economy** : Utilisez les modèles économiques d'Agent pour les tâches quotidiennes. Développez les **Paramètres avancés** pour gérer des fonctionnalités telles que App Testing et Code Optimizations.
* **Power** : Utilisez les modèles les plus performants d'Agent pour les problèmes complexes, les changements importants et les constructions longues. Dans les **Paramètres avancés**, vous pouvez également activer **Turbo** pour des constructions jusqu'à 2,5 fois plus rapides à un coût plus élevé (Pro uniquement).

<Note>Conseil : Vous pouvez également activer le **[Plan mode](/fr/core-concepts/agent/plan-mode)** pour examiner et itérer sur le plan d'Agent avant que la construction ne commence. Le mode Max n'est plus disponible ; utilisez Power pour les constructions standard les plus performantes.</Note>
...
* **Designs visuels et prototypes** — explorez des maquettes sur le [Design Canvas](/fr/replitai/canvas) avant de vous engager dans le code
* **Plusieurs sorties dans un même projet** — applications web, applications mobiles, diaporamas et vidéos partageant le même backend
* **Fichiers et documents** — CSV, PDF, fichiers PowerPoint, documents Markdown
* **Requêtes sur des services connectés** — récupérez des données depuis BigQuery, Linear, Slack, Notion, et plus encore directement depuis le chat

<Card title="Commencer à construire maintenant" icon="arrow-up-right" href="https://replit.com">
...
<Accordion title="Dois-je savoir programmer ?">
  Non. Agent s'occupe de tout le travail technique — écriture du code, configuration de l'infrastructure, paramétrage des bases de données. Vous décrivez ce que vous souhaitez en langage courant et Agent le construit.
</Accordion>

<Accordion title="Quelle est la différence entre Lite, Economy et Power ?">
  **Lite** est conçu pour des modifications rapides et ciblées (10 à 60 secondes) telles que des ajustements visuels, des corrections de bugs et des fonctionnalités ciblées. **Economy** est le meilleur choix par défaut pour la plupart des constructions lorsque vous souhaitez équilibrer coût et qualité. **Power** utilise les modèles les plus performants pour les tâches plus difficiles, et **Turbo** est un interrupteur optionnel réservé à Power dans les Paramètres avancés lorsque vous avez besoin des exécutions les plus rapides.
</Accordion>

<Accordion title="Puis-je créer plusieurs choses dans un projet ?">
  Oui. Vous pouvez ajouter des applications web, des applications mobiles, des diaporamas, des vidéos et des visualisations de données au [même projet](/fr/replitai/artifacts#multiple-artifacts-in-one-project) — tous partageant le même backend et les mêmes données.
</Accordion>

<Accordion title="Que se passe-t-il si Agent fait une erreur ?">
  Agent teste régulièrement son propre travail. Agent crée également des points de contrôle au fur et à mesure de son travail, vous permettant de revenir à n'importe quel état précédent. Vous pouvez également discuter avec Agent pour décrire ce qui s'est mal passé et il corrigera le problème.
</Accordion>
...
Pour des informations tarifaires détaillées, consultez [la facturation Agent](/fr/billing/ai-billing#agent-billing).

## Prochaines étapes

<CardGroup cols={2}>
  <Card title="Compétences Agent" icon="puzzle-piece" href="/fr/core-concepts/agent/skills">
    Enseignez à Agent des connaissances spécialisées — utilisez des compétences préconstruites ou créez les vôtres.
  </Card>
  ...
  <Card title="Design Canvas" icon="paintbrush" href="/fr/replitai/canvas">
    Maquettes visuelles et éditeur visuel interactif.
  </Card>

  <Card title="Connecteurs" icon="plug" href="/fr/core-concepts/agent/general-agent">
  ...
  <Card title="Guide vibe coding" icon="wand-magic-sparkles" href="/fr/tutorials/how-to-vibe-code">
