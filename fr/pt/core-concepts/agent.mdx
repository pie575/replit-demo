---
title: "Replit Agent"
description: "Replit Agent transforme vos idées en applications, designs, présentations et bien plus encore, le tout en langage naturel. Aucune compétence en programmation requise."
---
...
Agent est votre partenaire créatif. Il accueille vos idées, vous aide à les affiner, puis les concrétise. Contrairement à un chatbot qui se contente de répondre à vos questions, Agent agit : il configure votre projet, crée des applications, vérifie votre travail et résout les problèmes au fur et à mesure.

Décrivez ce que vous voulez dans vos propres mots. Aucun code ni aucune connaissance technique ne sont nécessaires. Agent s’occupe du reste, de la planification au déploiement.
...
<Steps>
  <Step title="Décrivez ce que vous souhaitez">
    Dans le [Project Editor](/fr/pt/core-concepts/project-editor), commencez simplement à discuter. Décrivez l&#39;application que vous souhaitez créer, posez une question, recherchez un sujet ou récupérez des données depuis des [services connectés](/fr/pt/core-concepts/agent/general-agent) comme BigQuery, Slack ou Notion. Il n&#39;y a aucune restriction. Replit Agent s&#39;occupe de tout ce dont vous avez besoin.

    <AgentInput />
  </Step>

  <Step title="Choisissez ce que vous souhaitez créer">
    Vous pouvez également sélectionner un type de projet : application web, application mobile, diapositives, design, visualisation de données, et bien plus encore. Si vous avez déjà décrit ce que vous souhaitez à l&#39;étape 1, Agent détermine automatiquement la configuration adaptée.

    <AgentCarousel />
  </Step>

  <Step title="Agent construit">
    Agent écrit le code, configure l&#39;infrastructure et teste le résultat. Dans votre projet, vous pouvez basculer entre les [modes d&#39;Agent](#agent-modes) pour contrôler sa manière de construire.
  </Step>

  <Step title="Itérez, testez, déployez">
    Discutez avec Agent pour affiner votre projet, puis publiez-le lorsqu&#39;il est prêt. Tous les artefacts sont déployés ensemble.
  </Step>
</Steps>

<Note>Vous n&#39;êtes pas limité à un seul type. Commencez par une application web, puis ajoutez une application mobile, des diapositives ou une vidéo, le tout dans le même projet.</Note>

<div id="modo-planejamento">
  ### Mode Planification
</div>

Activez le mode Planification pour explorer des idées, poser des questions et élaborer votre projet avant que l’Agent ne modifie du code ou des données. En mode Planification, l’Agent va :

* **Décomposer les projets complexes** en listes de tâches ordonnées
* **Explorer différentes approches** et évaluer les avantages et les inconvénients
* **Réviser et affiner** avant que du code ne soit écrit

Cliquez sur « Plan » dans le champ de Chat — ou demandez-le simplement à l’Agent — pour passer au mode Planification. Exemple : « Crée un plan pour développer un outil de suivi de projets pour mon équipe »

<AgentInput highlightPlan={true} showTypewriter={false} />

L’Agent crée une liste de tâches ordonnée que vous pouvez réviser et affiner. Lorsque vous êtes satisfait du plan, approuvez-le et l’Agent commence à le mettre en œuvre.
...
Pour en savoir plus, consultez le [guide du mode Planification](/fr/pt/core-concepts/agent/plan-mode).
...
* **Lite** : effectuez rapidement des modifications légères et économiques. Le mode Lite est idéal pour les ajustements visuels, les corrections de bugs et les fonctionnalités bien délimitées.
* **Economy** : utilisez les modèles de l&#39;Agent optimisés en coût pour les tâches du quotidien. Développez les **Paramètres avancés** pour gérer des fonctionnalités comme les Tests d&#39;App et les Optimisations de code.
* **Power** : utilisez les modèles les plus performants de l&#39;Agent pour les problèmes les plus complexes, les modifications d&#39;envergure et les builds plus longues. Dans les **Paramètres avancés**, vous pouvez aussi activer le **Turbo** pour des builds jusqu&#39;à 2,5x plus rapides, à un coût supérieur (Pro uniquement).

<Note>Astuce : vous pouvez également activer le **[mode Planification](/fr/pt/core-concepts/agent/plan-mode)** pour examiner et affiner le plan de l&#39;Agent avant le début de la construction. Le mode Max n&#39;est plus disponible ; utilisez Power pour les builds standard les plus performantes.</Note>

<div id="o-que-voc-pode-criar-com-o-replit-agent">
...
* **Designs visuels et prototypes** — explorez des maquettes dans le [Design Canvas](/fr/pt/replitai/canvas) avant de vous lancer dans le code
* **Plusieurs livrables dans un même projet** — applications web, applications mobiles, diapositives et vidéos partageant le même backend
...
<Accordion title="Dois-je savoir coder ?">
  Non. Agent s&#39;occupe de tout le travail technique : il écrit le code, configure l&#39;infrastructure et les bases de données. Vous décrivez ce que vous voulez en langage clair, et Agent le crée.
</Accordion>

<Accordion title="Quelle est la différence entre Lite, Economy et Power ?">
  **Lite** est destiné aux modifications rapides et ciblées (10 à 60 secondes), comme les ajustements visuels, les corrections de bugs et les fonctionnalités bien définies. **Economy** est le choix par défaut idéal pour la plupart des builds lorsque vous souhaitez équilibrer coût et qualité. **Power** utilise les modèles les plus performants pour les tâches plus complexes, et **Turbo** est une option disponible uniquement avec Power, dans les paramètres avancés, lorsque vous avez besoin des exécutions les plus rapides.
</Accordion>

<Accordion title="Puis-je créer plusieurs éléments dans un projet ?">
  Oui. Vous pouvez ajouter des applications web, des applications mobiles, des diapositives, des vidéos et des visualisations de données au [même projet](/fr/pt/replitai/artifacts#multiple-artifacts-in-one-project) — le tout en partageant le même backend et les mêmes données.
</Accordion>

<Accordion title="Que se passe-t-il si Agent commet une erreur ?">
  Agent teste régulièrement son propre travail. Il crée également des checkpoints au fur et à mesure, afin que vous puissiez revenir à n&#39;importe quel état antérieur. Vous pouvez aussi discuter avec Agent pour décrire ce qui s&#39;est mal passé, et il corrigera le problème.
</Accordion>
...
Pour connaître les tarifs détaillés, consultez [Facturation d&#39;Agent](/fr/pt/billing/ai-billing#agent-billing).
...
<CardGroup cols={2}>
  <Card title="Compétences d'Agent" icon="puzzle-piece" href="/fr/pt/core-concepts/agent/skills">
    Transmettez à Agent des connaissances spécialisées : utilisez des compétences prêtes à l&#39;emploi ou créez les vôtres.
  </Card>

  <Card title="Créer en parallèle" icon="list-check" href="/fr/pt/core-concepts/agent/task-system">
    Planification Kanban, tâches en arrière-plan et modifications réintégrées à la version principale.
  </Card>

  <Card title="Design Canvas" icon="paintbrush" href="/fr/pt/replitai/canvas">
    Maquettes visuelles et éditeur visuel pratique.
  </Card>

  <Card title="Connecteurs" icon="plug" href="/fr/pt/core-concepts/agent/general-agent">
  ...
  <Card title="Guide du vibe coding" icon="wand-magic-sparkles" href="/fr/pt/tutorials/how-to-vibe-code">
