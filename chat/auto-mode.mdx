---
title: "Use intelligent model routing"
description: "Let Replit select an appropriate model for each task, then choose a model manually only when you need that control."
sidebarTitle: "Intelligent model routing"
...
<div className="chat-mkt" role="img" aria-label="Anthropic, OpenAI, and Kimi marks representing intelligent model routing" style={{ margin: "28px 0 42px", borderTop: 0, paddingTop: 0 }}>
...
Intelligent model routing lets Replit select an appropriate model for each task while balancing quality and cost. When you see **Auto** in the product, Replit is performing intelligent model routing.
...
2. Select **Power** or **Max**, then find the **Primary model** section. Its label changes with the selected mode, such as **Primary Max model**.
3. If the primary model is already **Auto**, intelligent model routing is enabled. If a specific model is selected, open the primary model menu and select **Automatically choose**.

Free Mode always uses intelligent model routing and cannot select a model manually.

## Let the task guide the model

Intelligent model routing is useful when you want Replit to handle model selection as the work changes. A focused request can use a lower-cost model, while a complex task may need a more capable model.

In Power Mode or Max Mode, builders on Core and Pro can select an available model manually instead.

<Frame caption="Intelligent model routing, model selection, and Effort in the Agent settings.">
  <img src="/assets/mirror/auto-mode-model-and-effort-selector-fdcabef57097.jpg" alt="Agent settings showing Automatically choose recommended, available models, the Effort control, and Max Mode using intelligent model routing" width="1839" height="855" data-path="images/chat/auto-mode-model-and-effort-selector.jpg" />
</Frame>

## Learn more

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/features/agent/model-selector#intelligent-model-routing">
    ...
    <div className="chat-card-title">Intelligent model routing reference</div>

    <p>
      Learn how Replit selects a model and when manual selection is available.
    </p>
...
When you need more control than intelligent model routing provides, continue to [Choose a model](/chat/model-selector).
