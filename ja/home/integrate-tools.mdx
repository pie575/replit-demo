---
title: "ツールを連携する"
description: "コーヒーショップのローンチプランを構築するためのコンテキストをReplitに与える、ファイルとツールを連携する。"
---
...
インテグレーションについて学んだところで、コーヒーショップのローンチにそれを
適用してみましょう。

あなたはすでに方向性を選びました。駅前の通勤客向けバーではなく、**近隣型カフェ**を
作るというものです。この方向性を、実行に移せるプランに変えるために必要な素材を
Replitに与えましょう。

インテグレーションは、Replitをすでに使っているツールやファイルに接続します。これに
よって、Replitは関連情報を取り込んだり、タスクに必要なアクションを実行したりできる
ようになります。**[Slack](/ja/features/integrations/overview#connectors)**、
**[Google Drive](/ja/features/integrations/overview#connectors)**、
**[Notion](/ja/features/integrations/overview#connectors)**、
\*\*[Salesforce](/ja/features/integrations/overview#connectors)\*\*など、450を超える
サービスを接続できます。

このコーヒーショップの場合、Google Driveを使うことでReplitは予算、立地に関する
メモ、メニューのアイデア、写真、ビジュアルの参考資料にアクセスできるようになります。
Notionは、ローンチプラン、決定事項、未解決の質問をまとめて記録する場所を提供します。

[ツールを連携する](/ja/chat/integrate-your-tools)方法について詳しく見る。

<Steps>
  <Step title="Replitに素材をまとめるよう依頼する">
    まずは以下のリクエストから始めましょう。Replitはプランを完成させるために
    必要なものを確認し、アクセスが必要な場合はサービスの接続を求めます。

    <AiPrompt>
      私のカフェのファイルを使って、近隣型カフェの最初の1か月のローンチプランを
      作成してください。関連する前提を更新し、その上でコンセプト、主要な決定事項、
      未解決の質問、ローンチチェックリストをNotionに記録してください。
    </AiPrompt>
  </Step>

  <Step title="Replitが求めたらGoogle Driveを接続する">
    Replitが利用したい素材を確認できるように、Google Driveの接続を承認します。
    Replitは接続リクエストを会話内で直接開きます。

    <CoffeeShopScreenshotFrame src="/assets/mirror/connect-google-drive-f7951617b078.png" alt="カフェのファイルを確認できるようにGoogle Driveの接続を求めるReplit" />
  </Step>

  <Step title="Replitがプランを文書化する必要があるときにNotionを接続する">
    Replitがローンチブリーフを作成するために依頼してきたら、Notionを承認します。
    そうすることで、コンセプト、決定事項、未解決の質問、チェックリストを一箇所に
    記録できるようになります。
  </Step>
</Steps>

Google Driveは、Replitが作業の根拠とする証拠を提供します。Notionは、新しい質問に
答え、決定を下していく中で進化し続けるプランを一箇所にまとめておきます。

[コンテキストの追加](/ja/chat/add-context)、[ファイルのアップロード](/ja/chat/upload-and-use-files)、
[ツールの連携](/ja/chat/integrate-your-tools)について詳しく見る。

## 次のステップ

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ja/home/watch-the-competition">
    ...
    <div className="chat-example-tag">ルーティン</div>
    <div className="chat-card-title">競合を見守り続ける</div>
    <p>ローンチの準備をしている間、変化を見張るルーティンを設定しましょう。</p>
