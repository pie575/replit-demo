---
title: "Enterprise model controls"
description: "Choose which Agent model providers and models are available in each Enterprise Workspace."
sidebarTitle: "Model controls"
icon: "microchip"
---
...
Intelligent Model Routing is the default Agent experience for Enterprise Workspaces. With **Auto mode** turned on, Replit chooses the mode and model that best fit each task while balancing quality and cost.

Routing selects from the pool of models authorized by the account and Workspace policies. Providers and models outside that pool are not eligible for selection. Builders can keep **Auto mode** on or turn it off to select an authorized model manually.

<Frame caption="Turn on Auto mode to route tasks among models authorized for the Workspace.">
  <img src="/assets/mirror/enterprise-auto-mode-9365b7d49c0d.png" alt="Agent settings with Auto mode turned on and model selection set to Auto" width="558" height="356" data-path="images/changelog/2026-08-28/enterprise-auto-mode.png" />
</Frame>

Learn more about how routing works in [Use Auto Mode](/chat/auto-mode).
...
<Frame caption="The Anthropic provider policy set to Enable selected, with model access configured by Workspace.">
  <img src="/assets/mirror/anthropic-account-policy-workspace-models-38c0f71a609d.jpg" alt="Anthropic account policy set to Enable selected with model toggles for a Workspace" width="2000" height="930" data-path="images/workspace/anthropic-account-policy-workspace-models.jpg" />
</Frame>
...
* Model policies apply to Workspaces. You cannot set a different policy for a user group in the same Workspace.
* Intelligent Model Routing does not override account or Workspace policies. It cannot select a disabled provider or model.
* The **Fast mode models** toggle in **Agent modes and model providers** controls access to fast model variants across every Workspace on the account. Provider policies and Intelligent Model Routing do not change that setting.
