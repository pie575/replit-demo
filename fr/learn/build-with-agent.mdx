---
title: "Build avec Agent"
description: "Découvrez cinq bonnes pratiques pour travailler avec Agent : soyez précis, planifiez le travail, ajoutez du contexte, vérifiez et testez, puis utilisez des checkpoints."
sidebarTitle: "Build avec Agent"
icon: "robot"
---
...
Agent est l’outil de création par IA de Replit et votre compagnon pour transformer vos idées en logiciels. Agent peut planifier des modifications, écrire du code, expliquer le comportement de votre app, déboguer des problèmes et l’améliorer.

[Le vibe coding : les bases](/fr/learn/foundations/vibe-coding-101) présente l’état d’esprit à adopter, tandis que cette page explique comment utiliser Agent en pratique. [Rédiger des prompts efficaces](/fr/learn/effective-prompting) explique comment formuler des instructions plus claires.

Considérez Agent comme le quarterback sur le terrain : il analyse la situation, choisit une approche et fait avancer le travail. Vous êtes l’entraîneur : vous définissez la stratégie, annoncez l’action, examinez ce qui s’est passé et décidez de la prochaine étape.

Agent peut faire beaucoup de choses, mais il est plus efficace lorsque vous pilotez le processus : soyez précis, planifiez le travail, ajoutez du contexte, vérifiez et testez, et utilisez des checkpoints lorsque vous devez revenir en arrière.

<div id="five-tips-for-building-with-agent">
  ## Cinq conseils pour créer avec Agent
</div>

Adoptez ces bonnes pratiques lorsque vous travaillez avec Agent :

1. Soyez précis.
2. Planifiez le travail.
3. Ajoutez du contexte.
4. Vérifiez et testez.
5. Utilisez des checkpoints.
...
Une demande utile explique ce que vous souhaitez, ce qui compte et ce qui doit rester inchangé. Elle n&#39;a pas besoin de décrire chaque détail de l&#39;implémentation, mais elle doit donner à Agent suffisamment d&#39;indications pour prendre les bonnes décisions.

Au lieu de :

<AiPrompt>Améliore mon app.</AiPrompt>

Essayez :

<AiPrompt>
  Améliore la page d&#39;inscription afin que les visiteurs comprennent ce que fait le produit avant
  de créer un compte.

  <br />

  Conserve la palette de couleurs actuelle, garde les mêmes champs de formulaire et rends le
  bouton principal plus visible.
</AiPrompt>

Ce deuxième prompt donne à Agent un objectif, un domaine d&#39;intervention et des contraintes.

Lorsque votre demande comporte plusieurs parties, indiquez ce qui compte le plus.

<AiPrompt>
  Ajoute une vue d&#39;administration simple pour gérer les demandes de service traiteur.

  <br />

  Le plus important est de pouvoir voir les nouvelles demandes, les marquer comme
  contactées et les filtrer par statut.
...
Agent peut décomposer ce travail en étapes, mais vos instructions lui indiquent à quoi ressemble une réussite.

Pour plus d&#39;exemples d&#39;instructions précises, consultez [Rédiger des prompts efficacement](/fr/learn/effective-prompting).

<div id="plan-the-work">
  ## Planifier le travail
</div>

La planification est utile lorsque le travail comporte des incertitudes, plusieurs étapes, des compromis ou des éléments que vous souhaitez valider avant de modifier des fichiers.

Vous pouvez demander à Agent de créer un plan dans la conversation :

<AiPrompt>
  Avant d&#39;effectuer des modifications, créez un plan pour ajouter la commande en ligne.

  <br />

  Incluez le parcours utilisateur, les données nécessaires, les pages ou composants qui seront modifiés,
  les risques et la manière dont je devrais les tester.
</AiPrompt>

Utilisez le [Plan mode](/fr/features/agent/plan-mode) lorsque vous souhaitez qu&#39;Agent réfléchisse d&#39;abord et attende votre approbation avant de modifier des fichiers. Activez **Plan** dans le champ de prompt avant d&#39;envoyer votre demande.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="Le champ de prompt de la page d'accueil de Replit, avec un prompt pour une application en cours d'exécution et la case Plan activée" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>

<AgentInput defaultPrompt="Avant d'effectuer des modifications, créez un plan pour ajouter la commande en ligne. Incluez le parcours utilisateur, les données nécessaires, les pages ou composants qui seront modifiés, les risques et la manière dont je devrais les tester." highlightPlan={true} showTypewriter={false} />

Une fois qu&#39;Agent a fini de réfléchir, une bannière **le plan de tâche est prêt à être examiné** s&#39;affiche avec un bouton **Examiner maintenant**.

<Frame>
  <img src="/assets/mirror/agent-plan-ready-banner-5fbcf58a1668.png" alt="Une bannière indiquant que le plan d'une tâche est prêt à être examiné, avec un bouton Examiner maintenant en bas du panneau Agent" width="560" height="550" data-path="images/fitstart/work-with-agent/agent-plan-ready-banner.png" />
</Frame>
...
Vous pouvez ajouter du contexte sous forme de texte, de captures d’écran, de croquis, de fichiers, de données, d’erreurs ou d’annotations Canvas. Utilisez le format qui explique le mieux le problème.

Utilisez du texte lorsque la modification concerne le comportement, les objectifs ou les contraintes :

<AiPrompt>
  Contexte : cette app est destinée aux parents qui commandent des gâteaux d’anniversaire.

  <br />

  Gardez le formulaire court, rendez les informations de retrait bien visibles et évitez tout ce
  qui paraît trop institutionnel.
</AiPrompt>

Utilisez des captures d’écran ou des croquis lorsque la disposition est difficile à décrire avec des mots :

<AiPrompt>
  Utilisez cette capture d’écran comme référence pour la disposition du formulaire de demande.

  <br />

  Reprenez la structure générale, mais conservez les champs existants et le comportement
  d’envoi.
</AiPrompt>

Utilisez les annotations [Canvas](/fr/design/canvas) lorsque la modification est visuelle et que vous souhaitez indiquer une partie précise de l’app.

<Frame>
  <img src="/assets/mirror/agent-canvas-annotations-c88ab5856966.png" alt="Un frame d’application de fitness sur le Canvas avec un rectangle, une flèche et une étiquette de texte pointant vers la carte de progression, ainsi qu’un croquis au stylo et une étiquette sur la zone du formulaire" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-canvas-annotations.png" />
</Frame>

<AiPrompt>
  Utilisez mes annotations Canvas pour mettre à jour la section principale.

  <br />

  Appliquez les notes uniquement au titre, à l’image et au bouton principal.

  <br />

  Ne modifiez pas le formulaire de demande de service traiteur.
</AiPrompt>

Un bon contexte indique également ce qui ne doit pas changer. Cela aide Agent à améliorer une zone sans en réécrire accidentellement une autre.

<div id="review-and-test">
  ## Examiner et tester
</div>

Agent peut proposer un plan avant de commencer à créer, puis résumer les modifications apportées. Lisez les deux.

Ouvrez le plan en sélectionnant **View** sur la carte du plan de tâche. Le plan détaillé indique ce qu&#39;Agent prévoit de créer, les critères de réussite, ce qui ne fait pas partie du périmètre et les étapes qu&#39;Agent suivra.

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="Plan de tâche détaillé montrant les sections Ce que et Pourquoi, les critères de réussite, les éléments hors périmètre et les étapes de création numérotées" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>

Avant qu&#39;Agent ne crée l&#39;application, examinez le plan :

* Résout-il le bon problème ?
* Manque-t-il quelque chose ?
* Certains éléments sont-ils hors périmètre ?
* Existe-t-il une version plus petite à créer d&#39;abord ?
* Explique-t-il comment tester le résultat ?

Si le plan est trop vaste, demandez à Agent de le réduire.

<AiPrompt>
  Ce plan est trop vaste.

  <br />

  Réduis-le à la plus petite version que je peux tester aujourd&#39;hui.

  <br />

  Place tout le reste dans une liste d&#39;améliorations futures.
</AiPrompt>

Après qu&#39;Agent a créé l&#39;application, testez-la vous-même. Ouvrez Preview et utilisez l&#39;application comme le ferait la personne à qui elle est destinée.

Vérifiez :

* Une personne peut-elle effectuer l&#39;action principale ?
* La modification demandée a-t-elle été apportée ?
* L&#39;application fonctionne-t-elle toujours sur mobile ?
* Un comportement important a-t-il été altéré ?
* Agent a-t-il modifié quelque chose sans rapport ?
* Le résultat est-il suffisamment satisfaisant pour continuer à développer l&#39;application ?

Si quelque chose ne va pas, décrivez ce que vous avez constaté.

Au lieu de :

<AiPrompt>Corrige le formulaire.</AiPrompt>

Essayez plutôt :

<AiPrompt>
  Lorsque j&#39;envoie le formulaire de demande de service traiteur, le message de confirmation apparaît, mais le
  formulaire affiche toujours l&#39;ancien texte.

  <br />

  Vide le formulaire après l&#39;envoi et laisse le message de confirmation visible.
</AiPrompt>

Des retours précis aident Agent à résoudre le bon problème.

<div id="use-checkpoints">
  ## Utiliser les checkpoints
</div>

Les checkpoints vous permettent de revenir en arrière lorsqu&#39;une modification prend une mauvaise tournure.

Utilisez un retour ciblé lorsque le résultat est presque satisfaisant :

<AiPrompt>
  C&#39;est presque ça, mais le formulaire est maintenant trop long.

  <br />

  Conservez le nouveau style visuel, supprimez le champ de notes facultatif et rendez la
  date de retrait plus facile à lire.
</AiPrompt>

Utilisez un checkpoint lorsque l&#39;App est moins bonne qu&#39;avant, qu&#39;une fonctionnalité importante ne marche plus ou qu&#39;Agent a apporté plus de modifications que prévu.

Ouvrez **Historique** dans le panneau Agent pour afficher les checkpoints. Chaque checkpoint indique les modifications apportées et propose les commandes **Restaurer ici** et **Modifications**.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="Le panneau Historique d'Agent affichant un checkpoint intitulé 'Transitioned from Plan to Build mode', avec les boutons Restaurer ici et Modifications" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

Lorsque vous sélectionnez **Restaurer ici**, Agent confirme les éléments qui seront restaurés avant d&#39;effectuer la restauration.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-rollback-dialog-28de278b44da.png" alt="La boîte de dialogue de confirmation de restauration vers un checkpoint, indiquant le checkpoint cible et les éléments concernés : fichiers, mémoire d'Agent, tâches et une case à cocher Base de données" width="900" height="880" data-path="images/fitstart/work-with-agent/agent-checkpoints-rollback-dialog.png" />
</Frame>

Si vous ne savez pas s&#39;il vaut mieux corriger la version actuelle ou revenir en arrière, demandez à Agent de vous aider à comparer.

<AiPrompt>
  La dernière modification a empiré l&#39;App.

  <br />

  Comparez la version actuelle à la dernière version fonctionnelle.

  <br />

  Dites-moi s&#39;il est plus sûr de corriger la version actuelle ou de revenir en arrière, et expliquez pourquoi.
</AiPrompt>

Les fausses pistes sont normales. Les checkpoints vous permettent d&#39;explorer sans perdre la possibilité de revenir à une version plus sûre.

Pour plus de conseils sur la récupération, consultez [Checkpoints et retours en arrière](/fr/features/version-control/checkpoints-and-rollbacks).

<div id="what-good-agent-collaboration-feels-like">
  ## Les clés d’une bonne collaboration avec Agent
</div>

Vous êtes sur la bonne voie lorsque :

* Vos demandes précisent l’objectif et les contraintes
...
* Vous fournissez un retour ciblé lorsque quelque chose ne fonctionne pas comme prévu
* Vous utilisez des checkpoints plutôt que d’essayer indéfiniment de démêler une mauvaise modification
...
<CardGroup cols={2}>
  <Card icon="comment-dots" href="/fr/learn/effective-prompting">
    Découvrez comment rédiger des instructions plus claires qu’Agent pourra suivre en toute confiance.
  </Card>

  <Card title="Créez votre première app" icon="rocket" href="/fr/build/your-first-app">
    Entraînez-vous à travailler avec Agent, du premier prompt à une
    amélioration ciblée.
  </Card>

  <Card title="Checkpoints et retours en arrière" icon="rotate-left" href="/fr/features/version-control/checkpoints-and-rollbacks">
