---
title: "コンテキストを追加"
description: "会話をReplitが理解するために必要な、関連ファイル、情報源、接続済みツール、既存の作業をReplitに渡します。"
sidebarTitle: "コンテキストを追加"
icon: "paperclip"
...
Replitがリクエストを理解する助けになる場合は、関連するコンテキストやツールを追加してください。役立つコンテキストがあると、Replitの意図がより明確になり、成果物の品質も向上します。

## コンテキストを追加するには

プロンプトボックスの**プラス**メニューを選択すると、ファイルのアップロード、既存の作業のインポート、Skillやデザインシステムの利用、インテグレーションの追加ができます。

<Frame>
  <img src="/assets/mirror/prompt-box-plus-menu-8c1e2d807e0b.png" alt="ファイルをアップロード、新しく作成、既存のプロジェクトをインポート、スキルを使用、デザインシステムを使用、インテグレーションを追加が並ぶプロンプトボックスのプラスメニュー" width="1558" height="556" data-path="images/chat/prompt-box-plus-menu.png" />
</Frame>

## タスクに合わせてコンテキストを選ぶ

ファイル、スクリーンショットや画像、Web上の情報源やリサーチ結果、接続済みツールを追加できます。現在のタスクに役立つソースを選びましょう。

* **[ファイル、スクリーンショット、画像](/ja/chat/upload-and-use-files):** 役立つ証拠や要件が含まれるドキュメント、スプレッドシート、エクスポートデータ、スクリーンショット、視覚的な参考資料を追加します。
* **[Web上の情報源とリサーチ](/ja/features/agent/web-search):** 最新情報や外部の視点があれば結果が向上する場合、Replitにウェブ検索を依頼します。
* **[インテグレーション](/ja/chat/integrate-your-tools):** 必要なツールを接続し、Replitが考慮すべきサービス、チャンネル、レコード、その他の情報を伝えます。
* **[デザインのコンテキスト](/ja/chat/work-with-figma):** レイアウト、階層構造、ビジュアルの方向性が重要な場合は、Figmaのフレームを添付します。
* **[既存の作業](/ja/chat/import-a-project):** コード、デザイン、または他所で作成された作業から続けて構築したい場合は、プロジェクトをインポートします。プロジェクトのインポートは、証拠としてファイルを添付するのとは異なります。

リクエストに関連するリンクを貼り付けたり、Replitにウェブ検索を依頼したりすることもできます。

## コンテキストのソースを選ぶ

<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/ja/chat/upload-and-use-files">
    <FilesCardVisual />
    ...
    <div className="chat-card-title">ファイルをアップロードして使う</div>
    <p>アップロードしたファイルについて質問したり、証拠として使用したりします。</p>
    <div className="chat-card-link">詳しく見る →</div>
  </a>

  <a className="chat-example has-visual" href="/ja/chat/work-with-figma">
    <FigmaCardVisual />
    ...
    <div className="chat-card-title">Figmaと連携する</div>
    <p>ビジュアルの方向性が重要な場合に、特定のフレームを使用します。</p>
    <div className="chat-card-link">詳しく見る →</div>
  </a>

  <a className="chat-example has-visual" href="/ja/chat/integrate-your-tools">
    ...
    <p>関連情報を保持している、またはアクションをサポートするサービスを接続します。</p>
    <div className="chat-card-link">詳しく見る →</div>
  </a>

  <a className="chat-example has-visual" href="/ja/chat/import-a-project">
    ...
    <div className="chat-card-title">既存のプロジェクトをインポートする</div>

    <p>
      既存のコード、デザイン、その他のプロジェクトの作業から構築を続けます。
    </p>
...
現在の公開情報については、[Web検索ガイド](/ja/features/agent/web-search)を参照してください。[MCP](/ja/chat/connect-through-mcp)は、対応するツールやクライアント向けの別の拡張手段です。
