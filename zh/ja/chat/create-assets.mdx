---
title: "创建资源"
description: "创建图像、视频、矢量图形、动画和音频资源，并按照各工作流程的官方指南进行操作。"
sidebarTitle: "创建资源"
icon: "photo-film"
...
让 Replit 创建完成工作所需的媒体，并在 Conversation 或相应的产品页面中完善结果。选择符合预期成果的资源类型。
...
<section className="chat-mkt chat-workflow-grid has-visuals catalog-three-up">
  <a className="chat-example has-visual" href="/zh/ja/features/agent/image-generation">
    ...
    <p>
      为项目创建自定义图片、插图、背景、图标等视觉素材。
    </p>

    <div className="chat-card-link">打开图片生成 →</div>
  </a>

  <a className="chat-example has-visual" href="/zh/ja/design/toolbar#generate">
    ...
    <p>
      使用 Replit Design 的 Generate media 工具创建短视频片段和动画。
    </p>
  ...
  <a className="chat-example has-visual" href="/zh/ja/design/toolbar#generate">
    ...
    <p>
      在 Replit Design 中创建 SVG 图标、徽标、图示和风格化插图。
    </p>

    <div className="chat-card-link">打开 Design 指南 →</div>
  </a>

  <a className="chat-example has-visual" href="/zh/ja/features/artifact-types/animated-videos">
    ...
    <p>
      通过 Conversation 构建程序化动态图形，并将其导出为视频。
    </p>
...
创建资源会立即生成结果。对于需要按计划重复执行的工作，请参阅[运行 Routine 任务](/zh/ja/chat/routines)。

生成的视频片段和动画视频采用不同的工作流程。动画视频是可导出为 MP4 的程序化动态影像。有关视觉资源的示例，请参阅[设计第一个屏幕](/zh/ja/design/design-your-first-screen)。

<div id="section-2">
  ## 引入最新信息
</div>

Web Search 不会创建资源。当图片、音频片段、视频或图形需要基于最新信息时，请使用它，并让 Replit 根据这些信息创建合适的资源。
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/ja/features/agent/web-search">
    ...
    <p>
      在创建资源前，收集包含最新信息和来源的资料。
    </p>

    <div className="chat-card-link">打开 Web Search →</div>
  </a>
</section>