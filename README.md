...
- French, Korean, Russian, and Chinese translations (`fr/`, `ko/`, `ru/`, `zh/`),
  generated by [Locadex](https://generaltranslation.com/docs/locadex) and wired
  into `docs.json` via per-locale `docs.json` `$ref`s. Japanese and Portuguese
  are intentionally not included.
- `gt.config.json` / `gt-lock.json` — the Locadex configuration and translation
  lock file. `gt.config.json`'s `locales` list must include every locale
  directory present in the repo, or Locadex may mistake an untracked locale's
  files for source content and mistranslate or delete them.
