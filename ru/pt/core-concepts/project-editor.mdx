...
description: "Узнайте, что такое Редактор проектов: ваша рабочая среда в Replit, где вы общаетесь с Agent, следите за разработкой приложения и управляете всем."
---
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="Главный экран редактора проектов Replit с полем промпта, в котором вы описываете, что хотите создать" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>

<div id="o-que-o-editor-de-projetos">
  ## Что такое Редактор проектов?
</div>

Редактор проектов — ваша отправная точка в Replit: здесь вы создаёте и управляете проектами, а также наблюдаете, как ваши приложения оживают. Опишите, что хотите создать, и [Agent](/ru/pt/core-concepts/agent) позаботится обо всём остальном: напишет код, настроит проект и покажет интерактивный предпросмотр в реальном времени.

<div id="primeiros-passos">
  ## Начало работы
</div>

Введите в поле промпта, что хотите создать. Опишите свою идею простым языком — «трекер бюджета, который показывает мои расходы на графиках», «лендинг для моей кофейни», «генератор комиксов с ИИ для детей» — а Agent сделает всё остальное.

<AgentInput />

Вы также можете выбрать в карусели тип проекта, чтобы указать Agent, какой результат вам нужен: веб-приложение, мобильное приложение, презентация, дизайн и многое другое. Или пропустите выбор и просто опишите свою идею — Agent автоматически подберёт подходящую конфигурацию.

<AgentCarousel />

Чтобы начать прямо сейчас, воспользуйтесь [кратким руководством «Создание с ИИ»](/ru/pt/getting-started/quickstarts/ask-ai/).

<div id="modo-plan">
  ## Режим Plan
</div>

В режиме Plan можно обдумывать идеи, задавать вопросы и планировать проект. Agent создаёт упорядоченный список задач, который можно просмотреть и доработать. Когда план будет готов, утвердите его — и Agent начнёт разработку. Режим Plan также можно использовать для вопросов и изучения идей: Agent не внесёт никаких изменений, пока вы не выйдете из этого режима.

<AgentInput highlightPlan={true} showTypewriter={false} />

Включите режим Plan кнопкой в нижней части поля ввода. Подробнее — в [руководстве по режиму Plan](/ru/pt/core-concepts/agent/plan-mode).

<div id="o-que-voc-pode-criar">
  ## Что можно создать
</div>

Выберите тип проекта, чтобы указать Agent, что вам нужно, или просто опишите свою идею — Agent сам выберет подходящую конфигурацию.

* **Веб-приложения** — дашборды, внутренние инструменты, SaaS-продукты, лендинги
* **Мобильные приложения** — приложения для iOS и Android по одному промпту
* **Презентации и слайды** — создавайте презентации с продуманными раскладками
* **Анимации** — интерактивные руководства и моушн-графика
* **Дизайны** — визуальные макеты и прототипы в [Design Canvas](/ru/pt/replitai/canvas)
* **Визуализация данных** — графики, дашборды и инструменты с базами данных
* **Документы** — файлы CSV, PDF, PowerPoint и документы Markdown
* **Таблицы** — таблицы данных и структурированные результаты

<div id="o-editor-de-projetos">
  ## Редактор проектов
</div>

Когда Agent начинает создавать приложение, в Редакторе проектов вы можете следить за разработкой проекта. Он разделён на две панели: с одной стороны — ваш диалог с Agent, с другой — интерактивный предпросмотр вашего App.

<div id="threads">
  ### Ветки
</div>

Каждый разговор с Agent ведётся в отдельной ветке. В основной ветке вы описываете, что хотите сделать, и следите за тем, как Agent создаёт приложение. При использовании [системы задач](/ru/pt/core-concepts/agent/task-system) у каждой фоновой задачи есть собственная ветка — так Agent изолирует работу над задачами, не смешивая разговоры.

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="Ветка с запросом пользователя и Agent, создающим проект" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>
...
Доска задач распределяет работу по столбцам: **Черновики**, **Активные**, **Готово** и **Завершённые**. По мере выполнения задачи перемещаются слева направо, чтобы вы могли сразу увидеть, что запланировано, над чем работает Agent и что уже завершено. Подробнее см. в разделе [Параллельная разработка](/ru/pt/core-concepts/agent/task-system).

<Frame>
  <img src="/assets/mirror/task-board-active-355ecd390c49.png" alt="Доска задач с задачами, распределёнными по столбцам «Черновики», «Активные», «Готово» и «Завершённые»" width="3430" height="1986" data-path="images/replitai/task-board-active.png" />
</Frame>
...
Design Canvas — это визуальный редактор, в котором можно создавать и дорабатывать макеты перед переходом к коду. Перетаскивайте элементы, настраивайте раскладки и совершенствуйте дизайн, а затем поручите Agent превратить его в работающее приложение. Подробнее см. в разделе [Design Canvas](/ru/pt/replitai/canvas).

<div id="colaborao">
  ## Совместная работа
</div>

Создайте командный Workspace для совместной разработки. Приглашайте участников команды, делитесь проектами и работайте над одним App в реальном времени. Подробнее о настройке команды см. в разделе [Командные Workspaces](/ru/pt/core-concepts/workspaces/team-workspaces), а о совместной работе на уровне проекта — в разделе [Приглашение участников команды](/ru/pt/replitai/invite-teammates).

<div id="importar-cdigo-e-design">
  ## Импорт кода и дизайна
</div>

Уже есть проект? Импортируйте его в Replit из GitHub или Figma либо загрузите файлы напрямую. Agent может продолжить с того места, на котором вы остановились, и дальше создавать приложение на основе существующего кода или дизайна. См. [Импорт в Replit](/ru/pt/core-concepts/project-editor/app-setup/import).
...
<CardGroup cols={2}>
  <Card title="Редактор и инструменты" href="/ru/pt/core-concepts/project-editor/editor-and-tools" icon="window-restore">
    Основные панели для предпросмотра приложения, вывода Console и работы с другими инструментами
  </Card>

  <Card title="Настройка приложения" href="/ru/pt/core-concepts/project-editor/app-setup/configuration" icon="gear">
    Настройте параметры, зависимости, Secrets и среду приложения
  </Card>

  <Card title="Контроль версий" href="/ru/pt/core-concepts/project-editor/version-control" icon="clock-rotate-left">
    Отслеживайте изменения, возвращайтесь к предыдущим версиям и управляйте историей приложения
  </Card>

  <Card title="Публикация" href="/ru/category/replit-deployments" icon="rocket">
    Разверните приложение по общедоступному URL, чтобы им мог пользоваться любой пользователь
  </Card>
</CardGroup>
...
<Accordion title="Нужно ли уметь программировать?">
  Нет. Опишите простыми словами, что вам нужно, и Agent возьмёт на себя технические детали. При желании вы также можете вносить изменения вручную.
</Accordion>

<Accordion title="Что произойдёт, если я закрою браузер?">
  Ваш проект сохраняется в облаке. Вернитесь в любое время и продолжите с того места, где остановились: файлы, история и запущенное приложение сохранятся.
</Accordion>

<Accordion title="Можно ли работать с другими людьми?">
  Да. Вы можете пригласить участников в конкретный проект или создать командный Workspace, участники которого получат доступ ко всем проектам. См. [Командные Workspace](/ru/pt/core-concepts/workspaces/team-workspaces) и [Приглашение участников команды](/ru/pt/replitai/invite-teammates).
</Accordion>

<Accordion title="Как опубликовать приложение?">
  Когда приложение будет готово, разверните его по общедоступному URL прямо из Редактора проектов. См. [Развёртывания](/ru/category/replit-deployments).
</Accordion>
...
Редактор проектов доступен на всех тарифах. Некоторые функции, например повышенная вычислительная мощность и постоянно активные деплои, требуют платного тарифа. Подробнее см. в разделе [Биллинг](/ru/pt/billing/ai-billing).