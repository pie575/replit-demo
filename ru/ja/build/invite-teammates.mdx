...
description: "Пригласите участников команды в проект и работайте вместе — каждый сможет работать над одним проектом, создавая новые ветки в Agent."
---
...
В Replit несколько человек могут одновременно работать над одним проектом. Каждый может начать новую ветку планирования с Agent, а задачи всех участников отображаются на общей доске — так команда может создавать вместе, а не по очереди.

<div id="section-2">
  ## Начните создавать вместе с командой
</div>
...
1. Откройте проект
2. Нажмите **Пригласить** или поделитесь ссылкой на проект
3. Участники команды присоединятся и начнут новую ветку в Agent
...
Каждый может создавать новые ветки и работать независимо, не мешая другим. Задачи всех участников отображаются на общей канбан-доске, поэтому легко понять, кто над чем работает, и избежать дублирования.

<Note>
  В Core для всего проекта одновременно может выполняться только одна фоновая задача. В Pro можно параллельно выполнять до 10 задач — несколько человек могут одновременно использовать Agent для разработки разных частей проекта.
</Note>
...
Когда команда работает вместе, задачи всех участников отображаются на одной доске. Сразу видно, кто чем занимается, что сейчас в работе и что готово к применению.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Доска задач с задачами, распределёнными по столбцам «Черновик», «В работе», «Готово к применению» и «Завершено»" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', margin: '20px 0' }}>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Добавить возможность дарить истории семье и друзьям" description="Позволяет родителям создавать истории для других детей и отправлять их в подарок." status="draft-static" timestamp="2m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Черновик</strong> — Agent предложил эту задачу. План ожидает проверки.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Продолжение истории — новая глава" description="Добавляет кнопку «Продолжить приключение», которая создаёт продолжение с того места, где закончилась предыдущая история." status="active" timestamp="12m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Активный</strong> — Agent выполняет эту задачу в отдельной копии проекта.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Ночной режим с тёмной темой" description="Специальный ночной режим с тёплой тёмной темой и фоновыми звуками." status="ready-drop" timestamp="35m" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Готово к применению</strong> — Задача завершена. Проверьте изменения и примените их к основной версии.</span>
  </div>

  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    <TaskCard title="Чтение вслух с подсветкой слов" description="Функция преобразования текста в речь позволяет детям самостоятельно наслаждаться историями, следя за подсвеченными словами." status="done" timestamp="4h" width="100%" />

    <span style={{ fontSize: '13px', color: 'var(--kb-text-dimmer, #666)', padding: '0 4px' }}><strong>Завершено</strong> — Задача выполнена, а изменения включены в основную версию.</span>
  </div>
</div>

<div id="section-6">
  ### Использование досок в команде
</div>

Когда над проектом одновременно работают несколько человек, задачи каждого отображаются на общей доске. Например, один участник команды работает над продолжениями и повторной генерацией историй, другой — над возможностью отправлять подарки, а выполненные задачи остальных находятся в разделах «Готово к применению» и «Завершено».

<KanbanColumns
  columns={[
{ title: 'Активные', count: 3, cards: [
{ title: 'Продолжение истории — сиквел', description: 'Добавьте кнопку «Продолжить приключение», чтобы создавать продолжения с того места, где закончилась предыдущая история.', status: 'active', timestamp: '12m' },
{ title: 'Повторная генерация истории — переделать страницу', description: 'Добавьте возможность повторно генерировать отдельные страницы, чтобы родители могли контролировать результат и меньше разочаровывались.', status: 'active', timestamp: '8m' },
{ title: 'Добавить возможность отправлять подарки семье и друзьям', description: 'Позвольте родителям создавать истории для других детей и отправлять их в качестве подарка.', status: 'active-queued', timestamp: '1m' },
] },
{ title: 'Готово к применению', count: 2, cards: [
{ title: 'Тёмная тема для чтения перед сном', description: 'Специальный режим перед сном с тёплой тёмной темой и фоновыми звуками.', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: 'Ссылки на истории для публикации с OG-превью', description: 'Добавление общедоступных ссылок для публикации — самый важный рычаг роста.', status: 'ready-drop', timestamp: '31m', showMenu: true },
] },
{ title: 'Завершено', count: 3, cards: [
{ title: 'Режим чтения вслух с подсветкой слов', description: 'Функция преобразования текста в речь позволяет детям самостоятельно наслаждаться историями.', status: 'done', timestamp: '4h' },
{ title: 'Улучшенная настройка персонажей', description: 'Варианты внешности для по-настоящему персонализированных иллюстраций.', status: 'done', timestamp: '4h' },
{ title: 'Поддержка мобильных устройств и подготовка к продакшену', description: 'Улучшенные состояния ошибок, мобильные макеты и анимации загрузки.', status: 'done', timestamp: '4h' },
] }
]}
/>

Когда задача перемещается в раздел «Готово к применению», любой участник команды может проверить и применить изменения. Agent автоматически разрешает конфликты при применении нескольких задач.

<div id="section-7">
  ## Что можно создавать вместе
</div>

* **Распределять задачи в команде**: один человек создаёт фронтенд, а другой отвечает за бэкенд. Каждый работает в чате с Agent, координируя действия команды на канбан-доске.
* **Вводить новых участников команды в курс дела**: они могут изучать проект в своём чате с Agent, задавать вопросы о том, как всё устроено, и начинать вносить вклад, не блокируя других.
* **Выпускать быстрее с Pro**: благодаря параллельному выполнению 2–3 человека могут одновременно запускать свои задачи. Проект, на который один человек тратит целую сессию, можно завершить за часть этого времени.

<div id="section-8">
  ## Часто задаваемые вопросы
</div>

<Accordion title="Можно ли отправлять сообщения коллегам через Agent?">
  Нет. Agent не предназначен для обмена сообщениями — он всегда пытается выполнить действие в ответ на введённый вами текст. Для командного общения используйте Slack, Discord или другие инструменты.
</Accordion>

<Accordion title="Можем ли мы перезаписать работу друг друга?">
  Система задач предотвращает это, создавая независимую копию для каждой задачи и учитывая другие задачи. После завершения задачи примените её к основной версии и разрешите конфликты на этом этапе.
</Accordion>

<Accordion title="Можно ли узнать, чем занимаются коллеги?">
  Активные задачи отображаются на канбан-доске. Также можно видеть курсоры коллег и файлы, которые они просматривают.
</Accordion>

<Accordion title="Можно ли удалить кого-то из проекта?">
  Да. Владелец проекта может в любое время управлять участниками команды и отзывать доступ.
</Accordion>
