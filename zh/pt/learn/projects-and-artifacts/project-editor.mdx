---
title: "项目编辑器"
description: "了解项目编辑器：这是您在 Replit 上与 Agent 对话、查看 App 逐步成形并管理一切的核心环境。"
sidebarTitle: "项目编辑器"
icon: "window"
---
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="Replit 项目编辑器主页，显示用于描述你想构建内容的 prompt 输入框" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>
...
项目编辑器是您在 Replit 上开展工作的起点：您可以在这里开始构建、管理项目，并见证您的 App 焕发生机。只需描述您想构建的内容，[Agent](/zh/pt/features/agent/overview) 就会完成其余工作，包括编写代码、配置项目，并实时提供预览。

<div id="primeiros-passos">
  ## 开始使用
</div>

在 prompt 输入框中输入你想构建的内容。用简单的语言描述你的想法——“一个能将我的支出绘制成图表的预算追踪器”、“一个为我的咖啡店设计的落地页”、“一个面向儿童的 AI 漫画故事生成器”——其余的交给 Agent。

<AgentInput />

你还可以从轮播列表中选择项目类型，告诉 Agent 你想要什么类型的输出——web 应用、移动应用、幻灯片、设计等。或者跳过选择，直接描述你的想法；Agent 会自动确定合适的配置。

<AgentCarousel />

想快速开始，请参阅[构建你的第一个 app](/zh/pt/build/your-first-app)。

<div id="modo-de-plano">
  ## 规划模式
</div>

规划模式让您可以集思广益、提出问题并梳理项目。Agent 会创建一份按顺序排列的任务清单，供您审阅和完善。对计划满意后，批准即可让 Agent 开始构建。您也可以使用规划模式提问或探索想法；在您退出该模式前，Agent 不会进行任何更改。

<AgentInput highlightPlan={true} showTypewriter={false} />

通过输入框底部的按钮启用规划模式。更多信息请参阅[规划模式指南](/zh/pt/features/agent/plan-mode)。

<div id="o-que-voc-pode-construir">
  ## 您可以构建的内容
</div>

选择项目类型，告诉 Agent 您想构建什么；也可以直接描述您的想法，让 Agent 自动选择合适的配置。

* **web 应用** — 仪表板、内部工具、SaaS 产品、落地页
* **移动应用** — 通过一个 prompt 创建 iOS 和 Android 应用
* **幻灯片和演示文稿** — 生成具有丰富布局的演示文稿
* **动画** — 交互式演示和 motion graphics
* **设计** — 在 [Design Canvas](/zh/pt/design/canvas) 中创建视觉模拟稿和原型
* **数据可视化** — 图表、仪表板和数据库工具
* **文档** — CSV、PDF、PowerPoint 文件和 Markdown 文档
* **电子表格** — 数据表和结构化输出
...
Agent 开始构建后，你可以在项目编辑器中查看项目逐步成形。它分为多个面板：一侧是你与 Agent 的对话，另一侧是应用的实时预览。

<div id="threads">
  ### 线程
</div>

每次与 Agent 的对话都会在一个线程中进行。你可以在主线程中描述需求，并查看 Agent 如何构建项目。使用[任务系统](/zh/pt/core-concepts/agent/task-system)时，每个后台任务都有自己的线程，让 Agent 能够隔离处理各项任务，避免混淆对话。

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="一个线程，展示用户的 prompt 以及 Agent 正在构建项目" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>
...
任务面板将你的工作按列组织：**草稿**、**进行中**、**就绪**和**已完成**。每项任务会随着进展从左向右移动，让你能快速了解已规划的工作、Agent 正在处理的工作以及已完成的工作。有关详细信息，请参阅[任务系统](/zh/pt/core-concepts/agent/task-system)。

<Frame>
  <img src="/assets/mirror/task-board-active-355ecd390c49.png" alt="任务面板显示按草稿、进行中、就绪和已完成列组织的任务" width="3430" height="1986" data-path="images/replitai/task-board-active.png" />
</Frame>
...
Design Canvas 是一款可视化编辑器，您可以在开始编写代码前创建和完善设计稿。拖放元素、调整布局并不断迭代设计，然后让 Agent 将其转化为可运行的 App。有关详细信息，请参阅 [Design Canvas](/zh/pt/design/canvas)。

<div id="colaborao">
  ## 协作
</div>

创建团队 Workspace，与团队成员共同构建。邀请同事、共享项目，并实时协作开发同一个 App。有关团队设置，请参阅[团队 Workspace](/zh/pt/features/collaboration/team-workspaces)；有关项目级协作，请参阅[邀请队友](/zh/pt/build/invite-teammates)。

<div id="importe-cdigo-e-design">
  ## 导入代码和设计
</div>

已有项目？您可以从 GitHub、Figma 导入到 Replit，或直接上传文件。Agent 可以从您上次停下的地方继续，并在现有代码或设计的基础上继续构建。请参阅[导入到 Replit](/zh/pt/build/import-from-providers)。
...
<CardGroup cols={2}>
  <Card title="编辑器和工具" href="/zh/pt/features/editor/editor-and-tools" icon="window-restore">
    查看 App 预览、控制台输出及使用其他工具的主要面板
  </Card>

  <Card title="App 配置" href="/zh/pt/features/project-setup/configuration" icon="gear">
    配置 App 的设置、依赖项、Secrets 和环境
  </Card>

  <Card title="版本控制" href="/zh/pt/learn/projects-and-artifacts/version-control" icon="clock-rotate-left">
    跟踪更改、还原到早期版本并管理 App 的历史记录
  </Card>

  <Card title="部署" href="/zh/pt/learn/projects-and-artifacts/replit-deployments" icon="rocket">
    将 App 部署到线上 URL，供任何人使用
  </Card>
</CardGroup>
...
<Accordion title="我需要会编程吗？">
  不需要。只需用简单的语言描述你的需求，Agent 会处理技术细节。你也可以根据需要直接编辑。
</Accordion>

<Accordion title="关闭浏览器后会怎样？">
  你的项目会保存到云端。你可以随时回来并从上次停下的地方继续：文件、历史记录和正在运行的应用都会保留。
</Accordion>

<Accordion title="我可以与其他人协作吗？">
  可以。你可以邀请协作者加入特定项目，或创建团队 Workspace，让成员访问所有项目。请参阅[团队 Workspace](/zh/pt/features/collaboration/team-workspaces)和[邀请团队成员](/zh/pt/build/invite-teammates)。
</Accordion>

<Accordion title="如何发布我的应用？">
  当你的应用准备就绪后，可以直接通过项目编辑器将其部署到线上 URL。请参阅[Deployments](/zh/pt/learn/projects-and-artifacts/replit-deployments)。
</Accordion>
...
项目编辑器适用于所有方案。部分功能 (如增强计算和始终运行的部署) 需要付费计划。有关详情，请参阅[计费](/zh/pt/billing/ai-billing)。