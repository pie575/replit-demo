---
title: "任务看板"
description: "使用任务看板查看、启动、监控和应用 Agent 任务，跟踪其在各个阶段的进度。"
---
...
任务看板为您提供一个集中位置，用于查看、启动、监控和应用 Agent 任务。当 Agent 将大型请求拆分为多个部分、您想在后台执行任务，或需要了解项目中已作出的更改时，可以使用它。

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="任务看板显示按“草稿”、“进行中”、“已就绪”和“已完成”列组织的任务" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

任务会根据状态分列排列。看板会显示 Agent 计划的内容、正在执行的内容、可供审核的内容，以及已应用的内容。

每项任务都会随着进展从左向右移动：

* **草稿**：Agent 已计划但尚未启动的任务
* **进行中**：Agent 当前正在构建的任务，包括因活跃任务数量限制而排队等待的任务
* **已就绪**：Agent 已完成、可供您审核的任务
* **已完成**：已完成、已归档或已取消的任务
...
**草稿**是 Agent 尚未开始构建的已规划任务。每个草稿都包含标题、描述和分步计划。

使用草稿可在工作开始前审阅 Agent 提出的方案。你可以查看计划，请 Agent 调整计划，或在准备就绪后启动任务。

如果你想在投入后台工作前规划一项较大的功能，草稿会很有用。例如，Agent 可以将一个集成项目拆分为分别用于身份验证、个人资料配置、仪表板界面和部署优化的草稿。

<div id="ativo">
  ## 进行中
</div>

接受任务后，它会移至**进行中**列，Agent 随即开始构建。每项任务都会在项目各自独立的隔离副本中执行，因此在你应用这些更改之前，主版本不会发生任何变化。

有些任务依赖于其他任务。例如，构建仪表板的任务可能需要先完成数据库架构任务。依赖任务会以**队列**状态等待，直到其前置条件完成。如果你已达到套餐中活跃后台任务的数量上限，任务也会进入队列。

<KanbanColumns
  columns={[
{ title: '草稿', count: 2, cards: [
{ title: '为家人和朋友添加赠礼流程', description: '让父母可以为他人的孩子创作故事，并将其作为礼物赠送。这将开辟新的获客渠道。', status: 'draft-static', timestamp: '2m' },
{ title: '采用深色主题的睡前阅读模式', description: '专用的睡前模式搭配暖色调深色主题和环境音效，将 App 打造成完整的睡前仪式。', status: 'draft-static', timestamp: '2m' },
] },
{ title: '进行中', count: 3, cards: [
{ title: '故事续篇：续集', description: '添加“继续冒险”按钮后，可生成从上一段故事结束处继续的续集。', status: 'active', timestamp: '12m' },
{ title: '故事重新生成：重做页面', description: '添加重新生成单个页面的功能，让父母拥有更多控制权并减少挫败感。', status: 'active', timestamp: '8m' },
{ title: '为家人和朋友添加赠礼流程', description: '让父母可以为他人的孩子创作故事，并将其作为礼物赠送。', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
任务构建完成后，会进入**已就绪**状态。这表示工作已完成，但你的主版本尚未发生任何变化。

你可以通过两种方式应用更改：

* **在线程中**：打开任务线程，选择**将更改应用到主版本**，将工作内容应用到项目中；或者选择**丢弃**将其舍弃
* **在看板中**：在任意已就绪任务卡片上选择三点菜单，然后选择**应用更改**

<TaskReviewDrawer />

应用更改前，请先检查已就绪的任务。查看工作日志、测试输出和预览，了解 Agent 做了哪些更改。
...
已完成列还包括已归档的规划会话和已取消的任务。这样既能保留历史记录，又不会将已完成的工作重新归入草稿或进行中任务。

<KanbanColumns
  columns={[
{ title: '已就绪', count: 3, cards: [
{ title: '带深色主题的睡前阅读模式', description: '专为睡前设计的模式，配有暖色调深色主题和环境音效。', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: '带 OG 预览的可分享故事链接', description: '添加公开可分享链接是最重要的增长驱动力。', status: 'ready-drop', timestamp: '31m', showMenu: true },
{ title: '增强的角色自定义功能', description: '添加外观选项可让插图真正实现个性化。', status: 'ready-drop', timestamp: '28m', showMenu: true },
] },
{ title: '已完成', count: 4, cards: [
{ title: '带单词高亮的朗读模式', description: '文字转语音功能让孩子们能够独立享受故事。', status: 'done', timestamp: '4h' },
{ title: '带 OG 预览的可分享故事链接', description: '公开可分享链接是最重要的增长驱动力。', status: 'done', timestamp: '1h' },
{ title: '增强的角色自定义功能', description: '提供外观选项，让插图真正实现个性化。', status: 'done', timestamp: '4h' },
{ title: '移动端响应式设计与上线前优化', description: '更完善的错误状态、移动端布局和加载动画。', status: 'done', timestamp: '4h' },
] }
]}
/>

Agent 会在应用多项任务的更改时自动解决冲突。如果有任何问题，请让 Agent 修复。

<div id="encontrar-e-filtrar-tarefas">
  ## 查找和筛选任务
</div>

当项目中有许多进行中的任务时，可使用任务看板。你可以按标题或任务编号搜索、筛选草稿，并使用编辑模式批量管理任务组。

例如，当有人在线程、评审或 Slack 对话中提及某项任务时，可按任务编号搜索。若想专注于计划中的工作，而不必浏览进行中或已完成的任务，可筛选草稿。

如需查看建议的后续任务，请参阅[后续任务](/zh/pt/core-concepts/agent/task-system/follow-up-tasks)。