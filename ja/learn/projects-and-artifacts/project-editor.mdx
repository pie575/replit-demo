---
title: "プロジェクトエディター"
description: "プロジェクトエディターとは何か：Replitであなたの拠点となる環境。Agentと会話し、アプリが形になるのを見て、すべてを管理します。"
sidebarTitle: "プロジェクトエディター"
icon: "window"
---
...
<Frame>
  <img src="/assets/mirror/workspace-home-e7642fab34cb.png" alt="Replitプロジェクトエディターのホーム画面。何を構築したいかを説明するプロンプト入力欄が表示されている" width="3456" height="1994" data-path="images/workspace/workspace-home.png" />
</Frame>

## プロジェクトエディターとは何か？

プロジェクトエディターはReplitのあなたの拠点です：構築を始め、プロジェクトを管理し、アプリが形になるのを見る環境です。ここで構築したいものを説明すると、[Agent](/ja/features/agent/overview)があとを引き受け、コードを書き、プロジェクトをセットアップし、リアルタイムでライブプレビューを表示します。

## 始め方

プロンプトボックスに構築したいものを入力します。「支出をグラフ化する予算トラッカー」、「コーヒーショップのランディングページ」、「子供向けAIコミックブックジェネレーター」などと平易な言葉でアイデアを説明すると、Agentが残りを処理します。

<AgentInput />

カルーセルからプロジェクトタイプを選択して、Agentに何の出力が欲しいかを伝えることもできます — ウェブアプリ、モバイルアプリ、スライド、デザインなど。または選択をスキップしてアイデアを説明するだけでも、Agentが自動的に適切なセットアップを見つけます。

<AgentCarousel />

すぐに始めるには[最初のアプリを構築する](/ja/build/your-first-app)に従ってください。

## プランモード

プランモードではブレインストーミング、質問、プロジェクトのマッピングができます。Agentがレビューして改良できる順番付きタスクリストを作成します — プランに満足したら承認するとAgentが構築を開始します。プランモードを使って質問したりアイデアを探索することもできます；終了するまでAgentは変更を加えません。

<AgentInput highlightPlan={true} showTypewriter={false} />

入力欄の下部のボタンからプランモードをトグルします。詳細は[プランモードガイド](/ja/features/agent/plan-mode)をご覧ください。

## 構築できるもの

プロジェクトタイプを選択してAgentに何が欲しいかを伝えるか、アイデアを説明してAgentに適切なセットアップを選ばせましょう。

* **ウェブアプリ** — ダッシュボード、社内ツール、SaaSプロダクト、ランディングページ
* **モバイルアプリ** — 1つのプロンプトからiOSとAndroidアプリ
* **スライドとプレゼンテーション** — リッチなレイアウトのデッキを生成
* **アニメーション** — インタラクティブなウォークスルーとモーショングラフィックス
* **デザイン** — [デザインCanvas](/ja/design/canvas)上のビジュアルモックアップとプロトタイプ
* **データビジュアライゼーション** — チャート、ダッシュボード、データベースバックエンドツール
* **ドキュメント** — CSV、PDF、PowerPointファイル、Markdownドキュメント
* **スプレッドシート** — データテーブルと構造化出力

## プロジェクトエディター

Agentが構築を始めると、プロジェクトエディターでプロジェクトが形になるのを見ることができます。パネルに分かれています：一方はAgentとの会話、もう一方はアプリのライブプレビューです。

### スレッド

Agentとのすべての会話はスレッドで行われます。メインスレッドは何が欲しいかを説明してAgentが構築するのを見る場所です。[タスクシステム](/ja/core-concepts/agent/task-system)を使うと、各バックグラウンドタスクが独自のスレッドを持ちます — Agentは会話を混同することなくタスクの作業を独立して保つことができます。

<Frame>
  <img src="/assets/mirror/task-describe-idea-555338f515fd.png" alt="ユーザーのプロンプトとAgentがプロジェクトを構築するスレッド" width="3430" height="1986" data-path="images/replitai/task-describe-idea.png" />
</Frame>

### タスクボード

タスクボードはあなたの作業を**下書き**、**アクティブ**、**準備完了**、**完了**の列に整理します。各タスクは進行とともに左から右に移動するため、計画されているもの、Agentが取り組んでいるもの、完了したものをひと目で確認できます。詳細は[タスクシステム](/ja/core-concepts/agent/task-system)をご覧ください。

<Frame>
  <img src="/assets/mirror/task-board-active-355ecd390c49.png" alt="下書き、アクティブ、準備完了、完了の列にタスクが整理されたタスクボード" width="3430" height="1986" data-path="images/replitai/task-board-active.png" />
</Frame>

### デザインCanvas

デザインCanvasは、コードにコミットする前にモックアップを作成して改良できるビジュアルエディターです。要素をドラッグし、レイアウトを調整し、デザインを反復して — Agentに動作するアプリに変えてもらいましょう。詳細は[デザインCanvas](/ja/design/canvas)をご覧ください。

## コラボレーション

チームワークスペースを作成して一緒に構築しましょう。チームメイトを招待し、プロジェクトを共有し、同じアプリでリアルタイムで作業できます。チームのセットアップは[チームワークスペース](/ja/features/collaboration/team-workspaces)、プロジェクトレベルのコラボレーションは[チームメイトを招待する](/ja/build/invite-teammates)をご覧ください。

## コードとデザインをインポートする

すでにプロジェクトをお持ちですか？GitHub、Figmaからインポートするか、ファイルを直接アップロードしてReplitに持ち込みましょう。Agentは中断したところから続け、既存のコードやデザインから構築を続けることができます。[Replitにインポートする](/ja/build/import-from-providers)をご覧ください。

## 次のステップ

<CardGroup cols={2}>
  <Card title="エディタとツール" href="/ja/features/editor/editor-and-tools" icon="window-restore">
    アプリのプレビュー、コンソール出力、その他のツールを見るメインパネル
  </Card>

  <Card title="アプリのセットアップ" href="/ja/features/project-setup/configuration" icon="gear">
    アプリの設定、依存関係、シークレット、環境を設定する
  </Card>

  <Card title="バージョン管理" href="/ja/learn/projects-and-artifacts/version-control" icon="clock-rotate-left">
    変更を追跡し、以前のバージョンにロールバックし、アプリの履歴を管理する
  </Card>

  <Card title="公開" href="/ja/learn/projects-and-artifacts/replit-deployments" icon="rocket">
    アプリをライブURLにデプロイして誰でも使えるようにする
  </Card>
</CardGroup>

## よくある質問

<Accordion title="コードの書き方を知っている必要がありますか？">
  いいえ。平易な言葉で何が欲しいかを説明すれば、Agentが技術的な詳細を処理します。望む場合は直接編集することもできます。
</Accordion>

<Accordion title="ブラウザを閉じると何が起こりますか？">
  プロジェクトはクラウドに保存されます。いつでも戻ってきて中断したところから再開できます：ファイル、履歴、実行中のアプリはすべて保持されます。
</Accordion>

<Accordion title="他の人と一緒に作業できますか？">
  はい。特定のプロジェクトにコラボレーターを招待するか、メンバーがすべてのプロジェクトにアクセスできるチームワークスペースを作成できます。[チームワークスペース](/ja/features/collaboration/team-workspaces)と[チームメイトを招待する](/ja/build/invite-teammates)をご覧ください。
</Accordion>

<Accordion title="アプリを公開するにはどうすればよいですか？">
  アプリの準備ができたら、プロジェクトエディターから直接ライブURLにデプロイします。[デプロイメント](/ja/learn/projects-and-artifacts/replit-deployments)をご覧ください。
</Accordion>

## 利用可能な機能

プロジェクトエディターはすべてのプランで利用できます。増加したコンピュートや常時稼働デプロイメントなど一部の機能は有料プランが必要です。詳細は[課金](/ja/billing/ai-billing)をご覧ください。
