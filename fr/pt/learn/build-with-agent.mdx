...
description: "Découvrez cinq bonnes pratiques pour travailler avec Agent : soyez précis, planifiez le travail, ajoutez du contexte, relisez et testez, et utilisez les checkpoints."
...
Agent est le créateur d&#39;applications par IA de Replit et votre allié pour transformer vos idées en logiciels. Agent peut planifier des modifications, écrire du code, expliquer des comportements, déboguer des problèmes et améliorer votre app.

[Vibe coding 101](/fr/pt/learn/foundations/vibe-coding-101) présente l&#39;état d&#39;esprit à adopter, tandis que cette page explique comment travailler avec Agent en pratique. [Effective prompting](/fr/pt/learn/effective-prompting) explique comment rédiger des instructions plus claires.

Voyez Agent comme le quarterback sur le terrain : il analyse la situation, choisit une direction et fait avancer le jeu. Vous êtes l&#39;entraîneur : vous définissez la stratégie, annoncez la tactique, examinez ce qui s&#39;est passé et décidez du coup suivant.

Agent peut faire beaucoup de choses, mais il donne le meilleur de lui-même lorsque c&#39;est vous qui menez la barque : soyez précis, planifiez le travail, ajoutez du contexte, relisez et testez, et utilisez les checkpoints lorsque vous avez besoin de revenir en arrière.

<div id="cinco-dicas-para-construir-com-agent">
...
Agent peut gérer des tâches complexes, mais il a besoin de consignes claires.

Une bonne demande explique ce que vous voulez, ce qui compte et ce qui doit rester inchangé. Inutile de décrire chaque détail d&#39;implémentation, mais donnez à Agent assez d&#39;éléments pour prendre de bonnes décisions.
...
<AiPrompt>
  Improve the signup page so visitors understand what the product does before creating an account.<br />
  Keep the current color palette, keep the form fields the same, and make the primary button more obvious.
</AiPrompt>

Le second prompt donne à Agent un objectif, un périmètre et des contraintes.

Lorsque votre demande comporte plusieurs volets, précisez ce qui prime.

<AiPrompt>
  Add a simple admin view for managing catering requests.<br />
  The most important part is being able to see new requests, mark them as contacted, and filter by status.<br />
  Keep the first version simple.
</AiPrompt>

Agent peut découper ce travail en étapes, mais ce sont vos indications qui définissent ce qu&#39;est un résultat réussi.

Pour d&#39;autres exemples d&#39;instructions précises, consultez [Effective prompting](/fr/pt/learn/effective-prompting).

<div id="planeje-o-trabalho">
  ## Planifiez le travail
</div>

La planification est utile lorsque le travail comporte des incertitudes, plusieurs étapes, des compromis ou tout élément que vous souhaitez valider avant que des fichiers ne soient modifiés.
...
Utilisez le [Plan mode](/fr/pt/features/agent/plan-mode) lorsque vous souhaitez qu&#39;Agent réfléchisse d&#39;abord et attende votre approbation avant de modifier des fichiers. Activez **Plan** dans le composeur de prompts avant d&#39;envoyer.

<Frame>
  <img src="/assets/mirror/agent-plan-mode-prompt-059fe8d2c277.png" alt="La zone de prompt initiale de Replit avec un prompt d'app en cours d'exécution saisi et la case Plan activée" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-mode-prompt.png" />
</Frame>
...
Lorsqu&#39;Agent a terminé de réfléchir, un bandeau **task plan is ready for review** s&#39;affiche avec un bouton **Review now**.

<Frame>
  <img src="/assets/mirror/agent-plan-ready-banner-5fbcf58a1668.png" alt="Un bandeau « task plan is ready for review » avec un bouton « Review now » en bas du panneau Agent" width="560" height="550" data-path="images/fitstart/work-with-agent/agent-plan-ready-banner.png" />
</Frame>

<div id="adicione-contexto">
  ## Ajoutez du contexte
</div>

Le contexte aide Agent à comprendre ce qui compte vraiment.
...
Utilisez du texte lorsque la modification porte sur un comportement, des objectifs ou des contraintes :
...
Utilisez des captures d&#39;écran ou des croquis lorsque la mise en page est difficile à décrire avec des mots :
...
Un bon contexte précise aussi ce qui ne doit pas changer. Agent peut ainsi améliorer une zone sans en réécrire une autre par mégarde.

<div id="revise-e-teste">
  ## Révisez et testez
</div>

Agent peut proposer un plan avant de construire, puis résumer les modifications effectuées. Lisez les deux.

Ouvrez le plan en sélectionnant **View** sur la carte du plan de tâche. Le plan détaillé indique ce qu’Agent prévoit de construire, les critères de réussite, ce qui est hors périmètre et les étapes qu’Agent suivra.

<Frame>
  <img src="/assets/mirror/agent-plan-content-c298ea216ebc.png" alt="Plan de tâche détaillé montrant ce qui sera fait et pourquoi, les critères de réussite, les éléments hors périmètre et les étapes de construction numérotées" width="1440" height="900" data-path="images/fitstart/work-with-agent/agent-plan-content.png" />
</Frame>

Avant qu’Agent ne construise, examinez le plan :

* Résout-il le bon problème ?
* Manque-t-il quelque chose ?
* Y a-t-il quelque chose hors périmètre ?
* Existe-t-il une version plus simple à construire d’abord ?
* Explique-t-il comment tester le résultat ?

Si le plan est trop ambitieux, demandez à Agent de le simplifier.

<AiPrompt>
  Ce plan est trop ambitieux.<br />
  Ramène-le à la version la plus simple que je peux tester aujourd’hui.<br />
  Place tout le reste dans une liste d’améliorations futures.
</AiPrompt>

Après qu’Agent a construit l’app, testez-la vous-même. Ouvrez le Preview et utilisez-le comme le ferait la personne pour laquelle l’app a été créée.

Vérifiez :

* Une personne peut-elle effectuer l’action principale ?
* La modification demandée a-t-elle été effectuée ?
* L’app fonctionne-t-elle toujours sur les appareils mobiles ?
* Un comportement important a-t-il cessé de fonctionner ?
* Agent a-t-il modifié quelque chose qui n’était pas lié à la demande ?
* Le résultat est-il suffisamment bon pour poursuivre le développement ?

Si quelque chose ne va pas, décrivez ce que vous avez observé.
...
<AiPrompt>
  Lorsque j’envoie le formulaire de demande de service traiteur, le message de confirmation apparaît, mais le formulaire affiche toujours l’ancien texte.<br />
  Efface le formulaire après l’envoi et laisse le message de confirmation visible.
</AiPrompt>
...
Les checkpoints vous aident à revenir en arrière lorsqu’une modification ne donne pas le résultat escompté.

Utilisez un feedback ciblé lorsque le résultat est presque satisfaisant :

<AiPrompt>
  C’est presque ça, mais le formulaire est maintenant trop long.<br />
  Conservez le nouveau style visuel, supprimez le champ de notes facultatif et rendez la date de retrait plus facile à consulter.
</AiPrompt>

Utilisez un checkpoint lorsque l’App est moins bien qu’avant, qu’une fonctionnalité importante ne marche plus ou qu’Agent a modifié plus d’éléments que prévu.

Ouvrez **History** dans le panneau Agent pour afficher les checkpoints. Chaque checkpoint indique les modifications apportées et propose les commandes **Rollback here** et **Changes**.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-history-c33b3b0dd592.png" alt="Le panneau History d’Agent affichant un checkpoint intitulé 'Transitioned from Plan to Build mode' avec les boutons Rollback here et Changes" width="720" height="450" data-path="images/fitstart/work-with-agent/agent-checkpoints-history.png" />
</Frame>

Lorsque vous sélectionnez **Rollback here**, Agent confirme les éléments qui seront restaurés avant d’appliquer le rollback.

<Frame>
  <img src="/assets/mirror/agent-checkpoints-rollback-dialog-28de278b44da.png" alt="La boîte de dialogue de confirmation Rollback to checkpoint listant le checkpoint cible et les éléments concernés : fichiers, mémoire d’Agent, tâches et une case à cocher pour la base de données" width="900" height="880" data-path="images/fitstart/work-with-agent/agent-checkpoints-rollback-dialog.png" />
</Frame>

Si vous ne savez pas s’il vaut mieux corriger le problème ou revenir en arrière, demandez à Agent de vous aider à comparer.

<AiPrompt>
  La dernière modification a dégradé l’App.<br />
  Comparez la version actuelle à la dernière version fonctionnelle.<br />
  Dites-moi s’il est plus sûr de corriger le problème ou de revenir en arrière, et expliquez pourquoi.
</AiPrompt>

Il est normal de faire fausse route. Les checkpoints vous permettent d’explorer sans perdre la possibilité de revenir à une version plus fiable.

Pour plus d’informations sur la récupération, consultez [Checkpoints and rollbacks](/fr/pt/features/version-control/checkpoints-and-rollbacks).

<div id="como-uma-boa-colaborao-com-agent">
...
* Vous ajoutez des captures d&#39;écran, des fichiers ou des notes du Canvas lorsque les mots ne suffisent pas
* Vous testez les parcours importants après chaque modification
* Vous donnez un retour ciblé lorsque quelque chose ne va pas
* Vous utilisez les checkpoints plutôt que de vous acharner à démêler une modification ratée
...
<CardGroup cols={2}>
  <Card icon="comment-dots" href="/fr/pt/learn/effective-prompting">
    Apprenez à rédiger des instructions plus claires, qu&#39;Agent pourra suivre en toute confiance.
  </Card>

  <Card title="Créez votre première app" icon="rocket" href="/fr/pt/build/your-first-app">
    Entraînez-vous à travailler avec Agent, du premier prompt jusqu&#39;aux ajustements ciblés.
  </Card>

  <Card title="Checkpoints et rollbacks" icon="rotate-left" href="/fr/pt/features/version-control/checkpoints-and-rollbacks">
