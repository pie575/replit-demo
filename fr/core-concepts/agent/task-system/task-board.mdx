...
description: "Utilisez le tableau des tâches pour réviser, démarrer, surveiller et appliquer les tâches Agent au fur et à mesure de leur progression dans chaque étape."
---
...
Le tableau des tâches vous offre un seul endroit pour réviser, démarrer, surveiller et appliquer les tâches Agent. Utilisez-le lorsqu'Agent divise une grande demande en plusieurs parties, lorsque vous souhaitez exécuter du travail en arrière-plan, ou lorsque vous devez comprendre ce qui a déjà changé dans votre projet.

<Frame>
  <img src="/assets/mirror/task-board-progress-f4d3b5e7e93e.png" alt="Tableau des tâches affichant les tâches organisées dans les colonnes Brouillons, En cours, Prêt et Terminé" width="3456" height="1984" data-path="images/replitai/task-board-progress.png" />
</Frame>

Les tâches sont organisées en colonnes selon leur statut. Le tableau montre ce qu'Agent a planifié, ce qui est en cours d'exécution, ce qui est prêt pour révision et ce qui a déjà été appliqué.

Chaque tâche avance de gauche à droite au fil de sa progression :

* **Brouillons** : tâches qu'Agent a planifiées mais n'a pas encore démarrées
* **En cours** : tâches qu'Agent construit maintenant, y compris les tâches en file d'attente derrière votre limite de tâches actives
* **Prêt** : tâches qu'Agent a terminées et qui sont prêtes pour votre révision
* **Terminé** : tâches finies, archivées ou annulées

## Brouillons

Les **Brouillons** sont des tâches planifiées qu'Agent n'a pas encore commencé à construire. Chaque brouillon comprend un titre, une description et un plan étape par étape.

Utilisez les Brouillons pour réviser l'approche proposée d'Agent avant que le travail ne commence. Vous pouvez inspecter le plan, demander à Agent de l'ajuster, ou démarrer la tâche lorsque vous êtes prêt.

Les Brouillons sont utiles lorsque vous souhaitez planifier une fonctionnalité plus importante avant de vous engager dans un travail en arrière-plan. Par exemple, Agent pourrait diviser un projet d'intégration en brouillons séparés pour l'authentification, la configuration du profil, l'interface du tableau de bord et la finition du déploiement.

## En cours

Une fois que vous acceptez une tâche, elle passe dans la colonne **En cours** et Agent commence à construire. Chaque tâche s'exécute dans sa propre copie isolée de votre projet, de sorte que rien ne change dans votre version principale tant que vous n'avez pas appliqué le travail.

Certaines tâches dépendent d'autres. Par exemple, une tâche qui construit un tableau de bord peut nécessiter que la tâche du schéma de base de données soit terminée en premier. Les tâches dépendantes attendent **en file d'attente** jusqu'à ce que leurs prérequis soient terminés. Les tâches se mettent également en file d'attente si vous atteignez la limite de votre plan pour les tâches actives en arrière-plan.

<KanbanColumns
  columns={[
{ title: 'Brouillons', count: 2, cards: [
{ title: 'Ajouter un flux cadeau pour la famille et les amis', description: 'Permettre aux parents de créer une histoire pour l\'enfant de quelqu\'un d\'autre et de l\'envoyer en cadeau. Ouvre un nouveau canal d\'acquisition.', status: 'draft-static', timestamp: '2m' },
{ title: 'Mode lecture au coucher avec thème sombre', description: 'Un mode coucher dédié avec un thème sombre aux tons chauds et des sons ambiants transforme l\'application en un véritable rituel du coucher.', status: 'draft-static', timestamp: '2m' },
] },
{ title: 'En cours', count: 3, cards: [
{ title: 'Suites d\'histoires — séquelles', description: 'Ajouter un bouton « Continuer l\'aventure » génère une suite qui reprend là où la dernière s\'est arrêtée.', status: 'active', timestamp: '12m' },
{ title: 'Régénération d\'histoires — refaire des pages', description: 'Ajouter la possibilité de régénérer des pages individuelles donne aux parents le contrôle et réduit la frustration.', status: 'active', timestamp: '8m' },
{ title: 'Ajouter un flux cadeau pour la famille et les amis', description: 'Permettre aux parents de créer une histoire pour l\'enfant de quelqu\'un d\'autre et de l\'envoyer en cadeau.', status: 'active-queued', timestamp: '1m' },
] }
]}
/>
...
Quand une tâche finit de se construire, elle passe à **Prêt**. Cela signifie que le travail est terminé, mais rien n'a encore changé dans votre version principale.

Vous pouvez appliquer les modifications de deux façons :

* **Depuis le fil** : ouvrez le fil de la tâche et sélectionnez **Appliquer les modifications à la version principale** pour intégrer le travail dans votre projet, ou **Ignorer** pour le rejeter
* **Depuis le tableau** : sélectionnez le menu à trois points sur n'importe quelle carte de tâche Prête, puis sélectionnez **Appliquer les modifications**

<TaskReviewDrawer />

Révisez les tâches Prêtes avant de les appliquer. Vérifiez le journal de travail, les résultats des tests et l'aperçu pour comprendre ce qu'Agent a modifié.

## Terminé

Une fois appliquée, la carte de tâche passe dans la colonne **Terminé**.

La colonne Terminé comprend également les sessions de planification archivées et les tâches annulées. Cela vous donne un historique de ce qui s'est passé sans mélanger le travail terminé dans les Brouillons ou En cours.

<KanbanColumns
  columns={[
{ title: 'Prêt', count: 3, cards: [
{ title: 'Mode lecture au coucher avec thème sombre', description: 'Un mode coucher dédié avec un thème sombre aux tons chauds et des sons ambiants.', status: 'ready-drop', timestamp: '35m', showMenu: true },
{ title: 'Liens d\'histoires partageables avec aperçus OG', description: 'Ajouter des liens publics partageables est le levier de croissance le plus important.', status: 'ready-drop', timestamp: '31m', showMenu: true },
{ title: 'Personnalisation améliorée des personnages', description: 'Ajouter des options d\'apparence rend les illustrations vraiment personnalisées.', status: 'ready-drop', timestamp: '28m', showMenu: true },
] },
...
{ title: 'Liens d\'histoires partageables avec aperçus OG', description: 'Liens publics partageables comme levier de croissance le plus important.', status: 'done', timestamp: '1h' },
...
Agent gère automatiquement les conflits lorsque vous appliquez des modifications de plusieurs tâches. Si quelque chose ne semble pas correct, demandez à Agent de le corriger.

## Rechercher et filtrer les tâches

Utilisez le tableau des tâches lorsque votre projet a de nombreuses tâches en cours. Vous pouvez rechercher par titre de tâche ou numéro de tâche, filtrer les brouillons et utiliser le mode modification pour gérer des groupes de tâches à la fois.

Par exemple, recherchez par numéro de tâche lorsque quelqu'un fait référence à une tâche depuis un fil, une révision ou une conversation Slack. Filtrez les brouillons lorsque vous souhaitez vous concentrer sur le travail planifié sans analyser les tâches actives ou terminées.

Pour les suivis suggérés, consultez [Tâches de suivi](/fr/core-concepts/agent/task-system/follow-up-tasks).
