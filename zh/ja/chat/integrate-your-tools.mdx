---
title: "连接工具"
description: "连接受支持的服务，确定 Conversation 所需的信息和操作；有关配置，请参阅官方集成文档。"
sidebarTitle: "连接工具"
icon: "plug"
...
集成可将所支持服务的上下文和功能引入 Replit。

* 接入 **Slack** 后，Replit 可以读取 Slack 中的消息和会话串。
* **Google Drive** 让 Replit 能够访问文件和文件夹，Google Slides 则支持演示文稿。
* **Salesforce** 让 Replit 能够访问 CRM 数据及相关操作。
...
<Steps>
  <Step title="打开 Integrations">
    在工作区的侧边栏中选择 **Integrations**。

    <Frame>
      <img src="/assets/mirror/integrations-open-from-workspace-4dfe77901ec3.jpg" alt="左侧边栏中高亮显示 Integrations 的已脱敏 Replit 工作区主页界面" width="1451" height="1084" data-path="images/chat/integrations-open-from-workspace.jpg" />
    </Frame>
  </Step>
  ...
  <Step title="选择集成">
    ...
    <Frame>
      <img src="/assets/mirror/integrations-library-sanitized-e4b7670d4e3e.jpg" alt="显示推荐和可用集成的已脱敏 Replit 集成库" width="1606" height="979" data-path="images/chat/integrations-library-sanitized.jpg" />
    </Frame>
  </Step>

  <Step title="连接">
    确认该集成可访问的内容，然后按提示完成服务提供方的登录与授权流程。这里以 Salesforce 为例，其他集成的流程可能有所不同。
...
<section className="chat-mkt chat-workflow-grid has-visuals catalog-three-up">
  <a className="chat-example has-visual" href="/zh/ja/features/integrations/overview#connectors">
    ...
    <p>搜索会话和消息串，把其中的上下文用到你的工作中。</p>
    <div className="chat-card-link">查看连接器 →</div>
  </a>
  ...
  <a className="chat-example has-visual" href="/zh/ja/features/integrations/overview#connectors">
    ...
    <p>把相关的文件和文件夹纳入 Conversation 的上下文。</p>
    <div className="chat-card-link">查看连接器 →</div>
  </a>
  ...
  <a className="chat-example has-visual" href="/zh/ja/features/integrations/overview#connectors">
    ...
    <p>在 Conversation 中直接调用相关的客户和账户信息。</p>
    <div className="chat-card-link">查看连接器 →</div>
  </a>
</section>
...
<section className="chat-mkt chat-workflow-grid has-visuals">
  <a className="chat-example has-visual" href="/zh/ja/use-cases/prepare-for-a-customer-call">
    ...
    <p>汇总 Salesforce、Slack 和 Google Drive 中的上下文。</p>
    <div className="chat-card-link">查看此使用案例 →</div>
  </a>
</section>
...
原生集成可连接 Replit 连接器目录中提供的受支持服务。如果所需服务尚不受支持，您可以创建自定义 MCP，并将其作为集成添加到 Replit。

[了解如何通过 MCP 连接。](/zh/ja/chat/connect-through-mcp)

有关连接器类型、使用要求、配置方法、受支持服务和权限，请参阅[集成功能指南](/zh/ja/features/integrations/overview)。

当 Conversation 中具备足够的上下文来开展较复杂的工作时，了解如何[将其发展为项目](/zh/ja/chat/continue-or-create-a-project)。